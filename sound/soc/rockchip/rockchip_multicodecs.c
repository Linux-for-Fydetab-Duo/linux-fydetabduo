		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
    ...
    queue_delayed_work(system_power_efficient_wq,
                &mc_data->hp_blast_handler, msecs_to_jiffies(mc_data->pre_poweron_delayms));
		break;
...
static int rk_multicodecs_probe(struct platform_device *pdev)
{
    ...
    mc_data->pre_poweron_delayms = VOLUME_DELAY_TIME;
...
static void set_mute(struct multicodecs_data *mc_data) {
