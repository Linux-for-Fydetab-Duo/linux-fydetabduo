...
static int esdfs_create(struct mnt_idmap *mnt_idmap, struct inode *dir,
			struct dentry *dentry, umode_t mode, bool want_excl)
{
	...
	err = vfs_create(mnt_idmap, lower_inode, lower_dentry, mode,
			 want_excl);
	if (err)
		goto out;
...
static int esdfs_mkdir(struct mnt_idmap *mnt_idmap, struct inode *dir,
		       struct dentry *dentry, umode_t mode)
{
	...
	mode |= S_IFDIR;
	esdfs_set_lower_mode(ESDFS_SB(dir->i_sb), ESDFS_I(dir), &mode);
	err = vfs_mkdir(mnt_idmap, lower_parent_dentry->d_inode, lower_dentry,
			mode);
	if (err)
 ...
 */
static int esdfs_rename(struct mnt_idmap *mnt_idmap,
			...
			unsigned int flags)
{
	...
	rd.old_mnt_idmap = mnt_idmap;
	rd.old_dir = lower_old_dir_dentry->d_inode;
	rd.old_dentry = lower_old_dentry;
	rd.new_mnt_idmap = mnt_idmap;
...
static int esdfs_permission(struct mnt_idmap *mnt_idmap,
			    struct inode *inode, int mask)
{
	...
	err = generic_permission(mnt_idmap, inode, mask);
...
static int esdfs_setattr(struct mnt_idmap *mnt_idmap,
			 struct dentry *dentry, struct iattr *ia)
{
	 ...
	 */
	err = setattr_prepare(mnt_idmap, dentry, ia);
	if (err)
		return err;
	...
	err = notify_change(mnt_idmap, lower_dentry,
...
static int esdfs_getattr(struct mnt_idmap *mnt_idmap,
			 ...
			 u32 request_mask, unsigned int flags)
{
	...
	generic_fillattr(mnt_idmap, request_mask, inode, stat);
