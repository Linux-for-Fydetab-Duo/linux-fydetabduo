				       ...
				       struct rfkill_rk_platform_data *data)
{
	...
	gpio = of_get_named_gpio_flags_legacy(node, "uart_rts_gpios", 0, &flags);
	...
	gpio = of_get_named_gpio_flags_legacy(node, "BT,power_gpio", 0, &flags);
	if (gpio_is_valid(gpio)) {
	...
	gpio = of_get_named_gpio_flags_legacy(node, "BT,reset_gpio", 0, &flags);
	if (gpio_is_valid(gpio)) {
	...
	gpio = of_get_named_gpio_flags_legacy(node, "BT,wake_gpio", 0, &flags);
	if (gpio_is_valid(gpio)) {
	...
	gpio = of_get_named_gpio_flags_legacy(node, "BT,wake_host_irq", 0, &flags);
	if (gpio_is_valid(gpio)) {
