				   ...
				   const struct pwm_state *state)
{
	...
	if ((state->oneshot_count > 0 && state->oneshot_count <= pc->data->oneshot_cnt_max) &&
		...
		if (state->duty_offset > 0 &&
			...
			dev_err(pwmchip_parent(chip), "Duty_offset must be between %lld and %lld.\n",
				...
				struct pwm_capture *capture_res, unsigned long timeout_ms)
{
	...
	if (!pc->data->funcs.set_capture || !pc->data->funcs.get_capture_result) {
		dev_err(pwmchip_parent(chip), "Unsupported capture mode\n");
		return -EINVAL;
	}

	pwm_get_state(pwm, &curstate);
	if (curstate.enabled) {
		dev_err(pwmchip_parent(chip), "Failed to enable capture mode because PWM%d is busy\n",
			pc->channel_id);
		return -EBUSY;
	}
	...
	ret = pinctrl_select_state(pc->pinctrl, pc->active_state);
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to select pinctrl state\n");
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to enable capture mode\n");
		goto err_disable_pclk;
	}
	...
	if (pc->capture_cnt > 3) {
		...
		if (ret)
			dev_err(pwmchip_parent(chip), "Failed to get capture result\n");
	} else {
		dev_err(pwmchip_parent(chip), "Failed to wait for LPR/HPR interrupt\n");
		ret = -ETIMEDOUT;
	}
			     ...
			     bool enable)
{
	...
	if (!pc->counter_support ||
		...
		dev_err(pwmchip_parent(chip), "Unsupported counter mode\n");
		return -EINVAL;
	}

	pwm_get_state(pwm, &curstate);
	if (curstate.enabled) {
		dev_err(pwmchip_parent(chip), "Failed to enable counter mode because PWM%d is busy\n",
			pc->channel_id);
		return -EBUSY;
	}
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to abtain counter arbitration for PWM%d\n",
			pc->channel_id);
		goto err_disable_pclk;
	}
				    ...
				    unsigned long *counter_res, bool is_clear)
{
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to get counter result for PWM%d\n",
			pc->channel_id);
		return ret;
	}
				...
				unsigned long *freq_hz)
{
	...
	if (!pc->freq_meter_support ||
		...
		dev_err(pwmchip_parent(chip), "Unsupported frequency meter mode\n");
		return -EINVAL;
	}

	pwm_get_state(pwm, &curstate);
	if (curstate.enabled) {
		dev_err(pwmchip_parent(chip), "Failed to enable frequency meter mode because PWM%d is busy\n",
			pc->channel_id);
		return -EBUSY;
	}
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to abtain frequency meter arbitration for PWM%d\n",
			pc->channel_id);
	} else {
		ret = pc->data->funcs.get_freq_meter_result(chip, pwm, delay_ms, freq_hz);
		if (ret) {
			dev_err(pwmchip_parent(chip), "Failed to get frequency meter result for PWM%d\n",
				pc->channel_id);
		}
	}
				       ...
				       enum rockchip_pwm_global_ctrl_cmd cmd)
{
	...
	case PWM_GLOBAL_CTRL_GRANT:
		...
		if (!(val & arbiter)) {
			dev_err(pwmchip_parent(chip), "Failed to abtain global ctrl arbitration for PWM%d\n",
				pc->channel_id);
			return -EINVAL;
		}
	...
	case PWM_GLOBAL_CTRL_UPDATE:
		if (!pc->global_ctrl_grant) {
			dev_err(pwmchip_parent(chip), "CMD %d: get global ctrl arbitration first for PWM%d\n",
				cmd, pc->channel_id);
			return -EINVAL;
		}
	...
	case PWM_GLOBAL_CTRL_DISABLE:
		...
		break;
	default:
		dev_err(pwmchip_parent(chip), "Unsupported global ctrl cmd %d\n", cmd);
		return -EINVAL;
	}
...
int rockchip_pwm_global_ctrl(struct pwm_device *pwm, enum rockchip_pwm_global_ctrl_cmd cmd)
{
	...
	if (!pc->data->funcs.global_ctrl) {
		dev_err(pwmchip_parent(chip), "Unsupported global control\n");
		return -EINVAL;
	}

	pwm_get_state(pwm, &curstate);
	if (curstate.enabled) {
		dev_err(pwmchip_parent(chip), "Failed to execute global ctrl cmd %d because PWM%d is busy\n",
			cmd, pc->channel_id);
		return -EBUSY;
	}
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to execute global ctrl cmd %d for PWM%d\n",
			cmd, pc->channel_id);
		goto err_disable_pclk;
	}
					  ...
					  enum rockchip_pwm_wave_table_width_mode width_mode)
{
	...
	if (!table_config->table ||
		...
		dev_err(pwmchip_parent(chip), "The wave table to set is out of range for PWM%d\n",
			pc->channel_id);
		return -EINVAL;
	}
	...
	if (!(val & arbiter)) {
		dev_err(pwmchip_parent(chip), "Failed to abtain wave memory arbitration for PWM%d\n",
			pc->channel_id);
		return -EINVAL;
	}
...
int rockchip_pwm_set_wave(struct pwm_device *pwm, struct rockchip_pwm_wave_config *config)
{
	...
	if (!pc->wave_support ||
		...
		dev_err(pwmchip_parent(chip), "Unsupported wave generator mode\n");
		return -EINVAL;
	}

	if (!config->clk_rate) {
		dev_err(pwmchip_parent(chip), "clk rate can not be 0\n");
		return -EINVAL;
	}

	pc->scaler = DIV_ROUND_CLOSEST_ULL(pc->clk_rate, config->clk_rate * 2);
	if (pc->scaler > 256) {
		dev_err(pwmchip_parent(chip), "Unsupported scale factor %d(max: 512) for PWM%d\n",
	...
	if (config->period_table) {
		ret = pc->data->funcs.set_wave_table(chip, pwm, config->period_table,
						     config->width_mode);
		if (ret) {
			dev_err(pwmchip_parent(chip), "Failed to set wave period table for PWM%d\n",
				pc->channel_id);
			goto err_disable_pclk;
		}
	}
	...
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to set wave generator for PWM%d\n", pc->channel_id);
		goto err_disable_pclk;
	}
					...
					struct rockchip_pwm_biphasic_config *config)
{
	...
	if (config->enable) {
		if (!config->is_continuous && !config->delay_ms) {
			dev_err(pwmchip_parent(chip), "The delay_ms can not be 0 in normal mode for PWM%d\n",
				pc->channel_id);
			return -EINVAL;
		}
			      ...
			      unsigned long *biphasic_res)
{
	...
	if (!pc->biphasic_support ||
		...
		dev_err(pwmchip_parent(chip), "Unsupported biphasic counter mode\n");
		return -EINVAL;
	}

	pwm_get_state(pwm, &curstate);
	if (curstate.enabled) {
		dev_err(pwmchip_parent(chip), "Failed to enable biphasic counter mode because PWM%d is busy\n",
			pc->channel_id);
		return -EBUSY;
	}
	...
	if (!config->is_continuous) {
		...
		if (pc->biphasic_res_valid) {
			*biphasic_res = readl_relaxed(pc->base + BIPHASIC_RESULT_VALUE);
			...
			dev_err(pwmchip_parent(chip), "failed to wait for biphasic counter interrupt\n");
			return -ETIMEDOUT;
		}
	} else {
...
int rockchip_pwm_get_biphasic_result(struct pwm_device *pwm, unsigned long *biphasic_res)
{
	...
	if (!pc->biphasic_config) {
		dev_err(pwmchip_parent(chip), "Failed to parse biphasic counter config\n");
		return -EINVAL;
	}

	if (!pc->biphasic_config->is_continuous || !pc->biphasic_config->enable) {
		dev_err(pwmchip_parent(chip), "Unsupported to get result in real time in normal mode\n");
		return -EINVAL;
	}
	...
	ret = pc->data->funcs.get_biphasic_result(chip, pwm, biphasic_res);
	if (ret) {
		dev_err(pwmchip_parent(chip), "Failed to get biphasic counter result for PWM%d\n",
			pc->channel_id);
		return -EINVAL;
	}
...
{
	pc->debugfs = debugfs_create_file(dev_name(pwmchip_parent(pc->chip)),
...
static int rockchip_pwm_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err_probe(&pdev->dev, ret, "devm_pwmchip_add() failed\n");
		goto err_pclk;
	}
...
static void rockchip_pwm_remove(struct platform_device *pdev)
{
	...
	if (state.enabled) {
		if (pc->oneshot_en) {
			...
			state.enabled = false;
			pwm_apply_might_sleep(&chip->pwms[0], &state);
		}
	}

	/* pwmchip_remove is handled by devm */
