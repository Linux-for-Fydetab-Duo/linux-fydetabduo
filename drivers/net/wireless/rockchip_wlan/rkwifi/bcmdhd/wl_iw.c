...
static int dev_wlc_bufvar_get(struct net_device *dev, char *name, char *buf,
  ...
  if (!error) bcopy(ioctlbuf, buf, buflen);
...
int wl_iw_send_priv_event(struct net_device *dev, char *flag) {
  ...
  if (strlen(flag) > sizeof(extra)) return -1;
...
static int wl_iw_config_commit(struct net_device *dev,
                               struct iw_request_info *info,
                               union iwreq_data *zwrq, char *extra) {
  wlc_ssid_t ssid;
  int error;
...
static int wl_iw_set_freq(struct net_device *dev, struct iw_request_info *info,
  ...
  /* Setting by frequency */
    ...
    if (fwrq->e >= 6) {
      fwrq->e -= 6;
      while (fwrq->e--) fwrq->m *= 10;
    } else if (fwrq->e < 6) {
      while (fwrq->e++ < 6) fwrq->m /= 10;
    }
  ...
  if (wext_info) wext_info->conn_info.channel = chan;
...
static int wl_iw_get_freq(struct net_device *dev, struct iw_request_info *info,
  ...
  DHD_CHECK(dhd, dev);
  if ((error = dev_wlc_intvar_get(dev, "chanspec", &val))) return error;
  chanspec = val;
...
static int wl_iw_get_range(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((error = dev_wlc_intvar_get(dev, "nmode", &nmode))) return error;
  if ((error = dev_wlc_ioctl(dev, WLC_GET_PHYTYPE, &phytype, sizeof(phytype))))
    return error;
  if (nmode == 1 &&
      (((phytype == WLC_PHY_TYPE_LCN) || (phytype == WLC_PHY_TYPE_LCN40)))) {
    if ((error = dev_wlc_intvar_get(dev, "mimo_bw_cap", &bw_cap))) return error;
    if ((error = dev_wlc_intvar_get(dev, "sgi_tx", &sgi_tx))) return error;
    if ((error =
...
static int wl_iw_set_wap(struct net_device *dev, struct iw_request_info *info,
  ...
  /* Ignore "auto" or "off" */
  if (ETHER_ISBCAST(wrqu->ap_addr.sa_data) ||
      ETHER_ISNULLADDR(wrqu->ap_addr.sa_data)) {
    scb_val_t scbval;
  ...
  if (wext_info && wext_info->conn_info.ssid.SSID_len) {
    if ((error = wl_ext_connect(dev, &wext_info->conn_info))) return error;
  } else {
    if ((error = dev_wlc_ioctl(dev, WLC_REASSOC, wrqu->ap_addr.sa_data,
                               ETHER_ADDR_LEN))) {
      WL_ERROR(("WLC_REASSOC failed (%d).\n", error));
      return error;
    }
...
#ifdef WL_EXT_IAPSTA
  wl_ext_in4way_sync_wext(dev, STA_NO_BTC_IN4WAY, WL_EXT_STATUS_CONNECTING,
                          NULL);
#endif
...
static int wl_iw_get_wap(struct net_device *dev, struct iw_request_info *info,
  ...
  (void)dev_wlc_ioctl(dev, WLC_GET_BSSID, wrqu->ap_addr.sa_data,
                      ETHER_ADDR_LEN);
...
static int wl_iw_get_aplist(struct net_device *dev,
  ...
  for (i = 0, dwrq->length = 0; i < list->count && dwrq->length < IW_MAX_AP;
    ...
    /* Infrastructure only */
    if (!(dtoh16(bi->capability) & DOT11_CAP_ESS)) continue;
...
static int wl_iw_get_scan(struct net_device *dev, struct iw_request_info *info,
  ...
  if (ci.scan_channel) return -EAGAIN;
  ...
  for (i = 0; i < list->count && i < IW_MAX_AP; i++) {
    ...
    iwe.u.freq.m =
        wf_channel2mhz(CHSPEC_CHANNEL(bi->chanspec), (CHSPEC_IS2G(bi->chanspec))
  ...
  while (p_buf != iscan->list_cur) {
    ...
    for (ii = 0; ii < list->count && apcnt < IW_MAX_AP; apcnt++, ii++) {
      ...
      if (bi->rateset.count <= sizeof(bi->rateset.rates)) {
        if (event + IW_MAX_BITRATES * IW_EV_PARAM_LEN >= end) return -E2BIG;
...
static int wl_iw_set_nick(struct net_device *dev, struct iw_request_info *info,
  ...
  if (dwrq->length > sizeof(iw->nickname)) return -E2BIG;
...
static int wl_iw_set_rate(struct net_device *dev, struct iw_request_info *info,
  ...
  if (vwrq->fixed) {
    ...
    */
    ...
    if (error_bg && error_a) return (error_bg | error_a);
  } else {
    /*
            clear rate override
            Since the is a/b/g-blind, both a/bg_rate are enforced.
    */
    ...
    for (i = 0; i < rateset.count; i++)
      if ((rateset.rates[i] & 0x7f) > rate) break;
...
static int wl_iw_set_rts(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((error = dev_wlc_intvar_set(dev, "rtsthresh", rts))) return error;
...
static int wl_iw_get_rts(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((error = dev_wlc_intvar_get(dev, "rtsthresh", &rts))) return error;
...
static int wl_iw_set_frag(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((error = dev_wlc_intvar_set(dev, "fragthresh", frag))) return error;
...
static int wl_iw_set_txpow(struct net_device *dev, struct iw_request_info *info,
  ...
  if (disable & WL_RADIO_SW_DISABLE) return 0;

  /* Only handle mW */
  if (!(vwrq->flags & IW_TXPOW_MWATT)) return -EINVAL;

  /* Value < 0 means just "on" or "off" */
  if (vwrq->value < 0) return 0;
...
static int wl_iw_set_retry(struct net_device *dev, struct iw_request_info *info,
  ...
  if (vwrq->disabled || (vwrq->flags & IW_RETRY_LIFETIME)) return -EINVAL;
...
static int wl_iw_get_retry(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((vwrq->flags & IW_RETRY_TYPE) == IW_RETRY_LIFETIME) return -EINVAL;
  ...
  if (vwrq->flags & IW_RETRY_MAX) {
    ...
    vwrq->value = srl;
    if (srl != lrl) vwrq->flags |= IW_RETRY_MIN;
  }
...
static int wl_iw_set_encode(struct net_device *dev,
  ...
  if ((dwrq->flags & IW_ENCODE_INDEX) == 0) {
    ...
    /* Default to 0 */
    if (key.index == DOT11_MAX_DEFAULT_KEYS) key.index = 0;
  } else {
    key.index = (dwrq->flags & IW_ENCODE_INDEX) - 1;
    if (key.index >= DOT11_MAX_DEFAULT_KEYS) return -EINVAL;
  }

  /* Interpret "off" to mean no encryption */
  wsec = (dwrq->flags & IW_ENCODE_DISABLED) ? 0 : WEP_ENABLED;

  if ((error = dev_wlc_intvar_set(dev, "wsec", wsec))) return error;
  ...
  if (!extra || !dwrq->length || (dwrq->flags & IW_ENCODE_NOKEY)) {
    ...
    if (dwrq->length > sizeof(key.data)) return -EINVAL;
...
static int wl_iw_get_encode(struct net_device *dev,
  ...
  /* Get key */
  if (dwrq->length && extra) memcpy(extra, key.data, dwrq->length);
...
static int wl_iw_set_power(struct net_device *dev, struct iw_request_info *info,
  ...
  pm = htod32(pm);
  if ((error = dev_wlc_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm)))) return error;
...
static int wl_iw_get_power(struct net_device *dev, struct iw_request_info *info,
  ...
  if ((error = dev_wlc_ioctl(dev, WLC_GET_PM, &pm, sizeof(pm)))) return error;
...
static int wl_iw_set_encodeext(struct net_device *dev,
  ...
  /* Instead of bcast for ea address for default wep keys, driver needs it to be
   * Null */
   ...
   */
    ...
    if (iwe->key_len > sizeof(key.data)) return -EINVAL;
    ...
    if (error) return error;
#ifdef WL_EXT_IAPSTA
    wl_ext_in4way_sync_wext(dev, STA_NO_BTC_IN4WAY, WL_EXT_STATUS_ADD_KEY,
                            NULL);
#endif
  }
  return 0;
...
static int wl_iw_set_pmksa(struct net_device *dev, struct iw_request_info *info,
  ...
  if (pmk_list) pmkid_array = pmk_list->pmkids.pmkid;
  ...
  if (iwpmksa->cmd == IW_PMKSA_REMOVE) {
    ...
    for (; i < pmk_list->pmkids.npmkid; i++) {
      bcopy(&pmkid_array[i + 1].BSSID, &pmkid_array[i].BSSID, ETHER_ADDR_LEN);
      bcopy(&pmkid_array[i + 1].PMKID, &pmkid_array[i].PMKID, WPA2_PMKID_LEN);
    }
  ...
  for (i = 0; i < pmk_list->pmkids.npmkid; i++) {
    uint j;
    WL_TRACE(
        ("PMKID[%d]: %s = ", i, bcm_ether_ntoa(&pmkid_array[i].BSSID, eabuf)));
    for (j = 0; j < WPA2_PMKID_LEN; j++)
...
static int wl_iw_set_wpaauth(struct net_device *dev,
  ...
  switch (paramid) {
    case IW_AUTH_WPA_VERSION:
      ...
      if ((error = dev_wlc_intvar_set(dev, "wpa_auth", val))) return error;
      break;
    ...
    case IW_AUTH_CIPHER_GROUP: {
      ...
      if (cipher_combined & (IW_AUTH_CIPHER_WEP40 | IW_AUTH_CIPHER_WEP104))
        val |= WEP_ENABLED;
      if (cipher_combined & IW_AUTH_CIPHER_TKIP) val |= TKIP_ENABLED;
      if (cipher_combined & IW_AUTH_CIPHER_CCMP) val |= AES_ENABLED;
#ifdef BCMWAPI_WPI
      val &= ~SMS4_ENABLED;
      if (cipher_combined & IW_AUTH_CIPHER_SMS4) val |= SMS4_ENABLED;
#endif
    ...
    case IW_AUTH_PRIVACY_INVOKED: {
      ...
      if (paramval == 0) {
        ...
        if ((error = dev_wlc_intvar_get(dev, "wsec", &wsec))) return error;
    ...
    case IW_AUTH_WAPI_ENABLED:
      if ((error = dev_wlc_intvar_get(dev, "wsec", &val))) return error;
      if (paramval) {
        val |= SMS4_ENABLED;
        if ((error = dev_wlc_intvar_set(dev, "wsec", val))) {
        ...
        if ((error =
                 dev_wlc_intvar_set(dev, "wpa_auth", WAPI_AUTH_UNSPECIFIED))) {
...
#define VAL_PSK(_val) (((_val)&WPA_AUTH_PSK) || ((_val)&WPA2_AUTH_PSK) || ((_val)&WPA3_AUTH_SAE_PSK))
...
static int wl_iw_get_wpaauth(struct net_device *dev,
  ...
  switch (paramid) {
    case IW_AUTH_WPA_VERSION:
      /* supported wpa version disabled or wpa or wpa2 */
      if ((error = dev_wlc_intvar_get(dev, "wpa_auth", &val))) return error;
    ...
    case IW_AUTH_80211_AUTH_ALG:
      /* open, shared, leap */
      if ((error = dev_wlc_intvar_get(dev, "auth", &val))) return error;
      if (!val)
...
int wl_iw_ioctl(struct net_device *dev, struct ifreq *rq, int cmd) {
  ...
  if (max_tokens && wrq->u.data.pointer) {
    if (wrq->u.data.length > max_tokens) return -E2BIG;

    if (!(extra = kmalloc(max_tokens * token_size, GFP_KERNEL))) return -ENOMEM;
...
#define WL_IW_DONT_CARE 9999
  const conn_fail_event_map_t event_map[] = {
      ...
      {WLC_E_PRUNE, WL_IW_DONT_CARE, WLC_E_PRUNE_ENCR_MISMATCH, "Conn",
       "EncrypMismatch"},
      {WLC_E_PRUNE, WL_IW_DONT_CARE, WLC_E_RSN_MISMATCH, "Conn", "RsnMismatch"},
  ...
  if (cause) {
    ...
    (void)snprintf(stringBuf, buflen, "%s %s %02d %02d", name, cause, status,
                   reason);
...
void wl_iw_event(struct net_device *dev, void *argu, const wl_event_msg_t *e,
  ...
  switch (event_type) {
    ...
    case WLC_E_ASSOC_REQ_IE:
      ...
      if (datalen < sizeof(extra)) memcpy(extra, data, datalen);
      break;
    ...
    case WLC_E_PMKID_CACHE: {
      ...
      while (count) {
        ...
        if (pmkidcand->preauth) iwpmkidcand->flags |= IW_PMKID_CAND_PREAUTH;
...
int wl_iw_get_wireless_stats(struct net_device *dev,
  ...
  if (res) {
    WL_ERROR(
        ("wl_iw_get_wireless_stats counters failed error=%d ****** \n", res));
    goto done;
  }
...
static int wl_iw_iscan_prep(wl_scan_params_v1_t *params, wlc_ssid_t *ssid) {
  int err = 0;
  ...
  if (ssid && ssid->SSID_len) memcpy(&params->ssid, ssid, sizeof(wlc_ssid_t));
...
static int wl_iw_iscan(iscan_info_t *iscan, wlc_ssid_t *ssid, uint16 action) {
  int params_size =
      (WL_SCAN_PARAMS_V1_FIXED_SIZE + OFFSETOF(wl_iscan_params_v1_t, params));
...
static uint32 wl_iw_iscan_get(iscan_info_t *iscan) {
  ...
  if (iscan->list_cur) {
    ...
    if (!buf) return WL_SCAN_RESULTS_ABORTED;
    buf->next = NULL;
    if (!iscan->list_hdr)
      iscan->list_hdr = buf;
    else {
  ...
  (void)dev_iw_iovar_getbuf(iscan->dev, "iscanresults", &list,
                            WL_ISCAN_RESULTS_V109_FIXED_SIZE, buf->iscan_buf,
                            WLC_IW_ISCAN_MAXLEN);
