...
#define GSCAN_BATCH_RESULT_HDR_LEN                             \
  (SCAN_INDEX_HDR_LEN + SCAN_ID_HDR_LEN + SCAN_FLAGS_HDR_LEN + \
   GSCAN_NUM_RESULTS_HDR_LEN + GSCAN_CH_BUCKET_MASK_HDR_LEN +  \
...
enum andr_vendor_subcmd {
  ...
  WIFI_SUBCMD_THERMAL_MITIGATION =
      ANDROID_NL80211_SUBCMD_MITIGATION_RANGE_START,
  WIFI_SUBCMD_CUSTOM_MAPPING_OF_DSCP =
      ANDROID_NL80211_SUBCMD_CUSTOM_SETTING_START,
  WIFI_SUBCMD_CUSTOM_MAPPING_OF_DSCP_RESET,
  WIFI_SUBCMD_CELL_AVOID_SUBCMD_SET_CONFIG =
      ANDROID_NL80211_SUBCMD_CELL_AVOID_RANGE_START,
...
enum debug_attributes {
  ...
  DEBUG_ATTRIBUTE_BUF_RING_NUM,
  DEBUG_ATTRIBUTE_BUF_RING_MAP,
...
enum andr_wifi_attr {
  ...
  ANDR_WIFI_ATTRIBUTE_INDOOR_STATE,
...
typedef struct wlan_driver_wake_reason_cnt_t {
  ...
  int *
      cmd_event_wake_cnt; /* Individual wake count array, each index a reason */
  int cmd_event_wake_cnt_sz;   /* Max number of cmd event wake reasons */
  int cmd_event_wake_cnt_used; /* Number of cmd event wake reasons specific to
                                  the driver */
  int total_driver_fw_local_wake;  /* Total count of drive/fw wakes, for local
                                      reasons */
  int *driver_fw_local_wake_cnt;   /* Individual wake count array, each index a
                                      reason */
...
#define BRCM_VENDOR_GET_RCC_EVENT_BUF_LEN               \
  sizeof(uint32) + DOT11_MAX_SSID_LEN + sizeof(int32) + \
      (sizeof(uint16) * MAX_ROAM_CHANNEL)
...
typedef enum {
   ...
   * - Platform is trying to speculatively connect to a second network and
   * evaluate it without disrupting the primary connection.
   *
   * Requirements for Firmware:
   * - Do not reduce the number of tx/rx chains of primary connection.
   * - If using MCC, should set the MCC duty cycle of the primary connection to
   * be higher than the secondary connection (maybe 70/30 split).
   * - Should pick the best BSSID for the secondary STA (disregard the chip
   * mode) independent of the primary STA:
   ...
   * - For the peer to peer use case, BSSID for the secondary connection will be
   * chosen by the framework.
   *
   * Requirements for Firmware:
   * - Can choose MCC or DBS mode depending on the MCC efficiency and HW
   * capability.
   * - If using MCC, set the MCC duty cycle of the primary connection to be
   * equal to the secondary connection.
   * - Prefer BSSID candidates which will help provide the best "overall"
   * performance for both the connections.
   */
...
#else
static INLINE int wl_cfgvendor_attach(struct wiphy *wiphy, dhd_pub_t *dhd) {
  UNUSED_PARAMETER(wiphy);
  UNUSED_PARAMETER(dhd);
  return 0;
}
static INLINE int wl_cfgvendor_detach(struct wiphy *wiphy) {
  UNUSED_PARAMETER(wiphy);
  return 0;
}
static INLINE int wl_cfgvendor_send_async_event(struct wiphy *wiphy,
                                                struct net_device *dev,
                                                int event_id, const void *data,
                                                int len) {
  return 0;
}
...
int wl_cfgdbg_tput_debug_get_cmd(struct wiphy *wiphy, struct wireless_dev *wdev,
                                 const void *data, int len);
#endif /* TPUT_DEBUG_DUMP */
extern int wl_cfgvendor_multista_set_primary_connection(
