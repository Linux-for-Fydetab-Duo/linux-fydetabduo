...
/* Default KEEP_ALIVE Period is 55 sec to prevent AP from sending Keep Alive
 * probe frame */
...
#define DHD_BUS_BUSY_RPM_ALL                                              \
  (DHD_BUS_BUSY_RPM_SUSPEND_DONE | DHD_BUS_BUSY_RPM_SUSPEND_IN_PROGRESS | \
...
#define DHD_BUS_BUSY_CHECK_IN_SYSFS_DUMP(dhdp) \
  ((dhdp)->dhd_bus_busy_state & DHD_BUS_BUSY_IN_SYSFS_DUMP)
#define DHD_BUS_BUSY_CHECK_IDLE(dhdp) ((dhdp)->dhd_bus_busy_state == 0)

#define DHD_BUS_CHECK_SUSPEND_OR_SUSPEND_IN_PROGRESS(dhdp) \
  ((dhdp)->busstate == DHD_BUS_SUSPEND ||                  \
...
#define DHD_BUS_CHECK_SUSPEND_OR_ANY_SUSPEND_IN_PROGRESS(dhdp) \
  ((dhdp)->busstate == DHD_BUS_SUSPEND ||                      \
   DHD_BUS_CHECK_ANY_SUSPEND_IN_PROGRESS(dhdp))
...
#define DHD_BUS_CHECK_REMOVE(dhdp) ((dhdp)->busstate == DHD_BUS_REMOVE)

/* IOVar flags for common error checks */
#define DHD_IOVF_PWRREQ_BYPASS \
  (1 << 0) /* flags to prevent bp access during host sleep state */
...
/* (u32)remainder = (u64)dividend % (u32)divisor */
#define MOD_U64_BY_U32(dividend, divisor) \
  (uint32)((uint64)(dividend) % (uint32)(divisor))

#define SEC_USEC_FMT "%015llu.%06u"
#endif /* LINUX || linux */
...
/* Download Types */
typedef enum download_type { FW, NVRAM, CLM_BLOB, TXCAP_BLOB } download_type_t;
...
#define DHD_SCAN_ASSOC_ACTIVE_TIME \
  40 /* ms: Embedded default Active setting from DHD */
#ifndef CUSTOM_SCAN_UNASSOC_ACTIVE_TIME
#define DHD_SCAN_UNASSOC_ACTIVE_TIME \
  80 /* ms: Embedded def. Unassoc Active setting from DHD */
#else
#define DHD_SCAN_UNASSOC_ACTIVE_TIME CUSTOM_SCAN_UNASSOC_ACTIVE_TIME
#endif /* CUSTOM_SCAN_UNASSOC_ACTIVE_TIME */
#define DHD_SCAN_HOME_TIME \
  45 /* ms: Embedded default Home time setting from DHD */
#ifndef CUSTOM_SCAN_HOME_AWAY_TIME
#define DHD_SCAN_HOME_AWAY_TIME \
  100 /* ms: Embedded default Home Away time setting from DHD */
#else
#define DHD_SCAN_HOME_AWAY_TIME \
  CUSTOM_SCAN_HOME_AWAY_TIME /* ms: Custom setting from DHD */
#endif                       /* CUSTOM_SCAN_HOME_AWAY_TIME */
#ifndef CUSTOM_SCAN_PASSIVE_TIME
#define DHD_SCAN_PASSIVE_TIME \
  130 /* ms: Embedded default Passive setting from DHD */
#else
#define DHD_SCAN_PASSIVE_TIME \
  CUSTOM_SCAN_PASSIVE_TIME /* ms: Custom Passive setting from DHD */
...
#define DHD_IF_SET_TX_ACTIVE(ifp, context) ifp->tx_paths_active |= context;
#define DHD_IF_CLR_TX_ACTIVE(ifp, context) ifp->tx_paths_active &= ~context;
#define DHD_IF_IS_TX_ACTIVE(ifp) (ifp->tx_paths_active)
/**
...
typedef enum { TPUT_PKT_TYPE_NORMAL, TPUT_PKT_TYPE_STOP } tput_pkt_type_t;
...
enum {
   ...
   * 2. TCPACKs that don't need to hurry delivered remains longer in TXQ so can
   * be suppressed.
   */
  TCPACK_SUP_DELAYTX,
  TCPACK_SUP_HOLD,
...
#define DHD_TRF_MGMT_DWM_PRIO(dwm_tbl_entry) \
  ((dwm_tbl_entry)&DHD_TRF_MGMT_DWM_PRIO_BITS)
...
enum { CMD_DEFAULT, CMD_UNWANTED, CMD_DISCONNECTED, CMD_MAX };
...
#else
static INLINE uint16 dhd_plat_align_rxbuf_size(uint16 rxbufpost_sz) {
  return rxbufpost_sz;
}
#endif /* !__linux__ */
...
#define DHD_PTM_GET_CLKID(ts) \
  (((ts)&DHD_PTM_CLK_ID_MASK) >> DHD_PTM_CLK_ID_SHIFT)
...
typedef struct dhd_pub {
  ...
  ulong
      tx_big_packets; /* Dropped data packets that are larger than MAX_MTU_SZ */
  ...
  int suspend_disable_flag; /* "1" to disable all extra powersaving during
                               suspend */
...
#error "wlfc thread not enabled"
  ...
  bool dongle_trap_during_wifi_onoff; /* flag to indicate trap during wifi
                                         on/off */
...
#define MAXSKBPEND 1024
  ...
  void *logdump_cookie;
#endif                   /* DHD_LOG_DUMP */
  uint32 dhd_console_ms; /** interval for polling the dongle for console (log)
                            messages */
  ...
  osl_atomic_t set_ssid_rcvd; /* to track if WLC_E_SET_SSID is received during
                                 join IOVAR */
...
#ifdef GDB_PROXY
  /* True if firmware runs under gdb control (this may cause timeouts at any
   * point) */
  bool gdb_proxy_active;
  /* True if deadman_to shall be forced to 0 */
  bool gdb_proxy_nodeadman;
  /* Counter incremented at each firmware stop/go transition. LSB
   * (GDB_PROXY_STOP_MASK) is set when firmware is stopped, clear when running
   */
...
#define DHD_H2D_LOG_TIME_STAMP_MATCH (10000) /* 10 Seconds */
  /*
   * Interval for updating the dongle console message time stamp with the Host
   * (DHD) time stamp
   */
...
#else
static INLINE int dhd_wifi_platform_set_power(dhd_pub_t *pub, bool on) {
  return 0;
}
#endif /* __linux__ */
...
#define DHD_PKT_SET_PA(pkt, pkt_pa) DHD_PKTTAG_FD(pkt)->pa = (dmaaddr_t)(pkt_pa)
...
WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(dhd_workitem_context_t,
                                   dhd_get_dhd_workitem_context)
#endif /* (BCMWDF)  */
...
#define _DHD_PM_RESUME_WAIT(a, b)                               \
  do {                                                          \
    int retry = 0;                                              \
...
#define DHD_PM_RESUME_RETURN     \
  do {                           \
    if (dhd_mmc_suspend) return; \
  } while (0)

#define DHD_SPINWAIT_SLEEP_INIT(a) DECLARE_WAIT_QUEUE_HEAD(a);
#define SPINWAIT_SLEEP(a, exp, us)                   \
  do {                                               \
...
#define DHD_OS_WD_WAKE_LOCK(pub)                                             \
  do {                                                                       \
    printf("call dhd_os_WD_wake_lock: %s %d (%d)\n", __FUNCTION__, __LINE__, \
           dhd_wakelock_wd_counter_get(pub));                                \
...
#define DHD_OS_OOB_IRQ_WAKE_LOCK_TIMEOUT(pub, val) \
  dhd_os_oob_irq_wake_lock_timeout(pub, val)
...
/* interface operations (register, remove) should be atomic, use this lock to
 * prevent race condition among wifi on/off and interface operation functions
 */
#if defined(LINUX)
...
extern void wl_android_post_init(
    void);  // terence 20120530: fix critical section in dhd_open and
            // dhdsdio_probe
#endif      /* MULTIPLE_SUPPLICANT */

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && \
    defined(MULTIPLE_SUPPLICANT)
...
#else
#define DHD_MUTEX_IS_LOCK_RETURN(a) \
  do {                              \
  } while (0)
#define DHD_MUTEX_LOCK(a) \
  do {                    \
  } while (0)
#define DHD_MUTEX_UNLOCK(a) \
  do {                      \
  } while (0)
#endif
#endif /* defined (LINUX) || defined(linux) */
...
typedef struct ecountersv2_xtlv_list_elt {
  /* Not quite the exact bcm_xtlv_t type as data could be pointing to other
   * pieces in memory at the time of parsing arguments.
   */
  uint16 id;
  uint16 len;
  uint8 *data;
...
extern void dhd_eap_txcomplete(dhd_pub_t *dhdp, void *txp, bool success,
                               int ifidx);
...
#define WIFI_FEATURE_ADDITIONAL_STA \
  0x0800                         /* Support for two STAs             */
#define WIFI_FEATURE_TDLS 0x1000 /* Tunnel directed link setup       */
#define WIFI_FEATURE_TDLS_OFFCHANNEL \
...
#define WIFI_FEATURE_TX_TRANSMIT_POWER \
  0x400000 /* Capture Tx transmit power levels */
#define WIFI_FEATURE_CONTROL_ROAMING \
...
#define WIFI_FEATURE_SET_TX_POWER_LIMIT \
  0x4000000 /* Support Tx Power Limit setting   */
#define WIFI_FEATURE_USE_BODY_HEAD_SAR \
  0x8000000 /* Support Body/Head Proximity SAR  */
#define WIFI_FEATURE_SET_LATENCY_MODE \
  0x40000000 /* Support Latency mode setting     */
#define WIFI_FEATURE_P2P_RAND_MAC \
  0x80000000                            /* Support P2P MAC randomization    */
...
#ifdef PCIE_FULL_DONGLE
extern void dhd_wakeup_ioctl_event(dhd_pub_t *pub,
                                   dhd_ioctl_recieved_status_t reason);
#else
static INLINE void dhd_wakeup_ioctl_event(dhd_pub_t *pub,
                                          dhd_ioctl_recieved_status_t reason) {
  printf("%s is NOT implemented for SDIO", __FUNCTION__);
  return;
}
#endif
#ifdef SHOW_LOGTRACE
...
int dhd_msix_message_set(dhd_pub_t *dhdp, uint table_entry, uint message_number,
                         bool unmask);
...
extern void dhd_pcie_dump_core_regs(dhd_pub_t *pub, uint32 index,
                                    uint32 first_addr, uint32 last_addr);
...
#endif /* (BT_OVER_SDIO) */
extern void *dhd_os_open_image1(dhd_pub_t *pub,
                                char *filename); /* rev1 function signature */
...
extern int dhd_customer_oob_irq_map(void *adapter,
...
extern int syna_country_update_type_list(eCountry_flag_type type,
                                         char *list_str);

extern int dhd_sar_init_parameter(dhd_pub_t *dhd, eCountry_flag_type type,
...
#else
#if defined(DHD_SSSR_DUMP)
static INLINE void dhd_write_sssr_dump(dhd_pub_t *dhd, uint32 dump_mode) {
  return;
}
#endif /* DHD_SSSR_DUMP */
#ifdef DNGL_AXI_ERROR_LOGGING
static INLINE void dhd_schedule_axi_error_dump(dhd_pub_t *dhdp, void *type) {
  return;
}
#endif /* DNGL_AXI_ERROR_LOGGING */
...
#else
static INLINE void dhd_schedule_edl_work(dhd_pub_t *dhd, uint delay_ms) {
  return;
}
#endif /* linux || LINUX */
#endif /* EWP_EDL */
...
extern int dhd_packet_filter_add_remove(dhd_pub_t *dhdp, int add_remove,
                                        int num);
...
extern int dhd_dev_apf_read_filter_data(struct net_device *ndev, u8 *buf,
                                        uint32 buf_len);
#endif /* APF */
...
extern int net_os_send_hang_message_reason(struct net_device *dev,
                                           const char *string_num);
#endif
extern bool dhd_wowl_cap(void *bus);
extern int wl_host_event(dhd_pub_t *dhd_pub, int *idx, void *pktdata,
                         uint pktlen, wl_event_msg_t *, void **data_ptr,
                         void *);
extern int wl_process_host_event(dhd_pub_t *dhd_pub, int *idx, void *pktdata,
                                 uint pktlen, wl_event_msg_t *, void **data_ptr,
                                 void *);
extern void wl_event_to_host_order(wl_event_msg_t *evt);
extern int wl_host_event_get_data(void *pktdata, uint pktlen,
                                  bcm_event_msg_u_t *evu);
extern int dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifindex, wl_ioctl_t *ioc,
                        void *buf, int len);
extern int dhd_wl_ioctl_cmd(dhd_pub_t *dhd_pub, int cmd, void *arg, int len,
                            uint8 set, int ifindex);
...
#endif
extern int dhd_event_ifadd(struct dhd_info *dhd,
                           struct wl_event_data_if *ifevent, char *name,
                           uint8 *mac);
extern int dhd_event_ifdel(struct dhd_info *dhd,
                           struct wl_event_data_if *ifevent, char *name,
                           uint8 *mac);
extern int dhd_event_ifchange(struct dhd_info *dhd,
                              struct wl_event_data_if *ifevent, char *name,
                              uint8 *mac);
#ifdef DHD_UPDATE_INTF_MAC
extern int dhd_op_if_update(dhd_pub_t *dhdpub, int ifidx);
#endif /* DHD_UPDATE_INTF_MAC */
extern struct net_device *dhd_allocate_if(dhd_pub_t *dhdpub, int ifidx,
                                          const char *name, uint8 *mac,
                                          uint8 bssidx, bool need_rtnl_lock,
                                          const char *dngl_name);
extern int dhd_remove_if(dhd_pub_t *dhdpub, int ifidx, bool need_rtnl_lock);
#ifdef WL_STATIC_IF
extern s32 dhd_update_iflist_info(dhd_pub_t *dhdp, struct net_device *ndev,
                                  int ifidx, uint8 *mac, uint8 bssidx,
                                  const char *dngl_name, int if_state);
#endif /* WL_STATIC_IF */
...
extern void dhd_sendup_event_common(dhd_pub_t *dhdp, wl_event_msg_t *event,
                                    void *data);
/* Send event to host */
...
extern int dhd_bus_membytes(dhd_pub_t *dhdp, bool set, uint32 address,
...
static INLINE int dhd_os_ds_enter_wake(dhd_pub_t *pub) { return 0; }
#endif /* PCIE_INB_DW */
...
extern int dhd_os_busbusy_wait_condition(dhd_pub_t *pub, uint *var,
                                         uint condition);
...
static INLINE int dhd_os_tput_test_wake(dhd_pub_t *pub) { return 0; }
...
static INLINE int dhd_os_ds_exit_wait(dhd_pub_t *pub, uint *condition) {
  DHD_ERROR(("%s is Not supported for this platform", __FUNCTION__));
  return 0;
}
...
#endif /* PCIE_INB_DW */
static INLINE int dhd_os_busbusy_wait_negation(dhd_pub_t *pub,
                                               uint *condition) {
  return 1;
}
static INLINE int dhd_os_busbusy_wake(dhd_pub_t *pub) { return 0; }
static INLINE int dhd_os_busbusy_wait_condition(dhd_pub_t *pub, uint *var,
                                                uint condition) {
  return 0;
}
...
static INLINE int dhd_os_dmaxfer_wake(dhd_pub_t *pub) { return 0; }
...
#else  /* LINUX */
static INLINE bool dhd_sta_associated(dhd_pub_t *dhdp, uint32 bssidx,
                                      uint8 *mac) {
  return FALSE;
}
static INLINE void *dhd_find_sta(void *pub, int ifidx, void *ea) {
  return NULL;
}
static INLINE void *dhd_findadd_sta(void *pub, int ifidx, void *ea) {
  return NULL;
}
static INLINE void dhd_del_all_sta(void *pub, int ifidx) {}
static INLINE void dhd_del_sta(void *pub, int ifidx, void *ea) {}
static INLINE void dhd_update_sta_chanspec_info(void *pub, int ifidx,
                                                const uint8 *ea,
                                                chanspec_t chanspec) {}
static INLINE bool dhd_is_sta_htput(void *pub, int ifidx, void *ea) {
  return FALSE;
}
static INLINE int dhd_get_ap_isolate(dhd_pub_t *dhdp, uint32 idx) { return 0; }
static INLINE int dhd_set_ap_isolate(dhd_pub_t *dhdp, uint32 idx, int val) {
  return 0;
}
...
#if defined(OEM_ANDROID)
extern int wl_iw_iscan_set_scan_broadcast_prep(struct net_device *dev,
                                               uint flag);
...
#ifdef OEM_ANDROID
#define DEFAULT_DHD_CONSOLE_INTERVAL_MS \
  0 /* android by default disable console msgs */
#else
...
#define CUSTOM_TDLS_IDLE_MODE_SETTING \
  60000 /* 60sec to tear down TDLS of not active */
#endif
#ifndef CUSTOM_TDLS_RSSI_THRESHOLD_HIGH
#define CUSTOM_TDLS_RSSI_THRESHOLD_HIGH \
  -70 /* rssi threshold for establishing TDLS link */
#endif
#ifndef CUSTOM_TDLS_RSSI_THRESHOLD_LOW
#define CUSTOM_TDLS_RSSI_THRESHOLD_LOW \
  -80 /* rssi threshold for tearing down TDLS link */
#endif
#ifndef CUSTOM_TDLS_PCKTCNT_THRESHOLD_HIGH
#define CUSTOM_TDLS_PCKTCNT_THRESHOLD_HIGH \
  100 /* pkt/sec threshold for establishing TDLS link */
#endif
#ifndef CUSTOM_TDLS_PCKTCNT_THRESHOLD_LOW
#define CUSTOM_TDLS_PCKTCNT_THRESHOLD_LOW \
  10 /* pkt/sec threshold for tearing down TDLS link */
#endif
#endif /* WLTDLS */
...
#define MAX_DTIM_SKIP_BEACON_INTERVAL \
  100 /* max allowed associated AP beacon for DTIM skip */
#ifndef MAX_DTIM_ALLOWED_INTERVAL
#define MAX_DTIM_ALLOWED_INTERVAL \
  600 /* max allowed total beacon interval for DTIM skip */
#endif

#ifdef OEM_ANDROID
#ifndef MIN_DTIM_FOR_ROAM_THRES_EXTEND
#define MIN_DTIM_FOR_ROAM_THRES_EXTEND \
  600 /* minimum dtim interval to extend roam threshold */
#endif
#endif /* OEM_ANDROID */
...
extern int dhd_write_file_and_check(const char *filepath, char *buf,
                                    int buf_len);
...
static INLINE int dhd_check_module_mac(dhd_pub_t *dhdp) { return 0; }
    ...
    defined(USE_CID_CHECK) || defined(GET_MAC_FROM_OTP)
...
extern int dhd_check_module_bcm(char *module_type, int index,
                                bool *is_murata_fem);
extern naming_info_t *dhd_find_naming_info(dhd_pub_t *dhdp, char *module_type);
extern naming_info_t *dhd_find_naming_info_by_chip_rev(dhd_pub_t *dhdp,
                                                       bool *is_murata_fem);
...
#else  /* LINUX || linux */
static INLINE int dhd_process_cid_mac(dhd_pub_t *dhdp, bool prepost) {
  return 0;
}
#endif /* LINUX || linux */
...
#define IFLOCK(lock) \
  while (InterlockedCompareExchange((lock), 1, 0)) NdisStallExecution(1);
#define IFUNLOCK(lock) InterlockedExchange((lock), 0)
#define IFLOCK_FREE(lock)
#define FW_SUPPORTED(dhd, capa) \
  ((strstr(dhd->fw_capabilities, " " #capa " ") != NULL))
...
extern int concate_revision(struct dhd_bus *bus, char *fwpath, char *nvpath);
...
extern bool dhd_prec_drop_pkts(dhd_pub_t *dhdp, struct pktq *pq, int prec,
                               f_droppkt_t fn);
...
#define DHD_OS_PREALLOC(dhdpub, section, size) \
  dhd_os_prealloc(dhdpub, section, size, FALSE)
...
#endif /* HNDCTF */
extern void dhd_schedule_trap_log_dump(dhd_pub_t *dhdp, uint8 *buf,
                                       uint32 size);
/* When a new flowid is allocated/deallocated, inform dhd. */
extern void dhd_add_flowid(dhd_pub_t *dhdp, int ifidx, uint8 ac_prio, void *ea,
                           uint16 flowid);
extern void dhd_del_flowid(dhd_pub_t *dhdp, int ifidx, uint16 flowid);
#else /* ! BCM_ROUTER_DHD */
#define dhd_add_flowid(pub, ifidx, ac_prio, ea, flowid) \
  do {                                                  \
  } while (0)
#define dhd_del_flowid(pub, ifidx, flowid) \
  do {                                     \
  } while (0)
...
#define DHD_GENERAL_LOCK(dhdp, flags) (flags) = dhd_os_general_spin_lock(dhdp)
...
#define DHD_BACKPLANE_ACCESS_UNLOCK(lock, flags) \
...
#else
#define DHD_LINUX_GENERAL_LOCK(dhdp, flags) \
  do {                                      \
    BCM_REFERENCE(flags);                   \
  } while (0)
#define DHD_LINUX_GENERAL_UNLOCK(dhdp, flags) \
  do {                                        \
    BCM_REFERENCE(flags);                     \
  } while (0)
#endif
...
/*
 * Temporarily change log dump lock to spin_lock_irqsave as
 * DHD_ERROR/DHD_LOG_MEM are being called from dhdpcie_bus_isr. This will be
 * reverted after proper solution is implemented to handle isr prints
 */
#define DHD_LOG_DUMP_BUF_LOCK(lock, flags) (flags) = osl_spin_lock_irq(lock)
#define DHD_LOG_DUMP_BUF_UNLOCK(lock, flags) \
  osl_spin_unlock_irq((lock), (flags))
...
static INLINE bool dhd_check_cfg_in_progress(dhd_pub_t *dhdp) { return FALSE; }
#endif /* LINUX | linux */
...
extern int dhd_set_qosmap_up_table(dhd_pub_t *dhdp, uint32 idx,
...
#if defined(SET_RPS_CPUS) || defined(SET_XPS_CPUS)
 ...
 * "2" -> print event log messages as formatted CONSOLE_E if logstrs.bin etc.
 * files are available
 */
...
#else
static INLINE void dhd_get_memdump_info(dhd_pub_t *dhd) { return; }
#endif /* linux || LINUX */
...
#if defined(PCIE_FULL_DONGLE)
extern void dmaxfer_free_prev_dmaaddr(dhd_pub_t *dhdp,
...
#define DHD_LB_STATS_UPDATE_NAPI_HISTO(dhdp, x) \
  dhd_lb_stats_update_napi_histo(dhdp, x)
#else /* !DHD_LB_STATS */
...
#else
#define DHD_SSSR_MEMPOOL_INIT(dhdp) \
  do { /* noop */                   \
  } while (0)
#define DHD_SSSR_MEMPOOL_DEINIT(dhdp) \
  do { /* noop */                     \
  } while (0)
#define DHD_SSSR_DUMP_INIT(dhdp) \
  do { /* noop */                \
  } while (0)
#define DHD_SSSR_DUMP_DEINIT(dhdp) \
  do { /* noop */                  \
  } while (0)
#define DHD_SSSR_PRINT_FILEPATH(dhdp, path) \
  do { /* noop */                           \
  } while (0)
#define DHD_SSSR_REG_INFO_INIT(dhdp) \
  do { /* noop */                    \
  } while (0)
#define DHD_SSSR_REG_INFO_DEINIT(dhdp) \
  do { /* noop */                      \
  } while (0)
#endif /* DHD_SSSR_DUMP */
...
#else
#define DHD_COREDUMP_MEMPOOL_INIT(dhdp) \
  do { /* noop */                       \
  } while (0)
#define DHD_COREDUMP_MEMPOOL_DEINIT(dhdp) \
  do { /* noop */                         \
  } while (0)
#endif /* DHD_COREDUMP */
...
static INLINE bool dhd_bus_skip_clm(dhd_pub_t *dhd_pub) { return 0; }
#endif /* BCMPCIE */

#if defined(LINUX) || defined(linux)
void dhd_show_kirqstats(dhd_pub_t *dhd);
#else
static INLINE void dhd_show_kirqstats(dhd_pub_t *dhd) { return; }
#endif /* defined(LINUX) || defined(linux) */
...
static INLINE int dhd_start_scan_timer(dhd_pub_t *pub, bool is_escan) {
  return 0;
}
static INLINE int dhd_stop_scan_timer(dhd_pub_t *pub, bool is_escan,
                                      uint16 sync_id) {
  return 0;
}
...
static INLINE void dhd_get_scan_to_val(dhd_pub_t *pub, uint32 *to_val) {
  return;
}
static INLINE void dhd_set_scan_to_val(dhd_pub_t *pub, uint32 to_val) {
  return;
}
static INLINE void dhd_get_join_to_val(dhd_pub_t *pub, uint32 *to_val) {
  return;
}
static INLINE void dhd_set_join_to_val(dhd_pub_t *pub, uint32 to_val) {
  return;
}
static INLINE void dhd_get_cmd_to_val(dhd_pub_t *pub, uint32 *to_val) {
  return;
}
static INLINE void dhd_set_cmd_to_val(dhd_pub_t *pub, uint32 to_val) { return; }
static INLINE void dhd_get_bus_to_val(dhd_pub_t *pub, uint32 *to_val) {
  return;
}
...
extern void dhd_prhex(const char *msg, volatile uchar *buf, uint nbytes,
                      uint32 dbg_level);
int dhd_tput_test(dhd_pub_t *dhd, tput_test_t *tput_data);
void dhd_tput_test_rx(dhd_pub_t *dhd, void *pkt);
static INLINE int dhd_get_max_txbufs(dhd_pub_t *dhdp) { return -1; }
...
#if defined(DHD_HANG_SEND_UP_TEST)
extern void dhd_make_hang_with_reason(struct net_device *dev,
                                      const char *string_num);
#endif /* DHD_HANG_SEND_UP_TEST */
...
#define DHD_PCIE_CONFIG_RESTORE(bus) \
  do { /* noop */                    \
  } while (0)
#endif /* linux || LINUX */
...
typedef struct dhd_gdb_proxy_probe_data {
  ...
  uint32 hostmem_code_win_base;   /* Hostmem code window start in ARM physical
                                   * address space
                                   */
...
/* Intended usage is to keep repeating timeoutable operation (operation that
 *expected to succeed but may fail because of timeout) while timeout may be
 *explained by firmware stop in gdb Example: dhd_pub_t *dhd;
 *	...
 *	#ifdef GDB_PROXY
...
#define GDB_PROXY_TIMEOUT_WHILE(timeout_cond)                       \
  ...
  while ((timeout_cond) &&                                          \
         ((gdb_dhd->gdb_proxy_stop_count != gdb_prev_stop_count) || \
...
extern int dhd_d2h_h2d_ring_dump(dhd_pub_t *dhd, void *file,
...
#define DHD_EDL_RING_TCM_RD_UPDATE(dhdp) dhd_prot_edl_ring_tcm_rd_update(dhdp)
#else
#define DHD_EDL_MEM_INIT(dhdp) \
  do { /* noop */              \
  } while (0)
#define DHD_EDL_MEM_DEINIT(dhdp) \
  do { /* noop */                \
  } while (0)
...
#if defined(LINUX) || defined(linux)
/* configuration of ecounters. API's tp start/stop. currently supported only for
 * linux */
...
extern void dhd_dump_file_manage_enqueue(dhd_pub_t *dhd, char *dump_path,
                                         char *fname);
#endif /* DHD_DUMP_MNGR */
...
#else
static INLINE void dhd_set_tid_based_on_uid(dhd_pub_t *dhdp, void *pkt) {
  return;
}
#endif /* linux || LINUX */
#endif /* SUPPORT_SET_TID */
...
extern int dhd_sdtc_etb_hal_file_dump(void *dev, const void *user_buf,
                                      uint32 len);
#endif /* DHD_SDTC_ETB_DUMP */
...
/*
 * To avoid OOM, Flow control will be kicked in when packet size in
 * process_queue crosses LB_RXP_STOP_THR * rcpl ring size * 1500(pkt size) and
 * will stop when it goes below LB_RXP_STRT_THR * rcpl ring size * 1500(pkt
 * size) By default flow control is diabled. i.e tunables set to 0/0. to enable
 * flow control, these tunables need to be passed from platform Makefiles.
 */
...
extern void dhd_handle_pktdata(dhd_pub_t *dhdp, int ifidx, void *pkt,
                               uint8 *pktdata, uint32 pktid, uint32 pktlen,
                               uint16 *pktfate, uint8 *dhd_udr, uint8 *dhd_igmp,
                               bool tx, int pkt_wake, bool pkt_log);
#else
static INLINE void dhd_schedule_delayed_dpc_on_dpc_cpu(dhd_pub_t *dhdp,
                                                       ulong delay) {
  return;
}
static INLINE void dhd_handle_pktdata(dhd_pub_t *dhdp, int ifidx, void *pkt,
                                      uint8 *pktdata, uint32 pktid,
                                      uint32 pktlen, uint16 *pktfate,
                                      uint8 *dhd_udr, uint8 *dhd_igmp, bool tx,
                                      int pkt_wake, bool pkt_log) {
  return;
}
#endif /* __linux */

#if defined(BCMPCIE) && defined(__linux__)
extern int dhd_check_shinfo_nrfrags(dhd_pub_t *dhdp, void *pktbuf,
                                    dmaaddr_t *pa, uint32 pktid);
#else
static INLINE int dhd_check_shinfo_nrfrags(dhd_pub_t *dhdp, void *pktbuf,
                                           dmaaddr_t *pa, uint32 pktid) {
  return BCME_OK;
}
...
extern int dhd_dev_set_accel_force_reg_on(struct net_device *dev);
    ...
    defined(DHD_DEBUGABILITY_EVENT_RING)
#ifndef DEBUGABILITY
#error \
    "DHD_DEBUGABILITY_LOG_DUMP_RING or DHD_DEBUGABILITY_EVENT_RING without DEBUGABILITY"
#endif /* DEBUGABILITY */
#endif /* defined(DHD_DEBUGABILITY_LOG_DUMP_RING) || \
          defined(DHD_DEBUGABILITY_EVENT_RING) */
#ifdef WL_CFGVENDOR_SEND_ALERT_EVENT
...
void *BCMFASTPATH(dhd_rxpool_pktget)(osl_t *osh, struct dhd_info *dhd,
                                     uint16 len);
#endif /* RX_PKT_POOL */
...
static INLINE struct file *dhd_filp_open(const char *filename, int flags,
                                         int mode) {
...
static INLINE int dhd_kernel_read_compat(struct file *fp, loff_t pos, void *buf,
                                         size_t count) {
  return kernel_read_compat(fp, pos, buf, count);
}

static INLINE int dhd_vfs_read(struct file *filep, char *buf, size_t size,
                               loff_t *pos) {
  return vfs_read(filep, buf, size, pos);
}

static INLINE int dhd_vfs_write(struct file *filep, char *buf, size_t size,
                                loff_t *pos) {
...
#define DHD_VFS_UNLINK(dir, file_path, c) \
  vfs_unlink(DHD_VFS_INODE(dir), file_path.dentry)
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0))
#define DHD_VFS_UNLINK(dir, file_path, c) \
  vfs_unlink(mnt_idmap(file_path.mnt), DHD_VFS_INODE(dir), file_path.dentry, c)
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0))
#define DHD_VFS_UNLINK(dir, file_path, c)                                      \
  vfs_unlink(mnt_user_ns(file_path.mnt), DHD_VFS_INODE(dir), file_path.dentry, \
             c)
#else
#define DHD_VFS_UNLINK(dir, file_path, c) \
  vfs_unlink(DHD_VFS_INODE(dir), file_path.dentry, c)
...
static INLINE int dhd_filp_close(void *image, void *id) { return 0; }
static INLINE int dhd_i_size_read(const struct inode *inode) { return 0; }
static INLINE int dhd_kernel_read_compat(struct file *fp, loff_t pos, void *buf,
                                         size_t count) {
  return 0;
}
...
static INLINE int dhd_vfs_fsync(struct file *filep, int datasync) { return 0; }
static INLINE long dhd_vfs_size_read(struct file *fp) { return 0; }
static INLINE int dhd_kern_path(char *name, int flags, struct path *file_path) {
  return 0;
}
