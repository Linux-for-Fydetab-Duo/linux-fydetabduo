...
enum rtt_schedule_reason {
  ...
  RTT_SCHED_RNG_TERM_SUB_SVC_CANCEL =
      12,                              /* Due rng canc attempt, on sub cancel */
  RTT_SCHED_RNG_TERM_SUB_SVC_UPD = 13, /* Due rng canc attempt, on sub update */
  RTT_SCHED_RNG_TERM_PUB_RNG_CLEAR =
      14,                      /* Due rng canc attempt, on pub upd/timeout */
  RTT_SCHED_RNG_RESP_IND = 15, /* Due to rng resp ind */
  RTT_SCHED_RNG_DIR_EXCESS_TARGET =
      16 /* On ssn end, if excess dir tgt pending */
};
...
typedef struct rtt_status_info {
  ...
  uint32
      flags; /* indicate whether device is configured as initiator or target */
  struct capability {
    int32 proto : 8;
    int32 feature : 8;
    int32 preamble : 8;
