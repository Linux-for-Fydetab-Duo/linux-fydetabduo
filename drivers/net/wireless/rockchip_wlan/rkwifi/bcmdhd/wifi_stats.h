...
typedef enum { WIFI_ROAMING_IDLE = 0, WIFI_ROAMING_ACTIVE = 1 } wifi_roam_state;
...
typedef enum {
   ...
   * NAN3.1 Instant communication mode. This filter should only be applied to
   * NAN interface. If 5G is supported default discovery channel 149/44 is
   * considered, If 5G is not supported then channel 6 has to be considered.
   * Based on regulatory domain if channel 149 and 44 are restricted, channel 6
   * should be considered for instant communication channel
   */
...
#define WIFI_CAPABILITY_PROTECTED                       \
  0x00000002 /* set for protected association (802.11   \
              * beacon frame control protected bit set) \
              */
#define WIFI_CAPABILITY_INTERWORKING                                           \
  0x00000004                            /* set if 802.11 Extended Capabilities \
                                         * element interworking bit is set     \
                                         */
#define WIFI_CAPABILITY_HS20 0x00000008 /* set for HS20 association */
#define WIFI_CAPABILITY_SSID_UTF8                   \
  0x00000010 /* set is 802.11 Extended Capabilities \
              * element UTF-8 SSID bit is set       \
              */
#define WIFI_CAPABILITY_COUNTRY \
  0x00000020 /* set is 802.11 Country Element is present */
...
typedef struct {
  ...
  uint8 ap_country_str[3];            /* country string advertised by AP */
  uint8 country_str[3];               /* country string for this association */
  uint8 time_slicing_duty_cycle_percent; /* if this iface is being served using
                                          * time slicing on a radio with one or
                                          * more ifaces (i.e MCC), then the duty
                                          * cycle assigned to this iface in %.
                                          * If not using time slicing (i.e SCC
                                          * or DBS), set to 100.
                                          */
...
typedef struct {
  ...
  wifi_rate_stat
      rate_stats[1]; /* per rate statistics, number of entries  = num_rate */
} wifi_peer_info_v1;
...
typedef struct {
  ...
  uint32 contention_num_samples; /* num of data pkts used for contention
                                    statistics */
} wifi_wmm_ac_stat;
...
typedef struct {
  ...
  uint32 beacon_rx; /* access point beacon received count from
                     ...
                     */
  uint64
      average_tsf_offset;   /* average beacon offset encountered (beacon_TSF -
                             * TBTT)   The average_tsf_offset field is used so as to
                             * calculate   the typical beacon contention time on the
                             * channel as well   may be used to debug beacon
                             * synchronization and related   power consumption issue
                             */
  uint32 leaky_ap_detected; /* indicate that this AP
                             * typically leaks packets beyond
                             * the driver guard time.
                             */
  uint32 leaky_ap_avg_num_frames_leaked; /* average number of frame leaked by AP
                                          * after frame with PM bit set was
                                          * ACK'ed by AP
                                          */
  uint32 leaky_ap_guard_time;            /* guard time currently in force
                                          ...
                                          * driver waits before shutting down the radio and
                                          * after            receiving an ACK for a data frame with PM
                                          * bit set)
                                          */
