...
static inline int dhd_rxf_enqueue(dhd_pub_t *dhdp, void *skb) {
  ...
  if (dhdp->skbbuf[store_idx] != NULL) {
    ...
    DHD_TRACE(
        ("dhd_rxf_enqueue: pktbuf not consumed %p, store idx %d sent idx %d\n",
...
static inline void *dhd_rxf_dequeue(dhd_pub_t *dhdp) {
  ...
  if (skb == NULL) {
    dhd_os_rxfunlock(dhdp);
    DHD_ERROR(
        ("dhd_rxf_dequeue: Dequeued packet is NULL, store idx %d sent idx %d\n",
         store_idx, sent_idx));
    return NULL;
  }
...
#ifdef DHD_WMF
bool dhd_is_rxthread_enabled(dhd_pub_t *dhdp) {
...
void dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt,
  ...
  if (dhdp->conf->pktsetsum) PKTSETSUMGOOD(pktbuf, TRUE);
#endif
  ...
  for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
     ...
     * special ifidx of DHD_DUMMY_INFO_IF.  This is just internal to dhd to get
     * the data from dhd_msgbuf.c:dhd_prot_infobuf_cmplt_process() to here
     * (dhd_rx_frame).
     */
    if (ifidx == DHD_DUMMY_INFO_IF) {
       ...
       * Also in the new Dongle SW(4359, 4355 onwards) console prints too come
       * as events with type WLC_E_TRACE. We'll print this console logs from the
       * WorkQueue context by enqueing SKB here and Dequeuing will be done in
       * WorkQueue and will be freed only if logtrace_pkt_sendup is TRUE
       */
#ifdef SHOW_LOGTRACE
    ...
    if (dhdp->stop_in_progress) {
      DHD_ERROR_RLMT(
          ("%s: dhd_stop in progress ignore received packet\n", __FUNCTION__));
    ...
    if (ifidx >= DHD_MAX_IFS) {
      DHD_ERROR(
          ("%s: ifidx(%d) Out of bound. drop packet\n", __FUNCTION__, ifidx));
      RX_PKTFREE(dhdp->osh, eh->ether_type, pktbuf, FALSE);
      continue;
    }
    ...
    /* Dropping only data packets before registering net device to avoid kernel
     * panic */
    ...
    if (dhd_tcpdata_get_flag(dhdp, pktbuf) == FLAG_SYNC) {
      ...
      if (delta_sec > 1) {
        ...
        if (sync_per_sec > TCP_SYNC_FLOOD_LIMIT) {
          ...
          DHD_ERROR(
              ("ifx %d TCP SYNC Flood attack suspected! "
    ...
    dhd_handle_pktdata(dhdp, ifidx, skb, dump_data, FALSE, len, NULL, NULL,
                       NULL, FALSE, pkt_wake, TRUE);
    ...
    if (ntoh16(skb->protocol) == ETHER_TYPE_BRCM) {
      ...
      if (ret_event != BCME_OK) {
        DHD_ERROR(
            ("%s: wl_host_event_get_data err = %d\n", __FUNCTION__, ret_event));
        PKTFREE_CTRLBUF(dhdp->osh, pktbuf, FALSE);
        continue;
      }
...
#if defined(OEM_ANDROID)
      if (!tout_ctrl) tout_ctrl = DHD_PACKET_TIMEOUT_MS;
#endif /* OEM_ANDROID */
      ...
      if (numpkt != 1) {
        DHD_TRACE(
            ("%s: Got BRCM event packet in a chained packet.\n", __FUNCTION__));
      }
      ...
      if ((event_type == WLC_E_IF) && (ret_event > 0)) {
        DHD_ERROR(
            ("%s: interface is deleted. Free event packet\n", __FUNCTION__));
        PKTFREE_CTRLBUF(dhdp->osh, pktbuf, FALSE);
        continue;
      }
...
#undef ETHER_ICMPV6_TYPE
  ...
  if (dhd->rxthread_enabled && skbhead) dhd_sched_rxf(dhdp, skbhead);
...
#else  /* RXF_DEQUEUE_ON_BUSY */
  do {
    if (dhd_rxf_enqueue(dhdp, skb) == BCME_OK) break;
  } while (1);
  if (dhd->thr_rxf_ctl.thr_pid >= 0) {
    up(&dhd->thr_rxf_ctl.sema);
  } else {
...
#ifdef WL_MONITOR
#ifdef BCMSDIO
void dhd_rx_mon_pkt_sdio(dhd_pub_t *dhdp, void *pkt, int ifidx) {
  dhd_info_t *dhd = (dhd_info_t *)dhdp->info;

  if (!dhd->monitor_skb) {
    if ((dhd->monitor_skb = PKTTONATIVE(dhdp->osh, pkt)) == NULL) return;
  }
  ...
  void dhd_rx_mon_pkt(dhd_pub_t * dhdp, host_rxbuf_cmpl_t * msg, void *pkt,
    ...
    if (dhd->host_radiotap_conv) {
      ...
      len = bcmwifi_monitor(dhd->monitor_info, &pkt_info,
                            PKTDATA(dhdp->osh, pkt), PKTLEN(dhdp->osh, pkt),
                            PKTDATA(dhdp->osh, dhd->monitor_skb), &offset);
...
#endif /* HOST_RADIOTAP_CONV */
    {
      ...
      switch (amsdu_flag) {
        ...
        case BCMPCIE_PKT_FLAGS_MONITOR_FIRST_PKT:
          ...
          memcpy(PKTDATA(dhdp->osh, dhd->monitor_skb), PKTDATA(dhdp->osh, pkt),
                 PKTLEN(dhdp->osh, pkt));
    ...
    if (in_interrupt()) {
       ...
       * to do it manually.
       */
      bcm_object_trace_opr(dhd->monitor_skb, BCM_OBJDBG_REMOVE, __FUNCTION__,
                           __LINE__);
...
static int dhd_wl_host_event(dhd_info_t *dhd, int ifidx, void *pktdata,
                             uint16 pktlen, wl_event_msg_t *event,
                             void **data) {
  int bcmerror = 0;
#ifdef WL_CFG80211
...
int net_os_rxfilter_add_remove(struct net_device *dev, int add_remove,
                               int num) {
  int ret = 0;
  ...
  if (!dhd_master_mode) add_remove = !add_remove;
...
int net_os_wake_lock_rx_timeout_enable(struct net_device *dev, int val) {
  ...
  if (dhd) ret = dhd_os_wake_lock_rx_timeout_enable(&dhd->pub, val);
  return ret;
}
...
#ifdef RX_PKT_POOL
void *BCMFASTPATH(dhd_rxpool_pktget)(osl_t *osh, dhd_info_t *dhd, uint16 len) {
  void *p = NULL;
  ...
  if (dhd->rx_pktpool_thread.thr_pid >= 0) {
    if (rx_pool->rxbuf_sz == len) {
      ...
      DHD_ERROR_RLMT(
          ("%s: PKTGET_RX_POOL is called with length %u, "
           "but pkt pool created with length : %u\n",
           __FUNCTION__, len, rx_pool->rxbuf_sz));
    }
  } else {
...
static int dhd_rx_pktpool_thread(void *data) {
  tsk_ctl_t *tsk = (tsk_ctl_t *)data;
  dhd_info_t *dhd = (dhd_info_t *)tsk->parent;
  ...
  while (1) {
    if (!binary_sema_down(tsk)) {
      ...
      if (rx_pool->rxbuf_sz) {
        ...
        DHD_TRACE(
            ("%s: before alloc - skb_q len=%u, max_size=%u "
             "rxbuf_sz : %u\n",
             __FUNCTION__, qlen, rx_pool->max_size, rx_pool->rxbuf_sz));
        num_attempts = 0;
...
void dhd_rx_pktpool_create(dhd_info_t *dhd, uint16 rxbuf_sz) {
  pkt_pool_t *rx_pool = &dhd->rx_pkt_pool;
...
void dhd_rx_pktpool_init(dhd_info_t *dhd) {
  ...
  PROC_START(dhd_rx_pktpool_thread, dhd, &dhd->rx_pktpool_thread, 0,
             "dhd_rx_pktpool_thread");
}
