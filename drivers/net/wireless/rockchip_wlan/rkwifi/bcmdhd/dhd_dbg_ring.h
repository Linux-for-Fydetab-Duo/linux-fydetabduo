...
#define DBGRING_FLUSH_THRESHOLD(ring)                       \
  (ring->id != PACKET_LOG_RING_ID) ? (ring->ring_size / 3u) \
                                   : (ring->ring_size / 4u)
#define RING_STAT_TO_STATUS(ring, status)                                    \
  do {                                                                       \
    /* status.name/ring->name are the same length so no need to check return \
     * value */                                                              \
    (void)memcpy_s(status.name, sizeof(status.name), ring->name,             \
                   sizeof(ring->name));                                      \
...
#define DBG_RING_READ_AVAIL_SPACE(w, r, d) \
  (((w) >= (r)) ? ((w) - (r)) : ((d) - (r)))
#define DBG_RING_WRITE_SPACE_AVAIL_CONT(r, w, d) \
  (((w) >= (r)) ? ((d) - (w)) : ((r) - (w)))
#define DBG_RING_WRITE_SPACE_AVAIL(r, w, d) (d - (TXACTIVESZ(r, w, d)))
#define DBG_RING_CHECK_WRITE_SPACE(r, w, d) \
  MIN(DBG_RING_WRITE_SPACE_AVAIL(r, w, d),  \
      DBG_RING_WRITE_SPACE_AVAIL_CONT(r, w, d))
