...
#define CFG80211_READY_ON_CHANNEL(cfgdev, cookie, channel, channel_type, \
                                  duration, flags)                       \
  cfg80211_ready_on_channel(cfgdev, cookie, channel, duration, GFP_KERNEL);
#else
#define CFG80211_READY_ON_CHANNEL(cfgdev, cookie, channel, channel_type,     \
                                  duration, flags)                           \
  cfg80211_ready_on_channel(cfgdev, cookie, channel, channel_type, duration, \
                            GFP_KERNEL);
...
static wl_scan_results_v109_t *wl_escan_get_buf(struct bcm_cfg80211 *cfg,
                                                bool aborted) {
  u8 index;
  if (aborted) {
...
static int wl_escan_check_sync_id(struct bcm_cfg80211 *cfg, s32 status,
                                  u16 result_id, u16 wl_id) {
  if (result_id != wl_id) {
    WL_ERR(
        ("ESCAN sync id mismatch :status :%d "
    ...
    if (cfg->escan_info.prev_escan_aborted == FALSE) {
      wl_cfg80211_handle_hang_event(bcmcfg_to_prmry_ndev(cfg),
                                    HANG_REASON_ESCAN_SYNCID_MISMATCH,
...
#define wl_escan_increment_sync_id(a, b)      \
  ((u8)((a)->escan_info.cur_sync_id + b) == 0 \
       ? ((a)->escan_info.cur_sync_id = 1)    \
       : ((a)->escan_info.cur_sync_id += b))
...
#else
#define wl_escan_get_buf(a, b) \
  ((wl_scan_results_v109_t *)(a)->escan_info.escan_buf)
...
static void wl_update_hidden_ap_ie(wl_bss_info_v109_t *bi, const u8 *ie_stream,
                                   u32 *ie_size, bool update_ssid,
                                   u32 *ssid_len_from_ie) {
  u8 *ssidie;
   ...
   * 3. If in case of erroneous buffer input where ssid length doesnt match the
  ...
  *ssid_len_from_ie = (u32)ssidie[1];
  ...
  if (ssidie[1] > available_buffer_len) {
    WL_ERR_MEM(
        ("wl_update_hidden_ap_ie: skip wl_update_hidden_ap_ie : overflow\n"));
    return;
  }
  ...
  if (ssidie[1] != ssid_len) {
     ...
     * will not carry SSID and this can result in cfg80211_get_bss not finding a
     * match. so include the SSID element.
     */
...
static s32 wl_mrg_ie(struct bcm_cfg80211 *cfg, u8 *ie_stream, u16 ie_size) {
  ...
  if (ret) {
    WL_ERR(("memcpy failed:%d, destsz: %zu, n: %d\n", ret,
            (sizeof(ie->buf) - ie->offset), ie_size));
    return BCME_ERROR;
  }
  ie->offset += ie_size;
...
s32 wl_inform_single_bss(struct bcm_cfg80211 *cfg, wl_bss_info_v109_t *bi,
                         bool update_ssid) {
  ...
  wl_cp_ie(cfg, beacon_proberesp->variable,
           WL_BSS_INFO_MAX - offsetof(struct wl_cfg80211_bss_info, frame_buf));
  ...
  WL_SCAN((
      "BSSID %pM, channel %s-%-3d(%3d %3sMHz), rssi %3d, capa 0x%-4x, "
      "mgmt_type %d, "
      "frame_len %3d, SSID \"%s\"\n",
  ...
  if (!mgmt->u.probe_resp.timestamp) {
    ...
    mgmt->u.probe_resp.timestamp =
        ((u64)ts.tv_sec * USEC_PER_SEC) + (ts.tv_nsec / NSEC_PER_USEC);
#else
    struct osl_timespec tv;
    osl_do_gettimeofday(&tv);
    mgmt->u.probe_resp.timestamp = ((u64)tv.tv_sec * 1000000) + tv.tv_usec;
#endif
  }
  ...
  if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID) &&
    ...
    if (!event_data) {
      WL_ERR(
          ("%s: failed to allocate the log_conn_event_t with "
           "length(%d)\n",
           __func__, alloc_len));
      goto out_err;
    }
...
struct net_device *wl_get_scan_ndev(struct bcm_cfg80211 *cfg) {
...
void wl_cfg80211_update_bss_cache(struct bcm_cfg80211 *cfg) {
  ...
  int rssi;
#endif
  wl_scan_results_v109_t *bss_list = cfg->bss_list;
...
#ifdef WL_EXT_IAPSTA
  if (ndev) wl_ext_in4way_sync(ndev, 0, WL_EXT_STATUS_SCAN_COMPLETE, NULL);
#endif
...
#if defined(BSSCACHE)
    wl_ext_get_best_channel(ndev, &cfg->g_bss_cache_ctrl, &cfg->best_2g_ch,
                            &cfg->best_5g_ch, &cfg->best_6g_ch);
#else
...
#ifdef WL11U
bcm_tlv_t *wl_cfg80211_find_interworking_ie(const u8 *parse, u32 len) {
  bcm_tlv_t *ie;
...
s32 wl_cfg80211_clear_iw_ie(struct bcm_cfg80211 *cfg, struct net_device *ndev,
                            s32 bssidx) {
...
s32 wl_cfg80211_add_iw_ie(struct bcm_cfg80211 *cfg, struct net_device *ndev,
                          s32 bssidx, s32 pktflag, uint8 ie_id, uint8 *data,
                          uint8 data_len) {
  s32 err = BCME_OK;
  s32 buf_len;
  ie_setbuf_t *ie_setbuf;
  ...
  if (scan_status == WLC_E_STATUS_RXBCN) {
    ...
    if (ret) {
      WL_ERR(("memcpy failed:%d, destsz:%lu, n:%d\n", ret,
              sizeof(bcn_recv->SSID), bi->SSID_len));
      err = BCME_ERROR;
      goto exit;
    }
...
static void wl_cfg80211_find_removal_candidate(wl_bss_info_v109_t *bss,
  ...
  for (idx = 0; idx < BUF_OVERFLOW_MGMT_COUNT; idx++) {
    ...
    if (BSSRSSI(bss) < candidate[idx].RSSI) {
      if (len) {
         ...
         */
        (void)memcpy_s(&candidate[idx + 1], (sizeof(removal_element_t) * len),
...
static void wl_cfg80211_remove_lowRSSI_info(wl_scan_results_v109_t *list,
                                            removal_element_t *candidate,
  ...
  for (idx1 = 0; idx1 < BUF_OVERFLOW_MGMT_COUNT; idx1++) {
    ...
    if (candidate[idx1].RSSI >= BSSRSSI(bi)) continue;
...
s32 wl_escan_handler(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
  ...
  if (!ndev ||
    ...
    WL_ERR_RLMT(
        ("escan is not ready. drv_scan_status 0x%x"
         " e_type %d e_status %d\n",
         wl_get_drv_status(cfg, SCANNING, ndev), ntoh32(e->event_type),
         ntoh32(e->status)));
    goto exit;
  }
  ...
  if (status == WLC_E_STATUS_PARTIAL) {
    ...
    if (bi_length !=
        (dtoh32(escan_result->buflen) - WL_ESCAN_RESULTS_V109_FIXED_SIZE)) {
    ...
    if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
      ...
      if (p2p_dev_addr && !memcmp(p2p_dev_addr, cfg->afx_hdl->tx_dst_addr.octet,
                                  ETHER_ADDR_LEN)) {
...
#ifdef ESCAN_BUF_OVERFLOW_MGMT
      if (bi_length > ESCAN_BUF_SIZE - list->buflen) remove_lower_rssi = TRUE;
#endif /* ESCAN_BUF_OVERFLOW_MGMT */
      ...
      for (i = 0; i < list->count; i++) {
        ...
        WL_DBG(("%s(" MACDBG "), i=%d bss: RSSI %d list->count %d\n", bss->SSID,
                MAC2STRDBG(bss->BSSID.octet), i, bss->RSSI, list->count));
        ...
        if (!bcmp(&bi->BSSID, &bss->BSSID, ETHER_ADDR_LEN) &&
          ...
          WL_DBG(("%s(" MACDBG "), i=%d prev: RSSI %d"
                  ...
                  bss->SSID, MAC2STRDBG(bi->BSSID.octet), i, bss->RSSI,
                  bss->flags, bi->RSSI, bi->flags));
          ...
          if ((bss->flags & WL_BSS_FLAGS_RSSI_ONCHANNEL) ==
            ...
            WL_DBG(("%s(" MACDBG "), same onchan"
                    ", RSSI: prev %d new %d\n",
                    bss->SSID, MAC2STRDBG(bi->BSSID.octet), bss->RSSI,
                    bi->RSSI));
          ...
          if (dtoh32(bss->length) != bi_length) {
            ...
            WL_DBG(("%s(" MACDBG "), replacement!(%d -> %d)\n", bss->SSID,
                    MAC2STRDBG(bi->BSSID.octet), prev_len, bi_length));

            if ((list->buflen - prev_len) + bi_length > ESCAN_BUF_SIZE) {
              WL_ERR(
                  ("Buffer is too small: keep the"
                   " previous result of this AP\n"));
              /* Only update RSSI */
              bss->RSSI = bi->RSSI;
              bss->flags |= (bi->flags & WL_BSS_FLAGS_RSSI_ONCHANNEL);
              goto exit;
            }
          ...
          if ((err = memcpy_s((u8 *)bss,
            ...
            WL_ERR(
                ("Failed to copy the recent bss_info."
                 "err:%d recv_len:%d bi_len:%d\n",
                 err, ESCAN_BUF_SIZE - (list->buflen - bi_length), bi_length));
    ...
    if ((dhdp->dhd_induce_error == DHD_INDUCE_SCAN_TIMEOUT) ||
      ...
      WL_ERR(("Skip escan complete to induce scantimeout\n"));
      err = BCME_ERROR;
      goto exit;
    }
    ...
    if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
      ...
      if (cfg->afx_hdl->peer_chan == WL_INVALID) complete(&cfg->act_frm_scan);
    ...
    if (wl_get_drv_status_all(cfg, FINDING_COMMON_CHANNEL)) {
      ...
      if (!scan_req_match(cfg)) {
        WL_TRACE_HW4(
            ("SCAN ABORTED: scanned AP count=%d\n", cfg->bss_list->count));
      }
#ifdef DUAL_ESCAN_RESULT_BUFFER
...
#ifdef WL_SCHED_SCAN
s32 wl_cfgscan_pfn_handler(struct bcm_cfg80211 *cfg,
  ...
  if (!bi) {
    WL_ERR(
        ("Invalid pfn bss info (NULL pointer)"
...
s32 wl_cfgscan_pfn_scanresult_handler(struct bcm_cfg80211 *cfg,
                                      bcm_struct_cfgdev *cfgdev,
                                      const wl_event_msg_t *e, void *data) {
  s32 err = BCME_OK;
  wl_pfn_scanresult_v3_1_t *pfn_scanresult;
...
s32 wl_cfgscan_notify_pfn_complete(struct bcm_cfg80211 *cfg,
                                   bcm_struct_cfgdev *cfgdev,
                                   const wl_event_msg_t *e, void *data) {
  s32 err = BCME_OK;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0))
    cfg80211_sched_scan_results(wiphy, cfg->sched_scan_req->reqid);
#else
...
static const u8 *wl_retrieve_wps_attribute(const u8 *buf, u16 element_id) {
...
bool wl_is_wps_enrollee_active(struct net_device *ndev, const u8 *ie_ptr,
                               u16 len) {
  const u8 *ie;
  const u8 *attrib;
...
static s32 wl_find_listen_channel(struct bcm_cfg80211 *cfg, const u8 *ie,
  ...
  while (pos < end) {
    ...
    if (pos[0] == 6) {
      ...
      if (listen_channel == SOCIAL_CHAN_1 || listen_channel == SOCIAL_CHAN_2 ||
...
static u32 wl_cfgscan_map_nl80211_scan_type(
    struct bcm_cfg80211 *cfg, struct cfg80211_scan_request *request) {
  u32 scan_flags = 0;
...
#ifdef SCAN_SUPPRESS
static void wl_cfgscan_populate_scan_channel(
    struct bcm_cfg80211 *cfg, struct ieee80211_channel **channels,
    u32 n_channels, u16 *channel_list, struct wl_chan_info *chan_info) {
  u32 i, chanspec = 0;
  ...
  WL_SCAN(("chan: %s-%d, chanspec: %x\n", WLCBAND2STR(chan_info->band),
           chan_info->chan, chanspec));
}
#endif

static void wl_cfgscan_populate_scan_channels(
    struct bcm_cfg80211 *cfg, struct ieee80211_channel **channels,
    u32 n_channels, u16 *channel_list, u32 *num_channels, bool use_chanspecs,
    bool skip_dfs) {
  u32 i = 0, j = 0;
  u32 chanspec = 0;
  ...
  for (i = 0; i < n_channels; i++) {
    ...
    support_chanspec =
        wl_channel_to_chanspec(cfg->wdev->wiphy, bcmcfg_to_prmry_ndev(cfg),
                               CHSPEC_CHANNEL(chanspec), WL_CHANSPEC_BW_80);
    if (!support_chanspec) continue;
    ...
    if (channels[i]->band == IEEE80211_BAND_2GHZ) {
      if ((cfg->curr_band == WLC_BAND_5G) || (cfg->curr_band == WLC_BAND_6G)) {
        if
          !(is_p2p_scan && IS_P2P_SOCIAL_CHANNEL(CHSPEC_CHANNEL(chanspec))) {
    ...
    if (is_p2p_scan) {
      ...
      if (CHSPEC_IS5G(chanspec) &&
          (CHSPEC_CHANNEL(chanspec) >= 52 && CHSPEC_CHANNEL(chanspec) <= 144)) {
        if (is_printed == false) {
    ...
    WL_SCAN(("chan: %s-%d, chanspec: %x\n", CHSPEC2BANDSTR(chanspec),
             wf_chspec_ctlchan(chanspec), channel_list[j]));
    j++;
...
static s32 wl_scan_prep(struct bcm_cfg80211 *cfg, struct net_device *ndev,
...
#endif /* WL_SCAN_TYPE */
    (void)memcpy_s(&params_v3->bssid, ETHER_ADDR_LEN, &ether_bcast,
                   ETHER_ADDR_LEN);
  ...
  WL_INFORM(("n_channels:%d n_ssids:%d ver:%d\n", request->n_channels,
             request->n_ssids, cfg->scan_params_ver));

  cur_offset = channel_offset;
  /* Copy channel array if applicable */
#ifdef SCAN_SUPPRESS
  channel =
      wl_ext_scan_suppress(ndev, scan_params, cfg->scan_params_ver, &chan_info);
  if (channel) {
    n_channels = 1;
    if ((n_channels > 0) && chan_list) {
      if (len >= (scan_param_size + (n_channels * sizeof(u16)))) {
        wl_cfgscan_populate_scan_channel(
            cfg, request->channels, request->n_channels, chan_list, &chan_info);
    ...
    if (len >= (scan_param_size + (request->n_channels * sizeof(u16)))) {
      wl_cfgscan_populate_scan_channels(cfg, request->channels,
                                        request->n_channels, chan_list,
  ...
  if (request->n_ssids > 0) {
    ...
    if (len > (cur_offset + (request->n_ssids * sizeof(wlc_ssid_t)))) {
      ...
      wl_cfgscan_populate_scan_ssids(cfg, ((u8 *)scan_params + cur_offset),
  ...
  if (n_ssids || n_channels) {
    ...
    if (params_v3) {
      ...
      if (n_channels == 1) {
        ...
        params_v3->nprobes =
            htod32(params_v3->active_time / WL_SCAN_JOIN_PROBE_INTERVAL_MS);
      }
    } else {
      ...
      if (n_channels == 1) {
        ...
        params->nprobes =
            htod32(params->active_time / WL_SCAN_JOIN_PROBE_INTERVAL_MS);
      }
    }
  }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) && \
    defined(SUPPORT_RANDOM_MAC_SCAN)
static s32 wl_config_scan_macaddr(struct bcm_cfg80211 *cfg,
                                  struct net_device *ndev, bool randmac_enable,
  ...
  if (randmac_enable) {
    ...
    if (!mac_addr_mask && !mac_addr && !ETHER_ISNULLADDR(mac_addr_mask)) {
      err = wl_cfg80211_scan_mac_config(ndev, mac_addr, mac_addr_mask);
      if (unlikely(err)) {
...
static s32 wl_run_escan(struct bcm_cfg80211 *cfg, struct net_device *ndev,
  ...
  if (IS_SCAN_PARAMS_V3_V2(cfg)) {
    params_size =
        (WL_SCAN_PARAMS_V3_FIXED_SIZE + OFFSETOF(wl_escan_params_v3_t, params));
  } else {
    params_size =
        (WL_SCAN_PARAMS_V1_FIXED_SIZE + OFFSETOF(wl_escan_params_v1_t, params));
  }
  ...
  if (!cfg->p2p_supported || !p2p_scan(cfg)) {
    ...
    if (ndev == bcmcfg_to_prmry_ndev(cfg) && g_first_broadcast_scan == true) {
      ...
      for (i = 0; i < request->n_channels; i++) {
        ...
        if (j > CH_MAX_2G_CHANNEL) {
          WL_ERR(
              ("Index %d exceeds max 2.4GHz channels %d"
    ...
    n_ssids = request->n_ssids;
    if (n_channels % 2) /* If n_channels is odd, add a padd of u16 */
    ...
    if (unlikely(err)) {
      if (err == BCME_EPERM) /* Scan Not permitted at this point of time */
    ...
    if (request->n_channels) {
      ...
      wl_cfgscan_populate_scan_channels(cfg, request->channels,
                                        request->n_channels, default_chan_list,
                                        &num_chans, true, true);

      if (num_chans == SOCIAL_CHAN_CNT &&
          ((CHSPEC_CHANNEL(default_chan_list[0]) == SOCIAL_CHAN_1) &&
           (CHSPEC_CHANNEL(default_chan_list[1]) == SOCIAL_CHAN_2) &&
           (CHSPEC_CHANNEL(default_chan_list[2]) == SOCIAL_CHAN_3))) {
    ...
    if (!err) cfg->p2p->search_state = search_state;

    MFREE(cfg->osh, default_chan_list, chan_mem);
  } else {
    WL_ERR(
        ("escan not triggered - p2p_supported=%d, p2p_scan=%d, p2p_is_on=%d\n",
...
s32 wl_do_escan(struct bcm_cfg80211 *cfg, struct wiphy *wiphy,
  ...
  if (passive_channel_skip) {
    err =
        wldev_ioctl_get(ndev, WLC_GET_SCAN_PASSIVE_TIME, &passive_scan_time_org,
    ...
    passive_scan_time = 0;
    err = wldev_ioctl_set(ndev, WLC_SET_SCAN_PASSIVE_TIME, &passive_scan_time,
  ...
  if (passive_channel_skip) {
    err =
        wldev_ioctl_set(ndev, WLC_SET_SCAN_PASSIVE_TIME, &passive_scan_time_org,
                        sizeof(passive_scan_time_org));
    if (unlikely(err)) {
      WL_ERR(("== error (%d)\n", err));
      goto exit;
...
static bool is_p2p_ssid_present(struct cfg80211_scan_request *request) {
...
static void wl_set_p2p_scan_states(struct bcm_cfg80211 *cfg,
                                   struct net_device *ndev) {
...
static void wl_set_legacy_scan_states(struct bcm_cfg80211 *cfg,
                                      struct cfg80211_scan_request *request,
  ...
  if (request && (interworking_ie = wl_cfg80211_find_interworking_ie(
    ...
    if ((err = wl_cfg80211_add_iw_ie(cfg, ndev, bssidx, VNDR_IE_CUSTOM_FLAG,
...
static bool is_scan_allowed(struct bcm_cfg80211 *cfg, struct net_device *ndev) {
...
s32 __wl_cfg80211_scan(struct wiphy *wiphy, struct net_device *ndev,
...
#else
      if (!(IS_P2P_IFACE(ndev->ieee80211_ptr)))
#endif
      {
  ...
  if (cfg->p2p_supported && p2p_ssid) {
    ...
    if (wl_get_p2p_status(cfg, DISC_IN_PROGRESS)) {
       ...
       */
      WL_SD(
          ("P2P_FIND: Discovery offload is in progress."
           " Do nothing\n"));
      err = -EINVAL;
      goto scan_out;
    }
  ...
  if (request) {
    ...
    err = wl_cfg80211_set_mgmt_vndr_ies(cfg, ndev_to_cfgdev(ndev), bssidx,
                                        VNDR_IE_PRBREQ_FLAG, request->ie,
                                        request->ie_len);
    if (unlikely(err)) {
...
#ifdef WL_EXT_IAPSTA
  mutex_unlock(&cfg->scan_sync);
  err = wl_ext_in4way_sync(ndev, STA_FAKE_SCAN_IN_CONNECT,
...
/* Use wl_cfgscan_cancel_scan function for scan abort, as this would do a FW
 * abort followed by indication to upper layer, the current function
 * wl_cfgscan_scan_abort, does only FW abort.
 */
void wl_cfgscan_scan_abort(struct bcm_cfg80211 *cfg) {
  void *params = NULL;
  s32 params_size = 0;
...
static s32 wl_notify_escan_complete(struct bcm_cfg80211 *cfg,
  ...
  if (cfg->scan_request) {
    ...
    if (cfg->scan_request->dev != cfg->p2p_net) dev = cfg->scan_request->dev;
    ...
    WL_DBG(
        ("cfg->scan_request is NULL. Internal scan scenario."
  ...
  if (p2p_is_on(cfg)) wl_clr_p2p_status(cfg, SCANNING);
...
void wl_cfg80211_abort_scan(struct wiphy *wiphy, struct wireless_dev *wdev) {
  ...
  if (wl_get_drv_status_all(cfg, SCANNING)) {
     ...
     * the status of abort is needed to be communicated using cfg80211_scan_done
     * call. Here we just issue abort request and let the scan complete path to
     * indicate abort to cfg80211 layer.
     */
...
int wl_cfg80211_scan_suppress(struct net_device *dev, int suppress) {
  ...
  if ((ret = wldev_ioctl_set(dev, WLC_SET_SCANSUPPRESS, &suppress,
                             sizeof(int))) < 0) {
...
static u32 wl_p2p_find_peer_channel(struct bcm_cfg80211 *cfg, s32 status,
                                    wl_bss_info_v109_t *bi, u32 bi_length) {
  u32 ret;
...
int wl_cfg80211_random_mac_enable(struct net_device *dev) {
  ...
  if (wl_get_drv_status_all(cfg, CONNECTED) ||
      ...
      wl_get_drv_status_all(cfg, AP_CREATING)) {
    WL_ERR(("fail to Set random mac, current state is wrong\n"));
    return err;
  }

  (void)memcpy_s(random_mac, ETH_ALEN, bcmcfg_to_prmry_ndev(cfg)->dev_addr,
                 ETH_ALEN);
...
#if defined(LEGACY_RANDOM_MAC)
  /* of the six bytes of random_mac the bytes 3, 4, 5 are copied with contents
   * of rand_bytes So while copying 3 bytes of content no overflow would be
   * seen. Hence returning void.
   */
  (void)memcpy_s(&random_mac[3], (sizeof(u8) * 3), rand_bytes,
                 sizeof(rand_bytes));
  ...
  err = wldev_iovar_setbuf_bsscfg(dev, "scanmac", sm, len, cfg->ioctl_buf,
                                  WLC_IOCTL_SMLEN, 0, &cfg->ioctl_buf_sync);
...
int wl_cfg80211_random_mac_disable(struct net_device *dev) {
  ...
  err = wldev_iovar_setbuf_bsscfg(bcmcfg_to_prmry_ndev(cfg), "cur_etheraddr",
                                  ...
                                  cfg->ioctl_buf, WLC_IOCTL_SMLEN, 0,
                                  &cfg->ioctl_buf_sync);
 ...
 * as arg and it uses scanmac iovar to offload the mac randomization to
 * firmware.
 */
int wl_cfg80211_scan_mac_config(struct net_device *dev, uint8 *rand_mac,
                                uint8 *rand_mask) {
  int byte_index = 0;
  s32 err = BCME_ERROR;
  uint8 buffer[WLC_IOCTL_SMLEN] = {0};
  ...
  wl_scanmac_config_t *sm_config = NULL;
  struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
  uint8 random_mask_46_bits[ETHER_ADDR_LEN] = {0xFC, 0xFF, 0xFF,
                                               0xFF, 0xFF, 0xFF};
  ...
  if (!rand_mask) {
    ...
    (void)memcpy_s(&sm_config->random_mask.octet, ETH_ALEN, random_mask_46_bits,
                   ETH_ALEN);
  } else {
...
static bool is_ssid_in_list(struct cfg80211_ssid *ssid,
  ...
  if (!ssid || !ssid_list) return FALSE;
...
int wl_cfg80211_sched_scan_start(struct wiphy *wiphy, struct net_device *dev,
  ...
  if ((wl_cfgvif_get_iftype_count(cfg, WL_IF_TYPE_STA) >= 2) ||
      cfg->latency_mode) {
    WL_ERR(
        ("Sched scan not supported in multi sta connected state"
         " or latency mode %d\n",
         cfg->latency_mode));
    return -EOPNOTSUPP;
  }
  ...
  WL_INFORM_MEM(
      ("[%s] Enter. ssids:%d match_sets:%d pno_time:%d pno_repeat:%d "
       "channels:%d adaptive:%d\n",
       dev->name, request->n_ssids, request->n_match_sets, pno_time, pno_repeat,
       request->n_channels, adaptive_pno));
  ...
  if (request->n_channels && request->n_channels < WL_NUMCHANNELS) {
    ...
    wl_cfgscan_populate_scan_channels(
        cfg, request->channels, request->n_channels, chan_list, &num_channels,
        FW_MAJOR_VER_PFN_CHSPEC_SUPPORTED(cfg->wlc_ver) ? true : false, false);
  }
  ...
  if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
    ...
    if (!event_data) {
      WL_ERR(
          ("%s: failed to allocate log_conn_event_t with "
           "length(%d)\n",
           __func__, alloc_len));
      ret = -ENOMEM;
      goto exit;
  ...
  if (ssid_cnt) {
    ...
    if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
      ...
      for (i = 0; i < ssid_cnt; i++) {
        ...
        (void)memcpy_s(tlv_data->value, DOT11_MAX_SSID_LEN, ssids_local[i].SSID,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) && \
    defined(SUPPORT_RANDOM_MAC_SCAN)
  if ((ret = wl_config_scan_macaddr(
           cfg, dev, (request->flags & NL80211_SCAN_FLAG_RANDOM_ADDR),
...
void wl_cfgscan_sched_scan_stop_work(struct work_struct *work) {
...
static void wl_scan_timeout(unsigned long data) {
  ...
  if (dhd_pcie_check_lps_d3_acked(dhdp)) {
    ...
    mod_timer(&cfg->scan_timeout, jiffies + msecs_to_jiffies(100));
    mutex_unlock(&cfg->scan_sync);
    return;
  }
#endif /* BCMPCIE */
  ...
  if ((cfg->tsinfo.scan_deq < cfg->tsinfo.scan_enq) ||
    ...
    WL_ERR(
        ("***SCAN event timeout. WQ state:0x%x scan_enq_time:" SEC_USEC_FMT
         ...
         work_busy(&cfg->event_work), GET_SEC_USEC(cfg->tsinfo.scan_enq),
    ...
    WL_ERR(("lock_states:[%d:%d:%d:%d:%d:%d]\n", mutex_is_locked(&cfg->if_sync),
            mutex_is_locked(&cfg->usr_sync), mutex_is_locked(&cfg->pm_sync),
            mutex_is_locked(&cfg->scan_sync), spin_is_locked(&cfg->cfgdrv_lock),
...
#ifdef WL_SCHED_SCAN
static s32 wl_cfgscan_init_pno_escan(struct bcm_cfg80211 *cfg,
                                     struct net_device *ndev,
                                     struct cfg80211_scan_request *request) {
  struct wiphy *wiphy = bcmcfg_to_wiphy(cfg);
  dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
  ...
  if (wl_get_drv_status_all(cfg, SCANNING)) {
    WL_INFORM_MEM(
        ("scan in progress. cancel and trigger PNO targetted scan\n"));
    _wl_cfgscan_cancel_scan(cfg);
  }
...
static s32 wl_cfgscan_update_v3_schedscan_results(
    struct bcm_cfg80211 *cfg, struct net_device *ndev,
  ...
  if (pfn_result->count > 0) {
    ...
    ssid = (struct cfg80211_ssid *)MALLOCZ(
        cfg->osh, sizeof(struct cfg80211_ssid) * MAX_PFN_LIST_COUNT);
    ...
    channel = (struct ieee80211_channel *)MALLOCZ(
        cfg->osh, (sizeof(struct ieee80211_channel) * pfn_result->count));
    ...
    for (i = 0; i < pfn_result->count; i++) {
       ...
       */
      ssid[i].ssid_len = ssid_len =
          MIN(DOT11_MAX_SSID_LEN, netinfo->pfnsubnet.SSID_len);
      ...
      (void)memcpy_s(ssid_buf, IEEE80211_MAX_SSID_LEN, ssid[i].ssid, ssid_len);
      ssid_buf[ssid_len] = '\0';
      WL_INFORM_MEM(
          ("[PNO] SSID:%s SSID length:%d"
      ...
      if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
        ...
        (void)memcpy_s(tlv_data->value, DOT11_MAX_SSID_LEN, ssid[i].ssid,
                       ssid[i].ssid_len);
        payload_len += TLV_LOG_SIZE(tlv_data);
        tlv_data = TLV_LOG_NEXT(tlv_data);
        ...
        tlv_data->tag = WIFI_TAG_CHANNEL;
        tlv_data->len = sizeof(uint16);
        (void)memcpy_s(tlv_data->value, sizeof(uint16), &channel_req,
                       sizeof(uint16));
        payload_len += TLV_LOG_SIZE(tlv_data);
        tlv_data = TLV_LOG_NEXT(tlv_data);
        ...
        tlv_data->tag = WIFI_TAG_RSSI;
        tlv_data->len = sizeof(int16);
        (void)memcpy_s(tlv_data->value, sizeof(uint16), &netinfo->RSSI,
                       sizeof(int16));
        payload_len += TLV_LOG_SIZE(tlv_data);
        tlv_data = TLV_LOG_NEXT(tlv_data);

        dhd_os_push_push_ring_data(dhdp, DHD_EVENT_RING_ID, &event_data->event,
                                   payload_len);
      }
    }

    /* assign parsed ssid array */
    if (request->n_ssids) request->ssids = &ssid[0];
...
out_err:
  if (ssid) {
    MFREE(cfg->osh, ssid, sizeof(struct cfg80211_ssid) * MAX_PFN_LIST_COUNT);
  }
  if (request) {
...
static s32 wl_notify_sched_scan_results(struct bcm_cfg80211 *cfg,
   ...
   * compatible in size and SSID offset, allowing v1 to be used below except for
   * the results fields themselves (status, count, offset to netinfo).
   */
  ...
  if (pfn_result_v1->version == PFN_SCANRESULT_VERSION_V1) {
    ...
    if (n_pfn_results > 0) {
      ...
      for (i = 0; i < n_pfn_results; i++) {
         ...
         */
        (void)memcpy_s(tmp_buf, DOT11_MAX_SSID_LEN, netinfo->pfnsubnet.SSID,
                       netinfo->pfnsubnet.SSID_len);
        ...
        if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
          ...
          tlv_data->tag = WIFI_TAG_RSSI;
          tlv_data->len = sizeof(int16);
          (void)memcpy_s(tlv_data->value, sizeof(int16), &netinfo->RSSI,
                         sizeof(int16));
          payload_len += TLV_LOG_SIZE(tlv_data);
          tlv_data = TLV_LOG_NEXT(tlv_data);
    ...
    if (n_pfn_results > 0) {
      ...
      for (i = 0; i < n_pfn_results; i++) {
        ...
        if (DBG_RING_ACTIVE(dhdp, DHD_EVENT_RING_ID)) {
          ...
          tlv_data->tag = WIFI_TAG_RSSI;
          tlv_data->len = sizeof(int16);
          (void)memcpy_s(tlv_data->value, sizeof(uint16), &netinfo_v2->RSSI,
                         sizeof(int16));
          payload_len += TLV_LOG_SIZE(tlv_data);
          tlv_data = TLV_LOG_NEXT(tlv_data);
    ...
    err = wl_cfgscan_update_v3_schedscan_results(cfg, ndev, pfn_result_v3,
                                                 e->event_type);
    if (err) {
      goto out_err;
    }
...
s32 wl_notify_pfn_status(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
  ...
  if (!dhd_dev_is_legacy_pno_enabled(ndev)) return 0;
#endif /* GSCAN_SUPPORT */
...
s32 wl_notify_gscan_event(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
  ...
  switch (event) {
    ...
    case WLC_E_PFN_SCAN_COMPLETE:
      ...
      wl_cfgvendor_send_async_event(wiphy, ndev, GOOGLE_SCAN_COMPLETE_EVENT,
                                    &event_type, sizeof(int));
      break;
    case WLC_E_PFN_BSSID_NET_FOUND:
      ...
      if (ptr) {
        wl_cfgvendor_send_hotlist_event(wiphy, ndev, ptr, send_evt_bytes,
    ...
    case WLC_E_PFN_GSCAN_FULL_RESULT:
      ...
      if (ptr) {
        wl_cfgvendor_send_async_event(
            wiphy, ndev, GOOGLE_SCAN_FULL_RESULTS_EVENT, ptr, send_evt_bytes);
        MFREE(cfg->osh, ptr, send_evt_bytes);
      } else {
        err = -ENOMEM;
      }
...
void wl_cfgscan_listen_complete_work(struct work_struct *work) {
  struct bcm_cfg80211 *cfg = NULL;
...
s32 wl_cfgscan_notify_listen_complete(struct bcm_cfg80211 *cfg) {
...
static void wl_init_scan_params(struct bcm_cfg80211 *cfg, u8 *params,
                                u16 params_size, u32 scan_type, u32 action,
                                u32 passive_time) {
  u32 sync_id = 0;
  wl_escan_params_v1_t *eparams = NULL;
  wl_escan_params_v3_t *eparams_v3 = NULL;
  ...
  if (IS_SCAN_PARAMS_V3_V2(cfg)) {
    ...
    (void)memcpy_s(&scanparams_v3->bssid, ETHER_ADDR_LEN, &ether_bcast,
                   ETHER_ADDR_LEN);
    ...
    (void)memcpy_s(&scanparams->bssid, ETHER_ADDR_LEN, &ether_bcast,
                   ETHER_ADDR_LEN);
...
s32 wl_cfgscan_cancel_listen_on_channel(struct bcm_cfg80211 *cfg,
                                        bool notify_user) {
  WL_DBG(("Enter\n"));
...
s32 wl_cfgscan_listen_on_channel(struct bcm_cfg80211 *cfg,
                                 struct wireless_dev *wdev,
                                 struct ieee80211_channel *channel,
  ...
  wl_init_scan_params(cfg, params, params_size, scan_type, WL_SCAN_ACTION_START,
                      dwell);
  ...
  err = wldev_iovar_setbuf_bsscfg(ndev, "escan", params, params_size,
                                  cfg->escan_ioctl_buf, WLC_IOCTL_MEDLEN,
...
#ifdef WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST
static void wl_priortize_scan_over_listen(struct bcm_cfg80211 *cfg,
                                          struct net_device *ndev,
                                          unsigned int duration) {
...
#define IS_P2P_DISC_NDEV(wdev)                                               \
  (wdev->netdev ? (strncmp(wdev->netdev->name, "p2p0", strlen("p2p0")) == 0) \
                : false)
  ...
  if (err == BCME_OK) {
    ...
    *cookie = id;
    ...
    CFG80211_READY_ON_CHANNEL(cfgdev, *cookie, channel, channel_type, duration,
                              flags);
    WL_INFORM_MEM(
        ("listen started on channel:%d duration (ms):%d cookie:%llu\n",
...
s32 wl_cfgscan_cancel_remain_on_channel(struct wiphy *wiphy,
...
#ifdef WL_GET_RCC
int wl_android_get_roam_scan_chanlist(struct bcm_cfg80211 *cfg) {
  s32 err = BCME_OK;
  ...
  skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(ndev),
                                    BRCM_VENDOR_GET_RCC_EVENT_BUF_LEN,
                                    BRCM_VENDOR_EVENT_RCC_INFO, kflags);
  ...
  for (i = 0; i < channel_list.n; i++) {
    WL_DBG(("Chanspec[%d] CH: %s (0x%04x)\n", i,
            wf_chspec_ntoa_ex(channel_list.channels[i], chanspec_buf),
...
int wl_android_get_roam_scan_freqlist(struct bcm_cfg80211 *cfg) {
  s32 err = BCME_OK;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
  struct sk_buff *skb;
  gfp_t kflags;
  ...
  skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(ndev),
                                    BRCM_VENDOR_GET_RCC_EVENT_BUF_LEN,
                                    BRCM_VENDOR_EVENT_RCC_FREQ_INFO, kflags);
  ...
  for (i = 0; i < channel_list.n; i++) {
    ...
    u32 freq =
        wl_channel_to_frequency(wf_chspec_ctlchan(chsp), CHSPEC_BAND(chsp));
 ...
 */
s32 wl_get_assoc_channels(struct bcm_cfg80211 *cfg, struct net_device *dev,
  ...
  if (cfg->rcc_enabled) {
    ...
    if (memcpy_s(ssid.SSID, sizeof(ssid.SSID), info->ssid, info->ssid_len) !=
        BCME_OK) {
    ...
    if ((!info->targeted_join) || (info->bssid_hint) || (info->chan_cnt == 0)) {
...
bool wl_cfgscan_is_dfs_set(wifi_band band) {
  switch (band) {
...
s32 wl_cfgscan_get_band_freq_list(struct bcm_cfg80211 *cfg,
                                  struct wireless_dev *wdev, int band,
...
static int wl_cfgscan_acs_parse_result(acs_selected_channels_t *pResult,
  ...
  unsigned int chspec_band, chspec_ctl_freq, chspec_center_ch, chspec_bw,
      chspec_sb;

  if ((!pResult) || (!pParameter)) {
    WL_ERR(("parameter invalid\n"));
    return BCME_BADARG;
  } else if (!wf_chspec_valid(ch_chosen)) {
    WL_ERR(("ch_chosen=0x%X invalid\n", ch_chosen));
    return BCME_BADARG;
  }

  chspec_ctl_freq =
      wl_channel_to_frequency(wf_chspec_ctlchan((chanspec_t)ch_chosen),
  ...
  WL_TRACE(("ctl_freq=%d, center_ch=%d, band=0x%X, bw=0x%X, sb=0x%X\n",
            chspec_ctl_freq, chspec_center_ch, chspec_band, chspec_bw,
            chspec_sb));

  (void)memset_s(pResult, sizeof(acs_selected_channels_t), 0,
                 sizeof(acs_selected_channels_t));
  ...
  switch (chspec_bw) {
    ...
    case WL_CHANSPEC_BW_80:
      if ((pParameter->vht_enabled) || (pParameter->he_enabled)) {
        ...
        WL_TRACE(("HT80 to HT40\n"));
      } else {
        pResult->ch_width = 20;
        pResult->pri_freq = chspec_ctl_freq;
        pResult->sec_freq = 0;
        WL_TRACE(("HT80 to HT20\n"));
      }
      break;
    ...
    case WL_CHANSPEC_BW_8080:
      if ((pParameter->vht_enabled) || (pParameter->he_enabled)) {
        ...
        WL_TRACE(("HT160 to HT40\n"));
      } else {
        pResult->ch_width = 20;
        pResult->pri_freq = chspec_ctl_freq;
        pResult->sec_freq = 0;
        WL_TRACE(("HT160 to HT20\n"));
      }
      break;
  ...
  WL_TRACE(
      ("result: pri_freq=%d, sec_freq=%d, vht_seg0=%d, vht_seg1=%d,"
       " ch_width=%d, hw_mode=%d\n",
       pResult->pri_freq, pResult->sec_freq, pResult->vht_seg0_center_ch,
       pResult->vht_seg1_center_ch, pResult->ch_width, pResult->hw_mode));

  return 0;
}

static int wl_cfgscan_acs_parse_parameter_save(int *pLen, uint32 *pList,
                                               chanspec_t chspec) {
  int ret = 0;
  int qty = 0;
  int i;
  ...
  do {
    ...
    if (!wf_chspec_valid(chspec)) {
      WL_TRACE(("chanspec=0x%X invalid\n", chspec));
      ret = BCME_BADARG;
      break;
    }
    ...
    if (i == qty) {
      pList[qty++] = chspec;
      *pLen = qty;
      WL_TRACE(("fill list[%d] = 0x%X\n", qty, chspec));
    } else {
      WL_TRACE(("duplicate with [idx]=[%d]=0x%X\n", i, chspec));
      break;
    }
  } while (0);
...
static int wl_cfgscan_acs_parse_parameter(struct bcm_cfg80211 *cfg, int *pLen,
                                          uint32 *pList, unsigned int chanspec,
                                          drv_acs_params_t *pParameter) {
  unsigned int chspec = 0, chspec_ctl_ch = 0, chspec_band = 0, chspec_bw = 0,
               chspec_sb = 0;
  ...
  do {
    ...
    if (chspec_band == WL_CHANSPEC_BAND_2G ||
      ...
      chspec =
          (chanspec_t)(chspec_ctl_ch | chspec_band | chspec_bw | chspec_sb);
    ...
    /* bw80 */
    if ((bw == 80) && (pParameter->vht_enabled || pParameter->he_enabled)) {
      chspec = wf_create_chspec_from_primary(channel, WL_CHANSPEC_BW_80,
                                             chspec_band);
#ifdef WL_CELLULAR_CHAN_AVOID
    ...
    /* bw40 */
    if ((bw == 40) && ((pParameter->ht40_enabled) ||
                       (pParameter->vht_enabled) || (pParameter->he_enabled))) {
      chspec = wf_create_chspec_from_primary(channel, WL_CHANSPEC_BW_40,
                                             chspec_band);
#ifdef WL_CELLULAR_CHAN_AVOID
      if (!wl_cellavoid_is_safe_overlap(cfg->cellavoid_info, chspec)) {
        chspec = INVCHANSPEC;
    ...
    /* bw20 */
    if ((bw == 20) && ((pParameter->ht_enabled) || (pParameter->ht40_enabled) ||
                       (pParameter->vht_enabled) || (pParameter->he_enabled))) {
      chspec = wf_create_chspec_from_primary(channel, WL_CHANSPEC_BW_20,
                                             chspec_band);
#ifdef WL_CELLULAR_CHAN_AVOID
      if (!wl_cellavoid_is_safe_overlap(cfg->cellavoid_info, chspec)) {
        chspec = INVCHANSPEC;
...
static void wl_cfgscan_acs_result_event(struct work_struct *work) {
  ...
  do {
    ...
    if (wl_cfgscan_acs_parse_result(&result, ch_chosen, pParameter) < 0) {
      WL_ERR(("fail to convert the result\n"));
      ret = BCME_BADARG;
      break;
    }
    ...
    len = 200;
    kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
    WL_TRACE(("idx=%d, wiphy->n_vendor_events=%d\n", BRCM_VENDOR_EVENT_ACS,
...
#else
    skb =
        cfg80211_vendor_event_alloc(wiphy, len, BRCM_VENDOR_EVENT_ACS, kflags);
#endif /* CONFIG_ARCH_MSM SUPPORT_WDEV_CFG80211_VENDOR_EVENT_ALLOC */
    if (!skb) {
      WL_ERR(("Error, no memory for event\n"));
      ret = BCME_NOMEM;
      break;
    }

    if ((nla_put_u32(skb, BRCM_VENDOR_ATTR_ACS_PRIMARY_FREQ, result.pri_freq) <
         0) ||
        (nla_put_u32(skb, BRCM_VENDOR_ATTR_ACS_SECONDARY_FREQ,
                     result.sec_freq) < 0) ||
      ...
      WL_ERR(("Error, fail to fill the result\n"));
      ret = BCME_BADARG;
      break;
    }

    WL_INFORM_MEM(
        ("ACS event notified. freq:%d:%d\n", result.pri_freq, result.sec_freq));
    cfg80211_vendor_event(skb, kflags);
  } while (0);

  if (ret < 0) {
    if (skb) {
      WL_ERR(("free the event since fail with ret=%d\n", ret));
      dev_kfree_skb_any(skb);
    }
  }
}

static chanspec_t wl_cfgscan_acs_find_default_chanspec(struct bcm_cfg80211 *cfg,
                                                       u32 band, uint32 *pList,
                                                       int qty) {
  chanspec_t chosen;
...
static int wl_cfgscan_acs_do_apcs(struct net_device *dev,
                                  ...
                                  int len, unsigned char *pBuffer, int qty,
                                  uint32 *pList) {
  int channel = 0;
  int chosen = 0;
  int retry = 0;
  ...
  if ((pBuffer) && (0 < len)) {
    ...
    if (scan_info == NULL) {
      WL_ERR(("scan_info kzalloc failed\n"));
      ret = -ENOMEM;
      goto done;
    }
    memcpy(&scan_info->channels, pBuffer, buf_len);
    chosen = wl_ext_autochannel(dev, ACS_DRV_BIT, band, scan_info);
    if (chosen) {
      goto done2;
    } else
      goto done;
    ...
    ret = BCME_BADARG;
    WL_ERR(("Error, no parameter to go, ret=%d\n", ret));
  }
  ...
  while (retry--) {
    ...
    WL_DBG(("round=%d, ret=%d, chosen=0x%X\n", APCS_MAX_RETRY - retry, ret,
            chosen));
    ...
    if (chosen) {
      ...
      freq = wl_channel_to_frequency(wf_chspec_ctlchan((chanspec_t)chosen),
                                     CHSPEC_BAND((chanspec_t)chosen));

      WL_INFORM_MEM(("* good, selected chosen=0x%X, freq:%d channel = %d\n",
                     chosen, freq, channel));
      break;
    }
    OSL_SLEEP(300);
  ...
  if (!chosen) {
    WL_ERR(("retry=%d, ret=%d, chosen=0x%X. Attempt default chan\n", retry, ret,
            chosen));
...
#ifdef WL_ESCAN
  if (scan_info) kfree(scan_info);
#endif /* WL_ESCAN */
  return ret;
}

#ifdef DHD_ACS_CHECK_SCC_2G_ACTIVE_CH
bool wl_check_active_2g_chan(struct bcm_cfg80211 *cfg,
  ...
  ret = wldev_iovar_getbuf(dev, "per_chan_info", (void *)&sta_chanspec,
                           sizeof(sta_chanspec), ioctl_buf, WLC_IOCTL_SMLEN,
                           NULL);
  if (ret != BCME_OK) {
  ...
  if (bitmap &
      (WL_CHAN_PASSIVE | WL_CHAN_RESTRICTED | WL_CHAN_CLM_RESTRICTED)) {
...
static int wl_convert_freqlist_to_chspeclist(struct bcm_cfg80211 *cfg,
                                             u32 *pElem_freq, u32 freq_list_len,
                                             u32 *req_len, u32 *pList,
                                             drv_acs_params_t *parameter,
                                             int qty) {
  int i, j;
  u32 list_size;
  ...
  if ((parameter->freq_bands & WLC_BAND_5G) &&
    ...
    for (i = 0; i < freq_list_len; i++) {
      if (CHSPEC_CHANNEL(chspeclist[i]) == APCS_DEFAULT_5G_CH) {
        ...
        wl_cfgscan_acs_parse_parameter(cfg, req_len, pList, chspeclist[i],
                                       parameter);
        goto exit;
      }
    }
  ...
  if (safe_chspec_cnt) {
    ...
    ret = wl_handle_acs_concurrency_cases(cfg, &safe_param, safe_chspec_cnt,
                                          safe_chspeclist);
    if (ret == BCME_OK) {
      WL_INFORM_MEM(
          ("Succeded to handle the acs concurrency case "
           "with safe channels!\n"));
      safe_success = TRUE;
      goto success;
    }
  ...
  if (wl_cellavoid_mandatory_isset(cfg->cellavoid_info, NL80211_IFTYPE_AP)) {
    WL_INFORM_MEM(
        ("Mandatory flag for AP is set, skip the ACS, safe_chspec_cnt %d\n",
         safe_chspec_cnt));
    goto exit;
  }
  ...
  if (safe_chspec_cnt != freq_list_len) {
    ...
    ret = wl_handle_acs_concurrency_cases(cfg, parameter, freq_list_len,
                                          chspeclist);
    if (ret) {
...
success:
  ...
  for (i = 0; i < freq_list_len; i++) {
    ...
    wl_cfgscan_acs_parse_parameter(cfg, req_len, pList, p_chspec_list[i],
                                   parameter);
  }
...
int wl_cfgscan_acs(struct wiphy *wiphy, struct wireless_dev *wdev,
  ...
  if (!delay_work_acs.init_flag) {
    ...
    INIT_DELAYED_WORK(&delay_work_acs.acs_delay_work,
                      wl_cfgscan_acs_result_event);
  ...
  do {
    ...
    if (
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 12, 0))
        nla_parse(tb, BRCM_VENDOR_ATTR_ACS_LAST, (struct nlattr *)data, len,
                  NULL, NULL) ||
#else
        nla_parse(tb, BRCM_VENDOR_ATTR_ACS_LAST, (struct nlattr *)data, len,
                  NULL) ||
      ...
      WL_ERR(("***Error, parse fail\n"));
      ret = BCME_BADARG;
      break;
    }

    parameter = &delay_work_acs.parameter;
    (void)memset_s(parameter, sizeof(drv_acs_params_t), 0,
                   sizeof(drv_acs_params_t));
    ...
    if (tb[BRCM_VENDOR_ATTR_ACS_HT_ENABLED]) {
      ...
      WL_TRACE(("ht_enabled=%d\n", parameter->ht_enabled));
    }
    if (tb[BRCM_VENDOR_ATTR_ACS_HT40_ENABLED]) {
      parameter->ht40_enabled =
          nla_get_u8(tb[BRCM_VENDOR_ATTR_ACS_HT40_ENABLED]);
      WL_TRACE(("ht40_enabled=%d\n", parameter->ht40_enabled));
    }
    if (tb[BRCM_VENDOR_ATTR_ACS_VHT_ENABLED]) {
      parameter->vht_enabled = nla_get_u8(tb[BRCM_VENDOR_ATTR_ACS_VHT_ENABLED]);
      WL_TRACE(("vht_enabled=%d\n", parameter->vht_enabled));
    }
    ...
    if (tb[BRCM_VENDOR_ATTR_ACS_CH_LIST]) {
      ...
      WL_TRACE(("chan_list_len=%d\n", chan_list_len));
#else
      WL_ERR(("chan_list attribute not supported\n"));
#endif /* WL_ACS_CHANLIST */
    }
    ...
    WL_INFORM_MEM(
        ("hw_mode=%d, band=%d ht_enabled:%d vht_enabled:%d ch_width:%d "
         "parameter->he_enabled = %d\n",
         parameter->hw_mode, parameter->band, parameter->ht_enabled,
         parameter->vht_enabled, parameter->ch_width, parameter->he_enabled));
    if (WLC_BAND_INVALID == parameter->band) {
      ret = BCME_BADARG;
      WL_ERR(("*Error, hw_mode=%d based band invalid\n", parameter->hw_mode));
      break;
    }
    ...
    total = sizeof(uint32) * qty *
            (/* extra structure 'count' item */
             ...
             (1 + 2 + 4 + 8));
    WL_TRACE(
        ("qty=%d+%d=%d, total=%d\n", chan_list_len, freq_list_len, qty, total));

    if (total <= 0) {
      ret = BCME_BADARG;
      WL_ERR(("*Error, total number (%d) is invalid\n", total));
      break;
    }
    pReq = MALLOCZ(cfg->osh, total);
    if (!pReq) {
      WL_ERR(("*Error, no memory for %d bytes\n", total));
      ret = BCME_NOMEM;
      break;
    } else {
    ...
    if (pElem_chan) {
      ...
      for (i = 0; i < chan_list_len; i++) {
        ...
        wl_cfgscan_acs_parse_parameter(cfg, &req_len, pList, chanspec,
                                       parameter);
      }
    }
    WL_TRACE(("list_len=%d after ch_list\n", req_len));
#endif /* WL_ACS_CHANLIST */
    ...
    if (ret >= 0) {
      ...
      WL_INFORM_MEM(
          ("scheduled work for acs event (chspec:0x%X)\n", ch_chosen));
      ret = 0;
    }
  ...
  /* free and clean up */
  if (NULL != pReq) {
    WL_TRACE(("free the pReq=0x%p with total=%d\n", pReq, total));
...
void wl_get_ap_chanspecs(struct bcm_cfg80211 *cfg, wl_ap_oper_data_t *ap_data) {
...
bool wl_is_chanspec_restricted(struct bcm_cfg80211 *cfg,
                               chanspec_t sta_chanspec) {
  struct net_device *dev = bcmcfg_to_prmry_ndev(cfg);
  s32 ret = BCME_OK;
  uint bitmap = 0;
  ...
  bitmap = dtoh32(*(uint *)ioctl_buf);
  if (bitmap & (WL_CHAN_PASSIVE | WL_CHAN_RADAR | WL_CHAN_RESTRICTED |
  ...
  WL_INFORM_MEM(
      ("STA chanspec:0x%x per_chan_info:0x%x\n", sta_chanspec, bitmap));
  return FALSE;
}

static bool wl_find_matching_chanspec(chanspec_t sta_chanspec, int qty,
                                      uint32 *pList) {
  uint32 i = qty;
  ...
  while (i--) {
    if (pList[i]) {
      if (wf_chspec_ctlchspec((pList[i])) ==
          wf_chspec_ctlchspec(sta_chanspec)) {
...
static bool wl_acs_check_scc(struct bcm_cfg80211 *cfg,
                             drv_acs_params_t *parameter,
...
#ifdef WL_CELLULAR_CHAN_AVOID
    scc = wl_cellavoid_operation_allowed(cfg->cellavoid_info, sta_chanspec,
                                         NL80211_IFTYPE_AP);
    if (scc == FALSE) {
...
int wl_handle_acs_concurrency_cases(struct bcm_cfg80211 *cfg,
                                    drv_acs_params_t *parameter, int qty,
                                    uint32 *pList) {
  chanspec_t chspec = 0;
  ...
  if (chspec) {
    ...
    if (sta_band == WLC_BAND_2G) {
      ...
      if (wl_is_chanspec_restricted(cfg, chspec) ||
        ...
        WL_ERR(
            ("STA connected in 5G %x, but no channel available "
             "for ACS %x\n",
      ...
      if (wl_acs_check_scc(cfg, parameter, chspec, qty, pList)) {
        ...
        WL_ERR(
            ("STA connected in 6G %x, but no channel available "
             "for ACS %x\n",
             chspec, parameter->freq_bands));
        return -EINVAL;
      }
