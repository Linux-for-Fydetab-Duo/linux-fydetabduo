...
#define sd_err(x)                              \
  do {                                         \
    if (sd_msglevel & SDH_ERROR_VAL) printf x; \
  } while (0)
#define sd_trace(x)                            \
  do {                                         \
    if (sd_msglevel & SDH_TRACE_VAL) printf x; \
  } while (0)
#define sd_info(x)                            \
  do {                                        \
    if (sd_msglevel & SDH_INFO_VAL) printf x; \
  } while (0)
#define sd_debug(x)                            \
  do {                                         \
    if (sd_msglevel & SDH_DEBUG_VAL) printf x; \
  } while (0)
#define sd_data(x)                            \
  do {                                        \
    if (sd_msglevel & SDH_DATA_VAL) printf x; \
  } while (0)
#define sd_ctrl(x)                            \
  do {                                        \
    if (sd_msglevel & SDH_CTRL_VAL) printf x; \
  } while (0)
#define sd_dma(x)                            \
  do {                                       \
    if (sd_msglevel & SDH_DMA_VAL) printf x; \
  } while (0)
#else
#define sd_err(x)                              \
  do {                                         \
...
#define sd_log(x)                            \
  do {                                       \
    if (sd_msglevel & SDH_LOG_VAL) bcmlog x; \
  } while (0)
#else
#define sd_log(x)
#endif
...
#define SDIOH_REG_WINSZ \
  0x100 /* Number of registers in Standard Host Controller */

#define SDIOH_TYPE_ARASAN_HDK 1
#define SDIOH_TYPE_BCM27XX 2
#define SDIOH_TYPE_TI_PCIXX21 4 /* TI PCIxx21 Standard Host Controller */
#define SDIOH_TYPE_RICOH_R5C822 \
  5 /* Ricoh Co Ltd R5C822 SD/SDIO/MMC/MS/MSPro Host Adapter */
...
struct sdioh_info {
  ...
  int global_UHSI_Supp;       /* type of UHSI support in both host and card.
                               ...
                               * HOST_SDR_50_104_DDR: one of SDR50/SDR104 or DDR50
                               * supptd
                               */
...
extern int sdstd_waitbits(sdioh_info_t *sd, uint16 norm, uint16 err, bool yield,
                          uint16 *bits);
