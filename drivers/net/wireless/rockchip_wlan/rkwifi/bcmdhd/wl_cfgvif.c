...
void wl_validate_wps_ie(const char *wps_ie, s32 wps_ie_len, bool *pbc) {
...
static s32 wl_cfg80211_is_policy_config_allowed(struct bcm_cfg80211 *cfg) {
  ...
  if ((sta_assoc_state == TRUE) || (p2p_disc_on == TRUE) ||
      (nan_init_state == TRUE) || (active_sec_iface != WL_IFACE_NOT_PRESENT)) {
    WL_INFORM_MEM(
        ("Active iface matrix: sta_assoc_state = %d,"
 ...
 * deleted.
 */
static s32 wl_cfg80211_data_if_mgmt(struct bcm_cfg80211 *cfg,
...
static s32 wl_cfg80211_disc_if_mgmt(struct bcm_cfg80211 *cfg,
                                    wl_iftype_t new_wl_iftype,
...
static bool wl_cfg80211_is_associated_discovery(struct bcm_cfg80211 *cfg,
                                                wl_iftype_t new_wl_iftype) {
  struct net_device *p2p_ndev = NULL;
  p2p_ndev = wl_to_p2p_bss_ndev(cfg, P2PAPI_BSSCFG_CONNECTION1);

  if (new_wl_iftype == WL_IF_TYPE_P2P_DISC && p2p_ndev &&
      p2p_ndev->ieee80211_ptr && is_p2p_group_iface(p2p_ndev->ieee80211_ptr)) {
    return true;
  }
#ifdef WL_NAN
...
static s32 wl_cfg80211_handle_discovery_config(struct bcm_cfg80211 *cfg,
                                               wl_iftype_t new_wl_iftype) {
  s32 ret = BCME_OK;
  ...
  ret =
      wl_cfg80211_disc_if_mgmt(cfg, new_wl_iftype, &disable_nan, &disable_p2p);
  if (ret != BCME_OK) {
...
s32 wl_release_vif_macaddr(struct bcm_cfg80211 *cfg, const u8 *mac_addr,
                           u16 wl_iftype) {
...
bcm_struct_cfgdev *wl_cfg80211_add_virtual_iface(struct wiphy *wiphy,
...
s32 wl_cfg80211_del_virtual_iface(struct wiphy *wiphy,
...
static s32 wl_cfg80211_change_p2prole(struct wiphy *wiphy,
  ...
  (void)memcpy_s(p2p_dev_addr.octet, ETHER_ADDR_LEN, ndev->dev_addr,
                 ETHER_ADDR_LEN);
  ...
  wl_cfgp2p_ifchange(cfg, &p2p_dev_addr, htod32(wlif_type), chspec, conn_idx);
...
s32 wl_cfg80211_change_virtual_iface(struct wiphy *wiphy,
                                     struct net_device *ndev,
                                     enum nl80211_iftype type,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 12, 0))
                                     u32 *flags,
  ...
  wl_cfg80211_iface_state_ops(ndev->ieee80211_ptr, WL_IF_CHANGE_REQ, wl_iftype,
                              wl_mode);
  ...
  if (is_p2p_group_iface(ndev->ieee80211_ptr) &&
    ...
    WL_DBG_MEM(
        ("[%s] Set base role to GC, current role"
...
chanspec_t wl_channel_to_chanspec(struct wiphy *wiphy, struct net_device *dev,
                                  u32 channel, u32 bw_cap) {
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
  u8 *buf = NULL;
...
#define LOCAL_BUF_SIZE 2048
  ...
  for (i = 0; i < dtoh32(list->count); i++) {
    ...
    if (tmp_c != channel) continue;

    if ((tmp_bw > bw) && (tmp_bw <= bw_cap)) {
      bw = tmp_bw;
      ret_c = c;
      if (bw == bw_cap) goto exit;
    }
  }
exit:
  if (buf) {
  ...
  for_each_ndev(cfg, iter, next) {
    ...
    if (iter->ndev && (iter->ndev != primary_ndev)) {
...
#endif /* WL_STATIC_IF */
      {
         ...
         */
        WL_ERR(
            ("Setting forced reg_on to enusre clearing"
...
int wl_get_bandwidth_cap(struct net_device *ndev, uint32 band,
                         uint32 *bandwidth) {
  ...
  if (param.band) {
     ...
     */
    err = wldev_iovar_getbuf(ndev, "bw_cap", &param, sizeof(param), ioctl_buf,
...
s32 wl_get_nl80211_band(u32 wl_band) {
  s32 err = BCME_ERROR;
...
static struct ieee80211_channel *wl_cfg80211_get_ieee80211_chan(
    struct net_device *dev, struct cfg80211_ap_settings *info) {
...
bool wl_is_sta_connected(struct bcm_cfg80211 *cfg) {
...
s32 wl_cfg80211_set_channel(struct wiphy *wiphy, struct net_device *dev,
  ...
  if (!CHSPEC_IS6G(chspec)) {
    ...
    cur_chspec =
        wl_cellavoid_find_widechspec_fromchspec(cfg->cellavoid_info, chspec);
...
set_channel:
  ...
  cur_chspec = wf_create_chspec_from_primary(
      wf_chspec_primary20_chan(chspec), band_width, CHSPEC_BAND(chspec), 0);
#else
...
#ifdef WL_6G_BAND
  if (cfg->acs_chspec && CHSPEC_IS6G(cfg->acs_chspec) &&
      (wf_chspec_ctlchspec(cfg->acs_chspec) ==
       wf_chspec_ctlchspec(cur_chspec))) {
  ...
  if (wf_chspec_valid(cur_chspec)) {
    ...
    if (cur_chspec != INVCHANSPEC) {
      if ((err = wldev_iovar_setint(dev, "chanspec", cur_chspec)) ==
          BCME_BADCHAN) {
        u32 local_channel = CHSPEC_CHANNEL(chspec);
        if ((bw == WL_CHANSPEC_BW_80) || (bw == WL_CHANSPEC_BW_160))
          goto change_bw;
        err = wldev_ioctl_set(dev, WLC_SET_CHANNEL, &local_channel,
                              sizeof(local_channel));
        if (err < 0) {
          WL_ERR(
              ("WLC_SET_CHANNEL error %d"
        ...
        if ((wl_get_mode_by_netdev(cfg, dev) == WL_MODE_AP) &&
          ...
          if (CHSPEC_IS2G(chspec) && !wl_is_sta_connected(cfg)) {
            WL_DBG(
                ("Disabling frameburst on "
                 "stand-alone 2GHz SoftAP\n"));
            wl_cfg80211_set_frameburst(cfg, FALSE);
          } else if (!CHSPEC_IS2G(chspec) && cfg->frameburst_disabled) {
...
static s32 wl_validate_fils_ind_ie(struct net_device *dev,
  ...
  if (!iov_buf) {
    WL_ERR(
        ("%s: iov_buf alloc failed! %d bytes\n", __FUNCTION__, iov_buf_size));
    err = BCME_NOMEM;
    goto exit;
  }
...
#ifdef MFP
static int wl_get_mfp_capability(u8 rsn_cap, u32 *wpa_auth, u32 *mfp_val) {
  u32 mfp = 0;
...
static s32 wl_validate_wpa2ie(struct net_device *dev, const bcm_tlv_t *wpa2ie,
  ...
  if ((len -= WPA_SUITE_LEN) <= 0) return BCME_BADLEN;
  ...
  if (len >= WPA2_PMKID_COUNT_LEN) {
    pmkid = (const wpa_pmkid_list_t *)((const u8 *)&mgmt->list[suite_count] +
                                       RSN_CAP_LEN);
...
s32 wl_update_akm_from_assoc_ie(struct bcm_cfg80211 *cfg,
                                struct net_device *ndev, u8 *assoc_ies,
                                u32 assoc_ie_len) {
  const wpa_suite_mcast_t *mcast;
  const wpa_suite_ucast_t *ucast;
  const wpa_suite_auth_key_mgmt_t *mgmt;
...
static s32 wl_validate_wpaie_wpa2ie(struct net_device *dev,
  ...
  if (wpaie == NULL || wpa2ie == NULL) goto exit;
...
static s32 wl_set_ap_passphrase(struct net_device *dev) {
  ...
  if ((cfg->hostapd_ssid.SSID_len == 0) || (_net_info->passphrase_len == 0)) {
...
static s32 wl_cfg80211_bcn_validate_sec(struct net_device *dev,
  ...
  if (dev_role == NL80211_IFTYPE_P2P_GO && (ies->wpa2_ie)) {
    ...
    if (wl_validate_wpa2ie(dev, ies->wpa2_ie, bssidx) < 0) return BCME_ERROR;
    ...
    if ((ies->wpa_ie != NULL && ies->wpa2_ie != NULL)) {
      if (wl_validate_wpaie_wpa2ie(dev, ies->wpa_ie, ies->wpa2_ie, bssidx) <
          0) {
      ...
      if (bss->rsn_ie) {
        MFREE(cfg->osh, bss->rsn_ie, bss->rsn_ie[1] + WPA_RSN_IE_TAG_FIXED_LEN);
        bss->rsn_ie = NULL;
      }
      if (bss->wpa_ie) {
        MFREE(cfg->osh, bss->wpa_ie, bss->wpa_ie[1] + WPA_RSN_IE_TAG_FIXED_LEN);
        bss->wpa_ie = NULL;
      }
      if (bss->wps_ie) {
      ...
      if (ies->wpa_ie != NULL) {
        ...
        bss->rsn_ie = NULL;
        bss->wpa_ie =
            MALLOCZ(cfg->osh, ies->wpa_ie->length + WPA_RSN_IE_TAG_FIXED_LEN);
        if (bss->wpa_ie) {
        ...
        bss->wpa_ie = NULL;
        bss->rsn_ie =
            MALLOCZ(cfg->osh, ies->wpa2_ie->len + WPA_RSN_IE_TAG_FIXED_LEN);
        if (bss->rsn_ie) {
...
#ifdef WL_FILS
      if (ies->fils_ind_ie) {
        bss->fils_ind_ie = MALLOCZ(
            cfg->osh, ies->fils_ind_ie->len + FILS_INDICATION_IE_TAG_FIXED_LEN);
        if (bss->fils_ind_ie) {
...
static s32 wl_cfg80211_bcn_set_rate(struct cfg80211_ap_settings *info,
                                    struct net_device *dev) {
  struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
  struct wiphy *wiphy = bcmcfg_to_wiphy(cfg);
  u32 beacon_rate = 0, mask = 0;
  int band = IEEE80211_NUM_BANDS, i;
  s32 err = BCME_OK;

  if (cfg->ap_oper_channel != INVCHANSPEC) {
    struct ieee80211_supported_band *sband = NULL;
    if (CHSPEC_IS2G(cfg->ap_oper_channel))
      band = IEEE80211_BAND_2GHZ;
    else if (CHSPEC_IS5G(cfg->ap_oper_channel))
      band = IEEE80211_BAND_5GHZ;
#ifdef CFG80211_6G_SUPPORT
    else if (CHSPEC_IS6G(cfg->ap_oper_channel))
      band = IEEE80211_BAND_6GHZ;
#endif
    else
      WL_ERR(("invalid channel %s-%-3d(0x%x)\n",
              CHSPEC2BANDSTR(cfg->ap_oper_channel),
              wf_chspec_ctlchan(cfg->ap_oper_channel), cfg->ap_oper_channel));
    if (band < IEEE80211_NUM_BANDS) {
      mask = info->beacon_rate.control[band].legacy;
      sband = wiphy->bands[band];
    }
    if (sband && mask > 0) {
      for (i = 0; i < sband->n_bitrates; i++) {
        if (mask & (1 << i)) {
          beacon_rate = sband->bitrates[i].bitrate / 5;
          break;
        }
      }
    }
    if (beacon_rate) {
      WL_MSG(dev->name, "channel %s-%-3d, beacon_rate %dMbps\n",
             CHSPEC2BANDSTR(cfg->ap_oper_channel),
             wf_chspec_ctlchan(cfg->ap_oper_channel), beacon_rate / 2);
      if ((err = wldev_iovar_setint(dev, "force_bcn_rspec", beacon_rate)) < 0) {
        WL_ERR(("force_bcn_rspec set error, %d\n", err));
        return err;
      }
    } else {
      wldev_iovar_setint(dev, "force_bcn_rspec", 0);
    }
  }
...
s32 wl_cfg80211_parse_ies(const u8 *ptr, u32 len, struct parsed_ies *ies) {
  s32 err = BCME_OK;
  ...
  /* find rates IEs */
  if ((ies->rate_ie = bcm_parse_tlvs(ptr, len, DOT11_MNG_RATES_ID)) != NULL) {
    ies->rate_ie_len = ies->rate_ie->len;
  }

  if ((ies->ext_rate_ie = bcm_parse_tlvs(ptr, len, DOT11_MNG_EXT_RATES_ID)) !=
      NULL) {
...
s32 wl_cfg80211_set_ap_role(struct bcm_cfg80211 *cfg, struct net_device *dev) {
  s32 err = BCME_OK;
  s32 infra = 1;
  s32 ap = 0;
  s32 pm;
  ...
  if (bssidx == 0 && !new_chip) {
    /* To fix 43456 scan err (-4) issue while hostapd send scan during bringup
     * AP mode */
...
void wl_cfg80211_ap_timeout_work(struct work_struct *work) {
#if defined(BCMDONGLEHOST)
  struct bcm_cfg80211 *cfg = NULL;
  dhd_pub_t *dhdp = NULL;
...
static s32 wl_cfg80211_bcn_bringup_ap(struct net_device *dev,
  ...
  if (is_rsdb_supported < 0) return (-ENODEV);
#endif /* BCMDONGLEHOST */

  WL_DBG(
      ("Enter dev_role:%d bssidx:%d ifname:%s\n", dev_role, bssidx, dev->name));
  ...
  if (dev_role == NL80211_IFTYPE_P2P_GO) {
    ...
    if (!is_bssup && (ies->wpa2_ie != NULL)) {
      err = wldev_iovar_setbuf_bsscfg(
          dev, "ssid", &cfg->p2p->ssid, sizeof(cfg->p2p->ssid), cfg->ioctl_buf,
      ...
      wait_for_completion_timeout(&cfg->iface_up, msecs_to_jiffies(500));
    ...
    err = wldev_ioctl_set(dev, WLC_SET_SPECT_MANAGMENT, &spect, sizeof(s32));
    if (err < 0) {
    ...
    if (dhd->conf->chip == BCM43430_CHIP_ID && bssidx > 0 &&
      ...
      ret =
          wldev_ioctl_get(primary_ndev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN);
    ...
    err = wldev_iovar_setbuf(dev, "cur_etheraddr", dev->dev_addr, ETH_ALEN,
...
static s32 wl_cfg80211_config_bss_selector(struct net_device *dev,
                                           struct parsed_ies *ies, u32 dev_role,
                                           s32 bssidx) {
  int err = BCME_OK;
  ...
  if (sae_h2e_required) {
    ...
    err = wl_cfg80211_set_wsec_info(dev, &sae_pwe, sizeof(sae_pwe),
  ...
  WL_INFORM_MEM(
      ("Config BSS Selector Done err %d, h2e_req %d\n", err, sae_h2e_required));
...
s32 wl_cfg80211_parse_ap_ies(struct net_device *dev,
  ...
  /* Parse Beacon IEs */
  if (wl_cfg80211_parse_ies((const u8 *)info->tail, info->tail_len, ies) < 0) {
    WL_ERR(("Beacon get IEs failed \n"));
    err = -EINVAL;
    goto fail;
  }

  if ((err = wl_cfg80211_config_rsnxe_ie(cfg, dev, (const u8 *)info->tail,
                                         info->tail_len)) < 0) {
  ...
  if (dhd->op_mode & DHD_FLAG_HOSTAP_MODE) {
    ...
    if (mgmt != NULL) {
      ...
      vndr_ie_len =
          (uint32)(info->probe_resp_len - offsetof(const struct ieee80211_mgmt,
...
s32 wl_cfg80211_set_ies(struct net_device *dev,
  ...
  if (dhd->op_mode & DHD_FLAG_HOSTAP_MODE) {
    ...
    if ((interworking_ie = wl_cfg80211_find_interworking_ie(
             info->tail, info->tail_len)) != NULL) {
      WL_DBG(("INTERWORKING found\n"));
      if ((err = wl_cfg80211_add_iw_ie(
               cfg, dev, bssidx, VNDR_IE_CUSTOM_FLAG, interworking_ie->id,
               interworking_ie->data, interworking_ie->len)) != BCME_OK) {
      ...
      if (!bss->security_mode) {
        ...
        if (bss->wpa_ie) {
          if (memcmp(bss->wpa_ie, ies->wpa_ie,
            ...
            update_bss = true;
            bss->wpa_ie = MALLOCZ(
                cfg->osh, ies->wpa_ie->length + WPA_RSN_IE_TAG_FIXED_LEN);
            if (bss->wpa_ie) {
              memcpy(bss->wpa_ie, ies->wpa_ie,
                     ies->wpa_ie->length + WPA_RSN_IE_TAG_FIXED_LEN);
            }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)) || \
    defined(WL_COMPAT_WIRELESS)
s32 wl_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *dev,
 ...
 */
  ...
  if ((err =
           wl_cfg80211_set_channel(wiphy, dev,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2) || \
    defined(CFG80211_BKPORT_MLO)
...
#ifdef WL_BEACON_RATE
  if ((err = wl_cfg80211_bcn_set_rate(info, dev)) < 0) {
    WL_ERR(("Beacon rate set failed \n"));
  }
#endif
  ...
  if ((err = wl_cfg80211_config_bss_selector(dev, &ies, dev_role, bssidx)) <
      0) {
...
s32 wl_cfg80211_stop_ap(struct wiphy *wiphy, struct net_device *dev
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2) || \
    defined(CFG80211_BKPORT_MLO)
                        ,
  ...
  if (dev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) {
    ...
    err = wldev_iovar_setbuf(dev, "cur_etheraddr", null_mac, ETH_ALEN,
                             cfg->ioctl_buf, WLC_IOCTL_SMLEN,
                             &cfg->ioctl_buf_sync);
    if (err) {
...
s32 wl_cfg80211_change_beacon(struct wiphy *wiphy, struct net_device *dev,
...
#else
s32 wl_cfg80211_add_set_beacon(struct wiphy *wiphy, struct net_device *dev,
  ...
  if (((dev_role == NL80211_IFTYPE_P2P_GO) ||
    ...
    if (pbc) wl_add_remove_eventmsg(dev, WLC_E_PROBREQ_MSG, true);
  }
...
s32 wl_cfg80211_del_beacon(struct wiphy *wiphy, struct net_device *dev) {
  int err = 0;
  s32 bssidx = 0;
  int infra = 0;
...
s32 wl_get_auth_assoc_status(struct bcm_cfg80211 *cfg, struct net_device *ndev,
 ...
 */
#if !defined(WL_CFG80211_STA_EVENT) && \
    (LINUX_VERSION_CODE < KERNEL_VERSION(3, 2, 0))
static s32 wl_notify_connect_status_ap_legacy(
    struct bcm_cfg80211 *cfg, struct net_device *ndev const wl_event_msg_t *e,
    void *data) {
  s32 err = 0;
  u32 event = ntoh32(e->event_type);
  u32 reason = ntoh32(e->reason);
...
static void wl_update_sta_chanspec_info(struct bcm_cfg80211 *cfg,
                                        struct net_device *ndev,
                                        const u8 *addr) {
  ...
  /* get the sta info */
  ret = wldev_iovar_getbuf(ndev, "sta_info", addr, ETHER_ADDR_LEN, iovar_buf,
                           iovar_buf_len, NULL);
  if (ret < 0) {
  ...
  WL_INFORM_MEM(
      ("[%s] updated client sta info. chanspec:0x%x\n", ndev->name, chanspec));
exit:
  if (iovar_buf) {
...
s32 wl_notify_connect_status_ap(struct bcm_cfg80211 *cfg,
                                struct net_device *ndev,
                                const wl_event_msg_t *e, void *data) {
  s32 err = 0;
  u32 event = ntoh32(e->event_type);
  ...
  if ((event == WLC_E_ASSOC_IND) ||
      ...
      ((event == WLC_E_REASSOC_IND) && (reason == DOT11_SC_ASSOC_TRY_LATER) &&
    ...
    WL_MSG(ndev->name, "new sta event for " MACDBG "\n",
           MAC2STRDBG(e->addr.octet));
#ifdef WL_EXT_IAPSTA
    wl_ext_in4way_sync(ndev, AP_WAIT_STA_RECONNECT, WL_EXT_STATUS_STA_CONNECTED,
                       (void *)&e->addr);
#endif
#ifdef STA_MGMT
...
s32 wl_frame_get_mgmt(struct bcm_cfg80211 *cfg, u16 fc,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
        cfg80211_rx_mgmt(cfgdev, cfg->tdls_mgmt_freq, 0, cfg->tdls_mgmt_frame,
                         cfg->tdls_mgmt_frame_len, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 12, 0))
        cfg80211_rx_mgmt(cfgdev, cfg->tdls_mgmt_freq, 0, cfg->tdls_mgmt_frame,
                         cfg->tdls_mgmt_frame_len, 0, GFP_ATOMIC);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)) || \
    defined(WL_COMPAT_WIRELESS)
        cfg80211_rx_mgmt(cfgdev, cfg->tdls_mgmt_freq, 0, cfg->tdls_mgmt_frame,
                         cfg->tdls_mgmt_frame_len, GFP_ATOMIC);
#else
        cfg80211_rx_mgmt(cfgdev, cfg->tdls_mgmt_freq, cfg->tdls_mgmt_frame,
                         cfg->tdls_mgmt_frame_len, GFP_ATOMIC);
...
s32 wl_cfg80211_dfs_ap_move(struct net_device *ndev, char *data, char *command,
  ...
  if (!*data) {
    ...
    if (status->move_status != (int8)DFS_SCAN_S_IDLE) {
      ...
      if (chanspec != 0 && chanspec != INVCHANSPEC) {
        ...
        bytes_written =
            snprintf(command, total_len, "AP Target Chanspec %s (0x%x)\n",
                     chanbuf, chanspec);
      }
      bytes_written +=
          snprintf(command + bytes_written, total_len - bytes_written, "%s\n",
  ...
  if (abort == -1) {
    ...
    if (chanspec != 0) {
      ...
      if (val != INVCHANSPEC) {
        if ((err = wldev_iovar_setbuf(ndev, "dfs_ap_move", &val, sizeof(int),
...
#ifdef WL_CFG80211_ACL
int wl_cfg80211_set_mac_acl(struct wiphy *wiphy, struct net_device *cfgdev,
  ...
  if (macmode == MACLIST_MODE_DISABLED) {
    if ((ret = wl_android_set_ap_mac_list(cfgdev, macmode, NULL)) != 0)
      WL_ERR(
          ("wl_cfg80211_set_mac_acl: Setting MAC list"
           " failed error=%d\n",
           ret));
  ...
  if (macnum < 0 || macnum > MAX_NUM_MAC_FILT) {
    WL_ERR(
        ("wl_cfg80211_set_mac_acl: invalid number of MAC address entries %d\n",
         macnum));
    return -1;
  }
  ...
  if ((ret = wl_android_set_ap_mac_list(cfgdev, macmode, list)) != 0)
    WL_ERR(
        ("wl_cfg80211_set_mac_acl: Setting MAC list failed error=%d\n", ret));
...
int wl_cfg80211_set_monitor_channel(struct wiphy *wiphy,
  ...
  WL_ERR(("%s: target_channel(%d), target bandwidth(%d)\n", __FUNCTION__,
          CHSPEC_CHANNEL(chspec), chandef->width));
  ...
  WL_DBG(
      ("%s: chandef->width(%u), bw(%u)\n", __FUNCTION__, chandef->width, bw));
#ifdef WL_6G_320_SUPPORT
  chspec = wf_create_chspec_from_primary(wf_chspec_primary20_chan(chspec), bw,
                                         CHSPEC_BAND(chspec), 0);
#else
  chspec = wf_create_chspec_from_primary(wf_chspec_primary20_chan(chspec), bw,
                                         CHSPEC_BAND(chspec));
#endif /* WL_6G_320_SUPPORT */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
int wl_chspec_chandef(chanspec_t chanspec, struct cfg80211_chan_def *chandef,
                      struct wiphy *wiphy) {
  uint16 freq = 0;
  struct ieee80211_channel *chan;
  ...
  freq = wl_channel_to_frequency(wf_chspec_primary20_chan(chanspec),
                                 CHSPEC_BAND(chanspec));
...
s32 wl_ap_start_ind(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
                    const wl_event_msg_t *e, void *data) {
  struct net_device *ndev = NULL;
  chanspec_t chanspec;
...
s32 wl_csa_complete_ind(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
                        const wl_event_msg_t *e, void *data) {
  int error = 0;
  u32 chanspec = 0;
  struct net_device *ndev = NULL;
...
s32 wl_cfg80211_tdls_config(struct bcm_cfg80211 *cfg, enum wl_tdls_config state,
  ...
  if (state == TDLS_STATE_TEARDOWN) {
    ...
    if (wl_get_drv_status_all(cfg, CONNECTED) >= TDLS_MAX_IFACE_FOR_ENABLE) {
    ...
    if (wl_get_drv_status_all(cfg, CONNECTED) == 0) {
      ...
      for_each_ndev(cfg, iter, next) {
        ...
        if ((iter->ndev) && (wl_get_drv_status(cfg, CONNECTED, ndev)) &&
          ...
          WL_DBG(
              ("Non STA iface operational. cfg_iftype:%d"
               " Can't enable tdls.\n",
  ...
  if (update_reqd == true) {
    if (dhdp->tdls_enable == enable) {
      WL_DBG(
          ("No change in tdls state. Do nothing."
           " tdls_enable:%d\n",
           enable));
      goto exit;
    }
    ...
    WL_DBG(
        ("Skip tdls config. state:%d update_reqd:%d "
         "current_status:%d \n",
...
int wl_set_ap_beacon_rate(struct net_device *dev, int val, char *ifname) {
  ...
  error = wldev_iovar_getbuf(ndev, "rateset", NULL, 0, &rs,
                             sizeof(wl_rateset_args_v1_t), NULL);
  if (error < 0) {
  ...
  for (i = 0; i < rs.count && i < WL_NUMRATES; i++)
    if (rs.rates[i] & WLC_RATE_FLAG)
      if ((rs.rates[i] & RATE_MASK) == val) break;
...
int wl_update_ap_rps_params(struct net_device *dev, ap_rps_info_t *rps,
  ...
  if (!rps) return BCME_BADARG;

  if (rps->pps < RADIO_PWRSAVE_PPS_MIN) return BCME_BADARG;

  if (rps->level < RADIO_PWRSAVE_LEVEL_MIN ||
      rps->level > RADIO_PWRSAVE_LEVEL_MAX)
    return BCME_BADARG;

  if (rps->quiet_time < RADIO_PWRSAVE_QUIETTIME_MIN) return BCME_BADARG;
...
int wl_cfg80211_change_he_features(struct net_device *dev,
                                   struct bcm_cfg80211 *cfg, s32 bssidx,
                                   u32 he_flag, bool set) {
  ...
  read_he_xtlv.len = 0;
  err =
      wldev_iovar_getbuf_bsscfg(dev, "he", &read_he_xtlv, sizeof(read_he_xtlv),
...
int wl_cfg80211_set_he_features(struct net_device *dev,
                                struct bcm_cfg80211 *cfg, s32 bssidx,
                                u32 interface_type, uint features) {
  uint8 se_he_xtlv[32];
  int se_he_xtlv_len = sizeof(se_he_xtlv);
  he_xtlv_v32 v32;
  s32 err = 0;
...
int wl_cfg80211_channel_switch(struct wiphy *wiphy, struct net_device *dev,
  ...
  WL_ERR(
      ("netdev_ifidx(%d), target channel(%d) target bandwidth(%d),"
       " mode(%d), count(%d)\n",
       dev->ifindex, CHSPEC_CHANNEL(chspec), chandef->width, params->block_tx,
       params->count));

  if (wl_get_mode_by_netdev(cfg, dev) != WL_MODE_AP) {
    WL_ERR(
        ("Channel Switch doesn't support on "
         "the non-SoftAP mode\n"));
    return -EINVAL;
  }
  ...
  if (chspec == cfg->ap_oper_channel) {
    WL_ERR(
        ("Channel %d is same as current operating channel,"
         " so skip\n",
  ...
  if (
    ...
    if (err < 0) {
      WL_ERR(
          ("Failed to get bandwidth information,"
           " err=%d\n",
           err));
      return err;
    }
    ...
    chanspec_t *sta_chanspec =
        (chanspec_t *)wl_read_prof(cfg, primary_dev, WL_PROF_CHAN);
    ...
    if (DHD_OPMODE_STA_SOFTAP_CONCURR(dhdp) &&
        wl_get_drv_status(cfg, CONNECTED, primary_dev) && sta_chanspec &&
        (CHSPEC_CHANNEL(*sta_chanspec) != CHSPEC_CHANNEL(chspec))) {
      WL_ERR(
          ("Invalid 2G Channel in case of STA/SoftAP"
  ...
  err =
      wldev_iovar_setbuf(dev, "csa", &csa_arg, sizeof(wl_chan_switch_t),
  ...
  if (is_bssup && suspend) {
    ...
    while (TRUE) {
      ...
      ret = wait_event_interruptible_timeout(
          cfg->netif_change_event, wl_get_drv_status(cfg, AP_CREATED, ndev),
...
int wl_set_softap_elna_bypass(struct net_device *dev, char *ifname,
                              int enable) {
  struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
  ...
  if (iftype == NL80211_IFTYPE_AP) {
    ...
    if (unlikely(err)) {
      WL_ERR(
          ("%s: Failed to set softap_elnabypass, err=%d\n", __FUNCTION__, err));
    }
  } else {
...
int wl_get_softap_elna_bypass(struct net_device *dev, char *ifname,
                              void *param) {
  struct bcm_cfg80211 *cfg = wl_get_cfg(dev);
  ...
  if (iftype == NL80211_IFTYPE_AP) {
    ...
    if (unlikely(err)) {
      WL_ERR(
          ("%s: Failed to get softap_elnabypass, err=%d\n", __FUNCTION__, err));
    }
  } else {
    WL_ERR(("%s: softap_elnabypass should control in SoftAP mode only\n",
            __FUNCTION__));
...
static void wl_update_apchan_bwcap(struct bcm_cfg80211 *cfg,
  ...
  if (ctl_chan <= CH_MAX_2G_CHANNEL) return;

  /* Get WL BW CAP */
  ret = wl_get_bandwidth_cap(bcmcfg_to_prmry_ndev(cfg), CHSPEC_BAND(chanspec),
                             &bw_cap);
  if (ret < 0) {
  ...
  chanbw = CHSPEC_BW(
      wl_channel_to_chanspec(wiphy, ndev, wf_chspec_ctlchan(chanspec), bw_cap));
...
int wl_set_ap_bw(struct net_device *dev, u32 bw, char *ifname) {
  ...
  if ((DHD_OPMODE_STA_SOFTAP_CONCURR(dhdp) && wl_is_sta_connected(cfg)) ||
...
void wl_restore_ap_bw(struct bcm_cfg80211 *cfg) {
  int ret = BCME_OK;
  u32 bw;
  bool he = FALSE;
  struct net_info *iter, *next;
  ...
  for_each_ndev(cfg, iter, next) {
    ...
    if (iter->ndev) {
      if (iter->ndev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) {
        chanspec = (chanspec_t *)wl_read_prof(cfg, iter->ndev, WL_PROF_CHAN);
        if (CHSPEC_IS2G(*chanspec)) {
          ndev = iter->ndev;
          break;
        }
...
static s32 wl_roam_off_config(struct net_device *dev, bool val) {
  s32 err;
...
#ifdef WL_DUAL_APSTA
static void wl_roam_enable_on_connected(struct bcm_cfg80211 *cfg, bool enable) {
  struct net_info *iter, *next;
  bool roam_val = false;
 ...
 */
static void wl_dualsta_enable_roam(struct bcm_cfg80211 *cfg,
...
void wl_cfgvif_roam_config(struct bcm_cfg80211 *cfg, struct net_device *dev,
  ...
  if ((state == ROAM_CONF_ASSOC_REQ) && (cfg->stas_associated >= 1)) {
...
uint32 wl_get_configured_ap_bw(dhd_pub_t *dhdp) {
  WL_DBG(
      ("%s: Confitured SoftAP BW is %d\n", __FUNCTION__, dhdp->wl_softap_bw));

  return dhdp->wl_softap_bw;
}

uint32 wl_update_configured_bw(uint32 bw) {
...
uint32 wl_cfgvif_get_iftype_count(struct bcm_cfg80211 *cfg,
                                  wl_iftype_t iftype) {
  struct net_info *iter, *next;
  u32 count = 0;
...
int wl_set_softap_antenna(struct net_device *dev, char *ifname, int set_chain) {
  ...
  if (iftype == NL80211_IFTYPE_AP) {
    ...
    WL_ERR(
        ("%s: Chain set should control in SoftAP mode only\n", __FUNCTION__));
    err = BCME_BADARG;
  }
fail:
  return err;
...
chanspec_t wl_cfg80211_get_ap_bw_limited_chspec(struct bcm_cfg80211 *cfg,
  ...
  if (!cfg->ap_bw_limit || (wf_bw_chspec_to_mhz(candidate) <=
                            wf_bw_chspec_to_mhz(cfg->ap_bw_chspec))) {
    /* LIMIT is not set or narrower bandwidth is required */
    return candidate;
  }
  updated_chspec = wf_create_chspec_from_primary(
      wf_chspec_primary20_chan(candidate), cfg->ap_bw_chspec,
 ...
 */
int wl_cfg80211_set_softap_bw(struct bcm_cfg80211 *cfg, uint32 band,
                              uint32 limit) {
...
struct net_device *wl_cfg80211_register_static_if(struct bcm_cfg80211 *cfg,
  ...
  wl_cfg80211_update_iflist_info(cfg, ndev, ifidx, NULL, WL_BSSIDX_MAX, ifname,
...
void wl_cfg80211_unregister_static_if(struct bcm_cfg80211 *cfg) {
  ...
  for (i = 0; i < DHD_MAX_STATIC_IFS; i++) {
    if (cfg->static_ndev[i]) dhd_unregister_net(cfg->static_ndev[i], true);
  }
}
...
s32 wl_cfg80211_static_if_open(struct net_device *net) {
  ...
  WL_INFORM_MEM(
      ("[STATIC_IF] dev_open ndev %p and wdev %p\n", net, net->ieee80211_ptr));
...
s32 wl_cfg80211_static_if_close(struct net_device *net) {
  ...
  if (cfg->static_ndev_state[static_ifidx] == NDEV_STATE_FW_IF_CREATED) {
    if (mutex_is_locked(&cfg->if_sync) == TRUE) {
      ret =
          _wl_cfg80211_del_if(cfg, primary_ndev, net->ieee80211_ptr, net->name);
    } else {
      ret =
          wl_cfg80211_del_if(cfg, primary_ndev, net->ieee80211_ptr, net->name);
    }
