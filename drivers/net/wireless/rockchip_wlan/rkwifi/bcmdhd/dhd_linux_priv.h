/*
 * DHD Linux header file - contains private structure definition of the Linux
 * specific layer
 *
...
typedef struct dhd_info {
  ...
  wifi_adapter_info_t
      *adapter;             /* adapter information, interrupt, fw path etc. */
   ...
   * and the packet count processed is as follows (assume the bin counters are
   * 0) iteration 1 - 10 (the bin counter 2^4 increments to 1) iteration 2 - 30
   * (the bin counter 2^5 increments to 1) iteration 3 - 15 (the bin counter 2^4
   * increments by 1 to become 2)
   */
...
#if defined(BT_OVER_SDIO)
  struct mutex
      bus_user_lock;  /* lock for sdio bus apis shared between WLAN & BT */
