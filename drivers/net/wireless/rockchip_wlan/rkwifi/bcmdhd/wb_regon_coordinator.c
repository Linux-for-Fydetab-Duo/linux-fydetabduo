...
struct wbrc_pvt_data {
  ...
  wait_queue_head_t
      wlan_reset_waitq; /* waitq to wait till wlan reset is done */
  unsigned int
      wlan_reset_ack; /* condition variable to be check for wlan reset */
...
#define WBRC_LOCK(wbrc_data)                             \
  {                                                      \
    if (wbrc_data) mutex_lock(&(wbrc_data)->wbrc_mutex); \
  }
#define WBRC_UNLOCK(wbrc_data)                             \
  {                                                        \
    if (wbrc_data) mutex_unlock(&(wbrc_data)->wbrc_mutex); \
  }
...
static __poll_t wbrc_bt_dev_poll(struct file *filep, poll_table *wait) {
  ...
  if (wbrc_data->read_data_available) mask |= EPOLLIN | EPOLLRDNORM;

  if (!wbrc_data->bt_dev_opened) mask |= EPOLLHUP;

  return mask;
}

int wbrc_reset_wait_on_condition(wait_queue_head_t *reset_waitq, uint *var,
                                 uint condition);
...
int wbrc_init(void) {
  ...
  wbrc_data->wbrc_bt_dev_major_number =
      register_chrdev(0, DEVICE_NAME, &wbrc_bt_dev_fops);
  ...
  wbrc_data->wbrc_bt_dev_device = device_create(
      wbrc_data->wbrc_bt_dev_class, NULL,
      MKDEV(wbrc_data->wbrc_bt_dev_major_number, 0), NULL, DEVICE_NAME);
...
void wbrc_exit(void) {
  ...
  pr_info("%s\n", __func__);
  wake_up_interruptible(&wbrc_data->outmsg_waitq);
  device_destroy(wbrc_data->wbrc_bt_dev_class,
                 MKDEV(wbrc_data->wbrc_bt_dev_major_number, 0));
  class_destroy(wbrc_data->wbrc_bt_dev_class);
  unregister_chrdev(wbrc_data->wbrc_bt_dev_major_number, DEVICE_NAME);
  kfree(wbrc_data);
  g_wbrc_data = NULL;
...
/* WBRC_LOCK should be held from caller */
static int wbrc_signal_wlan_reset(struct wbrc_pvt_data *wbrc_data) {
#ifdef WBRC_BT2WL_RESET
  /* Force trap wl */
  dhd_wbrc_wl_trap();
#endif /* WBRC_BT2WL_RESET */
