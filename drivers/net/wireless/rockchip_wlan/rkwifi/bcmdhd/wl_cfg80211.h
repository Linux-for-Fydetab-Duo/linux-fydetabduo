...
/**
 * Older Linux versions support the 'iw' interface, more recent ones the
 * 'cfg80211' interface.
 */
...
#define WL_CFG_NET_LIST_SYNC_UNLOCK(lock, flags) \
...
#define WL_CFG_VNDR_OUI_SYNC_UNLOCK(lock, flags) \
  osl_spin_unlock((lock), (flags))
...
#error \
    "WL_SAE is for dongle-offload and WL_CLIENT_SAE is for wpa_supplicant. Please choose one."
#endif
...
#ifdef WL_SAE
#error \
    "WL_SAE is for dongle-offload and WL_CLIENT_SAE is for wpa_supplicant. \
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 17, 0)) && \
    !defined(WL_DISABLE_SCAN_TYPE) && !defined(WL_SCAN_TYPE)
#define WL_SCAN_TYPE
#endif /* WL_SCAN_TYPE */
...
#define IS_STA_INFO_VER(sta) \
  (dtoh16(sta->ver) >= WL_STA_VER_4 && dtoh16(sta->ver) <= WL_STA_VER_6)
...
#define CHAN2G(_channel, _freq, _flags)                           \
  {                                                               \
    ...
    .hw_value = HW_VALUE_CHANNEL_2G(_channel), .flags = (_flags), \
    .max_antenna_gain = 0, .max_power = 30,                       \
  }

#define CHAN5G(_channel, _flags)                                         \
  {                                                                      \
    .band = IEEE80211_BAND_5GHZ, .center_freq = 5000 + (5 * (_channel)), \
    .hw_value = HW_VALUE_CHANNEL_5G(_channel), .flags = (_flags),        \
    .max_antenna_gain = 0, .max_power = 30,                              \
  }
...
#define CHAN6G(_channel, _flags)                                         \
  {                                                                      \
    .band = IEEE80211_BAND_6GHZ, .center_freq = 5950 + (5 * (_channel)), \
    .hw_value = HW_VALUE_CHANNEL_6G(_channel), .flags = (_flags),        \
    .max_antenna_gain = 0, .max_power = 30,                              \
  }

#define CHAN6G_CHAN2(_flags)                                        \
  {                                                                 \
    .band = IEEE80211_BAND_6GHZ, .center_freq = 5935,               \
    .hw_value = HW_VALUE_CHANNEL_6G_2(_channel), .flags = (_flags), \
    .max_antenna_gain = 0, .max_power = 30,                         \
  }
#else
#define CHAN6G(_channel, _flags)                                         \
  {                                                                      \
    .band = IEEE80211_BAND_5GHZ, .center_freq = 5950 + (5 * (_channel)), \
    .hw_value = HW_VALUE_CHANNEL_6G(_channel), .flags = (_flags),        \
    .max_antenna_gain = 0, .max_power = 30,                              \
  }
...
#define CHAN6G_CHAN2(_flags)                                        \
  {                                                                 \
    .band = IEEE80211_BAND_5GHZ, .center_freq = 5935,               \
    .hw_value = HW_VALUE_CHANNEL_6G_2(_channel), .flags = (_flags), \
    .max_antenna_gain = 0, .max_power = 30,                         \
  }
...
#ifdef DHD_LOG_DUMP
extern void dhd_log_dump_write(int type, char *binary_data, int binary_len,
                               const char *fmt, ...);
...
#define DHD_LOG_DUMP_WRITE_PRSRV_TS_FN \
  DHD_LOG_DUMP_WRITE_PRSRV(DHD_PREFIX_TS_FN)
...
#define WL_DBG_PRINT_SYSTEM_TIME pr_cont("[%s]", OSL_GET_RTCTIME())
#else
#define WL_DBG_PRINT_SYSTEM_TIME
#endif /* defined(CUSTOMER_DBG_SYSTEM_TIME) && \
          defined(DHD_DEBUGABILITY_LOG_DUMP_RING) */

#if defined(CUSTOMER_DBG_PREFIX_ENABLE)
#ifdef CUSTOM_PREFIX_NORTCTIME
#define USER_PREFIX_CFG80211 CUSTOM_PREFIX_NORTCTIME
#else
...
#define WL_CONS_ONLY_MSG(x, args...)                          \
  do {                                                        \
    printf(USER_PREFIX_CFG80211 "%s : " x, __func__, ##args); \
  } while (0)
#define WL_CONS_ONLY(x) WL_CONS_ONLY_MSG x
#else
...
#define CHSPEC_TO_WLC_BAND(chspec)   \
...
#endif /* WL_6G_BAND */
#define CHSPEC_IS_6G_PSC(chspec) \
  (CHSPEC_IS6G(chspec) && ((wf_chspec_ctlchan(chspec) % 16) == 5))
...
/*
 * BRCM local.
 * Use a high number that's unlikely to clash with linux upstream for a while
 * until we can submit these changes to the community.
 */
#define NL80211_FEATURE_FW_4WAY_HANDSHAKE (1 << 31)

/* SCAN_SUPPRESS timer values in ms */
#define WL_SCAN_SUPPRESS_TIMEOUT \
  31000 /* default Framwork DHCP timeout is 30 sec */
...
#define WL_WOWLAN_PKT_FILTER_ID_LAST \
  (WL_WOWLAN_PKT_FILTER_ID_FIRST + WL_WOWLAN_MAX_PATTERNS - 1)
...
struct preinit_iov;
typedef int (*wl_iov_fn)(struct bcm_cfg80211 *cfg, struct net_device *dev,
                         struct preinit_iov *v);
...
enum wl_status {
  ...
  /* find a peer to go to a common channel before sending public action req
     frame */
  ...
  /* it will be set when upper layer requests listen and succeed in setting
   * listen mode. if set, other scan request can abort current listen state
   */
  WL_STATUS_REMAINING_ON_CHANNEL,
#ifdef WL_CFG80211_VSDB_PRIORITIZE_SCAN_REQUEST
  /* it's fake listen state to keep current scan state.
   * it will be set when upper layer requests listen but scan is running. then
   * just run a expire timer without actual listen state. if set, other scan
   * request does not need to abort scan.
   */
...
/* donlge escan state */
enum wl_escan_state { WL_ESCAN_STATE_IDLE, WL_ESCAN_STATE_SCANING };
/* fw downloading status */
enum wl_fw_status { WL_FW_LOADING_DONE, WL_NVRAM_LOADING_DONE };
...
typedef s32 (*EVENT_HANDLER)(struct bcm_cfg80211 *cfg,
...
typedef struct wl_cfgbss {
  ...
  u8 *fils_ind_ie;
  bool security_mode;
  struct wl_bss_vndr_ies
      ies; /* Common for STA, P2P GC, GO, AP, P2P Disc Interface */
} wl_cfgbss_t;
...
#define WLC_PMKDB_SUPPORT(ver) \
  ((ver.wlc_ver_major >= PMKDB_WLC_VER) || NEW_PMK_MGR_API_BACK_PORTED(ver))
...
enum wl_dd_state { WL_DD_STATE_IDLE, WL_DD_STATE_SEARCH, WL_DD_STATE_LISTEN };
...
typedef enum wl_prune_evt_reason {
  ...
  WIFI_PRUNE_CHANNEL_NOT_IN_VLP =
      3,                      /* Prune AP due to Chanspec not in VLP cat */
...
#define WL_ROAM_REVERT_STATUS 0x0002
  ...
  bool p2p_prb_noti;
#endif
  s32 (*state_notifier)(struct bcm_cfg80211 *cfg, struct net_info *_net_info,
  ...
  struct wireless_dev
      *nmi_wdev; /* representing cfg cfg80211 device for NAN NMI */
  ...
  uint32
      roam_allowed_band; /* roam allow band in order to purne roam candidate */
...
#define IS_5G_APCS_CHANNEL(channel) \
...
static inline wl_bss_info_v109_t *next_bss(wl_scan_results_v109_t *list,
                                           wl_bss_info_v109_t *bss) {
  return bss = bss ? (wl_bss_info_v109_t *)((uintptr)bss + dtoh32(bss->length))
                   : list->bss_info;
}

static inline void wl_probe_wdev_all(struct bcm_cfg80211 *cfg) {
...
static inline struct net_info *_wl_get_netinfo_by_fw_idx(
    struct bcm_cfg80211 *cfg, s32 bssidx, u8 ifidx) {
...
static inline struct net_info *wl_get_netinfo_by_fw_idx(
    struct bcm_cfg80211 *cfg, s32 bssidx, u8 ifidx) {
  unsigned long int flags;
...
static inline void wl_dealloc_netinfo_by_wdev(struct bcm_cfg80211 *cfg,
                                              struct wireless_dev *wdev) {
  struct net_info *_net_info, *next;
  unsigned long int flags;
...
static inline s32 wl_alloc_netinfo(struct bcm_cfg80211 *cfg,
                                   struct net_device *ndev,
                                   struct wireless_dev *wdev,
                                   wl_iftype_t iftype, bool pm_block, u8 bssidx,
                                   u8 ifidx) {
  ...
  if ((_net_info = wl_get_netinfo_by_fw_idx(cfg, bssidx, ifidx))) {
    ...
    if (bssidx != (u8)WL_INVALID) {
      WL_ERR(
          ("**Duplicate entry for bssidx=%d ifidx=%d present."
           " Can't add new entry\n",
           bssidx, ifidx));
  ...
  if (cfg->iface_cnt == IFACE_MAX_CNT) return -ENOMEM;
...
static inline void wl_delete_all_netinfo(struct bcm_cfg80211 *cfg) {
  struct net_info *_net_info, *next;
  unsigned long int flags;
...
static inline u32 wl_get_status_all(struct bcm_cfg80211 *cfg, s32 status)

{
  ...
  BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
    GCC_DIAGNOSTIC_POP();
    if (_net_info->ndev && test_bit(status, &_net_info->sme_state)) cnt++;
  }
  WL_CFG_NET_LIST_SYNC_UNLOCK(&cfg->net_list_sync, flags);
  return cnt;
}

static inline void wl_set_status_all(struct bcm_cfg80211 *cfg, s32 status,
                                     u32 op) {
  struct net_info *_net_info, *next;
  unsigned long int flags;
  ...
  BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
    ...
    switch (op) {
      ...
      case 2:
        ...
        if (cfg->state_notifier && test_bit(status, &(cfg->interrested_state)))
...
static inline void wl_track_if_states(struct bcm_cfg80211 *cfg,
                                      struct net_info *_net_info, s32 status,
                                      bool set) {
...
static inline void wl_set_status_by_netdev(struct bcm_cfg80211 *cfg, s32 status,
...
static inline wl_cfgbss_t *wl_get_cfgbss_by_wdev(struct bcm_cfg80211 *cfg,
                                                 struct wireless_dev *wdev) {
  struct net_info *_net_info, *next;
...
static inline u32 wl_get_status_by_netdev(struct bcm_cfg80211 *cfg, s32 status,
                                          struct net_device *ndev) {
  struct net_info *_net_info, *next;
  u32 stat = 0;
  unsigned long int flags;
...
static inline s32 wl_get_mode_by_netdev(struct bcm_cfg80211 *cfg,
                                        struct net_device *ndev) {
  struct net_info *_net_info, *next;
  s32 mode = -1;
  unsigned long int flags;
...
static inline s32 wl_get_bssidx_by_wdev(struct bcm_cfg80211 *cfg,
                                        struct wireless_dev *wdev) {
  struct net_info *_net_info, *next;
  s32 bssidx = -1;
  unsigned long int flags;
...
static inline struct wireless_dev *wl_get_wdev_by_fw_idx(
    struct bcm_cfg80211 *cfg, s32 bssidx, s32 ifidx) {
  struct net_info *_net_info, *next;
  struct wireless_dev *wdev = NULL;
  unsigned long int flags;

  if (bssidx < 0) return NULL;
  WL_CFG_NET_LIST_SYNC_LOCK(&cfg->net_list_sync, flags);
  GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
  BCM_LIST_FOR_EACH_ENTRY_SAFE(_net_info, next, &cfg->net_list, list) {
    GCC_DIAGNOSTIC_POP();
...
static inline struct wl_profile *wl_get_profile_by_netdev(
...
static inline struct net_info *wl_get_netinfo_by_netdev(
    struct bcm_cfg80211 *cfg, struct net_device *ndev) {
  struct net_info *_net_info, *next, *info = NULL;
  unsigned long int flags;
...
static inline struct net_info *_wl_get_netinfo_by_wdev(
    struct bcm_cfg80211 *cfg, struct wireless_dev *wdev) {
  struct net_info *_net_info, *next, *info = NULL;
...
static inline struct net_info *wl_get_netinfo_by_wdev(
    struct bcm_cfg80211 *cfg, struct wireless_dev *wdev) {
  struct net_info *info = NULL;
  unsigned long int flags;
...
static inline char *wl_iftype_to_str(int wl_iftype) {
  switch (wl_iftype) {
    case (WL_IF_TYPE_STA):
...
#define is_discovery_iface(iface) \
  (((iface == WL_IF_TYPE_P2P_DISC) || (iface == WL_IF_TYPE_NAN_NMI)) ? 1 : 0)
#define IS_P2P_GC(wdev) ((wdev->iftype == NL80211_IFTYPE_P2P_CLIENT) ? 1 : 0)
#define IS_P2P_GO(wdev) ((wdev->iftype == NL80211_IFTYPE_P2P_GO) ? 1 : 0)
#define is_p2p_group_iface(wdev)                 \
...
#define IS_P2P_IFACE(wdev)                                 \
  (wdev && ((wdev->iftype == NL80211_IFTYPE_P2P_DEVICE) || \
            (wdev->iftype == NL80211_IFTYPE_P2P_GO) ||     \
            (wdev->iftype == NL80211_IFTYPE_P2P_CLIENT)))
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
#define IS_P2P_IFACE(ieee80211_ptr)                                          \
  (ieee80211_ptr && ((ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_DEVICE) || \
                     (ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_GO) ||     \
                     (ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_CLIENT)))
#else
#define IS_P2P_IFACE(ieee80211_ptr)                                      \
  (ieee80211_ptr && ((ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_GO) || \
                     (ieee80211_ptr->iftype == NL80211_IFTYPE_P2P_CLIENT)))
...
#endif /* WL_DUAL_STA */
#define IS_STA_IFACE(wdev) (wdev && (wdev->iftype == NL80211_IFTYPE_STATION))

#define IS_AP_IFACE(wdev) (wdev && (wdev->iftype == NL80211_IFTYPE_AP))

#if defined(WL_ENABLE_P2P_IF)
#define ndev_to_wlc_ndev(ndev, cfg) \
  ((ndev == cfg->p2p_net) ? bcmcfg_to_prmry_ndev(cfg) : ndev)
#else
#define ndev_to_wlc_ndev(ndev, cfg) (ndev)
#endif /* WL_ENABLE_P2P_IF */

#define wdev_to_wlc_ndev(wdev, cfg) \
  (wdev_to_ndev(wdev) ? wdev_to_ndev(wdev) : bcmcfg_to_prmry_ndev(cfg))
#if defined(WL_CFG80211_P2P_DEV_IF)
...
#define discover_cfgdev(cfgdev, cfg) \
  (cfgdev->iftype == NL80211_IFTYPE_P2P_DEVICE)
#else
...
#if defined(WL_CFG80211_P2P_DEV_IF)
#define scan_req_match(cfg)                     \
  (((cfg) && (cfg->scan_request) &&             \
    (cfg->scan_request->wdev == cfg->p2p_wdev)) \
       ? true                                   \
       : false)
#elif defined(WL_ENABLE_P2P_IF)
#define scan_req_match(cfg)                                                   \
  (((cfg) && (cfg->scan_request) && (cfg->scan_request->dev == cfg->p2p_net)) \
       ? true                                                                 \
       : false)
#else
#define scan_req_match(cfg) \
  (((cfg) && p2p_is_on(cfg) && p2p_scan(cfg)) ? true : false)
#endif /* WL_CFG80211_P2P_DEV_IF */
...
#define PRINT_WDEV_INFO(cfgdev)                                 \
  {                                                             \
    ...
    WL_DBG(("wdev_ptr:%p ndev_ptr:%p ifname:%s iftype:%d\n",    \
            OSL_OBFUSCATE_BUF(wdev), OSL_OBFUSCATE_BUF(netdev), \
...
#define for_each_bss(list, bss, __i)                  \
  for (__i = 0; __i < list->count && __i < WL_AP_MAX; \
...
#define is_wps_conn(_sme)                                    \
  ((wl_cfgp2p_find_wpsie(_sme->ie, _sme->ie_len) != NULL) && \
   (!_sme->crypto.n_ciphers_pairwise) && (!_sme->crypto.cipher_group))
...
extern s32 wl_cfg80211_clear_per_bss_ies(struct bcm_cfg80211 *cfg,
                                         struct wireless_dev *wdev);
extern void wl_cfg80211_clear_p2p_disc_ies(struct bcm_cfg80211 *cfg);
#ifdef WL_STATIC_IF
extern int32 wl_cfg80211_update_iflist_info(struct bcm_cfg80211 *cfg,
...
extern s32 wl_cfg80211_notify_ifadd(struct net_device *dev, int ifidx,
                                    char *name, uint8 *mac, uint8 bssidx,
                                    uint8 role);
extern s32 wl_cfg80211_notify_ifdel(struct net_device *dev, int ifidx,
                                    char *name, uint8 *mac, uint8 bssidx);
extern s32 wl_cfg80211_notify_ifchange(struct net_device *dev, int ifidx,
                                       char *name, uint8 *mac, uint8 bssidx);
extern struct net_device *wl_cfg80211_allocate_if(struct bcm_cfg80211 *cfg,
                                                  int ifidx, const char *name,
                                                  uint8 *mac, uint8 bssidx,
                                                  const char *dngl_name);
extern int wl_cfg80211_register_if(struct bcm_cfg80211 *cfg, int ifidx,
                                   struct net_device *ndev,
                                   bool rtnl_lock_reqd);
extern int wl_cfg80211_remove_if(struct bcm_cfg80211 *cfg, int ifidx,
...
extern s32 wl_cfg80211_increase_p2p_bw(struct net_device *net, char *buf,
                                       int len);
#ifdef P2PLISTEN_AP_SAMECHN
...
extern s32 wl_cfg80211_sd_offload(struct net_device *net, char *cmd, char *buf,
                                  int len);
extern s32 wl_cfg80211_pause_sdo(struct net_device *dev,
                                 struct bcm_cfg80211 *cfg);
extern s32 wl_cfg80211_resume_sdo(struct net_device *dev,
                                  struct bcm_cfg80211 *cfg);
...
#define CHANNEL_IS_RADAR(channel) \
  (((channel & WL_CHAN_RADAR) || (channel & WL_CHAN_PASSIVE)) ? true : false)
#define CHANNEL_IS_2G(channel) \
  (((channel >= 1) && (channel <= 14)) ? true : false)
#define CHANNEL_IS_5G(channel) \
  (((channel >= 36) && (channel <= 165)) ? true : false)
...
extern void wl_stop_wait_next_action_frame(struct bcm_cfg80211 *cfg,
                                           struct net_device *ndev,
                                           u8 bsscfgidx);
#ifdef WL_HOST_BAND_MGMT
...
extern void wl_cfg80211_add_to_eventbuffer(wl_eventmsg_buf_t *ev, u16 event,
                                           bool set);
...
extern s32 wl_cfg80211_set_country_code(struct net_device *dev,
                                        char *country_code, bool notify,
...
#ifdef WLFBT
extern int wl_cfg80211_get_fbt_key(struct net_device *dev, uint8 *key,
                                   int total_len);
#endif
...
#ifdef WL_UNII4_CHAN
#define IS_5G_UNII4_165_CHANNEL(chspec) \
  (CHSPEC_IS5G(chspec) && (wf_chspec_primary20_chan(chspec) == 165))
#define UINII4_169 169
#define UINII4_173 173
#define UINII4_177 177
#define IS_UNII4_CHANNEL(channel)                        \
  ((channel == UINII4_169) || (channel == UINII4_173) || \
   (channel == UINII4_177))
#endif /* WL_UNII4_CHAN */
...
extern s32 wl_cfg80211_wbtext_delta_config(struct net_device *ndev, char *data,
                                           char *command, int total_len);
#endif /* WBTEXT */
extern s32 wl_cfg80211_get_band_chanspecs(struct net_device *ndev, void *buf,
                                          s32 buflen, chanspec_band_t band,
                                          bool acs_req);

extern s32 wl_cfg80211_bss_up(struct bcm_cfg80211 *cfg, struct net_device *ndev,
                              s32 bsscfg_idx, s32 up);
...
extern s32 wl_tdls_event_handler(struct bcm_cfg80211 *cfg,
...
#ifdef RTT_SUPPORT
extern s32 wl_cfgvendor_send_as_rtt_legacy_event(
...
struct wireless_dev *wl_cfg80211_add_if(struct bcm_cfg80211 *cfg,
                                        ...
                                        const u8 *mac);
extern s32 wl_cfg80211_del_if(struct bcm_cfg80211 *cfg,
                              struct net_device *primary_ndev,
...
#ifdef WL_STATIC_IF
extern struct net_device *wl_cfg80211_register_static_if(
    struct bcm_cfg80211 *cfg, u16 iftype, char *ifname, int static_ifidx);
...
extern struct net_device *wl_cfg80211_post_static_ifcreate(
...
extern s32 wl_cfg80211_post_static_ifdel(struct bcm_cfg80211 *cfg,
                                         struct net_device *ndev);
#endif /* WL_STATIC_IF */
extern struct wireless_dev *wl_cfg80211_get_wdev_from_ifname(
    struct bcm_cfg80211 *cfg, const char *name);
struct net_device *wl_get_netdev_by_name(struct bcm_cfg80211 *cfg,
                                         char *ifname);
extern int wl_cfg80211_ifstats_counters(struct net_device *dev,
                                        wl_if_stats_t *if_stats);
extern int wl_cfg80211_if_infra_enh_ifstats_counters(
...
#ifdef WL_WPS_SYNC
void wl_handle_wps_states(struct net_device *ndev, u8 *dump_data, u16 len,
                          bool direction);
#endif /* WL_WPS_SYNC */
extern int wl_features_set(u8 *array, uint8 len, u32 ftidx);
extern void *wl_read_prof(struct bcm_cfg80211 *cfg, struct net_device *ndev,
                          s32 item);
extern s32 wl_cfg80211_sup_event_handler(struct bcm_cfg80211 *cfg,
                                         bcm_struct_cfgdev *cfgdev,
...
extern s32 wl_cfg80211_config_suspend_events(struct net_device *ndev,
                                             bool enable);
#ifdef PCIE_INB_DW
...
#endif /* WES_SUPPORT */
#ifdef KEEP_ALIVE
extern int wl_cfg80211_start_mkeep_alive(
    struct net_device *ndev, struct bcm_cfg80211 *cfg, uint8 mkeep_alive_id,
    uint16 ether_type, uint8 *ip_pkt, uint16 ip_pkt_len, uint8 *src_mac_addr,
    uint8 *dst_mac_addr, uint32 period_msec);
extern int wl_cfg80211_stop_mkeep_alive(struct net_device *ndev,
                                        struct bcm_cfg80211 *cfg,
                                        uint8 mkeep_alive_id);
#endif /* KEEP_ALIVE */

extern s32 wl_cfg80211_handle_macaddr_change(struct net_device *dev,
                                             u8 *macaddr);
...
extern bool wl_cfg80211_find_gas_subtype(u8 subtype, u16 adv_id, u8 *data,
                                         s32 len);
...
#define WL_CHANNEL_ARRAY_INIT(band_chan_arr)                               \
  do {                                                                     \
    ...
    for (k = 0; k < arr_size; k++) {                                       \
      band_chan_arr[k].flags = CH_DEFAULT_FLAGS | IEEE80211_CHAN_NO_HT40 | \
...
extern int wl_cfg80211_config_rsnxe_ie(struct bcm_cfg80211 *cfg,
...
extern u32 wl_cfg80211_debug_data_dump(struct net_device *dev, u8 *buf,
                                       u32 buf_len);
extern void wl_cfg80211_concurrent_roam(struct bcm_cfg80211 *cfg, int enable);

extern void wl_cfg80211_iface_state_ops(struct wireless_dev *wdev,
...
extern s32 wl_bss_handle_sae_auth(struct bcm_cfg80211 *cfg,
                                  struct net_device *ndev,
                                  const wl_event_msg_t *event, void *data);
#ifdef WL_WPS_SYNC
extern s32 wl_wps_session_update(struct net_device *ndev, u16 state,
                                 const u8 *peer_mac);
#endif /* WL_WPS_SYNC */
extern s32 wl_update_prof(struct bcm_cfg80211 *cfg, struct net_device *ndev,
                          const wl_event_msg_t *e, const void *data, s32 item);
#ifdef WL_CLIENT_SAE
extern s32 wl_handle_auth_event(struct bcm_cfg80211 *cfg,
                                struct net_device *ndev,
                                const wl_event_msg_t *e, void *data);
#endif /* WL_CLIENT_SAE */
...
extern void wl_cfg80211_set_okc_pmkinfo(struct bcm_cfg80211 *cfg,
...
extern void wl_android_roamoff_dbg_save(struct net_device *dev, uint rsn,
                                        uint roamvar);
extern void wl_android_roamoff_dbg_dump(struct bcm_cfg80211 *cfg);

#define ROAMOFF_DBG_CLR(cfg) wl_android_roamoff_dbg_clr((cfg));
#define ROAMOFF_DBG_SAVE(dev, rsn, var) \
  wl_android_roamoff_dbg_save((dev), (rsn), (var));
#define ROAMOFF_DBG_DUMP(cfg) wl_android_roamoff_dbg_dump((cfg));
#else
...
extern int wl_get_all_sideband_chanspecs(uint center_channel,
                                         chanspec_band_t band, chanspec_bw_t bw,
                                         chanspec_t *chspecs, int *cnt);
...
extern u8 *wl_get_up_table_netinfo(struct bcm_cfg80211 *cfg,
                                   struct net_device *ndev);
