...
typedef enum { NONE_4WAY, M1_4WAY, M2_4WAY, M3_4WAY, M4_4WAY } msg_4way_t;
...
extern int dhd_get_suspend_bcn_li_dtim(dhd_pub_t *dhd, int *dtim_period,
                                       int *bcn_interval);
#else
...
typedef struct dhd_if {
  ...
  bool qosmap_up_table_enable; /* flag set only when app want to set additional
                                  UP */
...
int dhd_get_platform_naming_for_nvram_clmblob_file(download_type_t component,
                                                   char *file_name);
#ifdef USE_CID_CHECK
