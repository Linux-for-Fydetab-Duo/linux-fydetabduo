...
#ifndef BCMDRIVER
/*	Take an array of measurments representing a single channel over time and
   return a summary. Currently implemented as a simple average but could easily
   evolve into more cpomplex alogrithms.
*/
cca_congest_channel_req_t *cca_per_chan_summary(
    cca_congest_channel_req_t *input, cca_congest_channel_req_t *avg,
    bool percent) {
  int sec;
  ...
  if (!avg->num_secs || !totals.duration) return (avg);
...
static void cca_info(uint8 *bitmap, int num_bits, int *left, int *bit_pos) {
  int i;
...
/*
        Take an array of measumrements representing summaries of different
   channels. Return a recomended channel. Interference is evil, get rid of that
   first. Then hunt for lowest Other bss traffic. Don't forget that channels
   with low duration times may not have accurate readings. For the moment, do
   not overwrite input array.
*/
int cca_analyze(cca_congest_channel_req_t *input[], int num_chans, uint flags,
                chanspec_t *answer) {
  ...
  for (i = 0; i < num_chans; i++) {
    if (input[i]->chanspec) setbit(bitmap, i);
  }
  ...
  if (flags & CCA_FLAG_2G_ONLY) {
    for (i = 0; i < num_chans; i++) {
      if (!CHSPEC_IS2G(input[i]->chanspec)) clrbit(bitmap, i);
    }
  }
  cca_info(bitmap, num_chans, &left, &i);
  if (!left) {
  ...
  if (flags & CCA_FLAG_5G_ONLY) {
    for (i = 0; i < num_chans; i++) {
      if (!CHSPEC_IS5G(input[i]->chanspec)) clrbit(bitmap, i);
    }
  }
  cca_info(bitmap, num_chans, &left, &i);
  if (!left) {
  ...
  if (!(flags & CCA_FLAG_IGNORE_DURATION)) {
    for (i = 0; i < num_chans; i++) {
      if (input[i]->secs[0].duration < CCA_THRESH_MILLI) clrbit(bitmap, i);
    }
  }
  cca_info(bitmap, num_chans, &left, &i);
  if (!left) {
...
#define IDX_IN_WL_CNT_VER_6_T(cntmember)                                       \
  ((OFFSETOF(wl_cnt_ver_6_t, cntmember) - OFFSETOF(wl_cnt_ver_6_t, txframe)) / \
   sizeof(uint32))

#define IDX_IN_WL_CNT_VER_7_T(cntmember)                                       \
  ((OFFSETOF(wl_cnt_ver_7_t, cntmember) - OFFSETOF(wl_cnt_ver_7_t, txframe)) / \
   sizeof(uint32))

#define IDX_IN_WL_CNT_VER_11_T(cntmember)  \
  ((OFFSETOF(wl_cnt_ver_11_t, cntmember) - \
    OFFSETOF(wl_cnt_ver_11_t, txframe)) /  \
   sizeof(uint32))

/* Exclude version and length fields */
#define NUM_OF_CNT_IN_WL_CNT_VER_6_T \
  ((sizeof(wl_cnt_ver_6_t) - 2 * sizeof(uint16)) / sizeof(uint32))
/* Exclude macstat cnt variables. wl_cnt_ver_6_t only has 62 macstat cnt
 * variables. */
...
static int wl_copy_wlccnt(uint16 cntver, uint32 *dst, uint32 *src,
                          uint8 src_max_idx) {
  uint i;
...
static int wl_copy_macstat_upto_ver10(uint16 cntver, uint32 *dst, uint32 *src) {
  uint i;
...
static int wl_copy_macstat_ver11(uint32 *dst, uint32 *src) {
  uint i;
...
/**
 * Translate non-xtlv 'wl counters' IOVar buffer received by old driver/FW to
 *xtlv format. Parameters: cntbuf: pointer to non-xtlv 'wl counters' IOVar
 *buffer received by old driver/FW. Newly translated xtlv format is written to
 *this pointer. buflen: length of the "cntbuf" without any padding. corerev:
 *chip core revision of the driver/FW.
 */
int wl_cntbuf_to_xtlv_format(void *ctx, void *cntbuf, int buflen,
                             uint32 corerev) {
  ...
  if (NUM_OF_CNT_IN_WL_CNT_VER_6_T > ((uint8)(-1) + 1) ||
    ...
    printf(
        "wlcntverXXt_to_wlcntwlct and src_max_idx need"
        " to be of uint16 instead of uint8\n");
    res = BCME_ERROR;
    goto exit;
  }

  /* Exclude version and length fields in either wlc_cnt_ver_6_t or
   * wlc_cnt_ver_11_t */
  src_max_idx =
      (cntinfo->datalen - OFFSETOF(wl_cnt_info_t, data)) / sizeof(uint32);
  ...
  res = wl_copy_wlccnt(ver, (uint32 *)wlccnt, (uint32 *)cntinfo->data,
                       (uint8)src_max_idx);
  if (res != BCME_OK) {
  ...
  res = bcm_pack_xtlv_buf_from_mem(&xtlvbuf_p, &xtlvbuflen, xtlv_desc,
