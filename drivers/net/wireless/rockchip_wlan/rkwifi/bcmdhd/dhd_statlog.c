...
#define DHD_STATLOG_ERR_INTERNAL(fmt, ...) \
  DHD_ERROR(("STATLOG-" fmt, ##__VA_ARGS__))
#define DHD_STATLOG_INFO_INTERNAL(fmt, ...) \
  DHD_INFO(("STATLOG-" fmt, ##__VA_ARGS__))
...
#define DHD_STATLOG_VALID(stat) \
  (((stat) > (ST(INVALID))) && ((stat) < (ST(MAX))))

dhd_statlog_handle_t *dhd_attach_statlog(dhd_pub_t *dhdp, uint32 num_items,
                                         uint32 bdlog_num_items,
                                         uint32 logbuf_len) {
  ...
  if (!statlog) {
    DHD_STATLOG_ERR(
        ("%s: failed to allocate memory for dhd_statlog_t\n", __FUNCTION__));
    return NULL;
  }
  ...
  /* alloc ring buffer */
  statlog->bufsize =
      (uint32)(dhd_ring_get_hdr_size() + DHD_STATLOG_RING_SIZE(num_items));
  buf = VMALLOCZ(dhdp->osh, statlog->bufsize);
  if (!buf) {
    DHD_STATLOG_ERR(
        ("%s: failed to allocate memory for ring buffer\n", __FUNCTION__));
    goto error;
  }

  statlog->ringbuf =
      dhd_ring_init(dhdp, buf, statlog->bufsize, DHD_STATLOG_ITEM_SIZE,
                    num_items, DHD_RING_TYPE_SINGLE_IDX);
  if (!statlog->ringbuf) {
  ...
  if (!buf) {
    DHD_STATLOG_ERR(
        ("%s: failed to allocate memory for bigdata logging buffer\n",
         __FUNCTION__));
    goto error;
  }

  statlog->bdlog_ringbuf =
      dhd_ring_init(dhdp, buf, statlog->bdlog_bufsize, DHD_STATLOG_ITEM_SIZE,
                    bdlog_num_items, DHD_RING_TYPE_SINGLE_IDX);
  if (!statlog->bdlog_ringbuf) {
    DHD_STATLOG_ERR(
        ("%s: failed to init ring buffer for bigdata logging\n", __FUNCTION__));
...
void dhd_detach_statlog(dhd_pub_t *dhdp) {
...
static int dhd_statlog_ring_log(dhd_pub_t *dhdp, uint16 stat, uint8 ifidx,
                                uint8 dir, uint16 status, uint16 reason) {
  dhd_statlog_t *statlog;
  stat_elem_t *elem;
  ...
  if (isset(statlog->bdmask, stat)) {
    ...
    if (!elem_bd) {
      /* no available slot */
      DHD_STATLOG_ERR(
          ("%s: cannot allocate a new element for bigdata\n", __FUNCTION__));
      return BCME_ERROR;
    }
...
int dhd_statlog_ring_log_data(dhd_pub_t *dhdp, uint16 stat, uint8 ifidx,
                              uint8 dir, bool cond) {
...
int dhd_statlog_ring_log_data_reason(dhd_pub_t *dhdp, uint16 stat, uint8 ifidx,
                                     uint8 dir, uint16 reason) {
  return dhd_statlog_ring_log(dhdp, stat, ifidx, dir ? STDIR(TX) : STDIR(RX), 0,
                              reason);
}

int dhd_statlog_ring_log_ctrl(dhd_pub_t *dhdp, uint16 stat, uint8 ifidx,
                              uint16 reason) {
  return dhd_statlog_ring_log(dhdp, stat, ifidx, ST(DIR_TX), 0, reason);
}

int dhd_statlog_process_event(dhd_pub_t *dhdp, int type, uint8 ifidx,
...
uint32 dhd_statlog_get_logbuf_len(dhd_pub_t *dhdp) {
  uint32 length = 0;
  dhd_statlog_t *statlog;
...
void *dhd_statlog_get_logbuf(dhd_pub_t *dhdp) {
  dhd_statlog_t *statlog;
  void *ret_addr = NULL;
 ...
 */
static void dhd_statlog_stat_name(char *buf, uint32 buflen, uint32 state,
                                  uint8 dir) {
  char *stat_str = NULL;
...
static void dhd_statlog_get_timestamp(stat_elem_t *elem, uint64 *sec,
                                      uint64 *usec) {
...
static void dhd_statlog_convert_time(stat_elem_t *elem, uint8 *buf,
                                     uint32 buflen) {
  ...
  snprintf(buf, buflen, DHD_STATLOG_TZFMT_YYMMDDHHMMSSMS, tm.tm_year - 100,
...
#ifdef DHD_LOG_DUMP
static int dhd_statlog_dump(dhd_statlog_t *statlog, char *buf, uint32 buflen) {
  stat_elem_t *elem;
  struct bcmstrbuf b;
  ...
  while (elem) {
    if (DHD_STATLOG_VALID(elem->stat)) {
      dhd_statlog_stat_name(stat_str, sizeof(stat_str), elem->stat, elem->dir);
      dhd_statlog_get_timestamp(elem, &sec, &usec);
      dhd_statlog_convert_time(elem, ts_str, sizeof(ts_str));
      bcm_bprintf(strbuf,
                  "[%s][%5lu.%06lu] status=%s, ifidx=%d, "
                  "reason=%d, status=%d\n",
                  ts_str, (unsigned long)sec, (unsigned long)usec, stat_str,
...
int dhd_statlog_write_logdump(dhd_pub_t *dhdp, const void *user_buf, void *fp,
                              uint32 len, unsigned long *pos) {
  dhd_statlog_t *statlog;
  ...
  if (remain_len < 0) {
    DHD_STATLOG_ERR(
        ("%s: failed to write stat info to buffer\n", __FUNCTION__));
    return BCME_ERROR;
  }
  ...
  if (ret < 0) {
    DHD_STATLOG_ERR(
        ("%s: failed to write stat info, err=%d\n", __FUNCTION__, ret));
  }

  DHD_STATLOG_INFO(
      ("%s: Complete to write statlog file, err=%d\n", __FUNCTION__, ret));
...
int dhd_statlog_generate_bdmask(dhd_pub_t *dhdp, void *reqbuf) {
  dhd_statlog_t *statlog;
...
int dhd_statlog_get_latest_info(dhd_pub_t *dhdp, void *reqbuf) {
  dhd_statlog_t *statlog;
  stat_query_t *query;
  stat_elem_t *elem;
...
int dhd_statlog_query(dhd_pub_t *dhdp, char *cmd, int total_len) {
  ...
  if (resp_num > 0) {
    ...
    for (i = 0; i < resp_num; i++) {
      if (DHD_STATLOG_VALID(elem->stat)) {
        dhd_statlog_stat_name(stat_str, sizeof(stat_str), elem->stat,
                              elem->dir);
        dhd_statlog_get_timestamp(elem, &sec, &usec);
        dhd_statlog_convert_time(elem, ts_str, sizeof(ts_str));
        DHD_STATLOG_PRINT(
            ("[RAWTS:%llu][%s][%5lu.%06lu] status=%s,"
             " ifidx=%d, reason=%d, status=%d\n",
             elem->ts_tz, ts_str, (unsigned long)sec, (unsigned long)usec,
...
void dhd_statlog_dump_scr(dhd_pub_t *dhdp) {
  dhd_statlog_t *statlog;
  stat_elem_t *elem;
  char stat_str[DHD_STATLOG_STATSTR_BUF_LEN];
  char ts_str[DHD_STATLOG_TZFMT_BUF_LEN];
