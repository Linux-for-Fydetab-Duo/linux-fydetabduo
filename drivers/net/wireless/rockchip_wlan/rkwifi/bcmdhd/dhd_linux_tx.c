...
int BCMFASTPATH(__dhd_sendpkt)(dhd_pub_t *dhdp, int ifidx, void *pktbuf) {
  ...
  if (PKTLEN(dhdp->osh, pktbuf) >= ETHER_HDR_LEN) {
    ...
    if (ETHER_ISMULTI(eh->ether_dhost)) dhdp->tx_multicast++;
    ...
    dhd_dump_pkt(dhdp, ifidx, pktdata, (uint32)PKTLEN(dhdp->osh, pktbuf), TRUE,
                 NULL, NULL);
  } else {
    PKTCFREE(dhdp->osh, pktbuf, TRUE);
    return BCME_ERROR;
...
#endif
  {
...
#endif /* !PKTPRIO_OVERRIDE */
    {
     ...
     (defined(QOS_MAP_SET) || defined(WL_CUSTOM_MAPPING_OF_DSCP)))
  ...
  if (dhdp->tx_profile_enab && dhdp->num_profiles > 0 &&
      dhd_protocol_matches_profile(
          PKTDATA(dhdp->osh, pktbuf), PKTLEN(dhdp->osh, pktbuf),
          dhdp->protocol_filters, dhdp->host_sfhllc_supported)) {
...
#ifdef PROP_TXSTATUS
  {
    if (dhd_wlfc_commit_packets(dhdp, (f_commitpkt_t)dhd_bus_txdata, dhdp->bus,
                                pktbuf, TRUE) == WLFC_UNSUPPORTED) {
  ...
  /* Update the packet counters here, as it is called for LB Tx and non-LB Tx
   * too */
  if (ret) {
#ifdef BCMDBUS
    PKTCFREE(dhdp->osh, pktbuf, TRUE);
...
static uint16 BCMFASTPATH(dhd_select_queue)(struct net_device *net,
                                            struct sk_buff *skb,
                                            void *accel_priv,
...
netdev_tx_t BCMFASTPATH(dhd_start_xmit)(struct sk_buff *skb,
  ...
  if (ifp->wmf.wmf_enable && (ntoh16(eh->ether_type) == ETHER_TYPE_IP) &&
    ...
    if (ucast_convert) {
      ...
      if (ret == NETDEV_TX_OK) PKTFREE(dhd->pub.osh, pktbuf, TRUE);
...
void dhd_txflowcontrol(dhd_pub_t *dhdp, int ifidx, bool state) {
  ...
  if ((dhdp->dequeue_prec_map == (1 << dhdp->flow_prio_map[PRIO_8021D_NC])) &&
      (state == ON)) {
    DHD_ERROR_RLMT(
        ("%s: Roaming in progress, cannot stop network queue (0x%x:%d)\n",
         __FUNCTION__, dhdp->dequeue_prec_map,
         dhdp->flow_prio_map[PRIO_8021D_NC]));
    return;
  }
#endif
...
void dhd_txcomplete(dhd_pub_t *dhdp, void *txp, bool success) {
  ...
  if (dhdp->tput_data.tput_test_running) {
    ...
    if (success && dhdp->batch_tx_pkts_cmpl <= dhdp->batch_tx_num_pkts)
...
void dhd_handle_pktdata(dhd_pub_t *dhdp, int ifidx, void *pkt, uint8 *pktdata,
                        uint32 pktid, uint32 pktlen, uint16 *pktfate,
                        uint8 *dhd_udr, uint8 *dhd_igmp, bool tx, int pkt_wake,
                        bool pkt_log) {
  struct ether_header *eh;
  uint16 ether_type;
  uint32 pkthash;
  ...
  do {
    if (!OSL_ATOMIC_READ(dhdp->osh, &dhdp->pktlog->enable)) {
      ...
      if (pktlog_ring->pktcount <= DHD_PACKET_LOG_RING_RESUME_THRESHOLD) {
        ...
        dhd_os_dbg_urgent_pullreq(dhdp->dbg->private, PACKET_LOG_RING_ID);
        break;
      }
    }
    ...
    if (dhd_udr && (pkt_type == PKT_TYPE_DHCP || pkt_type == PKT_TYPE_DNS ||
                    pkt_type == PKT_TYPE_ARP)) {
      *dhd_udr = TRUE;
    }
#endif /* DHD_SBN */
  ...
  /* Dump packet data */
  if (!tx) dhd_dump_pkt(dhdp, ifidx, pktdata, pktlen, FALSE, &pkthash, NULL);
  switch (pkt_type) {
    case PKT_TYPE_DHCP:
