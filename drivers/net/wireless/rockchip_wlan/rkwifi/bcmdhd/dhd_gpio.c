...
extern void *dhd_wlan_mem_prealloc(uint bus_type, int index, int section,
                                   unsigned long size);
#else
...
static int dhd_wlan_set_power(int on, wifi_adapter_info_t *adapter) {
...
//		rk_pcie_power_on_atu_fixup();
    ...
    if (adapter->pci_dev) {
      ...
      if (adapter->pci_saved_state)
        pci_load_and_free_saved_state(adapter->pci_dev,
                                      &adapter->pci_saved_state);
      pci_restore_state(adapter->pci_dev);
      err = pci_enable_device(adapter->pci_dev);
      if (err < 0) printf("%s: PCI enable device failed", __FUNCTION__);
...
static int dhd_wlan_set_reset(int onoff) { return 0; }
...
static int dhd_wlan_get_mac_addr(unsigned char *buf, int ifidx) {
  ...
  {
    char macpad[56] = {
        0x00, 0xaa, 0x9c, 0x84, 0xc7, 0xbc, 0x9b, 0xf6, 0x02, 0x33, 0xa9, 0x4d,
        0x5c, 0xb4, 0x0a, 0x5d, 0xa8, 0xef, 0xb0, 0xcf, 0x8e, 0xbf, 0x24, 0x8a,
        0x87, 0x0f, 0x6f, 0x0d, 0xeb, 0x83, 0x6a, 0x70, 0x4a, 0xeb, 0xf6, 0xe6,
        0x3c, 0xe7, 0x5f, 0xfc, 0x0e, 0xa7, 0xb3, 0x0f, 0x00, 0xe4, 0x4a, 0xaf,
...
static void *dhd_wlan_get_country_code(char *ccode
  ...
  for (i = 0; i < size; i++)
    if (strcmp(ccode, locales[i].iso_abbrev) == 0) return &locales[i];
  return NULL;
}
...
static int dhd_wlan_init_gpio(wifi_adapter_info_t *adapter) {
#ifdef BCMDHD_DTS
  char wlan_node[32];
...
#ifdef CUSTOMER_OOB
    gpio_wl_host_wake =
        of_get_named_gpio(root_node, GPIO_WL_HOST_WAKE_PROPNAME, 0);
#endif
  } else
#endif
  {
  ...
  if (gpio_wl_host_wake >= 0) {
    ...
    if (host_oob_irq < 0) {
      printf("%s: gpio_to_irq(%d) for WL_HOST_WAKE failed %d\n", __FUNCTION__,
             gpio_wl_host_wake, host_oob_irq);
      gpio_free(gpio_wl_host_wake);
      return -1;
    }
  }
...
static void dhd_wlan_init_adapter(wifi_adapter_info_t *adapter) {
#ifdef ADAPTER_IDX
  if (ADAPTER_IDX == 0) {
    adapter->bus_num = 1;
    adapter->slot_num = 1;
