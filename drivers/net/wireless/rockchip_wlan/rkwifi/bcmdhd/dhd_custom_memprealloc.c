...
void *dhd_wlan_mem_prealloc(int section, unsigned long size) {
  ...
  if (section == WLAN_STATIC_DHD_INFO_BUF) {
    if (size > WLAN_DHD_INFO_BUF_SIZE) {
      pr_err(
          "request DHD_INFO size(%lu) is bigger than"
          " static size(%d).\n",
  ...
  if (section == WLAN_STATIC_DHD_IF_FLOW_LKUP) {
    if (size > WLAN_DHD_IF_FLOW_LKUP_SIZE) {
      pr_err(
          "request DHD_WLFC size(%lu) is bigger than"
          " static size(%d).\n",
  ...
  if (section == WLAN_STATIC_DHD_WLFC_HANGER) {
    if (size > WLAN_DHD_WLFC_HANGER_SIZE) {
      pr_err(
          "request DHD_WLFC_HANGER size(%lu) is bigger than"
          " static size(%d).\n",
  ...
  if (section == WLAN_STATIC_DHD_MEMDUMP_RAM) {
    if (size > WLAN_DHD_MEMDUMP_SIZE) {
      pr_err(
          "request DHD_MEMDUMP_RAM size(%lu) is bigger"
          " than static size(%d).\n",
  ...
  if (section == WLAN_STATIC_DHD_LOG_DUMP_BUF) {
    if (size > DHD_LOG_DUMP_BUF_SIZE) {
      pr_err(
          "request DHD_LOG_DUMP_BUF size(%lu) is bigger then"
          " static size(%d).\n",
  ...
  if (section == WLAN_STATIC_DHD_LOG_DUMP_BUF_EX) {
    if (size > DHD_LOG_DUMP_BUF_EX_SIZE) {
      pr_err(
          "request DHD_LOG_DUMP_BUF_EX size(%lu) is bigger then"
          " static size(%d).\n",
...
int dhd_init_wlan_mem(void) {
...
#ifdef CONFIG_BCMDHD_PCIE
      wlan_mem_array[i].mem_ptr = kvmalloc(wlan_mem_array[i].size, GFP_KERNEL);
#else
...
#ifdef CONFIG_BCMDHD_PCIE
  wlan_static_if_flow_lkup = kvmalloc(WLAN_DHD_IF_FLOW_LKUP_SIZE, GFP_KERNEL);
  ...
  wlan_static_dhd_wlfc_hanger = kmalloc(WLAN_DHD_WLFC_HANGER_SIZE, GFP_KERNEL);
...
err_skb_alloc:
  /*
   * When all the skb alloc buf couldn't alloced, free these buf with alloced
   * size dhd_exit_wlan_mem will free with total size (don't know alloced size)
   */
