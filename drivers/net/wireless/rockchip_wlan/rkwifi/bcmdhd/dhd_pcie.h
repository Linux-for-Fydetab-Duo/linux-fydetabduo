...
#define DEVICE_TX_STUCK_WARN_DURATION \
  (DEVICE_TX_STUCK_TIMEOUT / DEVICE_TX_STUCK_CKECK_TIMEOUT)
...
#define IDMA_CAPABLE(bus) \
  (((bus)->sih->buscorerev == 19) || ((bus)->sih->buscorerev >= 23))
...
typedef struct dhd_bus {
...
#ifdef DEVICE_TX_STUCK_DETECT
  /* Flag to enable/disable device tx stuck monitor by DHD IOVAR
   * dev_tx_stuck_monitor */
  ...
  bool deep_sleep; /* Indicates deep_sleep set or unset by the DHD IOVAR
                      deep_sleep */
...
#ifdef D2H_MINIDUMP
  bool d2h_minidump; /* This flag will be set if Host and FW handshake to
                        collect minidump */
  ...
  uint32 xtal_to_alp_ratio; /* xtal to ALP ratio which can change from chip to
                               chip */
#endif                      /* BCMSLTGT */
...
enum { PCIE_INTX = 0, PCIE_MSI = 1 };

enum { PCIE_D2H_INTMASK_CTRL = 0, PCIE_HOST_IRQ_CTRL = 1 };

static INLINE bool __dhd_check_bus_in_lps(dhd_bus_t *bus) {
  bool ret = (bus->bus_low_power_state == DHD_BUS_D3_INFORM_SENT) ||
             (bus->bus_low_power_state == DHD_BUS_D3_ACK_RECIEVED);
  return ret;
}

static INLINE bool dhd_check_bus_in_lps(dhd_bus_t *bus) {
...
static INLINE bool __dhd_check_bus_lps_d3_acked(dhd_bus_t *bus) {
  bool ret = (bus->bus_low_power_state == DHD_BUS_D3_ACK_RECIEVED);
  return ret;
}

static INLINE bool dhd_check_bus_lps_d3_acked(dhd_bus_t *bus) {
  unsigned long flags_bus;
  bool ret;
  DHD_BUS_LP_STATE_LOCK(bus->bus_lp_state_lock, flags_bus);
...
static INLINE void __dhd_set_bus_not_in_lps(dhd_bus_t *bus) {
  bus->bus_low_power_state = DHD_BUS_NO_LOW_POWER_STATE;
  return;
}

static INLINE void dhd_set_bus_not_in_lps(dhd_bus_t *bus) {
  unsigned long flags_bus;
  DHD_BUS_LP_STATE_LOCK(bus->bus_lp_state_lock, flags_bus);
...
static INLINE void __dhd_set_bus_lps_d3_informed(dhd_bus_t *bus) {
  bus->bus_low_power_state = DHD_BUS_D3_INFORM_SENT;
  return;
}

static INLINE void dhd_set_bus_lps_d3_informed(dhd_bus_t *bus) {
  unsigned long flags_bus;
  DHD_BUS_LP_STATE_LOCK(bus->bus_lp_state_lock, flags_bus);
...
static INLINE void __dhd_set_bus_lps_d3_acked(dhd_bus_t *bus) {
  bus->bus_low_power_state = DHD_BUS_D3_ACK_RECIEVED;
  return;
}

static INLINE void dhd_set_bus_lps_d3_acked(dhd_bus_t *bus) {
  unsigned long flags_bus;
  DHD_BUS_LP_STATE_LOCK(bus->bus_lp_state_lock, flags_bus);
...
extern int dhdpcie_bus_attach(osl_t *osh, dhd_bus_t **bus_ptr,
                              volatile char *regs, volatile char *tcm,
                              void *pci_dev, wifi_adapter_info_t *adapter);
extern uint32 dhdpcie_bus_cfg_read_dword(struct dhd_bus *bus, uint32 addr,
                                         uint32 size);
extern void dhdpcie_bus_cfg_write_dword(struct dhd_bus *bus, uint32 addr,
...
extern uint32 dhdpcie_set_l1_entry_time(struct dhd_bus *bus,
                                        int force_l1_entry_time);
...
extern void dhdpcie_set_pmu_min_res_mask(struct dhd_bus *bus,
                                         uint min_res_mask);
...
extern uint32 dhdpcie_rc_access_cap(dhd_bus_t *bus, int cap, uint offset,
                                    bool is_ext, bool is_write,
                                    uint32 writeval);
extern uint32 dhdpcie_ep_access_cap(dhd_bus_t *bus, int cap, uint offset,
                                    bool is_ext, bool is_write,
                                    uint32 writeval);
...
static INLINE uint32 dhdpcie_rc_access_cap(dhd_bus_t *bus, int cap, uint offset,
                                           bool is_ext, bool is_write,
                                           uint32 writeval) {
  return -1;
}
static INLINE uint32 dhdpcie_ep_access_cap(dhd_bus_t *bus, int cap, uint offset,
                                           bool is_ext, bool is_write,
                                           uint32 writeval) {
  return -1;
}
...
extern void dhd_bus_flow_ring_resume_response(struct dhd_bus *bus,
                                              uint16 flowid, int32 status);
extern int dhd_bus_flow_ring_suspend_request(struct dhd_bus *bus, void *arg);
extern void dhd_bus_flow_ring_suspend_response(struct dhd_bus *bus,
                                               uint16 flowid, uint32 status);
extern void dhd_flow_ring_move_to_active_list_head(
    struct dhd_bus *bus, flow_ring_node_t *flow_ring_node);
extern void dhd_flow_ring_add_to_active_list(struct dhd_bus *bus,
                                             flow_ring_node_t *flow_ring_node);
extern void dhd_flow_ring_delete_from_active_list(
    struct dhd_bus *bus, flow_ring_node_t *flow_ring_node);
extern void __dhd_flow_ring_delete_from_active_list(
    struct dhd_bus *bus, flow_ring_node_t *flow_ring_node);
#endif /* IDLE_TX_FLOW_MGMT */
...
extern enum dhd_bus_ds_state dhdpcie_bus_get_pcie_inband_dw_state(
    dhd_bus_t *bus);
...
static INLINE void dhdpcie_dongle_flr_or_pwr_toggle(dhd_bus_t *bus) { return; }
...
static INLINE uint32 dhd_pcie_config_read(dhd_bus_t *bus, uint offset,
                                          uint size) {
...
static INLINE uint32 dhd_pcie_corereg_read(si_t *sih, uint val) {
  /* For 4375 or prior chips to 4375 */
  if (sih->buscorerev <= 64) {
    OSL_DELAY(100);
  }
  si_corereg(sih, sih->buscoreidx, OFFSETOF(sbpcieregs_t, configaddr), ~0, val);
  return si_corereg(sih, sih->buscoreidx, OFFSETOF(sbpcieregs_t, configdata), 0,
                    0);
}
