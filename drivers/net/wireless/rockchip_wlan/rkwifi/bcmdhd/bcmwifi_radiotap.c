...
void radiotap_add_vendor_ns(ieee80211_radiotap_header_t *hdr);

void radiotap_encode_multi_rssi(monitor_pkt_rxsts_t *rxsts,
                                ieee80211_radiotap_header_t *hdr);
void radiotap_encode_bw_signaling(uint16 mask, struct wl_rxsts *rxsts,
                                  ieee80211_radiotap_header_t *hdr);
...
static void wl_rtapParseReset(radiotap_parse_t *rtap) {
  rtap->idx = 0;    /* reset parse index */
  rtap->offset = 0; /* reset current field pointer */
}

static void *wl_rtapParseFindField(radiotap_parse_t *rtap, uint search_idx) {
  ...
  if (search_idx > IEEE80211_RADIOTAP_EXT) return ptr;

  if (search_idx < rtap->idx) wl_rtapParseReset(rtap);
  ...
  while (idx <= search_idx) {
    ...
    if ((1 << idx) & btmap) {
       ...
       */
      if (rtap_parse_info[idx].align == 0) break;
       ...
       * just bail out
       */
      if (field_offset + len > rtap->fields_len) break;

      /* did we find the field? */
      if (idx == search_idx) ptr = (uint8 *)rtap->fields + field_offset;
...
ratespec_t wl_calcRspecFromRTap(uint8 *rtap_header) {
  ratespec_t rspec = 0;
  ...
  if (rate != 0) {
    ...
    if (mcs <= 23 || mcs == 32) {
      uint32 override = 0;
      if (mcs_known &
          (IEEE80211_RADIOTAP_MCS_HAVE_GI | IEEE80211_RADIOTAP_MCS_HAVE_FMT |
...
bool wl_rtapFlags(uint8 *rtap_header, uint8 *flags) {
  radiotap_parse_t rtap;
  void *p;
...
void wl_rtapParseInit(radiotap_parse_t *rtap, uint8 *rtap_header) {
  uint rlen;
  uint32 *present_word;
...
uint wl_radiotap_rx(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
  ...
  if (CHSPEC_IS2G(rxsts->chanspec)) {
    ...
    channel_frequency =
        wf_channel2mhz(wf_chspec_ctlchan(rxsts->chanspec), WF_CHAN_FACTOR_6_G);
  }
  ...
  if ((fc & FC_WEP) == FC_WEP) flags |= IEEE80211_RADIOTAP_F_WEP;

  if ((fc & FC_MOREFRAG) == FC_MOREFRAG) flags |= IEEE80211_RADIOTAP_F_FRAG;

  if (rxsts->pkterror & WL_RXS_CRC_ERROR) flags |= IEEE80211_RADIOTAP_F_BADFCS;
  ...
  if (rxsts->encoding == WL_RXS_ENCODING_HT ||
    ...
    *cp++ = (int8)rxsts->antenna;
    ...
    if (rxsts->encoding == WL_RXS_ENCODING_HT) {
      ...
      tail->u.vht.vht_known =
          (IEEE80211_RADIOTAP_VHT_HAVE_STBC |
           ...
           IEEE80211_RADIOTAP_VHT_HAVE_BF | IEEE80211_RADIOTAP_VHT_HAVE_BW |
           IEEE80211_RADIOTAP_VHT_HAVE_GID | IEEE80211_RADIOTAP_VHT_HAVE_PAID);
      ...
      tail->u.vht.vht_mcs_nss[0] =
          (rxsts->mcs << 4) | (rxsts->nss & IEEE80211_RADIOTAP_VHT_NSS);
    ...
    /*
     * Header length is complicated due to dynamic presence of signal and noise
     * fields Start with length of wl_radiotap_legacy plus signal/noise/ant
     */
    bsd_header_len = sizeof(struct wl_radiotap_hdr) + pad_len;
    ...
    /* add in signal/noise/ant */
    cp = bsd_header->pad;
    if (rxsts->signal != 0) *cp++ = (int8)rxsts->signal;
    if (rxsts->noise != 0) *cp++ = (int8)rxsts->noise;
    *cp++ = (int8)rxsts->antenna;
  }
  return bsd_header_len;
}

static int wl_radiotap_rx_channel_frequency(wl_rxsts_t *rxsts) {
  if (CHSPEC_IS2G(rxsts->chanspec)) {
...
static uint16 wl_radiotap_rx_channel_flags(wl_rxsts_t *rxsts) {
  if (CHSPEC_IS2G(rxsts->chanspec)) {
...
static uint8 wl_radiotap_rx_flags(struct dot11_header *mac_header,
                                  wl_rxsts_t *rxsts) {
  uint8 flags;
  uint16 fc;
  ...
  if (fc & FC_WEP) flags |= IEEE80211_RADIOTAP_F_WEP;

  if (fc & FC_MOREFRAG) flags |= IEEE80211_RADIOTAP_F_FRAG;

  return flags;
}

uint wl_radiotap_rx_legacy(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
  ...
  wl_radiotap_legacy_t *rtl =
      (wl_radiotap_legacy_t *)((uint8 *)rtap_hdr + rtap_len);
...
uint wl_radiotap_rx_ht(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
                       ieee80211_radiotap_header_t *rtap_hdr) {
  int channel_frequency;
  uint16 channel_flags;
...
uint wl_radiotap_rx_vht(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
                        ieee80211_radiotap_header_t *rtap_hdr) {
  int channel_frequency;
  uint16 channel_flags;
  uint8 flags;
  ...
  wl_radiotap_vht_t *rtvht =
      (wl_radiotap_vht_t *)((uint8 *)rtap_hdr + rtap_len);
  ...
  rtvht->vht_known =
      (IEEE80211_RADIOTAP_VHT_HAVE_STBC | IEEE80211_RADIOTAP_VHT_HAVE_TXOP_PS |
       IEEE80211_RADIOTAP_VHT_HAVE_GI |
       IEEE80211_RADIOTAP_VHT_HAVE_SGI_NSYM_DA |
       IEEE80211_RADIOTAP_VHT_HAVE_LDPC_EXTRA | IEEE80211_RADIOTAP_VHT_HAVE_BF |
       IEEE80211_RADIOTAP_VHT_HAVE_BW | IEEE80211_RADIOTAP_VHT_HAVE_GID |
       IEEE80211_RADIOTAP_VHT_HAVE_PAID);

  STATIC_ASSERT(WL_RXS_VHTF_STBC == IEEE80211_RADIOTAP_VHT_STBC);
  STATIC_ASSERT(WL_RXS_VHTF_TXOP_PS == IEEE80211_RADIOTAP_VHT_TXOP_PS);
  STATIC_ASSERT(WL_RXS_VHTF_SGI == IEEE80211_RADIOTAP_VHT_SGI);
  STATIC_ASSERT(WL_RXS_VHTF_SGI_NSYM_DA == IEEE80211_RADIOTAP_VHT_SGI_NSYM_DA);
  STATIC_ASSERT(WL_RXS_VHTF_LDPC_EXTRA == IEEE80211_RADIOTAP_VHT_LDPC_EXTRA);
  STATIC_ASSERT(WL_RXS_VHTF_BF == IEEE80211_RADIOTAP_VHT_BF);

  rtvht->vht_flags = (uint8)HTOL16(rxsts->vhtflags);

  STATIC_ASSERT(WL_RXS_VHT_BW_20 == IEEE80211_RADIOTAP_VHT_BW_20);
  STATIC_ASSERT(WL_RXS_VHT_BW_40 == IEEE80211_RADIOTAP_VHT_BW_40);
  STATIC_ASSERT(WL_RXS_VHT_BW_20L == IEEE80211_RADIOTAP_VHT_BW_20L);
  STATIC_ASSERT(WL_RXS_VHT_BW_20U == IEEE80211_RADIOTAP_VHT_BW_20U);
  STATIC_ASSERT(WL_RXS_VHT_BW_80 == IEEE80211_RADIOTAP_VHT_BW_80);
  STATIC_ASSERT(WL_RXS_VHT_BW_40L == IEEE80211_RADIOTAP_VHT_BW_40L);
  STATIC_ASSERT(WL_RXS_VHT_BW_40U == IEEE80211_RADIOTAP_VHT_BW_40U);
  STATIC_ASSERT(WL_RXS_VHT_BW_20LL == IEEE80211_RADIOTAP_VHT_BW_20LL);
  STATIC_ASSERT(WL_RXS_VHT_BW_20LU == IEEE80211_RADIOTAP_VHT_BW_20LU);
  STATIC_ASSERT(WL_RXS_VHT_BW_20UL == IEEE80211_RADIOTAP_VHT_BW_20UL);
  STATIC_ASSERT(WL_RXS_VHT_BW_20UU == IEEE80211_RADIOTAP_VHT_BW_20UU);
  ...
  STATIC_ASSERT(WL_RXS_VHTF_CODING_LDCP == IEEE80211_RADIOTAP_VHT_CODING_LDPC);
...
uint wl_radiotap_rx_he(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
                       ieee80211_radiotap_header_t *rtap_hdr) {
  int channel_frequency;
  uint16 channel_flags;
  uint8 flags;
...
uint wl_radiotap_rx_eht(struct dot11_header *mac_header, wl_rxsts_t *rxsts,
                        ieee80211_radiotap_header_t *rtap_hdr) {
  ASSERT(!"wl_radiotap_rx_eht: not implemented!");
  return 0;
}

uint16 wl_rxsts_to_rtap(monitor_pkt_rxsts_t *pkt_rxsts, void *payload,
...
void radiotap_encode_alignpad(ieee80211_radiotap_header_t *hdr,
                              uint16 pad_req) {
  uint16 cur_len = LTOH16(hdr->it_len);
  uint8 *vend_p = (uint8 *)hdr + cur_len;
  radiotap_vendor_ns_t *vendor_ns = (radiotap_vendor_ns_t *)vend_p;
