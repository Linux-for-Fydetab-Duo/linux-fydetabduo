...
typedef enum stat_log_stat {
  ...
  ST(EAPOL_GROUPKEY_M1) =
      21, /* send or receive the EAPOL Group key handshake M1 */
  ST(EAPOL_GROUPKEY_M2) =
      22, /* send or receive the EAPOL Group key handshake M2 */
...
/* functions */
extern dhd_statlog_handle_t *dhd_attach_statlog(dhd_pub_t *dhdp,
                                                uint32 num_items,
...
#define DHD_STATLOG_DATA_RSN(dhdp, stat, ifidx, dir, reason) \
  dhd_statlog_ring_log_data_reason((dhdp), (stat), (ifidx), (dir), (reason))
