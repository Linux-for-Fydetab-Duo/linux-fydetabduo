...
#define FCLOW (FCHI / 2)
#define PRIOMASK                                \
  7 /* XXX FixMe: should come from elsewhere... \
...
#define MAX_MEMBLOCK \
  (32 * 1024) /* Block size used for downloading of dongle image */

#define MAX_DATA_BUF \
  (64 * 1024) /* Must be large enough to hold biggest possible glom */
...
#define REMAP_ISADDR(bus, a) \
  (((a) >= ((bus)->orig_ramsize)) && ((a) < ((bus)->ramsize)))
...
#define PKTGEN_RCV_ONGOING (1)
  ...
  uint32
      fw_memmap_download_addr; /* Dongle address of FWS memory-info download */
  uint32
      fw_memmap_download_len; /* Length in bytes of FWS memory-info download */

  char fwsig_filename[DHD_MAX_PATH]; /* Name of FW signature file */
  char
      bootloader_filename[DHD_FILENAME_MAX]; /* Name of bootloader image file */
 ...
 * of adding one macro check as part of the if condition that checks for
 * DHD_IDLE_IMMEDIATE In case of non BT over SDIO builds, this macro will always
 * return TRUE. In case of the builds where BT_OVER_SDIO is enabled, it will
 * expand to a condition check that checks if bt_use_count is zero. So this
 * macro will return equate to 1 if bt_use_count is 0, indicating that there are
 * no active users and if bt_use_count is non zero it would return 0 there by
 * preventing the caller from executing the sleep calls.
 */
#ifdef BT_OVER_SDIO
...
/** the debugger layer will call back into this (bus) layer to read/write dongle
 * memory */
...
#define PKTALIGN(osh, p, len, align)                   \
  do {                                                 \
    ...
    if (datalign) PKTPULL((osh), (p), (uint)datalign); \
    PKTSETLEN((osh), (p), (len));                      \
  } while (0)
...
bool dhdsdio_is_dataok(dhd_bus_t *bus) {
  return (((uint8)(bus->tx_max - bus->tx_seq) - bus->dhd->conf->tx_max_offset >
           1) &&
          (((uint8)(bus->tx_max - bus->tx_seq) & 0x80) == 0));
}

uint8 dhdsdio_get_databufcnt(dhd_bus_t *bus) {
  return ((uint8)(bus->tx_max - bus->tx_seq) - 1 -
          bus->dhd->conf->tx_max_offset);
}
#endif
...
#else
#define DATABUFCNT(bus) ((uint8)(bus->tx_max - bus->tx_seq) - 1)
#endif
...
#define BUS_WAKE(bus)                                    \
  do {                                                   \
    bus->idlecount = 0;                                  \
    if ((bus)->sleeping) dhdsdio_bussleep((bus), FALSE); \
  } while (0);
 ...
 *		Device sets a new bit in the intstatus whenever there is a
 *packet available in fifo.  Host can't clear this specific status bit until all
 *the packets are read from the FIFO.  No need to ack dongle intstatus. Mode 2:
 *(sdiod core rev >= 4) Device sets a bit in the intstatus, and host acks this
 *by writing one to this bit.  Dongle won't generate anymore packet interrupts
 *		until host reads all the packets from the dongle and reads a zero
 *to figure that there are no more packets.  No need to disable host ints. Need
 *to ack the intstatus.
 */
...
#define PKT_AVAILABLE(bus, intstatus) \
  (bcmsdh_get_dstatus(bus->sdh) & STATUS_F2_PKT_AVAILABLE)
...
#define GSPI_PR55150_BAILOUT                                                \
  do {                                                                      \
    ...
    uint32 dstatushw =                                                      \
        bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL); \
...
#define FRAME_AVAIL_MASK(bus)                                   \
  ((bus->rxint_mode == SDIO_DEVICE_HMB_RXINT) ? I_HMB_FRAME_IND \
                                              : I_XMTDATA_AVAIL)
...
#ifdef DHD_DEBUG
static int dhd_serialconsole(dhd_bus_t *bus, bool get, bool enable,
                             int *bcmerror);
#endif /* DHD_DEBUG */
...
static int dhd_bcmsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
                               ...
                               bcmsdh_cmplt_fn_t complete, void *handle,
                               int max_retry);
static int dhdsdio_txpkt(dhd_bus_t *bus, uint chan, void **pkts, int num_pkt,
                         bool free_pkt);
static int dhdsdio_txpkt_preprocess(dhd_bus_t *bus, void *pkt, int chan,
                                    int txseq, int prev_chain_total_len,
                                    bool last_chained_pkt, int *pad_pkt_len,
                                    void **new_pkt
#if defined(BCMSDIOH_TXGLOM_EXT)
                                    ,
                                    int first_frame
...
static int dhdsdio_bus_write_fwsig(dhd_bus_t *bus, char *fwsig_path,
                                   char *nvsig_path);
...
#if defined(BT_OVER_SDIO)
static int extract_hex_field(char *line, uint16 start_pos, uint16 num_chars,
                             uint16 *value);
static int read_more_btbytes(struct dhd_bus *bus, void *file, char *line,
                             int *addr_mode, uint16 *hi_addr, uint32 *dest_addr,
                             uint8 *data_bytes, uint32 *num_bytes);
 ...
 */
static void dhdsdio_tune_fifoparam(struct dhd_bus *bus) {
  int err;
  uint8 devctl, wm, mes;
  ...
  if (bus->sih->buscorerev >= 15) {
    ...
    */
    ...
    DHD_INFO(
        ("skip fifotune: SdioRev(%d) is lower than minimal requested ver\n",
         bus->sih->buscorerev));
    return;
  }
  ...
  DHD_INFO(
      ("Apply overflow WAR: 0x%02x 0x%02x 0x%02x\n",
...
#ifdef BCMSPI
static void dhdsdio_wkwlan(dhd_bus_t *bus, bool on) {
  int err;
  uint32 regdata;
  ...
  if (bus->sih->buscoretype == SDIOD_CORE_ID) {
    /* wake up wlan function :WAKE_UP goes as ht_avail_request and
     * alp_avail_request */
...
#ifdef USE_OOB_GPIO1
static int dhdsdio_oobwakeup_init(dhd_bus_t *bus) {
 ...
 */
static bool dhdsdio_sr_cap(dhd_bus_t *bus) {
  bool cap = FALSE;
  ...
  if (bus->sih->chip == BCM43430_CHIP_ID ||
    ...
    if (bcmsdh_reg_read(bus->sdh, addr, 4) != 0) cap = TRUE;
  ...
  if (0) {
    ...
    core_capext = bcmsdh_reg_read(
        bus->sdh,
        si_get_pmu_reg_addr(bus->sih, OFFSETOF(chipcregs_t, core_cap_ext)), 4);

    core_capext = (core_capext & CORE_CAPEXT_SR_SUPPORTED_MASK);
  }
  if (!(core_capext)) return FALSE;

  if ((bus->sih->chip == BCM4335_CHIP_ID) ||
      (bus->sih->chip == BCM4339_CHIP_ID) || BCM4345_CHIP(bus->sih->chip) ||
      (bus->sih->chip == BCM4354_CHIP_ID) ||
      (bus->sih->chip == BCM4356_CHIP_ID) ||
      (bus->sih->chip == BCM4358_CHIP_ID) ||
      (bus->sih->chip == BCM43569_CHIP_ID) ||
    ...
    if ((bus->sih->chip == BCM4350_CHIP_ID) || BCM4345_CHIP(bus->sih->chip) ||
        (bus->sih->chip == BCM4354_CHIP_ID) ||
        (bus->sih->chip == BCM4356_CHIP_ID) ||
        (bus->sih->chip == BCM4358_CHIP_ID) ||
        (bus->sih->chip == BCM43569_CHIP_ID) ||
    ...
    data = bcmsdh_reg_read(
        bus->sdh,
        si_get_pmu_reg_addr(bus->sih, OFFSETOF(chipcregs_t, retention_ctl)), 4);
...
static int dhdsdio_sr_init(dhd_bus_t *bus) {
  uint8 val, wkctl;
  int err = 0;
  ...
  if (!(wkctl & val)) {
    DHD_ERROR(
        ("%s:Set wakeup ctrl failed. SBSDIO_FUNC1_WAKEUPCTRL %08x, expect "
         "%08x\n",
...
#ifdef USE_CMD14
  /* Add CMD14 Support */
  dhdsdio_devcap_set(bus, (SDIOD_CCCR_BRCM_CARDCAP_CMD14_SUPPORT |
 ...
 */
static int dhdsdio_clk_kso_init(dhd_bus_t *bus) {
  uint8 val;
  int err = 0;
...
static void dhdsdio_set_wakeupctrl(dhd_bus_t *bus) {
  uint8 val;
  int err = 0;
...
static int dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on) {
  ...
  if (on) sdio_retune_hold_now(adapter->sdio_func);
#elif (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 2, 0))
  host = func->card->host;
  ...
  /* In case of 43012 chip, the chip could go down immediately after KSO bit is
   * cleared. So the further reads of KSO register could fail. Thereby just
   * bailing out immediately after clearing KSO bit, to avoid polling of KSO
   * bit.
   */
  ...
  if (on) {
     ...
     * So during a wake we write 0x1 for 5 msec to guarantee that chip is a
     * wake.
     */
    for (int i = 0; i < KSO_WAKE_RETRY_COUNT; i++) {
      bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, wr_val,
                       &err);
      OSL_DELAY(KSO_WAIT_US);
    }

    rd_val =
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, &err);
    ...
    cmp_val = 0;
    /* XXX only check for bit0, bit1(devon status) may not get cleared right
     * away */
  ...
  if (conf->kso_try_max) try_max = conf->kso_try_max;
  do {
    /*
    ...
    rd_val =
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, &err);
    if (((rd_val & bmask) == cmp_val) && !err) break;
  ...
  if (try_cnt > 2) {
    KSO_DBG(("%s> op:%s, try_cnt:%d, rd_val:%x, ERR:%x \n", __FUNCTION__,
             (on ? "KSO_SET" : "KSO_CLR"), try_cnt, rd_val, err));
  }
  if (try_cnt > try_max) {
    DHD_ERROR(("%s> op:%s, ERROR: try_cnt:%d, rd_val:%x, ERR:%x \n",
               __FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt, rd_val,
               err));
#ifdef KSO_DEBUG
    {
      int i;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0)) && \
    !defined(ANDROID13_KERNEL515_BKPORT)
  if (on) sdio_retune_release(adapter->sdio_func);
...
static int dhdsdio_clk_kso_iovar(dhd_bus_t *bus, bool on) {
  int err = 0;
  ...
  if (on == FALSE) {
    ...
    DHD_ERROR((
        ...
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err)));
...
static uint8 dhdsdio_sleepcsr_get(dhd_bus_t *bus) {
  ...
  /* XXX: Propagate error */
  if (err) DHD_TRACE(("Failed to read SLEEPCSR: %d\n", err));

  return val;
}

uint8 dhdsdio_devcap_get(dhd_bus_t *bus) {
...
static int dhdsdio_devcap_set(dhd_bus_t *bus, uint8 cap) {
  ...
  if (err) DHD_ERROR(("%s: devcap set err: 0x%x\n", __FUNCTION__, err));

  return 0;
}

static int dhdsdio_clk_devsleep_iovar(dhd_bus_t *bus, bool on) {
  int err = 0, retry;
  uint8 val;
  ...
  if (on == TRUE) {
    ...
    if (!SLPAUTO_ENAB(bus))
      ...
      if ((val & SBSDIO_CSR_MASK) == 0) {
        DHD_ERROR(
            ("%s: No clock before enter sleep:0x%x\n", __FUNCTION__, val));
...
#if !defined(NDIS)
      err =
          bcmsdh_gpioout(bus->sdh, GPIO_DEV_WAKEUP, FALSE); /* GPIO_1 is off */
#endif /* !defined(NDIS) */
    }
#endif /* USE_CMD14 */
    ...
    if (err == 0) {
      ...
      SPINWAIT_SLEEP(sdioh_spinwait_sleep,
                     (((csr = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
                                              SBSDIO_FUNC1_CHIPCLKCSR, &err)) &
                       SBSDIO_HT_AVAIL) != (SBSDIO_HT_AVAIL)),
                     (DHD_WAIT_HTAVAIL));
...
#define HT_AVAIL_ERROR_MAX 10
  ...
  if (!KSO_ENAB(bus)) return BCME_OK;
...
static int dhdsdio_set_sdmode(dhd_bus_t *bus, int32 sd_mode) {
  ...
  err = bcmsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0, &sd_mode, sizeof(sd_mode),
                        TRUE);
  if (err) {
...
static int dhdsdio_sdclk(dhd_bus_t *bus, bool on) {
  ...
  if (on) {
    if (bus->idleclock == DHD_IDLE_STOP) {
      ...
      iovalue = 1;
      err = bcmsdh_iovar_op(bus->sdh, "sd_clock", NULL, 0, &iovalue,
                            sizeof(iovalue), TRUE);
      if (err) {
      ...
      err = bcmsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0, &iovalue,
                            sizeof(iovalue), TRUE);
      if (err) {
...
static int dhdsdio_clkctl(dhd_bus_t *bus, uint target, bool pendok) {
  ...
  switch (target) {
    case CLK_AVAIL:
      ...
      if (bus->clkstate == CLK_NONE) dhdsdio_sdclk(bus, TRUE);
    ...
    case CLK_SDONLY:
      ...
      if (bus->bt_use_count != 0) {
        DHD_INFO(
            ("%s(): Req CLK_SDONLY, BT is active %d not switching off \r\n",
    ...
    case CLK_NONE:
      ...
      if (bus->bt_use_count != 0) {
        DHD_INFO(
            ("%s(): Request CLK_NONE BT is active %d not switching off \r\n",
             __FUNCTION__, bus->bt_use_count));
        ret = BCME_OK;
        break;
      }
      ...
      if (bus->clkstate == CLK_AVAIL) ret = dhdsdio_htclk(bus, FALSE, FALSE);
      /* Now remove the SD clock */
      ret = dhdsdio_sdclk(bus, FALSE);
      if (bus->poll == 0) dhd_os_wd_timer(bus->dhd, 0);
      break;
  }
#ifdef DHD_DEBUG
...
static int dhdsdio_bussleep(dhd_bus_t *bus, bool sleep) {
  ...
  DHD_INFO(("dhdsdio_bussleep: request %s (currently %s)\n",
            (sleep ? "SLEEP" : "WAKE"), (bus->sleeping ? "SLEEP" : "WAKE")));

  if (bus->dhd->hang_was_sent) return BCME_ERROR;

  /* Done if we're already in the requested state */
  if (sleep == bus->sleeping) return BCME_OK;
  ...
  if (sleep) {
     ...
     * From WLAN driver, either from the active contexts OR from the Watchdog
     * contexts we will be attempting to Go to Sleep. AT that moment if we see
     * that BT is still actively using the bus, we will return BCME_BUSY from
     * here, but the bus->sleeping state would not have changed. So the caller
     * can then schedule the Watchdog again which will come and attempt to sleep
     * at a later point.
     *
     * In case if BT is the only one and is the last user, we don't switch off
     * the clock immediately, we allow the WLAN to decide when to sleep i.e from
     * the watchdog. Now if the watchdog becomes active and attempts to switch
     * off the clock and if another WLAN context is active they are any way
    ...
    err =
        bcmsdh_iovar_op(bus->sdh, "sd3_tuning_disable", NULL, 0,
    ...
    if (err == 0) {
      ...
      err = bcmsdh_iovar_op(bus->sdh, "sd3_tuning_disable", NULL, 0,
                            &sd3_tuning_disable, sizeof(sd3_tuning_disable),
                            TRUE);
#endif /* BCMSDIOH_STD */
    }
  }
 ...
 *            FALSE if the caller cannot wait
 */
int __dhdsdio_clk_disable(struct dhd_bus *bus, bus_owner_t owner,
                          int can_wait) {
  int ret = BCME_ERROR;
...
int dhdsdio_func_blocksize(dhd_pub_t *dhd, int function_num, int block_size) {
  ...
  bcmerr = dhd_bus_iovar_op(dhd, "sd_blocksize", &func_blk_size, sizeof(int),
                            &result, sizeof(int), IOV_GET);
  ...
  if (result != block_size) {
    ...
    bcmerr = dhd_bus_iovar_op(dhd, "sd_blocksize", NULL, 0, &func_blk_size,
                              sizeof(int32), IOV_SET);
    if (bcmerr != BCME_OK) {
...
int dhd_bus_txdata(struct dhd_bus *bus, void *pkt) {
  ...
  if (bus->dhd->conf->orphan_move == 1) PKTORPHAN(pkt, bus->dhd->conf->tsq);

  /* Check for existing queue, current flow-control, pending event, or pending
   * clock */
  if (dhd_deferred_tx || bus->fcstate || pktq_n_pkts_tot(&bus->txq) ||
      bus->dpc_sched || (!DATAOK(bus)) || (bus->flowcontrol & NBITVAL(prec)) ||
      (bus->clkstate != CLK_AVAIL)) {
    bool deq_ret;
    int pkq_len = 0;
...
#ifdef PROP_TXSTATUS
      wlfc_enabled =
          (dhd_wlfc_flowcontrol(bus->dhd, ON, FALSE) != WLFC_UNSUPPORTED);
#endif
...
/* align packet data pointer and packet length to n-byte boundary, process
 *packet headers, a new packet may be allocated if there is not enough head
 *and/or tail from for padding. the caller is responsible for updating the glom
 *size in the head packet (when glom is used)
 *
 * pad_pkt_len: returns the length of extra padding needed from the padding
 *packet, this parameter is taken in tx glom mode only
 *
 * new_pkt: out, pointer of the new packet allocated due to insufficient head
 *room for alignment padding, NULL if not needed, the caller is responsible for
 *freeing the new packet
 *
 * return: positive value - length of the packet, including head and tail
 *padding negative value - errors
 */
static int dhdsdio_txpkt_preprocess(dhd_bus_t *bus, void *pkt, int chan,
                                    int txseq, int prev_chain_total_len,
                                    bool last_chained_pkt, int *pad_pkt_len,
  ...
  *new_pkt = NULL;
...
#endif /* PKT_STATICS */
#ifdef DHD_DEBUG
  if (PKTPRIO(pkt) < ARRAYSIZE(tx_packets)) tx_packets[PKTPRIO(pkt)]++;
#endif /* DHD_DEBUG */

  /* align the data pointer, allocate a new packet if there is not enough space
   * (new packet data pointer will be aligned thus no padding will be needed)
   */
  ...
  if (PKTHEADROOM(osh, pkt) < head_padding) {
     ...
     * unless it is a single pkt chain with total length less than one block
     * size, which we prefer sending by byte mode.
     *
     ...
     */
    cur_chain_total_len =
        prev_chain_total_len + (head_padding + pkt_len + tail_padding);
    if (last_chained_pkt && bus->blocksize != 0 &&
        (cur_chain_total_len > (int)bus->blocksize ||
         prev_chain_total_len > 0)) {
  ...
  if (alloc_new_pkt) {
    ...
    /* update the tail padding as it depends on the head padding, since a new
     * packet is allocated, the head padding is non longer needed and packet
     * length is chagned
     */
  ...
  if (head_padding) PKTPUSH(osh, pkt, head_padding);
   ...
   *			2-byte frame channel and frame flags (e.g. next frame
   *following) 2-byte header length 2-byte tail padding size
   *
   ...
   * packet and sees it as the total lenght of the glom (including tail
   * padding), for each packet in the glom, the packet length needs to be
   * updated, (see below PKTSETLEN)
   *
   ...
   */
#if defined(BCMSDIOH_TXGLOM_EXT)
  if (bus->dhd->conf->txglom_bucket_size) tail_padding = 0;
#endif
  ...
  *(((uint16 *)frame) + 1) = (uint16)htol16(~pkt_len);
    ...
    if (bus->dhd->conf->txglom_ext && !last_chained_pkt) {
      ...
      if (PKTTAILROOM(osh, pkt) < real_pad) {
        DHD_INFO(("%s : insufficient tailroom %d for %d real_pad\n", __func__,
                  (int)PKTTAILROOM(osh, pkt), real_pad));
  ...
  swheader = ((chan << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK) |
             ...
             (((head_padding + sdpcm_hdrlen) << SDPCM_DOFFSET_SHIFT) &
              SDPCM_DOFFSET_MASK);
...
static int dhdsdio_txpkt_postprocess(dhd_bus_t *bus, void *pkt) {
  ...
  int tail_padding;
  int swhdr_offset =
      SDPCM_FRAMETAG_LEN + (bus->txglom_enable ? SDPCM_HWEXT_LEN : 0);
  ...
  if (bus->txglom_enable) {
     ...
     * We cannot refer to this field for txglom pkts as the first pkt of the
     * chain will have the field for the total length of the chain.
     */
...
static int dhdsdio_txpkt(dhd_bus_t *bus, uint chan, void **pkts, int num_pkt,
  ...
  if (bus->dhd->dongle_reset) return BCME_NOTREADY;

  if (num_pkt <= 0) return BCME_BADARG;

  sdh = bus->sdh;
  osh = bus->dhd->osh;
  /* init new_pkts[0] to make some compiler happy, not necessary as we check
   * new_pkt_num */
  new_pkts[0] = NULL;
  ...
  for (i = 0; i < num_pkt; i++) {
    ...
    pkt_len =
        dhdsdio_txpkt_preprocess(bus, pkt, chan, bus->tx_seq + i, total_len,
                                 last_pkt, &pad_pkt_len, &new_pkt
#if defined(BCMSDIOH_TXGLOM_EXT)
                                 ,
                                 i
#endif
    ...
    if (pkt_len <= 0) {
      if (new_pkt) PKTFREE(osh, new_pkt, TRUE);
      goto done;
    }
    if (new_pkt) {
      pkt = new_pkt;
  ...
  if ((bus->dhd->conf->data_drop_mode == TXPKT_DROP) && (total_len > 500))
    ...
    ret = dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
                              PKTDATA(osh, head_pkt), total_len, pkt_chain,
                              NULL, NULL, TXRETRIES);
  if (ret == BCME_OK)
    bus->tx_seq = (bus->tx_seq + num_pkt) % SDPCM_SEQUENCE_WRAP;

  /* if a padding packet was needed, remove it from the link list as it not a
   * data pkt */
  if (pad_pkt_len && pkt) PKTSETNEXT(osh, pkt, NULL);
  ...
  for (i = 0; i < num_pkt; i++) {
    ...
    if (DHD_PKTTAG_WLFCPKT(PKTTAG(pkt))) {
      wlfc_enabled =
          (dhd_wlfc_txcomplete(bus->dhd, pkt, ret == 0) != WLFC_UNSUPPORTED);
    }
#endif /* PROP_TXSTATUS */
    if (!wlfc_enabled) {
      PKTSETNEXT(osh, pkt, NULL);
      dhd_txcomplete(bus->dhd, pkt, ret != 0);
      if (free_pkt) PKTFREE(osh, pkt, TRUE);
    }
  }

  for (i = 0; i < new_pkt_num; i++) PKTFREE(osh, new_pkts[i], TRUE);
...
static uint dhdsdio_sendfromq(dhd_bus_t *bus, uint maxframes) {
  ...
  for (cnt = 0; (cnt < maxframes) && DATAOK(bus);) {
    ...
    for (i = 0; i < num_pkt; i++) {
      ...
      if (eh->ether_type == hton16(ETHER_TYPE_802_1X)) {
        ...
        if (prio == PRIO_8021D_NC) {
          ...
          bdc_header->priority = (dhd->prio_8021x & BDC_PRIORITY_MASK);
#endif
        }
      }
#endif /* DHD_LOSSLESS_ROAMING */
      ...
      if (!bus->dhd->conf->orphan_move) PKTORPHAN(pkts[i], bus->dhd->conf->tsq);
    ...
    if (dhdsdio_txpkt(bus, SDPCM_DATA_CHANNEL, pkts, i, TRUE) != BCME_OK)
      ...
      bus->tx_statics.glom_cnt_us[num_pkt - 1] =
          (bus->tx_statics.glom_cnt[num_pkt - 1] *
               bus->tx_statics.glom_cnt_us[num_pkt - 1] +
           bcmsdh_get_spend_time(bus->sdh)) /
          (bus->tx_statics.glom_cnt[num_pkt - 1] + 1);
#endif
    }
    cnt += i;
#ifdef PKT_STATICS
    ...
    if (!bus->intr && cnt) {
      ...
      bus->f2txdata++;
      if (bcmsdh_regfail(bus->sdh)) break;
      if (intstatus & bus->hostintmask) bus->ipend = TRUE;
    }
  }
...
static void dhdsdio_sendpendctl(dhd_bus_t *bus) {
  bcmsdh_info_t *sdh = bus->sdh;
  int ret;
  uint8 *frame_seq = bus->ctrl_frame_buf + SDPCM_FRAMETAG_LEN;

  if (bus->txglom_enable) frame_seq += SDPCM_HWEXT_LEN;

  if (*frame_seq != bus->tx_seq) {
    DHD_INFO(
        ("%s IOCTL frame seq lag detected!"
  ...
  ret = dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
                            ...
                            (uint32)bus->ctrl_frame_len, NULL, NULL, NULL, 1);
  if (ret == BCME_OK) bus->tx_seq = (bus->tx_seq + 1) % SDPCM_SEQUENCE_WRAP;
...
int dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen) {
  ...
  if (bus->roundup && bus->blocksize && (len > bus->blocksize)) {
    ...
    if ((pad <= bus->roundup) && (pad < bus->blocksize)) len += pad;
  ...
  if (forcealign && (len & (ALIGNMENT - 1))) len = ROUNDUP(len, ALIGNMENT);
...
#ifdef BCMQT
  /* Make sure firmware is ready before request HT clock for PMU maximum
   * resource setting */
  ...
  *(((uint16 *)frame) + 1) = htol16(~msglen);
  ...
  if (bus->txglom_enable) {
    ...
    swheader =
        ((SDPCM_CONTROL_CHANNEL << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK) |
        bus->tx_seq | ((doff << SDPCM_DOFFSET_SHIFT) & SDPCM_DOFFSET_MASK);
    htol32_ua_store(swheader, frame + SDPCM_FRAMETAG_LEN + SDPCM_HWEXT_LEN);
    htol32_ua_store(
        0, frame + SDPCM_FRAMETAG_LEN + SDPCM_HWEXT_LEN + sizeof(swheader));
    ...
    if (bus->ctrl_frame_stat == FALSE) {
      ...
      if (!bus->dhd->hang_was_sent) {
        ...
        DHD_ERROR(("%s: rxlim_en=%d, rxlim enable=%d, rxlim_addr=0x%x\n",
                   __FUNCTION__, bus->dhd->conf->rxlim_en, bus->rxlim_en,
                   bus->rxlim_addr));
#endif /* BCMSDIO_RXLIM_POST */
      }
...
int dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen) {
  ...
  if (timeleft == 0) {
    if (rxlen == 0) bus->dhd->rxcnt_timeout++;
  ...
  if (bus->dhd->dongle_trap_occured) return -EREMOTEIO;
...
static void dhd_dump_pct(struct bcmstrbuf *strbuf, char *desc, uint num,
                         uint div) {
  uint q1, q2;
...
void dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf) {
  ...
  bcm_bprintf(
      ...
      bus->fcstate, pktq_n_pkts_tot(&bus->txq), bus->tx_seq, bus->tx_max,
              ...
              bus->f2txdata, bus->f1regdata);
  {
    ...
    dhd_dump_pct(
        strbuf, ", pkts/sd", (bus->dhd->tx_packets + bus->dhd->rx_packets),
  ...
  if (bus->pktgen_count) {
    ...
    bcm_bprintf(strbuf, "send attempts %u rcvd %u fail %u\n", bus->pktgen_sent,
                bus->pktgen_rcvd, bus->pktgen_fail);
  }
#endif /* SDTEST */
#ifdef DHD_DEBUG
  ...
  bcm_bprintf(strbuf,
              ...
              bus->sleeping);
  dhd_dump_pct(strbuf, "Tx: glom pct", (100 * bus->txglompkts),
               bus->dhd->tx_packets);
...
static int dhdsdio_pktgen_set(dhd_bus_t *bus, uint8 *arg) {
  ...
  if (pktgen.version != DHD_PKTGEN_VERSION) return BCME_BADARG;
...
static int dhdsdio_membytes(dhd_bus_t *bus, bool write, uint32 address,
  ...
  while (size) {
    ...
    if (dsize <= MAX_MEM_BUF) {
      pdata = bus->membuf;
      if (write) memcpy(bus->membuf, data, dsize);
    } else {
      pdata = data;
    }
    ...
    if (dsize <= MAX_MEM_BUF && !write) memcpy(data, bus->membuf, dsize);
...
static void dhdsdio_txseq_sync(dhd_bus_t *bus, sdpcm_shared_t *sh) {
  struct dhd_conf *conf = bus->dhd->conf;
  ...
  if (sh->flags & SDPCM_SHARED_RXLIM_POST) {
    ...
    if (conf->rxlim_en) {
      ...
      if (0 ==
          dhdsdio_membytes(bus, FALSE, sh->txseq_sync_addr, (uint8 *)&val, 1)) {
        if (bus->tx_seq != val) {
...
#endif /* BCMSDIO_TXSEQ_SYNC */
    DHD_INFO(
        ("%s: TXSEQ_SYNC and RXLIM_POST not supported in fw\n", __FUNCTION__));
  }
}
...
static int dhdsdio_readshared(dhd_bus_t *bus, sdpcm_shared_t *sh) {
  ...
  if ((rv = dhdsdio_membytes(bus, FALSE, addr, (uint8 *)sh,
                             sizeof(sdpcm_shared_t))) < 0)
    return rv;
...
void dhd_bus_check_srmemsize(dhd_pub_t *dhdp) {
  dhd_bus_t *bus = dhdp->bus;
  uint32 srmem_size = 0;
  int err = BCME_OK;
  ...
  err = dhd_iovar(dhdp, 0, "bus:srmem_size", NULL, 0, (char *)&srmem_size,
                  sizeof(srmem_size), FALSE);
  if (err) {
  ...
  if (srmem_size != bus->srmemsize) {
    sdpcm_shared_t shared;
    if (err == BCME_OK) bus->srmemsize = srmem_size;
    dhd_os_sdlock(dhdp);
...
static int dhdsdio_readconsole(dhd_bus_t *bus) {
  ...
  if (bus->console_addr == 0) return 0;

  if (!KSO_ENAB(bus)) return 0;

  /* Read console log struct */
  addr = bus->console_addr + OFFSETOF(hnd_cons_t, log);
  if ((rv = dhdsdio_membytes(bus, FALSE, addr, (uint8 *)&c->log,
                             sizeof(c->log))) < 0)
    return rv;
...
static int dhdsdio_checkdied(dhd_bus_t *bus, char *data, uint size) {
  ...
  if (DHD_NOCHECKDIED_ON()) return 0;
  ...
  if ((bcmerror = dhdsdio_readshared(bus, &l_sdpcm_shared)) < 0) goto done;
  ...
  if ((l_sdpcm_shared.flags & (SDPCM_SHARED_ASSERT | SDPCM_SHARED_TRAP)) == 0) {
    ...
    bcm_bprintf(
        ...
        (l_sdpcm_shared.flags & SDPCM_SHARED_ASSERT_BUILT) ? "/assrt" : "");
  } else {
    if (l_sdpcm_shared.flags & SDPCM_SHARED_ASSERT) {
      ...
      if (l_sdpcm_shared.assert_exp_addr != 0) {
        str[0] = '\0';
        if ((bcmerror =
                 dhdsdio_membytes(bus, FALSE, l_sdpcm_shared.assert_exp_addr,
      ...
      if (l_sdpcm_shared.assert_file_addr != 0) {
        str[0] = '\0';
        if ((bcmerror =
                 dhdsdio_membytes(bus, FALSE, l_sdpcm_shared.assert_file_addr,
                                  (uint8 *)str, maxstrlen)) < 0)
          goto done;
    ...
    if (l_sdpcm_shared.flags & SDPCM_SHARED_TRAP) {
      ...
      if ((bcmerror = dhdsdio_membytes(bus, FALSE, l_sdpcm_shared.trap_addr,
      ...
      if ((rv = dhdsdio_membytes(bus, FALSE, addr, (uint8 *)&console_ptr,
                                 sizeof(console_ptr))) < 0)
        goto printbuf;

      addr = l_sdpcm_shared.console_addr + OFFSETOF(hnd_cons_t, log.buf_size);
      if ((rv = dhdsdio_membytes(bus, FALSE, addr, (uint8 *)&console_size,
                                 sizeof(console_size))) < 0)
        goto printbuf;

      addr = l_sdpcm_shared.console_addr + OFFSETOF(hnd_cons_t, log.idx);
      if ((rv = dhdsdio_membytes(bus, FALSE, addr, (uint8 *)&console_index,
      ...
      for (i = 0, n = 0; i < console_size; i += n + 1) {
        ...
        if (n > 0) {
          ...
          if (dhd_msg_level & DHD_ERROR_VAL) printf("CONSOLE: %s\n", line);
        }
      }
    }
  }
...
printbuf:
  ...
  if (bus->dhd->memdump_enabled && (l_sdpcm_shared.flags & SDPCM_SHARED_TRAP)) {
     ...
     * with a better solution but with the current structure of sdlocks it is
     * very unlikely to have a better fix for now. The better Rearch of sdio bus
...
#ifdef DHD_DEBUG
static int dhd_serialconsole(dhd_bus_t *bus, bool set, bool enable,
                             int *bcmerror) {
  int int_val;
  ...
  *bcmerror = 0;
  ...
  if (!set) return (int_val & uart_enab);
  if (enable)
    int_val |= uart_enab;
  else
    int_val &= ~uart_enab;
...
static int dhdsdio_doiovar(dhd_bus_t *bus, const bcm_iovar_t *vi,
                           uint32 actionid, const char *name, void *params,
                           uint plen, void *arg, uint len, uint val_size) {
  int bcmerror = 0;
  int32 int_val = 0;
  bool bool_val = 0;
  ...
  if ((vi->varid == IOV_KSO) && (IOV_ISSET(actionid))) {
  ...
  } else if ((vi->varid == IOV_DEVSLEEP) && (IOV_ISSET(actionid))) {
    {
      ...
      if (!SLPAUTO_ENAB(bus) && (bool_val == FALSE) && (bus->ipend)) {
        DHD_ERROR(
            ("INT pending in devsleep 1, dpc_sched: %d\n", bus->dpc_sched));
        if (!bus->dpc_sched) {
          bus->dpc_sched = TRUE;
          dhd_sched_dpc(bus->dhd);
        }
  ...
  switch (actionid) {
    ...
    case IOV_SVAL(IOV_SET_DOWNLOAD_INFO): {
      ...
      bcmerror = dhdsdio_bus_save_download_info(
    ...
    case IOV_SVAL(IOV_READAHEAD):
      if (bool_val && !dhd_readahead) bus->nextlen = 0;
    ...
    case IOV_GVAL(IOV_SDREG): {
      ...
      if (bcmsdh_regfail(bus->sdh)) bcmerror = BCME_SDIO_ERROR;
    ...
    case IOV_GVAL(IOV_SDCIS): {
      *(char *)arg = 0;
      ...
      bcmstrcat(arg, "\nFunc 0\n");
      bcmsdh_cis_read(bus->sdh, 0x10, (uint8 *)arg + strlen(arg),
                      SBSDIO_CIS_SIZE_LIMIT);
      bcmstrcat(arg, "\nFunc 1\n");
      bcmsdh_cis_read(bus->sdh, 0x11, (uint8 *)arg + strlen(arg),
                      SBSDIO_CIS_SIZE_LIMIT);
      bcmstrcat(arg, "\nFunc 2\n");
      bcmsdh_cis_read(bus->sdh, 0x12, (uint8 *)arg + strlen(arg),
                      SBSDIO_CIS_SIZE_LIMIT);
      break;
    }
    ...
    case IOV_GVAL(IOV_SPROM): {
       ...
       */
      if (bus->bus == SPI_BUS) size = SBSDIO_CIS_SIZE_LIMIT;

      /* Do some validation */
      dsize = set ? plen - (2 * sizeof(int)) : len;
      if (dsize < size) {
        DHD_ERROR(("%s: error on srom %s, addr 0x%08x size %d dsize %d\n",
                   __FUNCTION__, (set ? "write" : "read"), offset, size,
                   dsize));
        bcmerror = BCME_BADARG;
        break;
      }
      ...
      if (!set) {
        ...
        bcmerror = srom_read(bus->sih, DHD_BUS, (void *)bus->regs,
                             bus->dhd->osh, offset, size, (uint16 *)arg, FALSE);
        GCC_DIAGNOSTIC_POP();
        ...
        GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
        bcmerror = srom_write(bus->sih, DHD_BUS, (void *)bus->regs,
                              bus->dhd->osh, offset, size, (uint16 *)arg);
        GCC_DIAGNOSTIC_POP();
      }
      break;
    }
    ...
    case IOV_SVAL(IOV_WATERMARK):
      ...
      watermark = (watermark > SBSDIO_WATERMARK_MASK) ? SBSDIO_WATERMARK_MASK
                                                      : watermark;
      DHD_ERROR(("Setting watermark as 0x%x.\n", watermark));
      bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK,
                       (uint8)watermark, NULL);
      break;
    ...
    case IOV_SVAL(IOV_DEVRESET):
      ...
      break;
      /*
       * softap firmware is updated through module parameter or android private
       * command
       */
...
static int dhdsdio_write_vars(dhd_bus_t *bus) {
  ...
  bcmerror =
      dhdsdio_membytes(bus, TRUE, (phys_size - 4), (uint8 *)&varsizew, 4);
...
static int dhdsdio_download_state(dhd_bus_t *bus, bool enter) {
  ...
  if (enter) {
    ...
    if (!foundcr4) {
      ...
      if (bus->ramsize) {
        uint32 zeros = 0;
        if (dhdsdio_membytes(bus, TRUE, bus->ramsize - 4, (uint8 *)&zeros, 4) <
            0) {
          bcmerror = BCME_SDIO_ERROR;
          goto fail;
        }
    ...
    if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
      ...
      if (do_wr_flops) {
        ...
        bcmerror = dhdsdio_membytes(bus, TRUE, 0, (uint8 *)&bus->resetinstr,
        ...
        if (bcmerror == BCME_OK) {
          ...
          /* verify write */
          bcmerror =
              dhdsdio_membytes(bus, FALSE, 0, (uint8 *)&tmp, sizeof(tmp));
...
static int dhdsdio_bus_download_fw_signature(dhd_bus_t *bus, bool *do_write) {
  ...
  if ((bcmerror = dhdsdio_bus_download_ram_bootloader(bus)) != BCME_OK) {
...
static int dhdsdio_bus_write_fw_signature(dhd_bus_t *bus) {
  ...
  if ((bcmerror = dhdsdio_bus_write_fwsig(bus, bus->fwsig_filename, NULL))) {
    DHD_ERROR(
 ...
 * existing rTLVs */
static int dhdsdio_download_rtlv(dhd_bus_t *bus, dngl_rtlv_type_t type,
...
static int dhdsdio_bus_write_fws_status(dhd_bus_t *bus) {
  ...
  ret = dhdsdio_download_rtlv(bus, DNGL_RTLV_TYPE_FWSIGN_STATUS,
...
static int dhdsdio_bus_write_fws_mem_info(dhd_bus_t *bus) {
  ...
  ret = dhdsdio_download_rtlv(bus, DNGL_RTLV_TYPE_FWSIGN_MEM_MAP,
...
static int dhdsdio_download_sig_file(dhd_bus_t *bus, char *path, uint32 type) {
  int bcmerror = BCME_OK;
  int srcsize = 0;
  uint32 dest_size = 0; /* dongle RAM dest size */
...
exit:
  if (sig) dhd_os_close_img_fwreq(sig);
...
static int dhdsdio_bus_write_fwsig(dhd_bus_t *bus, char *fwsig_path,
  ...
  bcmerror =
      dhdsdio_download_sig_file(bus, fwsig_path, DNGL_RTLV_TYPE_FW_SIGNATURE);
  if (bcmerror) {
    goto exit;
  }
...
int dhd_bus_iovar_op(dhd_pub_t *dhdp, const char *name, void *params, uint plen,
  ...
  if ((vi = bcm_iovar_lookup(dhdsdio_iovars, name)) == NULL) {
    ...
    if (set && strcmp(name, "sd_divisor") == 0) {
      if (bcmsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0, &bus->sd_divisor,
                          sizeof(int32), FALSE) != BCME_OK) {
        bus->sd_divisor = -1;
    ...
    if (set && strcmp(name, "sd_mode") == 0) {
      if (bcmsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0, &bus->sd_mode,
                          sizeof(int32), FALSE) != BCME_OK) {
        bus->sd_mode = -1;
        DHD_ERROR(("%s: fail on %s get\n", __FUNCTION__, name));
      } else {
  ...
  bcmerror = dhdsdio_doiovar(bus, vi, actionid, name, params, plen, arg, len,
                             val_size);
...
void dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex) {
  ...
  if (enforce_mutex) dhd_os_sdlock(bus->dhd);
  ...
  if (bus->glomd) PKTFREE(osh, bus->glomd, FALSE);

  if (bus->glom) PKTFREE(osh, bus->glom, FALSE);
  ...
  if (enforce_mutex) dhd_os_sdunlock(bus->dhd);
}

#if defined(BCMSDIOH_TXGLOM) && defined(BCMSDIOH_STD)
extern uint sd_txglom;
#endif
void dhd_txglom_enable(dhd_pub_t *dhdp, bool enable) {
...
#ifdef BCMSDIOH_STD
  if (enable) enable = sd_txglom;
#endif /* BCMSDIOH_STD */

  if (enable) {
    rxglom = 1;
    ret = dhd_iovar(dhdp, 0, "bus:rxglom", (char *)&rxglom, sizeof(rxglom),
                    NULL, 0, TRUE);
    if (ret >= 0)
...
int dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex) {
  ...
  while (!enable && !dhd_timeout_expired(&tmo)) {
    dstatus =
        bcmsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);
    if (dstatus & STATUS_F2_RX_READY) enable = TRUE;
  }
  ...
  if (enable) {
    ...
    DHD_ERROR(
        ("dstatus when timed out on f2-fifo not ready = 0x%x\n", dstatus));
  ...
  DHD_ERROR(("%s: enable 0x%02x, ready 0x%02x (waited %uus)\n", __FUNCTION__,
             enable, ready, tmo.elapsed));

#if defined(SDIO_ISR_THREAD)
  if (dhdp->conf->intr_extn) {
    intr_extn =
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_INTR_EXTN, NULL);
    if (intr_extn & 0x1) {
      intr_extn |= 0x2;
      bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_INTR_EXTN, intr_extn,
                       NULL);
    }
  }
#endif
  ...
  if (ready == enable) {
...
#ifdef DEBUG_LOST_INTERRUPTS
    {
      ...
      devpend =
          bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_INTPEND, NULL);

      R_SDREG(intstatus, &bus->regs->intstatus, sdr_retries);
      intstatus &= bus->hostintmask;

      DHD_ERROR(
          ("%s: interrupts -- host %s device ena/pend 0x%02x/0x%02x\n"
           "intstatus 0x%08x, hostmask 0x%08x\n",
           __FUNCTION__, (hostpending ? "PENDING" : "NOT PENDING"), devena,
           devpend, intstatus, bus->hostintmask));
    }
  ...
  if (dhdsdio_sr_cap(bus)) {
    ...
    bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, saveclk,
                     &err);
  }
#endif /* !BCMSPI */

  /* If we didn't come up, turn off backplane clock */
  if (dhdp->busstate != DHD_BUS_DATA) dhdsdio_clkctl(bus, CLK_NONE, FALSE);
...
static void dhdsdio_read_control(dhd_bus_t *bus, uint8 *hdr, uint len,
                                 uint doff) {
  bcmsdh_info_t *sdh = bus->sdh;
  uint rdlen, pad;
  ...
  if ((bus->bus == SPI_BUS) && (!bus->usebufpool)) goto gotpkt;
...
static uint8 dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq) {
  ...
  DHD_TRACE(
      ("dhdsdio_rxglom: start: glomd %p glom %p\n", bus->glomd, bus->glom));
    ...
    if (pnext) {
      ...
      if (DHD_GLOM_ON() && bus->nextlen) {
        if (totlen != bus->nextlen) {
          DHD_GLOM(
              ("%s: glomdesc mismatch: nextlen %d glomdesc %d "
               "rxseq %d\n",
               __FUNCTION__, bus->nextlen, totlen, rxseq));
        }
      }
      bus->glom = pfirst;
      pfirst = pnext = NULL;
    } else {
      if (pfirst) PKTFREE(osh, pfirst, FALSE);
      bus->glom = NULL;
      num = 0;
    }
  ...
  if (bus->glom) {
    if (DHD_GLOM_ON()) {
      ...
      for (pnext = bus->glom; pnext; pnext = PKTNEXT(osh, pnext)) {
        DHD_GLOM(("    %p: %p len 0x%04x (%d)\n", pnext,
                  (uint8 *)PKTDATA(osh, pnext), PKTLEN(osh, pnext),
                  PKTLEN(osh, pnext)));
      }
    }
     ...
     */
    if (usechain) {
      errcode = dhd_bcmsdh_recv_buf(
          bus, bcmsdh_cur_sbwad(bus->sdh), SDIO_FUNC_2, F2SYNC,
          (uint8 *)PKTDATA(osh, pfirst), dlen, pfirst, NULL, NULL);
    } else if (bus->dataptr) {
      errcode =
          dhd_bcmsdh_recv_buf(bus, bcmsdh_cur_sbwad(bus->sdh), SDIO_FUNC_2,
                              F2SYNC, bus->dataptr, dlen, NULL, NULL, NULL);
...
#ifdef DHD_DEBUG
    if (DHD_GLOM_ON()) {
      prhex("SUPERFRAME", PKTDATA(osh, pfirst), MIN(PKTLEN(osh, pfirst), 48));
    }
#endif
    ...
    for (num = 0, pnext = pfirst; pnext && !errcode;
      ...
      if ((uint16) ~(sublen ^ check)) {
        DHD_ERROR(
            ("%s (subframe %d): HW hdr error: "
             "len/check 0x%04x/0x%04x\n",
             __FUNCTION__, num, sublen, check));
        errcode = -1;
      } else if ((sublen > dlen) || (sublen < SDPCM_HDRLEN)) {
        DHD_ERROR(
            ("%s (subframe %d): length mismatch: "
    ...
    for (num = 0; pfirst; rxseq++, pfirst = pnext) {
      ...
      if (DHD_GLOM_ON()) {
        DHD_GLOM(("%s subframe %d to stack, %p(%p/%d) nxt/lnk %p/%p\n",
                  __FUNCTION__, num, pfirst, PKTDATA(osh, pfirst),
                  PKTLEN(osh, pfirst), PKTNEXT(osh, pfirst), PKTLINK(pfirst)));
        prhex("", (uint8 *)PKTDATA(osh, pfirst), MIN(PKTLEN(osh, pfirst), 32));
      }
#endif /* DHD_DEBUG */
    }
    dhd_os_sdunlock_rxq(bus->dhd);
  ...
  *finished = FALSE;
  ...
  if (bus->bus == SPI_BUS) {
    ...
    if ((dstatus & STATUS_F2_PKT_AVAILABLE) &&
      ...
      if (bus->dwordmode) bus->nextlen = bus->nextlen << 2;
  ...
  for (rxseq = bus->rx_seq, rxleft = maxframes;
    ...
    if (TXCTLOK(bus) && bus->ctrl_frame_stat && (bus->clkstate == CLK_AVAIL)) {
    ...
    } else if (bus->dotxinrx && (bus->clkstate == CLK_AVAIL) && !bus->fcstate &&
               DATAOK(bus) &&
       ...
       */
      bus->dotxinrx =
          (bus->dhd->tcpack_sup_mode == TCPACK_SUP_DELAYTX) ? FALSE : TRUE;
#endif
    }
    ...
    if (dhd_readahead && bus->nextlen) {
      ...
      if (!(pkt = PKTGET(osh, rdlen + DHD_SDALIGN, FALSE))) {
        if (bus->bus == SPI_BUS) {
          ...
          DHD_ERROR(
              ("%s (nextlen): PKTGET failed: len %d rdlen %d "
               "expected rxseq %d\n",
        ...
        if (bus->bus == SPI_BUS) bus->usebufpool = TRUE;
        ...
        if (bcmsdh_get_dstatus((void *)bus->sdh) & STATUS_UNDERFLOW) {
          ...
          *finished = TRUE;
          DHD_ERROR(
              ("%s (nextlen): read %d bytes failed due "
               "to spi underflow\n",
               __FUNCTION__, rdlen));
          PKTFREE(bus->dhd->osh, pkt, FALSE);
          bus->dhd->rx_errors++;
      ...
      if ((uint16) ~(len ^ check)) {
        DHD_ERROR(
            ("%s (nextlen): HW hdr error: nextlen/len/check"
             " 0x%04x/0x%04x/0x%04x\n",
             __FUNCTION__, nextlen, len, check));
        dhd_os_sdlock_rxq(bus->dhd);
        PKTFREE2();
        dhd_os_sdunlock_rxq(bus->dhd);
        bus->rx_badhdr++;
      ...
      if (len_consistent) {
        ...
        DHD_ERROR(
            ("%s (nextlen): mismatch, nextlen %d len %d rnd %d; "
             "expected rxseq %d\n",
      ...
      if (bus->bus == SPI_BUS) {
        ...
        if ((bus->nextlen << 4) > MAX_RX_DATASZ) {
          DHD_INFO(
              ("%s (nextlen): got frame w/nextlen too large"
               " (%d), seq %d\n",
      ...
      if (chan == SDPCM_CONTROL_CHANNEL) {
        if (bus->bus == SPI_BUS) {
          ...
          DHD_ERROR(
              ("%s (nextlen): readahead on control"
               " packet %d?\n",
               __FUNCTION__, seq));
    ...
    if (!(pkt = PKTGET(osh, (rdlen + firstread + DHD_SDALIGN), FALSE))) {
      /* Give up on data, request rtx of events */
      DHD_ERROR(
          ("%s: PKTGET failed: rdlen %d chan %d\n", __FUNCTION__, rdlen, chan));
      bus->dhd->rx_dropped++;
      dhd_os_sdunlock_rxq(bus->dhd);
    ...
    sdret = dhd_bcmsdh_recv_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
                                ((uint8 *)PKTDATA(osh, pkt)), rdlen, pkt, NULL,
                                NULL);
    bus->f2rxdata++;
    ASSERT(sdret != BCME_PENDING);
  ...
  deliver:
    ...
    dhd_os_sdlock(bus->dhd);
#if defined(SDIO_ISR_THREAD)
    /* terence 20150615: fix for below error due to bussleep in watchdog after
     * dhd_os_sdunlock here, so call BUS_WAKE to wake up bus again
...
static uint32 dhdsdio_hostmail(dhd_bus_t *bus, uint32 *hmbd) {
  sdpcmd_regs_t *regs = bus->regs;
  uint32 intstatus = 0;
  uint32 hmb_data;
  uint8 fcbits;
  ...
  if (retries <= retry_limit) W_SDREG(SMB_INT_ACK, &regs->tosbmailbox, retries);
  bus->f1regdata += 2;
  ...
  if (hmb_data & (HMB_DATA_DEVREADY | HMB_DATA_FWREADY)) {
    bus->sdpcm_ver =
        (hmb_data & HMB_DATA_VERSION_MASK) >> HMB_DATA_VERSION_SHIFT;
...
// open FW cons and log by default
   ...
   * method isn't used any more.  Leave this here for possibly remaining
   * backward compatible with older dongles
   */
  if (hmb_data & HMB_DATA_FC) {
    fcbits = (hmb_data & HMB_DATA_FCDATA_MASK) >> HMB_DATA_FCDATA_SHIFT;

    if (fcbits & ~bus->flowcontrol) bus->fc_xoff++;
    if (bus->flowcontrol & ~fcbits) bus->fc_xon++;
  ...
  if (hmb_data & ~(HMB_DATA_DEVREADY | HMB_DATA_FWHALT | HMB_DATA_NAKHANDLED |
                   HMB_DATA_FC | HMB_DATA_FWREADY | HMB_DATA_FCDATA_MASK |
...
static uint32 dhdsdio_read_intstatus_byte(dhd_bus_t *bus) {
  bcmsdh_info_t *sdh = bus->sdh;
  sdpcmd_regs_t *regs = bus->regs;
  ...
  if (bus->dhd->conf->read_intr_mode) {
    ...
    if (!err1) newstatus |= intstatus_byte;
    intstatus_byte =
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
                        ((unsigned long)&regs->intstatus & 0xffff) + 1, &err2)
        << 8;
    if (!err2) newstatus |= intstatus_byte;
    intstatus_byte |=
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
                        ((unsigned long)&regs->intstatus & 0xffff) + 2, &err3)
        << 16;
    if (!err3) newstatus |= intstatus_byte;
    intstatus_byte |=
        bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
                        ((unsigned long)&regs->intstatus & 0xffff) + 3, &err4)
        << 24;
    if (!err4) newstatus |= intstatus_byte;

    if (!err1 || !err2 || !err3 || !err4) sdh->regfail = FALSE;
  } else {
    R_SDREG(newstatus, &regs->intstatus, retries);
  }
...
exit_ucode:
  ...
  if (PKT_AVAILABLE(bus, intstatus)) {
    ...
    if (rxdone || bus->rxskip) intstatus &= ~FRAME_AVAIL_MASK(bus);
...
clkwait:
...
#endif /* !defined(NDIS) */
#ifdef BCMSPI_ANDROID
    if (*dhd_spi_lockcount == 0) bcmsdh_oob_intr_set(bus->sdh, TRUE);
#endif /* BCMSPI_ANDROID */
  }
  ...
  if (!DATAOK(bus) && bus->rxlim_en) {
    uint8 rxlim = 0;
    if (0 ==
        dhdsdio_membytes(bus, FALSE, bus->rxlim_addr, (uint8 *)&rxlim, 1)) {
      if (bus->tx_max != rxlim) {
        DHD_INFO(("%s: bus->tx_max/rxlim=%d/%d\n", __FUNCTION__, bus->tx_max,
                  rxlim));
        bus->tx_max = rxlim;
      }
    }
...
#ifdef PROP_TXSTATUS
  dhd_wlfc_commit_packets(bus->dhd, (f_commitpkt_t)dhd_bus_txdata, (void *)bus,
                          NULL, FALSE);
#endif
  ...
  if (TXCTLOK(bus) && bus->ctrl_frame_stat && (bus->clkstate == CLK_AVAIL))
    dhdsdio_sendpendctl(bus);
#ifdef CONSOLE_DPC
  else if (DATAOK(bus) && strlen(bus->cons_cmd) &&
  ...
  if (bus->ctrl_frame_stat) {
    if (bus->dhd->conf->txctl_tmo_fix) {
      ...
      if (!kthread_should_stop()) schedule_timeout(1);
  ...
  if ((bus->dhd->busstate == DHD_BUS_DOWN) || bcmsdh_regfail(sdh)) {
    if ((bus->sih && bus->sih->buscorerev >= 12) &&
       ...
       * E.g. if we get occasional 'bcmsdh_regfail' we should be able to
       * continue operation. May want to make the decision to enable or not
       * based on count of failures, so in case of bus lock up we avoid
    ...
    /* Awaiting I_CHIPACTIVE; don't resched */
             ...
             (!bus->fcstate &&
              (pktq_mlen(&bus->txq, ((~bus->flowcontrol)
#ifdef DHD_LOSSLESS_ROAMING
...
exit:
  ...
  if (bus->ctrl_wait && TXCTLOK(bus)) wake_up_interruptible(&bus->ctrl_tx_wait);
  dhd_os_sdunlock(bus->dhd);
#ifdef DEBUG_DPC_THREAD_WATCHDOG
  if (bus->dhd->dhd_bug_on) {
    DHD_INFO((
        "%s: resched = %d ctrl_frame_stat = %d intstatus 0x%08x"
        " ipend = %d pktq_mlen = %d is_resched_by_readframe = %d \n",
        __FUNCTION__, resched, bus->ctrl_frame_stat, bus->intstatus, bus->ipend,
...
#ifdef DHD_WAKE_STATUS
  if (dhdp && dhdp->in_suspend) bcmsdh_set_get_wake(bus->sdh, 1);
#endif
...
void dhdsdio_isr(void *arg) {
  ...
  /* terence 20150209: dpc should be scheded again if dpc_sched is TRUE or
     dhd_bus_txdata can not schedule anymore because dpc_sched is TRUE now.
   */
  if (dhdsdio_dpc(bus)) {
    bus->dpc_sched = TRUE;
    dhd_sched_dpc(bus->dhd);
...
#ifdef PKT_STATICS
void dhd_bus_dump_txpktstatics(dhd_pub_t *dhdp) {
  dhd_bus_t *bus = dhdp->bus;
  uint32 total = 0;
  uint i;

  printf("%s: TYPE EVENT: %d pkts (size=%d) transfered\n", __FUNCTION__,
         bus->tx_statics.event_count, bus->tx_statics.event_size);
  printf("%s: TYPE CTRL:  %d pkts (size=%d) transfered\n", __FUNCTION__,
         bus->tx_statics.ctrl_count, bus->tx_statics.ctrl_size);
  printf("%s: TYPE DATA:  %d pkts (size=%d) transfered\n", __FUNCTION__,
         bus->tx_statics.data_count, bus->tx_statics.data_size);
  ...
  printk(KERN_CONT DHD_LOG_PREFIXS);
  for (i = 0; i < bus->tx_statics.glom_max; i++) {
    printk(KERN_CONT "%02d:%5d%%", i + 1,
           (bus->tx_statics.glom_cnt[i] * 100) / total);
    if ((i + 1) % 8)
      printk(KERN_CONT ", ");
    else {
      printk("\n");
...
void dhd_bus_clear_txpktstatics(dhd_pub_t *dhdp) {
  dhd_bus_t *bus = dhdp->bus;
  ...
  for (pktcount = 0; pktcount < bus->pktgen_count; pktcount++) {
    ...
    switch (bus->pktgen_mode) {
      ...
      case DHD_PKTGEN_RXBURST:
        ...
        *data++ =
            (uint8)bus->pktgen_count; /* Just for backward compatability */
        break;
    ...
    *data++ = (bus->pktgen_len >> 8);
    ...
    if (bus->pktgen_mode == DHD_PKTGEN_RXBURST) break;
  }
}
...
static void dhdsdio_sdtest_set(dhd_bus_t *bus, uint count) {
  ...
  /* Allocate the packet */
  if (!(pkt = PKTGET(osh,
                     SDPCM_HDRLEN + SDPCM_TEST_HDRLEN +
...
static void dhdsdio_testrcv(dhd_bus_t *bus, void *pkt, uint seq) {
  ...
  if (cmd == SDPCM_TEST_DISCARD || cmd == SDPCM_TEST_ECHOREQ ||
    ...
    if (pktlen != len + SDPCM_TEST_HDRLEN) {
      DHD_ERROR(
          ("dhdsdio_testrcv: frame length mismatch, pktlen %d seq %d"
           " cmd %d extra %d len %d\n",
  ...
  switch (cmd) {
    ...
    case SDPCM_TEST_ECHORSP:
      ...
      for (offset = 0; offset < len; offset++, data++) {
        if (*data != SDPCM_TEST_FILL(offset, extra)) {
          DHD_ERROR(
              ("dhdsdio_testrcv: echo data mismatch: "
      ...
      for (i = 0; i < len; i++) {
        if (*prn != testval) {
          DHD_ERROR(("DIErr@Pkt#:%d,Ix:%d, expected:0x%x, got:0x%x\n", i,
                     bus->pktgen_rcvd_rcvsession, testval, *prn));
          prn++;
          testval++;
        }
      }
    ...
    default:
      DHD_INFO(
          ("dhdsdio_testrcv: unsupported or unknown command, pktlen %d seq %d"
           " cmd %d extra %d len %d\n",
           pktlen, seq, cmd, extra, len));
      PKTFREE(osh, pkt, FALSE);
      break;
...
extern bool dhd_bus_watchdog(dhd_pub_t *dhdp) {
  ...
  if (bus->dhd->dongle_reset) return FALSE;
  ...
  if (!SLPAUTO_ENAB(bus) && bus->sleeping) return FALSE;
  ...
  // terence 20160615: remove !SLPAUTO_ENAB(bus) to fix not able to polling if
  // sr supported
...
#ifdef CONSOLE_DPC
extern int dhd_bus_txcons(dhd_pub_t *dhdp, uchar *msg, uint msglen) {
  dhd_bus_t *bus = dhdp->bus;
  int ret = BCME_OK;
  ...
  if (!strlen(bus->cons_cmd)) {
    ...
    DHD_CTL(("%s: \"%s\" delay send, tx_max %d, tx_seq %d, pktq_len %d\n",
             __FUNCTION__, bus->cons_cmd, bus->tx_max, bus->tx_seq,
             pktq_n_pkts_tot(&bus->txq)));
    if (!bus->dpc_sched) {
      bus->dpc_sched = TRUE;
      dhd_sched_dpc(bus->dhd);
...
static bool dhdsdio_chipmatch(uint16 chipid) {
  if (chipid == BCM4330_CHIP_ID) return TRUE;
  if (chipid == BCM43362_CHIP_ID) return TRUE;
  if (chipid == BCM43340_CHIP_ID) return TRUE;
  if (chipid == BCM43341_CHIP_ID) return TRUE;
  if (chipid == BCM4334_CHIP_ID) return TRUE;
  if (chipid == BCM4324_CHIP_ID) return TRUE;
  if (chipid == BCM4335_CHIP_ID) return TRUE;
  if (chipid == BCM4339_CHIP_ID) return TRUE;
  if (BCM4345_CHIP(chipid)) return TRUE;
  if (chipid == BCM4350_CHIP_ID) return TRUE;
  if (chipid == BCM4354_CHIP_ID) return TRUE;
  if (chipid == BCM4356_CHIP_ID) return TRUE;
  if (chipid == BCM4358_CHIP_ID) return TRUE;
  if (chipid == BCM43569_CHIP_ID) return TRUE;
  if (chipid == BCM4371_CHIP_ID) return TRUE;
  if (chipid == BCM43430_CHIP_ID) return TRUE;
  if (chipid == BCM43018_CHIP_ID) return TRUE;
  if (BCM4349_CHIP(chipid)) return TRUE;
  if (chipid == BCM4364_CHIP_ID) return TRUE;
  ...
  if (chipid == BCM43012_CHIP_ID) return TRUE;

  if (chipid == BCM43014_CHIP_ID) return TRUE;

  if (chipid == BCM43013_CHIP_ID) return TRUE;

  if (chipid == BCM4369_CHIP_ID) return TRUE;

  if (BCM4378_CHIP(chipid)) {
    return TRUE;
  }

  if (chipid == BCM4362_CHIP_ID) return TRUE;
  if (chipid == BCM43751_CHIP_ID) return TRUE;
  if (chipid == BCM43752_CHIP_ID) return TRUE;
  if (chipid == BCM43756_CHIP_ID) return TRUE;
  if (chipid == BCM43711_CHIP_ID) return TRUE;
  return FALSE;
}
...
static void *dhdsdio_probe(uint16 venid, uint16 devid, uint16 bus_no,
  ...
  bus->usebufpool =
      FALSE; /* Use bufpool if allocated, else use locally malloced rxbuf */
#ifdef BT_OVER_SDIO
  bus->bt_use_count = 0;
#endif
  ...
  if (bus->intr) {
    ...
    DHD_INTR(
        ("%s: disable SDIO interrupts (not interested yet)\n", __FUNCTION__));
    bcmsdh_intr_disable(sdh); /* XXX New API: bcmsdh_intr_mask()? */
    if ((ret = bcmsdh_intr_reg(sdh, dhdsdio_isr, bus)) != 0) {
      DHD_ERROR(
          ("%s: FAILED: bcmsdh_intr_reg returned %d\n", __FUNCTION__, ret));
      goto fail;
    }
    DHD_INTR(("%s: registered SDIO interrupt function ok\n", __FUNCTION__));
  } else {
    DHD_INFO(
        ("%s: SDIO interrupt function is NOT registered due to polling mode\n",
         __FUNCTION__));
  }
   ...
   * started based upon dhd_download_fw_on_driverload check, so increase the bus
   * user count, this count will only be disabled inside dhd_register_if()
   * function if flag dhd_download_fw_on_driverload is set to false, i.e FW
   * download during insmod is not needed, otherwise it will not be decremented
  ...
  wl_android_post_init();  // terence 20120530: fix critical section in dhd_open
                           // and dhdsdio_probe
...
static bool dhdsdio_probe_attach(struct dhd_bus *bus, osl_t *osh, void *sdh,
                                 void *regsva, uint16 devid) {
#ifndef BCMSPI
  int err = 0;
  uint8 clkctl = 0;
  ...
  bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, DHD_INIT_CLKCTL1,
                   &err);
  if (!err)
    clkctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
  ...
  if (err || ((clkctl & ~SBSDIO_AVBITS) != DHD_INIT_CLKCTL1)) {
    DHD_ERROR(
        ("dhdsdio_probe: ChipClkCSR access: err %d wrote 0x%02x read 0x%02x\n",
  ...
  if (DHD_INFO_ON()) {
    ...
    for (fn = 0; fn <= numfn; fn++) {
      ...
      if ((err = bcmsdh_cis_read(sdh, fn, cis, SBSDIO_CIS_SIZE_LIMIT))) {
  ...
  if (!(bus->sih = si_attach((uint)devid, osh, regsva, DHD_BUS, sdh, &bus->vars,
                             &bus->varsz))) {
  ...
  if (!DHD_NOPMU(bus)) {
    ...
    if (dhd_dongle_ramsize) dhd_dongle_setramsize(bus, dhd_dongle_ramsize);
...
static bool dhdsdio_probe_malloc(dhd_bus_t *bus, osl_t *osh, void *sdh) {
  ...
  if (bus->dhd->maxctl) {
    bus->rxblen =
        ROUNDUP((bus->dhd->maxctl + SDPCM_HDRLEN), ALIGNMENT) + DHD_SDALIGN;
    if (!(bus->rxbuf =
              DHD_OS_PREALLOC(bus->dhd, DHD_PREALLOC_RXBUF, bus->rxblen))) {
      DHD_ERROR(
          ("%s: MALLOC of %d-byte rxbuf failed\n", __FUNCTION__, bus->rxblen));
      goto fail;
    }
  }
  /* Allocate buffer to receive glomed packet */
  if (!(bus->databuf =
            DHD_OS_PREALLOC(bus->dhd, DHD_PREALLOC_DATABUF, MAX_DATA_BUF))) {
    DHD_ERROR(
        ("%s: MALLOC of %d-byte databuf failed\n", __FUNCTION__, MAX_DATA_BUF));
    /* release rxbuf which was already located as above */
    if (!bus->rxblen) DHD_OS_PREFREE(bus->dhd, bus->rxbuf, bus->rxblen);
    goto fail;
  }
  ...
  if (bus->membuf == NULL) {
    DHD_ERROR(
        ("%s: MALLOC of %d-byte membuf failed\n", __FUNCTION__, MAX_MEM_BUF));
    if (bus->databuf) {
  ...
  if ((uintptr)bus->databuf % DHD_SDALIGN)
    bus->dataptr =
        bus->databuf + (DHD_SDALIGN - ((uintptr)bus->databuf % DHD_SDALIGN));
  else
...
static bool dhdsdio_probe_init(dhd_bus_t *bus, osl_t *osh, void *sdh) {
  ...
  if (bcmsdh_iovar_op(sdh, "sd_divisor", NULL, 0, &bus->sd_divisor,
                      sizeof(int32), FALSE) != BCME_OK) {
  ...
  if (bcmsdh_iovar_op(sdh, "sd_mode", NULL, 0, &bus->sd_mode, sizeof(int32),
                      FALSE) != BCME_OK) {
...
#ifdef DHDENABLE_TAILPAD
  if (bus->pad_pkt) PKTFREE(osh, bus->pad_pkt, FALSE);
   ...
   */
  if (bcmsdh_iovar_op(sdh, "sd_rxchain", NULL, 0, &bus->sd_rxchain,
                      sizeof(int32), FALSE) != BCME_OK) {
...
int dhd_set_bus_params(struct dhd_bus *bus) {
  ...
  if (bus->hdrbufp) {
    MFREE(bus->dhd->osh, bus->hdrbufp,
          bus->dhd->conf->max_hdr_read + DHD_SDALIGN);
  }
  bus->hdrbufp =
      MALLOC(bus->dhd->osh, bus->dhd->conf->max_hdr_read + DHD_SDALIGN);
  if (bus->hdrbufp == NULL) {
...
static int dhdsdio_download_firmware(struct dhd_bus *bus, osl_t *osh,
                                     void *sdh) {
  int ret;
...
static void dhdsdio_release_dongle(dhd_bus_t *bus, osl_t *osh,
  ...
  if ((bus->dhd && bus->dhd->dongle_reset) && reset_flag) return;
  ...
  if (bus->sih) {
    ...
    si_detach(bus->sih);
    bus->sih = NULL;
    if (bus->vars && bus->varsz) MFREE(osh, bus->vars, bus->varsz);
    bus->vars = NULL;
  }
...
uint64 dhdsdio_get_last_suspend_time(dhd_pub_t *dhdp) {
...
static int dhdsdio_suspend(void *context) {
  ...
  if (SLPAUTO_ENAB(bus) && (!ret) && (bus->dhd->up) &&
      (bus->dhd->op_mode != DHD_FLAG_HOSTAP_MODE)) {
...
#if defined(OOB_INTR_ONLY) || defined(BCMSPI_ANDROID)
  if (dhd_os_check_if_up(bus->dhd)) bcmsdh_oob_intr_set(bus->sdh, TRUE);
#endif /* defined(OOB_INTR_ONLY) || defined(BCMSPI_ANDROID) */
...
static bcmsdh_driver_t dhd_sdio = {dhdsdio_probe, dhdsdio_disconnect,
                                   dhdsdio_suspend, dhdsdio_resume};
...
#if defined(BCMLXSDMMC)
/* Register a dummy SDIO client driver in order to be notified of new SDIO
 * device */
int dhd_bus_reg_sdio_notify(void *semaphore) {
  return bcmsdh_reg_sdio_notify(semaphore);
}

void dhd_bus_unreg_sdio_notify(void) { bcmsdh_unreg_sdio_notify(); }
#endif /* defined(BCMLXSDMMC) */
...
static int dhdsdio_download_code_array(struct dhd_bus *bus) {
  ...
  while ((offset + MEMBLOCK) < sizeof(dlarray)) {
    ...
    bcmerror = dhdsdio_membytes(bus, TRUE, offset, (uint8 *)(dlarray + offset),
                                MEMBLOCK);
    if (bcmerror) {
      DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
  ...
  /* Upload and compare the downloaded code */
  {
    ...
    while ((offset + MEMBLOCK) < sizeof(dlarray)) {
      bcmerror =
          dhdsdio_membytes(bus, FALSE, offset, ularray + offset, MEMBLOCK);
      if (bcmerror) {
    ...
    if (offset < sizeof(dlarray)) {
      bcmerror = dhdsdio_membytes(bus, FALSE, offset, ularray + offset,
                                  sizeof(dlarray) - offset);
      if (bcmerror) {
        DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
                   __FUNCTION__, bcmerror, sizeof(dlarray) - offset, offset));
...
err:
  if (ularray) MFREE(bus->dhd->osh, ularray, bus->ramsize);
  return bcmerror;
}
#endif /* BCMEMBEDIMAGE */

#ifdef DHD_LINUX_STD_FW_API
static int dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path) {
  int bcmerror = -1;
  int offset = 0;
  int len;
  ...
  /* XXX: Should succeed in opening image if it is actually given through
   * registry entry or in module param.
   */
   ...
   */
#ifndef NDIS
  if (sd_f1_blocksize == 512) memblock_size = MAX_MEMBLOCK;
#endif /* !NDIS */
...
err:
  if (memblock) MFREE(bus->dhd->osh, memblock, memblock_size + DHD_SDALIGN);
#ifdef CHECK_DOWNLOAD_FW
  if (bus->dhd->conf->fwchk) {
  ...
  static int dhdsdio_download_code_file(struct dhd_bus * bus, char *pfw_path) {
    ...
    if (bcmerror == BCME_UNSUPPORTED) {
      ...
      DHD_ERROR(
          ("%s Using COMBINED image (size %d)\n", __FUNCTION__, file_size));
    }
    bus->fw_download_len = file_size;
    bus->fw_download_addr = bus->dongle_ram_base;

    /* Update the dongle image download block size depending on the F1 block
     * size */
#ifndef NDIS
    if (sd_f1_blocksize == 512) memblock_size = MAX_MEMBLOCK;
#endif /* !NDIS */
  ...
  err:
    ...
    if (image) dhd_os_close_image1(bus->dhd, image);
 ...
 */
static uint32 dhdsdio_ucode_base(struct dhd_bus *bus) {
  uint32 ucode_base = 0;
...
void dhd_bus_ucode_download(struct dhd_bus *bus) {
...
static int dhdsdio_download_nvram(struct dhd_bus *bus) {
  ...
  if (len > 0 && len < MAX_NVRAMBUF_SIZE) {
    ...
    bufp += len;
    *bufp++ = 0;
    if (len) bcmerror = dhdsdio_downloadvars(bus, memblock, len + 1);
    if (bcmerror) {
      DHD_ERROR(("%s: error downloading vars: %d\n", __FUNCTION__, bcmerror));
    }
  } else {
    DHD_ERROR(
        ("%s: error reading nvram file: %s %d\n", __FUNCTION__, pnv_path, len));
    bcmerror = BCME_SDIO_ERROR;
  }

err:
  if (memblock) dhd_free_download_buffer(bus->dhd, memblock, memblock_len);
...
static int dhd_bcmsdh_recv_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
  ...
  status = bcmsdh_recv_buf(bus->sdh, addr, fn, flags, buf, nbytes, pkt,
                           complete_fn, handle);
...
static int dhd_bcmsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
  ...
  sdh = bus->sdh;
  do {
    ret = bcmsdh_send_buf(bus->sdh, addr, fn, flags, buf, nbytes, pkt,
                          complete_fn, handle);
    ...
    if (ret == BCME_NODEVICE) {
      ...
      bus->dhd->tx_errors++;
      bcmsdh_abort(sdh, SDIO_FUNC_2);
      bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_FRAMECTRL, SFC_WF_TERM,
                       NULL);
...
uint8 dhd_bus_is_ioready(struct dhd_bus *bus) {
  uint8 enable;
  bcmsdh_info_t *sdh;
  ASSERT(bus);
  ASSERT(bus->sih != NULL);
...
void dhd_bus_set_dotxinrx(struct dhd_bus *bus, bool val) {
  bus->dotxinrx = val;
}
    ...
    if (bus->dhd->dongle_reset) {
      ...
      if (dhdsdio_probe_attach(bus, bus->dhd->osh, bus->sdh,
        ...
        if (dhdsdio_probe_init(bus, bus->dhd->osh, bus->sdh) &&
          ...
          DHD_ERROR(
              ("%s Failed to download binary to the dongle\n", __FUNCTION__));
          if (bus->sih != NULL) {
            si_detach(bus->sih);
            bus->sih = NULL;
          }
      ...
      if (bcmerror) return bcmerror;
#if defined(HW_OOB) || defined(FORCE_WOWLAN)
      dhd_conf_set_hw_oob_intr(
          bus->sdh, bus->sih);  // terence 20120615: fix for OOB initial issue
#endif
...
#if defined(LINUX)
int dhd_bus_suspend(dhd_pub_t *dhdpub) { return bcmsdh_stop(dhdpub->bus->sdh); }
...
int dhd_bus_membytes(dhd_pub_t *dhdp, bool set, uint32 address, uint8 *data,
                     uint size) {
  dhd_bus_t *bus;
...
#if defined(SUPPORT_MULTIPLE_REVISION)
static int concate_revision_bcm4335(dhd_bus_t *bus, char *fw_path,
                                    char *nv_path) {
  uint chipver;
...
static int concate_revision_bcm4339(dhd_bus_t *bus, char *fw_path,
                                    char *nv_path) {
  uint chipver;
#if defined(SUPPORT_MULTIPLE_CHIPS)
...
static int concate_revision_bcm4350(dhd_bus_t *bus, char *fw_path,
                                    char *nv_path) {
  uint32 chip_ver;
#if defined(SUPPORT_MULTIPLE_CHIPS)
  char chipver_tag[10] = {
...
static int concate_revision_bcm4354(dhd_bus_t *bus, char *fw_path,
                                    char *nv_path) {
  uint32 chip_ver;
#if defined(SUPPORT_MULTIPLE_CHIPS)
...
static int concate_revision_bcm43454(dhd_bus_t *bus, char *fw_path,
                                     char *nv_path) {
  char chipver_tag[10] = {
      0,
  };
...
int concate_revision(dhd_bus_t *bus, char *fw_path, char *nv_path) {
  ...
  switch (bus->sih->chip) {
    ...
    default:
      ...
      /* XXX: if revision specific feature is not required then return true
       * always */
      return res;
  }
...
#if defined(NDIS)
void dhd_bus_reject_ioreqs(dhd_pub_t *dhdp, bool reject) {
  DHD_TRACE(("%s: Enter\n", __FUNCTION__));

  bcmsdh_reject_ioreqs(dhdp->bus->sdh, reject);
}

void dhd_bus_waitfor_iodrain(dhd_pub_t *dhdp) {
  DHD_TRACE(("%s: Enter\n", __FUNCTION__));
...
void dhd_bus_update_fw_nv_path(struct dhd_bus *bus, char *pfw_path,
                               char *pnv_path, char *pclm_path,
                               char *pconf_path) {
  bus->fw_path = pfw_path;
  bus->nv_path = pnv_path;
...
#ifdef BCMSDIO
int dhd_sr_config(dhd_pub_t *dhd, bool on) {
  dhd_bus_t *bus = dhd->bus;

  if (!bus->_srenab) return -1;
...
void dhd_bus_cfg_write(void *h, uint fun_num, uint32 addr, uint8 val,
                       int *err) {
  dhd_pub_t *dhdp = (dhd_pub_t *)h;
  dhd_bus_t *bus = (dhd_bus_t *)dhdp->bus;
...
static int _dhdsdio_download_btfw(struct dhd_bus *bus) {
  ...
  mem_ptr = mem_blk =
      MALLOC(bus->dhd->osh, BTFW_DOWNLOAD_BLK_SIZE + DHD_SDALIGN);
  if (mem_blk == NULL) {
    DHD_ERROR(("%s: Failed to allocate memory %d bytes\n", __FUNCTION__,
  ...
  while (read_more_btbytes(bus, image, line, &addr_mode, &hiAddress, &dest_addr,
    ...
    if (offset_len <= 0x1000) {
      ...
      bcm_error =
          dhdsdio_membytes(bus, TRUE, start_addr, mem_ptr, bytes_to_write);
      if (bcm_error) {
        DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
                   __FUNCTION__, bcm_error, num_bytes, start_addr));
        goto err;
...
err:
  ...
  if (data_ptr) MFREE(bus->dhd->osh, data_ptr, BTFW_DOWNLOAD_BLK_SIZE - 8);

  if (line) MFREE(bus->dhd->osh, line, BTFW_MAX_STR_LEN);
...
int dhd_bus_download_btfw(struct dhd_bus *bus, osl_t *osh, char *pbtfw_path) {
  int ret;
...
void dhd_bus_dump_trap_info(dhd_bus_t *bus, struct bcmstrbuf *strbuf) {
  ...
  bcm_bprintf(strbuf,
              ...
              ltoh32(tr->pc), ltoh32(bus->dongle_trap_addr), ltoh32(tr->r0),
              ltoh32(tr->r1), ltoh32(tr->r2), ltoh32(tr->r3), ltoh32(tr->r4),
              ltoh32(tr->r5), ltoh32(tr->r6), ltoh32(tr->r7));
}
...
static int dhd_bcmsdh_send_buffer(void *bus, uint8 *frame, uint16 len) {
  ...
  ret = dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(((dhd_bus_t *)bus)->sdh),
                            SDIO_FUNC_2, F2SYNC, frame, len, NULL, NULL, NULL,
                            TXRETRIES);

  if (ret == BCME_OK)
    ((dhd_bus_t *)bus)->tx_seq =
        (((dhd_bus_t *)bus)->tx_seq + 1) % SDPCM_SEQUENCE_WRAP;

  return ret;
}

/* Function to set the min res mask depending on the chip ID used */
bool dhd_bus_set_default_min_res_mask(struct dhd_bus *bus) {
  ...
  switch (bus->sih->chip) {
    case BCM4339_CHIP_ID:
      ...
      if (bcmsdh_regfail(bus->sdh)) {
        DHD_ERROR(
            ("%s:%d Setting min_res_mask failed\n", __FUNCTION__, __LINE__));
        return FALSE;
      }
      break;
    ...
    case BCM43014_CHIP_ID:
      bcmsdh_reg_write(
          bus->sdh,
          si_get_pmu_reg_addr(bus->sih, OFFSETOF(pmuregs_t, min_res_mask)), 4,
...
void dhd_bus_pmu_reg_reset(dhd_pub_t *dhdp) {
  struct dhd_bus *bus = dhdp->bus;
  bcmsdh_reg_write(
      bus->sdh, si_get_pmu_reg_addr(bus->sih, OFFSETOF(pmuregs_t, swscratch)),
      4, 0x0);
  if (bcmsdh_regfail(bus->sdh)) {
  ...
  if (bus) {
    ...
    if (sleep) {
      if (SLPAUTO_ENAB(bus)) {
        ...
        if (ret != BCME_BUSY) dhd_os_wd_timer(bus->dhd, 0);
      } else
        dhdsdio_clkctl(bus, CLK_NONE, FALSE);
    }
    dhd_os_sdunlock(dhdp);
