...
int wl_cfgvendor_send_supp_eventstring(const char *func_name, const char *fmt,
  ...
  prefix_len = snprintf(buf, SUPP_LOG_LEN,
                        "[DHD]<%s> %s: ", wl_get_kernel_timestamp(), __func__);
...
#ifdef WL_CFGVENDOR_CUST_ADVLOG
int wl_cfgvendor_send_supp_advlog(const char *fmt, ...) {
  char buf[SUPP_LOG_LEN] = {0};
  struct bcm_cfg80211 *cfg;
  struct wiphy *wiphy;
  va_list args;
  ...
  wiphy = cfg->wdev->wiphy;
  prefix_len = snprintf(buf, SUPP_LOG_LEN, "[%s]", wl_get_kernel_timestamp());
  /* Remaining buffer len */
  rem_len = SUPP_LOG_LEN - (prefix_len + 1);
  /* Print the arg list on to the remaining part of the buffer */
  va_start(args, fmt);
  ...
  return wl_cfgvendor_send_async_event(wiphy, bcmcfg_to_prmry_ndev(cfg),
                                       BRCM_VENDOR_EVENT_CONNECTIVITY_LOG, buf,
                                       len);
}
#endif /* WL_CFGVENDOR_CUST_ADVLOG */
#endif /* WL_SUPP_EVENT */
...
static int wl_cfgvendor_get_feature_set(struct wiphy *wiphy,
  ...
  if (unlikely(err)) WL_ERR(("Vendor Command reply failed ret:%d \n", err));
...
static int wl_cfgvendor_set_rand_mac_oui(struct wiphy *wiphy,
  ...
  if (type == ANDR_WIFI_ATTRIBUTE_RANDOM_MAC_OUI) {
    ...
    if (unlikely(err)) WL_ERR(("Bad OUI, could not set:%d \n", err));
  }
exit:
  return err;
}
...
static int wl_cfgvendor_gscan_get_batch_results(struct wiphy *wiphy,
  ...
  iter = results;
  complete_flag =
      nla_reserve(skb, GSCAN_ATTRIBUTE_SCAN_RESULTS_COMPLETE, sizeof(is_done));
  ...
  nla_for_each_nested(cur, prev, rem) {
    ...
    switch (type) {
      ...
      case GSCAN_ATTRIBUTE_BUCKET_PERIOD:
        ...
        ch_bucket[num].bucket_freq_multiple = nla_get_u32(cur) / MSEC_PER_SEC;
        break;
      ...
      case GSCAN_ATTRIBUTE_BUCKET_CHANNELS:
        nla_for_each_nested(next, cur, rem) {
          if (k >= GSCAN_MAX_CHANNELS_IN_BUCKET) break;
      ...
      case GSCAN_ATTRIBUTE_BUCKET_MAX_PERIOD:
        ...
        ch_bucket[num].bucket_max_multiple = nla_get_u32(cur) / MSEC_PER_SEC;
        break;
      default:
    ...
    switch (type) {
      ...
      case GSCAN_ATTRIBUTE_HOTLIST_BSSIDS:
        ...
        nla_for_each_nested(outer, iter, tmp) {
          if (j >= hotlist_params->nbssid) break;
      ...
      case GSCAN_ATTRIBUTE_EPNO_SSID_LIST:
        nla_for_each_nested(outer, iter, tmp) {
          ssid_elem = (dhd_pno_ssid_t *)dhd_dev_pno_get_gscan(
              bcmcfg_to_prmry_ndev(cfg), DHD_PNO_GET_NEW_EPNO_SSID_ELEM, NULL,
              &num);
          if (!ssid_elem) {
          ...
          nla_for_each_nested(inner, outer, tmp1) {
            ...
            switch (type) {
              case GSCAN_ATTRIBUTE_EPNO_SSID:
                memcpy(ssid_elem->SSID, nla_data(inner), DOT11_MAX_SSID_LEN);
                break;
              ...
              case GSCAN_ATTRIBUTE_EPNO_FLAGS:
                ...
                ssid_elem->hidden =
                    ((ssid_elem->flags & DHD_EPNO_HIDDEN_SSID) != 0);
                break;
...
exit:
  ...
  if (err < 0) {
    dhd_dev_pno_set_cfg_gscan(bcmcfg_to_prmry_ndev(cfg), DHD_PNO_EPNO_CFG_ID,
                              NULL, TRUE);
    dhd_dev_flush_fw_epno(bcmcfg_to_prmry_ndev(cfg));
  } else if (type != GSCAN_ATTRIBUTE_EPNO_FLUSH) {
    /* If the last attribute was FLUSH, nothing else to do */
    dhd_dev_pno_set_cfg_gscan(bcmcfg_to_prmry_ndev(cfg), DHD_PNO_EPNO_PARAMS_ID,
                              &params, FALSE);
...
static int wl_cfgvendor_get_wake_reason_stats(struct wiphy *wiphy,
  ...
  ret = nla_put_u32(skb, WAKE_STAT_ATTRIBUTE_CMD_EVENT_COUNT_USED,
                    rc_event_used_cnt);
#endif /* CUSTOM_WAKE_REASON_STATS */
  if (unlikely(ret)) {
  ...
  if (unlikely(ret)) {
    WL_ERR(
        ("Failed to put Total wake due to RX IPV4 MULTICAST, ret=%d\n", ret));
    goto exit;
  }
  ...
  if (unlikely(ret)) {
    WL_ERR(
        ("Failed to put Total wake due to RX IPV6 MULTICAST, ret=%d\n", ret));
    goto exit;
  }
  ...
  if (unlikely(ret)) {
    WL_ERR(
        ("Failed to put Total wake due to Other RX Multicast, ret=%d\n", ret));
    goto exit;
  }
  ...
  /* On cfg80211_vendor_cmd_reply() skb is consumed and freed in case of success
   * or failure */
  return ret;
...
static int wl_cfgvendor_set_hal_pid(struct wiphy *wiphy,
                                    struct wireless_dev *wdev, const void *data,
                                    int len) {
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
  int ret = BCME_OK;
...
static int wl_cfgvendor_set_hal_started(struct wiphy *wiphy,
  ...
  if (!data) {
    ...
    if (len > 0) {
      ...
      if (type == SET_HAL_START_ATTRIBUTE_PRE_INIT) {
        if (nla_len(data)) {
          WL_INFORM(
              ("%s, HAL version: %s\n", __FUNCTION__, (char *)nla_data(data)));
        }
  ...
  if ((!ETHER_IS_LOCALADDR(&primary_mac)) &&
    ...
    WL_DBG_MEM(
        ("%s, Local admin bit not set, randomize"
         "STA MAC address \n",
         __FUNCTION__));
...
static int wl_cfgvendor_set_latency_mode(struct wiphy *wiphy,
                                         struct wireless_dev *wdev,
                                         const void *data, int len) {
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (type) {
      case ANDR_WIFI_ATTRIBUTE_LATENCY_MODE:
        ...
        err = wldev_iovar_setint(wdev->netdev, "latency_critical_data", enable);
        if (err != BCME_OK) {
          WL_ERR(
              ("failed to set latency_critical_data "
               "enable %d, error = %d\n",
               enable, err));
...
bool wl_check_wdev(struct wireless_dev *wdev) {
  ...
  if (cfg->wdev->wiphy != wdev->wiphy) {
    WL_ERR(
        ("wiphy is not the same with wiphy "
         "cfg->wdev:%px wdev:%px "
...
void wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data) {
...
#else
    skb = cfg80211_vendor_event_alloc(wiphy, 100, GOOGLE_RTT_COMPLETE_EVENT,
                                      kflags);
#endif /* (defined(CONFIG_ARCH_MSM) && \
          defined(SUPPORT_WDEV_CFG80211_VENDOR_EVENT_ALLOC)) || */
    /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0) */
  ...
  list_for_each_entry(rtt_header, rtt_cache_list, list) {
    ...
    list_for_each_entry(rtt_result, &rtt_header->result_list, list) {
      ...
      ret = nla_put(skb, RTT_ATTRIBUTE_RESULT_DETAIL, rtt_result->detail_len,
                    &rtt_result->rtt_detail);
      if (ret < 0) {
...
static void wl_cfgvendor_rtt_sort_targets(rtt_config_params_t *rtt_param) {
  ...
  for (i = 0; i < (target_count - 1); i++) {
    ...
    if (i != min_index) {
      /* swap */
      rtt_tmp_target = rtt_target[i];
      rtt_target[i] = rtt_target[min_index];
      rtt_target[min_index] = rtt_tmp_target;
    }
  }
}
    ...
    switch (type) {
      ...
      case RTT_ATTRIBUTE_TARGET_INFO:
        ...
        nla_for_each_nested(iter1, iter, rem1) {
          if ((uint8 *)rtt_target >=
              ((uint8 *)rtt_param.target_info + TARGET_INFO_SIZE(target_cnt))) {
          ...
          nla_for_each_nested(iter2, iter1, rem2) {
            ...
            switch (type) {
              ...
              case RTT_ATTRIBUTE_TARGET_NUM_FTM_BURST:
                rtt_target->num_frames_per_burst = nla_get_u32(iter2);
                break;
              case RTT_ATTRIBUTE_TARGET_NUM_RETRY_FTM:
                rtt_target->num_retries_per_ftm = nla_get_u32(iter2);
                break;
              case RTT_ATTRIBUTE_TARGET_NUM_RETRY_FTMR:
                rtt_target->num_retries_per_ftmr = nla_get_u32(iter2);
              ...
              case RTT_ATTRIBUTE_TARGET_BURST_DURATION:
                if ((nla_get_u32(iter2) > 1 && nla_get_u32(iter2) < 12)) {
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (type) {
      ...
      case RTT_ATTRIBUTE_TARGET_MAC:
        if (mac_list == NULL) {
          WL_ERR(
              ("ATTRIBUTE_TARGET_CNT not found before "
...
static int wl_cfgvendor_enable_lazy_roam(struct wiphy *wiphy,
  ...
  if (type == GSCAN_ATTRIBUTE_LAZY_ROAM_ENABLE) {
    ...
    if (unlikely(err)) WL_ERR(("Could not enable lazy roam:%d \n", err));
  }
...
static int wl_cfgvendor_set_bssid_pref(struct wiphy *wiphy,
  ...
  nla_for_each_attr(iter, data, len, tmp2) {
    ...
    switch (type) {
      case GSCAN_ATTRIBUTE_NUM_BSSID:
        ...
        if ((bssid_pref = create_bssid_pref_cfg(cfg, num, &buf_len)) == NULL) {
          ...
          nla_for_each_nested(inner, outer, tmp1) {
            ...
            switch (type) {
              case GSCAN_ATTRIBUTE_BSSID_PREF:
                ...
                memcpy(&(bssids[bssid_pref->count].bssid), nla_data(inner),
                       ETHER_ADDR_LEN);
                /* not used for now */
  ...
  err = dhd_dev_set_lazy_roam_bssid_pref(bcmcfg_to_prmry_ndev(cfg), bssid_pref,
                                         flush);
exit:
  if (bssid_pref) {
  ...
  nla_for_each_attr(iter, data, len, tmp) {
    ...
    switch (type) {
      case GSCAN_ATTRIBUTE_NUM_WL_SSID:
        ...
        mem_needed = sizeof(wl_ssid_whitelist_t) + sizeof(wlc_ssid_t) * num;
        ssid_whitelist = (wl_ssid_whitelist_t *)MALLOCZ(cfg->osh, mem_needed);
        ...
        nla_for_each_nested(iter1, iter, tmp1) {
          ...
          switch (type) {
            ...
            case GSCAN_ATTRIBUTE_WHITELIST_SSID:
              ...
              memcpy(ssid_elem->SSID, nla_data(iter1), ssid_elem->SSID_len);
              ssid_found = 1;
              break;
          }
          if (ssid_found) {
...
typedef enum { FW_ROAMING_DISABLE, FW_ROAMING_ENABLE } fw_roaming_state_t;
...
static int wl_cfgvendor_set_fw_roaming_state(struct wiphy *wiphy,
  ...
  ROAMOFF_DBG_SAVE(wdev_to_ndev(wdev), SET_ROAM_VNDR_POLICY,
                   !cfg->disable_fw_roam);
...
int wl_cfgvendor_connect_params_handler(struct wiphy *wiphy,
                                        struct wireless_dev *wdev,
                                        const void *data, int len) {
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (attr_type) {
      ...
      case BRCM_ATTR_SAE_PWE: {
        ...
        ret = wl_cfg80211_set_wsec_info(net, &sae_pwe, sizeof(sae_pwe),
                                        WL_WSEC_INFO_BSS_SAE_PWE);
        if (unlikely(ret)) {
...
int wl_cfgvendor_start_ap_params_handler(struct wiphy *wiphy,
                                         struct wireless_dev *wdev,
                                         const void *data, int len) {
  struct net_device *net = wdev->netdev;
  int ret = BCME_OK;
...
static int wl_cfgvendor_set_td_policy(struct wiphy *wiphy,
                                      struct wireless_dev *wdev,
                                      const void *data, int len) {
  struct net_device *net = wdev->netdev;
  int ret = BCME_OK;
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (attr_type) {
      case BRCM_ATTR_TD_POLICY: {
        ...
        ret = wl_cfg80211_set_wsec_info(net, &td_policy, sizeof(td_policy),
                                        WL_WSEC_INFO_BSS_TD_POLICY);
        if (unlikely(ret)) {
...
nan_status_type_t wl_cfgvendor_brcm_to_nanhal_status(int32 vendor_status) {
  ...
  switch (vendor_status) {
    ...
    default:
      WL_ERR(
          ("%s Unknown vendor status, status = %d\n", __func__, vendor_status));
      /* Generic error */
...
static int wl_cfgvendor_nan_cmd_reply(struct wiphy *wiphy, int nan_cmd,
  ...
  err = wl_cfgvendor_send_cmd_reply(wiphy, nan_req_resp, sizeof(*nan_req_resp));
...
static int wl_cfgvendor_nan_parse_scid_params(struct bcm_cfg80211 *cfg,
                                              nan_str_data_t *scid,
                                              const struct nlattr *iter,
                                              int attr_type) {
  int ret = BCME_OK;
  ...
  switch (attr_type) {
    ...
    case NAN_ATTRIBUTE_SCID:
      ...
      ret = memcpy_s(scid->data, scid->dlen, nla_data(iter), nla_len(iter));
      if (ret != BCME_OK) {
...
int8 chanbuf[CHANSPEC_STR_LEN];
static int wl_cfgvendor_nan_parse_datapath_args(
    struct wiphy *wiphy, const void *buf, int len,
    nan_datapath_cmd_data_t *cmd_data) {
  int ret = BCME_OK;
  int attr_type;
  ...
  nla_for_each_attr(iter, buf, len, rem) {
    ...
    switch (attr_type) {
      ...
      case NAN_ATTRIBUTE_PEER_DISC_MAC_ADDR:
        ...
        ret = memcpy_s((char *)&cmd_data->peer_disc_mac_addr, ETHER_ADDR_LEN,
                       (char *)nla_data(iter), nla_len(iter));
        if (ret != BCME_OK) {
      ...
      case NAN_ATTRIBUTE_PEER_NDI_MAC_ADDR:
        ...
        ret = memcpy_s((char *)&cmd_data->peer_ndi_mac_addr, ETHER_ADDR_LEN,
                       (char *)nla_data(iter), nla_len(iter));
        if (ret != BCME_OK) {
      ...
      case NAN_ATTRIBUTE_SERVICE_NAME:
        ...
        cmd_data->svc_hash.data = MALLOCZ(cfg->osh, cmd_data->svc_hash.dlen);
        if (!cmd_data->svc_hash.data) {
      ...
      case NAN_ATTRIBUTE_KEY_DATA:
        if ((!cmd_data->key.dlen) || (nla_len(iter) != cmd_data->key.dlen)) {
        ...
        ret = memcpy_s(cmd_data->key.data, NAN_MAX_PMK_LEN, nla_data(iter),
                       nla_len(iter));
        if (ret != BCME_OK) {
      ...
      /* Fall through */
      case NAN_ATTRIBUTE_SCID:
        ret = wl_cfgvendor_nan_parse_scid_params(cfg, &cmd_data->scid, iter,
                                                 attr_type);
        if (ret != BCME_OK) {
...
static int wl_cfgvendor_nan_parse_discover_args(
    struct wiphy *wiphy, const void *buf, int len,
    nan_discover_cmd_data_t *cmd_data) {
  int ret = BCME_OK;
  int attr_type;
  ...
  nla_for_each_attr(iter, buf, len, rem) {
    ...
    switch (attr_type) {
      ...
      case NAN_ATTRIBUTE_RX_MATCH_FILTER:
        ...
        cmd_data->rx_match.data = MALLOCZ(cfg->osh, cmd_data->rx_match.dlen);
      ...
      case NAN_ATTRIBUTE_TX_MATCH_FILTER:
        ...
        cmd_data->tx_match.data = MALLOCZ(cfg->osh, cmd_data->tx_match.dlen);
      ...
      case NAN_ATTRIBUTE_MAC_ADDR_LIST:
        ...
        cmd_data->mac_list.list = MALLOCZ(
            cfg->osh, (cmd_data->mac_list.num_mac_addr * ETHER_ADDR_LEN));
      ...
      case NAN_ATTRIBUTE_SDEA_SERVICE_SPECIFIC_INFO:
        ...
        ret = memcpy_s(cmd_data->sde_svc_info.data, cmd_data->sde_svc_info.dlen,
                       nla_data(iter), nla_len(iter));
        if (ret != BCME_OK) {
  ...
  nla_for_each_attr(iter, buf, len, rem) {
    ...
    switch (attr_type) {
      ...
      case NAN_ATTRIBUTE_CHANNEL: {
        ...
        WL_TRACE(
            ("valid chanspec, chanspec = 0x%04x \n", cmd_data->chanspec[0]));
        break;
      }
      ...
      case NAN_ATTRIBUTE_2G_AWAKE_DW:
        ...
        if (nla_get_u32(iter)) {
          cmd_data->awake_dws.dw_interval_2g = 1 << (nla_get_u32(iter) - 1);
        }
      ...
      case NAN_ATTRIBUTE_5G_AWAKE_DW:
        ...
        if (nla_get_u32(iter)) {
          cmd_data->awake_dws.dw_interval_5g = 1 << (nla_get_u32(iter) - 1);
        }
...
exit:
  ...
  NAN_DBG_EXIT();
  if (ret) {
    WL_ERR(("%s: Failed to parse attribute %d ret %d", __FUNCTION__, attr_type,
            ret));
  }
  return ret;
}

static int wl_cfgvendor_nan_dp_estb_event_data_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
  ...
  if (nan_sched_info->num_channels > 0) {
    ...
    ret = nla_put_u32(msg, NAN_ATTRIBUTE_NUM_CHANNELS,
                      nan_sched_info->num_channels);
    if (unlikely(ret)) {
...
static int wl_cfgvendor_nan_dp_ind_event_data_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
  ...
  if (event_data->scid.dlen && event_data->scid.data) {
    ...
    ret = nla_put(msg, NAN_ATTRIBUTE_SCID, event_data->scid.dlen,
                  event_data->scid.data);
    if (unlikely(ret)) {
...
static int wl_cfgvendor_nan_tx_followup_ind_event_data_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
  ...
  if (event_data->status == NAN_STATUS_SUCCESS) {
    ...
    ret = nla_put(msg, NAN_ATTRIBUTE_REASON, strlen("NAN_STATUS_NO_OTA_ACK"),
                  event_data->nan_reason);
    if (unlikely(ret)) {
      WL_ERR(("Failed to put nan reason, ret=%d\n", ret));
      goto fail;
...
static int wl_cfgvendor_nan_svc_terminate_event_filler(
  ...
  if (event_id == GOOGLE_NAN_EVENT_SUBSCRIBE_TERMINATED) {
    ret =
        nla_put_u16(msg, NAN_ATTRIBUTE_SUBSCRIBE_ID, event_data->local_inst_id);
    if (unlikely(ret)) {
  ...
  if (event_data->status == NAN_STATUS_SUCCESS) {
    ...
    ret =
        nla_put(msg, NAN_ATTRIBUTE_REASON,
...
static int wl_cfgvendor_nan_opt_params_filler(struct sk_buff *msg,
  ...
  if (event_data->sde_svc_info.dlen && event_data->sde_svc_info.data) {
    ...
    ret = nla_put(msg, NAN_ATTRIBUTE_SDEA_SERVICE_SPECIFIC_INFO,
                  event_data->sde_svc_info.dlen, event_data->sde_svc_info.data);
    if (unlikely(ret)) {
...
static int wl_cfgvendor_nan_match_expiry_event_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
...
static int wl_cfgvendor_nan_tx_followup_event_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
...
static int wl_cfgvendor_nan_sub_match_event_filler(
    struct sk_buff *msg, nan_event_data_t *event_data) {
  int ret = BCME_OK;
...
#ifdef WL_NAN_INSTANT_MODE
  ret = nla_put_u8(msg, NAN_ATTRIBUTE_CIPHER_SUITE_TYPE,
                   event_data->peer_cipher_suite);
  if (unlikely(ret)) {
  ...
  if (event_data->publish_rssi) {
    ...
    ret =
        nla_put_u8(msg, NAN_ATTRIBUTE_RSSI_PROXIMITY, event_data->publish_rssi);
    if (unlikely(ret)) {
...
static int wl_cfgvendor_nan_de_event_filler(struct sk_buff *msg,
  ...
  ret =
      nla_put_u8(msg, NAN_ATTRIBUTE_DE_EVENT_TYPE, event_data->nan_de_evt_type);
  if (unlikely(ret)) {
...
s32 wl_cfgvendor_send_as_rtt_legacy_event(struct wiphy *wiphy,
  ...
  report->type = RTT_TWO_WAY;
     ...
     defined(SUPPORT_WDEV_CFG80211_VENDOR_EVENT_ALLOC)) || \
    LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0)
  msg = cfg80211_vendor_event_alloc(wiphy, NULL, 100, GOOGLE_RTT_COMPLETE_EVENT,
                                    kflags);
#else
  msg = cfg80211_vendor_event_alloc(wiphy, 100, GOOGLE_RTT_COMPLETE_EVENT,
                                    kflags);
#endif /* (defined(CONFIG_ARCH_MSM) && \
          defined(SUPPORT_WDEV_CFG80211_VENDOR_EVENT_ALLOC)) || */
  /* LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0) */
  ...
  return ret;
exit:
  if (msg) dev_kfree_skb_any(msg);
  WL_ERR(
      ("Failed to send event GOOGLE_RTT_COMPLETE_EVENT,"
       " -- Free skb, ret = %d\n",
       ret));
  if (report) MFREE(cfg->osh, report, sizeof(*report));
  NAN_DBG_EXIT();
  return ret;
}
#endif /* RTT_SUPPORT */
...
static int wl_cfgvendor_send_nan_async_resp(struct wiphy *wiphy,
  ...
  msg = CFG80211_VENDOR_EVENT_ALLOC(wiphy, wdev, buf_len, event_id, kflags);
  if (!msg) {
    WL_ERR(("%s: fail to allocate skb for vendor event\n", __FUNCTION__));
    return -ENOMEM;
  }
...
fail:
  ...
  WL_ERR(
      ("%s: Event not implemented or unknown -- Free skb, event_id = %d, ret = "
       "%d\n",
...
int wl_cfgvendor_nan_send_async_disable_resp(struct wireless_dev *wdev) {
  ...
  ret = wl_cfgvendor_send_nan_async_resp(
      wiphy, wdev, NAN_ASYNC_RESPONSE_DISABLED, (u8 *)&nan_req_resp,
      sizeof(nan_req_resp));
...
int wl_cfgvendor_send_nan_event(struct wiphy *wiphy, struct net_device *dev,
  ...
  /* Allocate the skb for vendor event */
  msg = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(dev), buf_len, event_id,
                                    kflags);
  if (!msg) {
    WL_ERR(("%s: fail to allocate skb for vendor event\n", __FUNCTION__));
    return -ENOMEM;
  ...
  switch (event_id) {
    ...
    case GOOGLE_NAN_EVENT_PUBLISH_TERMINATED: {
      ...
      ret = wl_cfgvendor_nan_svc_terminate_event_filler(msg, cfg, event_id,
                                                        event_data);
      if (unlikely(ret)) {
...
static int wl_cfgvendor_nan_req_publish(struct wiphy *wiphy,
  ...
  if (unlikely(ret) || unlikely(cmd_data->status)) {
    WL_ERR(
        ("failed to publish error[%d], status[%d]\n", ret, cmd_data->status));
...
exit:
  if (ret == BCME_OK) {
    nancfg->enab_reason = cmd_data->chre_req ? ENABLE_FOR_CHRE : ENABLE_FOR_APP;
    WL_INFORM_MEM(("Enabled successful for reason %d\n", nancfg->enab_reason));
  }

  ret = wl_cfgvendor_nan_cmd_reply(wiphy, NAN_WIFI_SUBCMD_ENABLE, &nan_req_resp,
  ...
  *ssn_exists = false;
  ...
  for (i = 0; i < NAN_MAX_NDP_PEER; i++) {
    if (nancfg->ndp_id[i]) {
      ...
      if ((ret == BCME_OK) && cfg->nancfg->nan_enable && dhdp->up) {
        *ssn_exists = true;
      }
    }
  }
...
static int wl_cfgvendor_nan_stop_handler(struct wiphy *wiphy,
  ...
  if (nancfg->nan_enable == false) {
    ...
    schedule_delayed_work(&nancfg->nan_disable, msecs_to_jiffies(delay_ms));
  }
exit:
...
exit:
  ret = wl_cfgvendor_nan_cmd_reply(wiphy, NAN_WIFI_SUBCMD_CONFIG, &nan_req_resp,
                                   ret, cmd_data ? cmd_data->status : BCME_OK);
  if (cmd_data) {
    if (cmd_data->scid.data) {
      MFREE(cfg->osh, cmd_data->scid.data, cmd_data->scid.dlen);
...
exit:
  ret =
      wl_cfgvendor_nan_cmd_reply(wiphy, NAN_WIFI_SUBCMD_TRANSMIT, &nan_req_resp,
                                 ret, cmd_data ? cmd_data->status : BCME_OK);
  wl_cfgvendor_free_disc_cmd_data(cfg, cmd_data);
  NAN_DBG_EXIT();
  return ret;
...
static int wl_cfgvendor_nan_data_path_iface_create(struct wiphy *wiphy,
  ...
  if (cfg->nancfg->nan_enable) { /* new framework Impl, iface create called
                                    after nan enab */
...
static int wl_cfgvendor_nan_enable_merge(struct wiphy *wiphy,
                                         struct wireless_dev *wdev,
                                         const void *data, int len) {
  int ret = 0;
  ...
  ret = wl_cfgnan_set_enable_merge(wdev->netdev, cfg, cmd_data->enable_merge,
                                   &status);
  if (unlikely(ret) || unlikely(status)) {
    WL_ERR(("Enable merge: failed to set config request  [%d]\n", ret));
    /* As there is no cmd_reply, return status if error is in status else return
     * ret */
    if (status) {
      ret = status;
    }
...
static int wl_cfgvendor_send_stats_info(struct wiphy *wiphy, const void *data,
                                        int len) {
  int ret = 0;
  struct sk_buff *skb;
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
...
static void fill_chanspec_to_channel_info(chanspec_t cur_chanspec,
  ...
  if (CHSPEC_IS160(cur_chanspec) || CHSPEC_IS8080(cur_chanspec)) {
    channel->center_freq0 = wl_channel_to_frequency(
        wf_chspec_primary80_channel(cur_chanspec), band);
    channel->center_freq1 = wl_channel_to_frequency(
        wf_chspec_secondary80_channel(cur_chanspec), band);
  } else {
    channel->center_freq0 =
        wl_channel_to_frequency(CHSPEC_CHANNEL(cur_chanspec), band);
    channel->center_freq1 = 0;
  }
}

#ifdef CONFIG_COMPAT
static s32 wl_cfgvendor_get_radio_stats_compat(struct bcm_cfg80211 *cfg,
                                               struct net_device *ndev,
                                               wifi_channel_stat *chan_stats,
                                               int num_channels, char **output,
                                               uint *total_len) {
  s32 err = 0;
  ...
  /* Radio stat field */
  radio_stats_size =
      cfg->num_radios * (offsetof(compat_wifi_radio_stat, channels)) +
      chan_stats_size;
  ...
  for (i = 0; i < cfg->num_radios; i++) {
    ...
    err = wldev_iovar_getbuf(ndev, "radiostat", &radio_req_v2,
                             sizeof(radio_req_v2), iovar_buf, sizeof(iovar_buf),
                             NULL);
    ...
    if ((err == BCME_OK) &&
      ...
      if (radio_v2->radio != WL_RADIOSTAT_SLICE_INDEX_SCAN) {
        radio_h_v2.on_time_scan = (uint32)(radio_v2->on_time_scan / 1000);
      } else {
      ...
      radio_h_v2.on_time_pno_scan = (uint32)(radio_v2->on_time_pno_scan / 1000);
      ...
      err = memcpy_s(out_radio_stat, avail_radio_stat_len, &radio_h_v2,
                     offsetof(compat_wifi_radio_stat, channels));
      if (err) {
...
#endif /* LINKSTAT_EXT_SUPPORT  */
      if (radio_v2->radio != WL_RADIOSTAT_SLICE_INDEX_SCAN) {
        radio_h.on_time_scan = (uint32)(radio_v2->on_time_scan / 1000);
      } else {
        radio_h.on_time_scan = 0;
      }
      radio_h.on_time_nbd = (uint32)(radio_v2->on_time_nbd / 1000);
      radio_h.on_time_gscan = (uint32)(radio_v2->on_time_gscan / 1000);
      radio_h.on_time_roam_scan = radio_v2->on_time_roam_scan;
      radio_h.on_time_pno_scan = (uint32)(radio_v2->on_time_pno_scan / 1000);
      ...
      err = memcpy_s(out_radio_stat, avail_radio_stat_len, &radio_h,
      ...
      /* Retry the VERSION_1 */
      err = wldev_iovar_getbuf(ndev, "radiostat", NULL, 0, iovar_buf,
      ...
      if (radio_v2->radio != WL_RADIOSTAT_SLICE_INDEX_SCAN) {
        radio_h_v2.on_time_scan = (uint32)(radio_v1->on_time_scan / 1000);
      } else {
        radio_h_v2.on_time_scan = 0;
      }
      radio_h_v2.on_time_nbd = (uint32)(radio_v1->on_time_nbd / 1000);
      radio_h_v2.on_time_gscan = (uint32)(radio_v1->on_time_gscan / 1000);
      radio_h_v2.on_time_roam_scan = radio_v1->on_time_roam_scan;
      radio_h_v2.on_time_pno_scan = (uint32)(radio_v1->on_time_pno_scan / 1000);
      ...
      if (radio_v1->radio != WL_RADIOSTAT_SLICE_INDEX_SCAN) {
        radio_h.on_time_scan = (uint32)(radio_v1->on_time_scan / 1000);
      } else {
        radio_h.on_time_scan = 0;
      }
      radio_h.on_time_nbd = (uint32)(radio_v1->on_time_nbd / 1000);
      radio_h.on_time_gscan = (uint32)(radio_v1->on_time_gscan / 1000);
      radio_h.on_time_roam_scan = radio_v1->on_time_roam_scan;
  ...
  /* Update all channels */
  err = memcpy_s(out_radio_stat, avail_radio_stat_len, chan_stats,
                 chan_stats_size);
  if (err) {
...
static s32 wl_cfgvendor_get_radio_stats(struct bcm_cfg80211 *cfg,
                                        struct net_device *ndev,
                                        wifi_channel_stat *chan_stats,
                                        int num_channels, char **output,
                                        uint *total_len) {
  ...
  if (is_compat_task())
#endif /* LINUX_VER >= 4.6 */
    return wl_cfgvendor_get_radio_stats_compat(cfg, ndev, chan_stats,
  ...
  /* Radio stat field */
  radio_stats_size =
      cfg->num_radios * (offsetof(wifi_radio_stat, channels)) + chan_stats_size;
  radio_stat_ptr = (char *)MALLOCZ(cfg->osh, radio_stats_size);
  if (radio_stat_ptr == NULL) {
    WL_ERR(("radio_stat_ptr alloc failed\n"));
    err = BCME_NOMEM;
  ...
  for (i = 0; i < cfg->num_radios; i++) {
    ...
    if ((err == BCME_OK) &&
      ...
      err = memcpy_s(out_radio_stat, avail_radio_stat_len, &radio_h_v2,
                     offsetof(wifi_radio_stat, channels));
      if (err) {
        WL_ERR(("failed to copy radio_stat_h : %d\n", err));
        goto exit;
      }
      ...
      radio_h.radio = radio_v1->radio;
      if (radio_v1->radio != WL_RADIOSTAT_SLICE_INDEX_SCAN) {
        radio_h.on_time = (uint32)(radio_v1->on_time_scan / 1000);
      } else {
        radio_h.on_time = 0;
      }
      radio_h.tx_time = radio_v1->tx_time;
...
static int wl_cfgvendor_lstats_get_info(struct wiphy *wiphy,
  ...
  cca_congest_ext_channel_req_v3_t *cca_v3_req = NULL;
  cca_congest_ext_channel_req_v2_t *cca_v2_req = NULL;
  uint cca_v3_req_size = sizeof(cca_congest_ext_channel_req_v3_t);
  ...
  err =
      wldev_iovar_getbuf(inet_ndev, "cca_get_stats_ext", all_chan_req,
                         all_chan_req_size, iovar_buf, WLC_IOCTL_MAXLEN, NULL);
  ...
  if (err != BCME_OK && err != BCME_UNSUPPORTED) {
    ...
    fill_chanspec_to_channel_info(cur_chanspec, &chan_stats->channel,
                                  &cur_band);
  }
  ...
  if ((err == BCME_OK) &&
    ...
    per_chspec_stats = (cca_congest_ext_channel_req_v2_t *)MALLOCZ(
        cfg->osh, per_chspec_stats_size);
    ...
    for (i = 0; i < num_channels; i++, all_chan_stats++) {
      ...
      all_chan_stats->on_time = per_chspec_stats[i].secs[0].radio_on_time;
      all_chan_stats->cca_busy_time = per_chspec_stats[i].secs[0].cca_busy_time;
  ...
  if (cca_v3_req == NULL) {
    err = BCME_NOMEM;
    WL_ERR(("cca_v3_req alloc failed\n"));
    goto exit;
  }
  ...
  err = wldev_iovar_getbuf(inet_ndev, "cca_get_stats_ext", cca_v3_req,
                           cca_v3_req_size, iovar_buf, WLC_IOCTL_MAXLEN, NULL);
  ...
  if ((err == BCME_OK) && (dtoh16(cca_ver) == WL_CCA_EXT_REQ_VER_V3)) {
    cca_v2_results =
  ...
  } else if ((err == BCME_OK) && (dtoh16(cca_ver) == WL_CCA_EXT_REQ_VER_V2)) {
    ...
    err =
        wldev_iovar_getbuf(inet_ndev, "cca_get_stats_ext", cca_v2_req,
    ...
    err = wldev_iovar_getbuf(inet_ndev, "cca_get_stats", cca_req, cca_req_size,
    ...
    /* If cca_get_stats is unsupported, cca_busy_time has zero value as initial
     * value */
    chan_stats->cca_busy_time = cca_busy_time;
  }
  WL_TRACE(
      ("chanspec : %x, BW : %d, Cur Band : %x, freq : %d, freq0 :%d, freq1 : "
       "%d\n",
       cur_chanspec, chan_stats->channel.width, cur_band,
       chan_stats->channel.center_freq, chan_stats->channel.center_freq0,
       chan_stats->channel.center_freq1));

  err = wl_cfgvendor_get_radio_stats(cfg, inet_ndev, chan_stats, num_channels,
                                     &output, &total_len);
  if (unlikely(err)) {
  ...
  err = wldev_iovar_getbuf(inet_ndev, "counters", NULL, 0, iovar_buf,
  ...
  if_infra_enh_stats = (wl_if_infra_enh_stats_v2_t *)MALLOCZ(
      cfg->osh, sizeof(wl_if_infra_enh_stats_v2_t));
  ...
  if (FW_SUPPORTED(dhdp, ifst)) {
    ...
    err = wldev_iovar_getbuf(inet_ndev, "if_counters", NULL, 0,
  ...
  if (!err) {
    ...
    if (dtoh16(if_stats->version) > WL_IF_STATS_T_VERSION_1) {
      WL_ERR(
          ("incorrect version of wl_if_stats_t,"
           " expected=%u got=%u\n",
...
#ifdef LINKSTAT_EXT_SUPPORT
  err = wldev_iovar_getbuf(inet_ndev, "bssload_report", NULL, 0, iovar_buf,
                           WLC_IOCTL_MAXLEN, NULL);

  if (err == BCME_OK) {
    bssload = (wl_bssload_t *)iovar_buf;
    COMPAT_ASSIGN_VALUE(iface, peer_info->bssload.sta_count,
                        bssload->sta_count);
    COMPAT_ASSIGN_VALUE(iface, peer_info->bssload.chan_util,
                        bssload->chan_util);
  } else if (err == BCME_UNSUPPORTED) {
    WL_INFORM(("bssload_report is unsupported \n"));
...
#endif /* LINKSTAT_EXT_SUPPORT */
  COMPAT_MEMCOPY_IFACE(output, total_len, wifi_iface_stat, iface,
                       wifi_rate_stat);

  err = wldev_iovar_getbuf(inet_ndev, "ratestat", NULL, 0, iovar_buf,
                           WLC_IOCTL_MAXLEN, NULL);
  if (err != BCME_OK && err != BCME_UNSUPPORTED) {
    WL_ERR(
  ...
  total_len = total_len - sizeof(wifi_peer_info) +
              NUM_PEER * (sizeof(wifi_peer_info_v1) - sizeof(wifi_rate_stat) +
...
exit:
  ...
  if (cca_v3_req) {
    MFREE(cfg->osh, cca_v3_req, cca_v3_req_size);
  }
#endif /* LINKSTAT_EXT_SUPPORT */
  if (outdata) {
...
static int wl_cfgvendor_get_buf_data(const struct nlattr *iter,
...
#endif /* LINUX_VER >= 4.6 */
  {
    struct compat_buf_data *compat_buf =
        (struct compat_buf_data *)nla_data(iter);
...
#endif /* CONFIG_COMPAT */
  {
    ...
    ret = memcpy_s(buf, sizeof(struct buf_data), (void *)nla_data(iter),
                   nla_len(iter));
    if (ret) {
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (type) {
      ...
      case DUMP_BUF_ATTR_TIMESTAMP:
        ret =
            dhd_print_time_str(buf->data_buf[0], NULL, (uint32)buf->len, &pos);
        break;
      ...
      case DUMP_BUF_ATTR_EWP_HW_INIT_LOG:
        ret = dhd_print_init_dump_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                       buf->data_buf[0], NULL, (uint32)buf->len,
                                       &pos, LOG_DUMP_SECTION_EWP_HW_INIT_LOG);
        break;
      case DUMP_BUF_ATTR_EWP_HW_MOD_DUMP:
        ret = dhd_print_init_dump_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                       buf->data_buf[0], NULL, (uint32)buf->len,
                                       &pos, LOG_DUMP_SECTION_EWP_HW_MOD_DUMP);
        break;
      case DUMP_BUF_ATTR_EWP_HW_REG_DUMP:
        ret = dhd_print_init_dump_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                       buf->data_buf[0], NULL, (uint32)buf->len,
                                       &pos, LOG_DUMP_SECTION_EWP_HW_REG_DUMP);
        break;
#endif /* EWP_DACS */
      ...
      case DUMP_BUF_ATTR_RTT_LOG:
        ret =
            dhd_print_rtt_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                               buf->data_buf[0], NULL, (uint32)buf->len, &pos);
        break;
#endif /* EWP_RTT_LOGGING */
      case DUMP_BUF_ATTR_DHD_DUMP:
        ret =
            dhd_print_dump_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                buf->data_buf[0], NULL, (uint32)buf->len, &pos);
        break;
#if defined(BCMPCIE)
      ...
      case DUMP_BUF_ATTR_PKTID_MAP_LOG:
        ret = dhd_print_pktid_map_log_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                           buf->data_buf[0], NULL,
                                           (uint32)buf->len, &pos, TRUE);
        break;

      case DUMP_BUF_ATTR_PKTID_UNMAP_LOG:
        ret = dhd_print_pktid_map_log_data(bcmcfg_to_prmry_ndev(cfg), NULL,
                                           buf->data_buf[0], NULL,
                                           (uint32)buf->len, &pos, FALSE);
        break;
      ...
      case DUMP_BUF_ATTR_SSSR_C0_D11_BEFORE:
        ret = dhd_sssr_dump_d11_buf_before(
            bcmcfg_to_prmry_ndev(cfg), buf->data_buf[0], (uint32)buf->len, 0);
        break;
      ...
      case DUMP_BUF_ATTR_SSSR_C1_D11_BEFORE:
        ret = dhd_sssr_dump_d11_buf_before(
            bcmcfg_to_prmry_ndev(cfg), buf->data_buf[0], (uint32)buf->len, 1);
        break;
#endif /* DHD_SSSR_DUMP_BEFORE_SR */
      ...
      case DUMP_BUF_ATTR_SSSR_C2_D11_BEFORE:
        ret = dhd_sssr_dump_d11_buf_before(
            bcmcfg_to_prmry_ndev(cfg), buf->data_buf[0], (uint32)buf->len, 2);
        break;
#endif /* DHD_SSSR_DUMP_BEFORE_SR */
...
static int wl_cfgvendor_dbg_start_logging(struct wiphy *wiphy,
  ...
  ret = dhd_os_start_logging(dhd_pub, ring_name, log_level, flags, time_intval,
                             threshold);
  if (ret < 0) {
...
static int wl_cfgvendor_dbg_get_ring_data(struct wiphy *wiphy,
  ...
  if (ring_id == DEBUG_DUMP_RING1_ID) {
    ...
    WL_MEM(
        ("Doing dump_start op for ring_id %d ring:%s\n", ring_id, ring_name));
...
static int wl_cfgvendor_dbg_get_buf_ring_map(struct wiphy *wiphy,
  ...
  /* Alloc the SKB for vendor_event */
  skb = cfg80211_vendor_cmd_alloc_reply_skb(
      wiphy, nla_total_size(sizeof(dhd_buf_ring_map)) +
                 nla_total_size(sizeof(map_cnt)));
  if (!skb) {
    WL_ERR(("skb allocation is failed\n"));
    ret = BCME_NOMEM;
    goto fail;
...
#ifdef DEBUGABILITY
static void wl_cfgvendor_dbg_ring_send_evt(
    void *ctx, const int ring_id, const void *data, const uint32 len,
  ...
  /* Alloc the SKB for vendor_event */
    ...
    LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0)
  skb =
      cfg80211_vendor_event_alloc(wiphy, NULL, len + CFG80211_VENDOR_EVT_SKB_SZ,
...
static int wl_cfgvendor_nla_put_debug_dump_data(struct sk_buff *skb,
  ...
  for (i = LOG_DUMP_SECTION_EWP_HW_INIT_LOG;
       i <= LOG_DUMP_SECTION_EWP_HW_REG_DUMP; ++i) {
...
static int wl_cfgvendor_nla_put_pktlogdump_data(struct sk_buff *skb,
  ...
  if (pktlogdbg) {
    ret = nla_put_string(skb, DUMP_FILENAME_ATTR_PKTLOG_DEBUG_DUMP,
                         pktlogdump_path);
    if (ret) {
      WL_ERR(("Failed to put filename\n"));
      return ret;
 ...
 */
#ifdef DHD_SDTC_ETB_DUMP
static int wl_cfgvendor_nla_put_sdtc_etb_dump_data(struct sk_buff *skb,
                                                   struct net_device *ndev) {
  dhd_pub_t *dhdp = wl_cfg80211_get_dhdp(ndev);
  char memdump_path[MEMDUMP_PATH_LEN];
  int ret = BCME_OK;
  ...
  ret =
      nla_put_u32(skb, DUMP_LEN_ATTR_SDTC_ETB_DUMP, DHD_SDTC_ETB_MEMPOOL_SIZE);
  if (unlikely(ret)) {
...
static int wl_cfgvendor_nla_put_dump_data(dhd_pub_t *dhd_pub,
                                          struct sk_buff *skb,
                                          struct net_device *ndev,
                                          const uint32 fw_len) {
  int ret = BCME_OK;
...
#endif /* DHD_PKT_LOGGING */
  {
    if ((ret = wl_cfgvendor_nla_put_dump_data(dhd_pub, skb, ndev, fw_len)) <
        0) {
      WL_ERR(("nla put failed\n"));
      goto done;
    }
  }
  /* TODO : Similar to above function add for debug_dump, sssr_dump, and pktlog
   * also. */
  cfg80211_vendor_event(skb, kflags);
  return;
done:
...
static int wl_cfgvendor_dbg_get_version(struct wiphy *wiphy,
  ...
  ret =
      dhd_os_get_version(bcmcfg_to_prmry_ndev(cfg), dhd_ver, &buf_ptr, buf_len);
  if (ret < 0) {
...
static int __wl_cfgvendor_dbg_get_pkt_fates(
  ...
  if (!req_count || !user_buf) {
    WL_ERR(
        ("invalid request, user_buf=%p, req_count=%u\n", user_buf, req_count));
    ret = -EINVAL;
    goto exit;
  }
...
static int wl_cfgvendor_start_mkeep_alive(struct wiphy *wiphy,
  ...
  ret = wl_cfg80211_start_mkeep_alive(ndev, cfg, mkeep_alive_id, ether_type,
...
static int wl_cfgvendor_apf_read_filter_data(struct wiphy *wiphy,
                                             struct wireless_dev *wdev,
                                             const void *data, int len) {
  struct net_device *ndev = wdev_to_ndev(wdev);
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
  ...
  /* As the IOVar returns the data in 'wl_apf_program_t' structure format,
   * account for this also along with the queried APF buffer size for the IOvar
   * buffer.
   */
  ...
  /* Skip the initial fixed portion of 'wl_apf_program_t' and copy only the data
   * portion */
  ret = nla_put(skb, APF_ATTRIBUTE_PROGRAM, max_len,
                (buf + WL_APF_PROGRAM_FIXED_LEN));
  if (ret < 0) {
    WL_ERR(("Failed to put APF_ATTRIBUTE_MAX_LEN, ret=%d\n", ret));
    goto fail;
  }
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (type) {
      case BRCM_ATTR_DRIVER_KEY_PMK:
        ...
        ret = memcpy_s(pmk.key, sizeof(pmk.key), (uint8 *)nla_data(iter),
                       nla_len(iter));
        if (ret) {
...
#endif /* !BCMSUP_4WAY_HANDSHAKE || LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, \
          0) */
...
#ifdef WL_P2P_RAND
static int wl_cfgvendor_set_p2p_rand_mac(struct wiphy *wiphy,
                                         struct wireless_dev *wdev,
                                         const void *data, int len) {
  int err = 0;
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
  ...
  WL_INFORM_MEM(
      ("randomized p2p_dev_addr - " MACDBG "\n", MAC2STRDBG(nla_data(data))));
  ...
  if (type == BRCM_ATTR_DRIVER_RAND_MAC) {
    ...
    (void)memcpy_s(wl_to_p2p_bss_macaddr(cfg, P2PAPI_BSSCFG_DEVICE),
                   ETHER_ADDR_LEN, nla_data(data), ETHER_ADDR_LEN);
    (void)memcpy_s(wdev->address, ETHER_ADDR_LEN, nla_data(data),
                   ETHER_ADDR_LEN);
...
static int wl_cfgvendor_thermal_mitigation(struct wiphy *wiphy,
                                           struct wireless_dev *wdev,
                                           const void *data, int len) {
  int err = BCME_ERROR, rem, type;
...
static int wl_cfgvendor_tx_power_scenario(struct wiphy *wiphy,
                                          struct wireless_dev *wdev,
                                          const void *data, int len) {
  int err = BCME_ERROR, rem, type;
  ...
  if (cfg->wifi_tx_power_mode == wifi_tx_power_mode) {
    WL_INFORM_MEM(
        ("SAR: tx_power_mode %d is already set\n", wifi_tx_power_mode));
    err = BCME_OK;
    goto exit;
  }
...
#endif /* WL_SAR_TX_POWER_CONFIG */
  {
    ...
    switch (wifi_tx_power_mode) {
      ...
      default:
        WL_ERR(
            ("SAR: invalid wifi tx power scenario = %d\n", sar_tx_power_val));
        err = -EINVAL;
        goto exit;
    }
  }
  WL_DBG_MEM(
      ("SAR: sar_mode %d airplane_mode %d\n", sar_tx_power_val, airplane_mode));
#ifdef SYNA_SAR_CUSTOMER_PARAMETER
  if (sar_tx_power_val) {
    struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
    dhd_pub_t *dhd = (dhd_pub_t *)(cfg->pub);
    err = dhd_sar_set_parameter(dhd, sar_tx_power_val);
    if (unlikely(err)) {
      WL_ERR(("SAR: Failed to set sar_params - error (%d)\n", err));
      goto exit;
    }
    /* as already set sar_params for sar_enable 1 */
    sar_tx_power_val = 1;
  }
...
enum andr_user_ac { BEST_EFFORT = 0, BACKGROUND = 1, VIDEO = 2, VOICE = 3 };
...
static int wl_set_dscp_default_priority(uint8 *table) {
  int err = BCME_ERROR;
...
static int wl_cfgvendor_custom_mapping_of_dscp(struct wiphy *wiphy,
                                               struct wireless_dev *wdev,
                                               const void *data, int len) {
  int err = BCME_OK, rem, type;
  const struct nlattr *iter;
  ...
  for (dscp = dscp_start; dscp <= dscp_end; dscp++) {
    ...
    if ((def_dscp_pri != 0xff) && (def_dscp_pri != priority)) {
      ...
      WL_ERR(("Request priority is %d from %d to %d.\n", priority, dscp_start,
              dscp_end));
      WL_ERR(
          ("But conflict with defalut priority of DSCP %d and priority %d.\n",
           dscp, def_dscp_pri));
      goto exit;
    }
  }
...
static int wl_cfgvendor_custom_mapping_of_dscp_reset(struct wiphy *wiphy,
                                                     struct wireless_dev *wdev,
                                                     const void *data,
                                                     int len) {
  struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
...
int wl_cfgvendor_multista_set_primary_connection(struct wiphy *wiphy,
                                                 struct wireless_dev *wdev,
                                                 const void *data, int len) {
  int err = BCME_OK;
  struct bcm_cfg80211 *cfg = NULL;
...
static int wl_cfgvendor_multista_set_use_case(struct wiphy *wiphy,
                                              struct wireless_dev *wdev,
                                              const void *data, int len) {
  int err = BCME_OK;
  int rem, type;
...
static int wl_cfgvendor_twt_setup(struct wiphy *wiphy,
                                  struct wireless_dev *wdev, const void *data,
                                  int len) {
  wl_twt_config_t val;
  s32 bw;
  ...
  bw = bcm_pack_xtlv_entry(&rem, &rem_len, WL_TWT_CMD_CONFIG, sizeof(val),
  ...
  bw = wldev_iovar_setbuf(wdev_to_ndev(wdev), "twt", mybuf,
                          sizeof(mybuf) - rem_len, resp_buf, WLC_IOCTL_SMLEN,
                          NULL);
  if (bw < 0) {
    WL_ERR(("twt config set failed. ret:%d\n", bw));
  } else {
    WL_INFORM(
        ("twt config setup succeeded, config ID %d "
         "Negotiation type %d flow flags %d\n",
         val.desc.configID, val.desc.negotiation_type, val.desc.flow_flags));
  }
...
static int wl_cfgvendor_twt_teardown(struct wiphy *wiphy,
                                     struct wireless_dev *wdev,
                                     const void *data, int len) {
  ...
  bw = wldev_iovar_setbuf(wdev_to_ndev(wdev), "twt", mybuf,
                          sizeof(mybuf) - rem_len, res_buf, WLC_IOCTL_SMLEN,
                          NULL);
  if (bw < 0) {
    WL_ERR(("twt teardown failed. ret:%d\n", bw));
  } else {
    WL_INFORM(
        ("twt teardown succeeded, config ID %d "
         "Negotiation type %d alltwt %d\n",
         val.configID, val.teardesc.negotiation_type, val.teardesc.alltwt));
  }
...
static int wl_cfgvendor_twt_info_frame(struct wiphy *wiphy,
                                       struct wireless_dev *wdev,
                                       const void *data, int len) {
  wl_twt_info_t val;
  int bw;
  ...
  bw = bcm_pack_xtlv_entry(&rem, &rem_len, WL_TWT_CMD_INFO, sizeof(val),
                           (uint8 *)&val, BCM_XTLV_OPTION_ALIGN32);
  if (bw != BCME_OK) {
    goto exit;
  }
...
static int wl_cfgvendor_twt_stats_update_v2(struct wiphy *wiphy,
                                            wl_twt_stats_v2_t *stats) {
  u32 i;
  ...
  for (i = 0; i < stats->num_stats; i++) {
    ...
    WL_INFORM_MEM(("%u %u %u %u %u", peer_stats->eosp_dur_avg,
    ...
    ret =
        nla_put_u32(skb, ANDR_TWT_ATTR_AVG_PKT_NUM_TX, peer_stats->tx_pkts_avg);
    if (ret < 0) {
      WL_ERR(("Failed to put ANDR_TWT_ATTR_AVG_PKT_NUM_TX, ret:%d\n", ret));
      goto fail;
    }
    ret = nla_put_u32(skb, ANDR_TWT_ATTR_AVG_PKT_SIZE_TX,
                      peer_stats->tx_pkt_sz_avg);
    if (ret < 0) {
      WL_ERR(("Failed to put ANDR_TWT_ATTR_AVG_PKT_SIZE_TX, ret:%d\n", ret));
      goto fail;
    }
    ret =
        nla_put_u32(skb, ANDR_TWT_ATTR_AVG_PKT_NUM_RX, peer_stats->rx_pkts_avg);
    if (ret < 0) {
      WL_ERR(("Failed to put ANDR_TWT_ATTR_AVG_PKT_NUM_RX, ret:%d\n", ret));
      goto fail;
    }
    ret = nla_put_u32(skb, ANDR_TWT_ATTR_AVG_PKT_SIZE_RX,
                      peer_stats->rx_pkt_sz_avg);
    if (ret < 0) {
      WL_ERR(("Failed to put ANDR_TWT_ATTR_AVG_PKT_SIZE_RX, ret:%d\n", ret));
      goto fail;
    }
    ret =
        nla_put_u32(skb, ANDR_TWT_ATTR_AVG_EOSP_DUR, peer_stats->eosp_dur_avg);
    if (ret < 0) {
...
static int wl_cfgvendor_send_twt_status_map_v1(struct wiphy *wiphy,
                                               uint8 config_id,
  ...
  for (i = 0; i < WL_TWT_MAX_ITWT; i++) {
    if ((status->itwt_status[i].configID != WL_TWT_INV_CONFIG_ID) &&
      ...
      WL_DBG_MEM(
          ("response o/p for config_id %u: setup_cmd: %d, flow_flags: %u,"
           " flow_id: %u, channel: %u, wake_dur: %u,"
           " wake_int: %u, neg_type: %d",
           status->itwt_status[i].configID, (int)sdesc->setup_cmd,
           sdesc->flow_flags, (int)sdesc->flow_id, (int)sdesc->channel,
           sdesc->wake_dur, sdesc->wake_int, sdesc->negotiation_type));

      ret =
          nla_put_u8(skb, ANDR_TWT_ATTRIBUTE_SETUP_CMD, (int)sdesc->setup_cmd);
      if (ret < 0) {
...
static int wl_cfgvendor_twt_stats(struct wiphy *wiphy,
                                  struct wireless_dev *wdev, const void *data,
                                  int len, bool clear_stats) {
  ...
  pxtlv = (uint8 *)iovbuf;
  ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_TWT_CMD_STATS, sizeof(query),
                            (uint8 *)&query, BCM_XTLV_OPTION_ALIGN32);
  if (ret != BCME_OK) {
    WL_ERR(("%s : Error return during pack xtlv :%d\n", __FUNCTION__, ret));
    goto exit;
  }

  if ((ret = wldev_iovar_getbuf(wdev_to_ndev(wdev), "twt", iovbuf,
                                bufstart - buflen, iovresp, WLC_IOCTL_MEDLEN,
                                NULL))) {
  ...
  if (dtoh16(stats_v2.version) == WL_TWT_STATS_VERSION_2) {
    if (!clear_stats) {
      ...
      ret =
          wl_cfgvendor_twt_stats_update_v2(wiphy, (wl_twt_stats_v2_t *)iovresp);
    }
  } else {
    ret = BCME_UNSUPPORTED;
...
static int wl_cfgvendor_twt_get_response(struct wiphy *wiphy,
                                         struct wireless_dev *wdev,
                                         const void *data, int len) {
  int ret = BCME_OK;
  char iovbuf[WLC_IOCTL_SMLEN] = {
  ...
  status_cmd.length =
      sizeof(status_cmd) - OFFSETOF(wl_twt_status_cmd_v1_t, peer);
  ...
  ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_TWT_CMD_STATUS,
                            sizeof(status_cmd), (uint8 *)&status_cmd,
  ...
  if (dtoh16(result.version) == WL_TWT_CMD_STATUS_VERSION_1) {
    ret = wl_cfgvendor_send_twt_status_map_v1(wiphy, status_cmd.configID,
                                              &result);
    if (ret != BCME_OK) {
...
static int wl_cfgvendor_twt_get_stats(struct wiphy *wiphy,
                                      struct wireless_dev *wdev,
                                      const void *data, int len) {
  return wl_cfgvendor_twt_stats(wiphy, wdev, data, len, false);
}

static int wl_cfgvendor_twt_clear_stats(struct wiphy *wiphy,
                                        struct wireless_dev *wdev,
                                        const void *data, int len) {
  return wl_cfgvendor_twt_stats(wiphy, wdev, data, len, true);
}

static int wl_cfgvendor_twt_update_cap(struct wiphy *wiphy,
                                       wl_twt_cap_t *result) {
  struct sk_buff *skb;
  int32 mem_needed;
  int ret = BCME_OK;
...
static int wl_cfgvendor_twt_cap(struct wiphy *wiphy, struct wireless_dev *wdev,
                                const void *data, int len) {
  int ret = BCME_OK;
  char iovbuf[WLC_IOCTL_SMLEN] = {
      0,
  ...
  ret = bcm_pack_xtlv_entry(&pxtlv, &buflen, WL_TWT_CMD_CAP, sizeof(cmd_cap),
                            (uint8 *)&cmd_cap, BCM_XTLV_OPTION_ALIGN32);
  if (ret != BCME_OK) {
    WL_ERR(("%s : Error return during pack xtlv :%d\n", __FUNCTION__, ret));
    goto exit;
  }
...
static int wl_cfgvendor_twt_update_setup_response(struct sk_buff *skb,
                                                  void *event_data) {
  s32 err = BCME_OK;
  ...
  err =
      nla_put_u8(skb, ANDR_TWT_ATTR_NEGOTIATION_TYPE, sdesc->negotiation_type);
  if (unlikely(err)) {
...
static int wl_cfgvendor_twt_update_teardown_response(struct sk_buff *skb,
                                                     void *event_data) {
  s32 err = BCME_OK;
...
static int wl_cfgvendor_twt_update_infoframe_response(struct sk_buff *skb,
                                                      void *event_data) {
  s32 err = BCME_OK;
...
static int wl_cfgvendor_twt_update_notify_response(struct sk_buff *skb,
                                                   void *event_data) {
  s32 err = BCME_OK;
  ...
  err =
      nla_put_u8(skb, ANDR_TWT_ATTR_TWT_NOTIFICATION, notif_cplt->notification);
  if (unlikely(err)) {
...
s32 wl_cfgvendor_notify_twt_event(struct bcm_cfg80211 *cfg,
  ...
  skb = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(ndev),
                                    BRCM_TWT_HAL_VENDOR_EVENT_BUF_LEN,
  ...
  switch (twt_event->event_type) {
    case WL_TWT_EVENT_SETUP:
      err = wl_cfgvendor_twt_update_setup_response(
          skb, (void *)twt_event->event_info);
      break;
    case WL_TWT_EVENT_TEARDOWN:
      err = wl_cfgvendor_twt_update_teardown_response(
          skb, (void *)twt_event->event_info);
      break;
    case WL_TWT_EVENT_INFOFRM:
      err = wl_cfgvendor_twt_update_infoframe_response(
          skb, (void *)twt_event->event_info);
      break;
    case WL_TWT_EVENT_NOTIFY:
      err = wl_cfgvendor_twt_update_notify_response(
          skb, (void *)twt_event->event_info);
      break;
    default:
  ...
  WL_INFORM(
      ("Successfully sent TWT vendor event type %d\n", twt_event->event_type));
  return BCME_OK;
...
static void wl_set_ota_nvram_ext(dhd_pub_t *dhd) {
...
static int wl_cfgvendor_get_ota_current_info(struct wiphy *wiphy,
                                             struct wireless_dev *wdev,
                                             const void *data, int len) {
  int ret, mem_needed;
  struct sk_buff *skb = NULL;
...
static int wl_cfgvendor_ota_download(struct wiphy *wiphy,
                                     struct wireless_dev *wdev,
                                     const void *data, int len) {
  int err = BCME_OK;
  int rem, type;
  ...
  nla_for_each_attr(iter, data, len, rem) {
    ...
    switch (type) {
      ...
      case OTA_DOWNLOAD_CLM_ATTR:
        ...
        memcpy_s(buf, sizeof(*buf), (void *)nla_data(iter), nla_len(iter));
        ...
        err = copy_from_user(ota_info->clm_buf, buf[0], ota_info->clm_len);
        if (err) {
      ...
      case OTA_DOWNLOAD_NVRAM_ATTR:
        ...
        err = copy_from_user(ota_info->nvram_buf, buf[0], ota_info->nvram_len);
        if (err) {
...
#ifdef OEM_ANDROID
static int wl_cfgvendor_set_dtim_config(struct wiphy *wiphy,
                                        struct wireless_dev *wdev,
                                        const void *data, int len) {
  int err = BCME_OK, rem, type;
  const struct nlattr *iter;
...
static int wl_cfgvendor_get_usable_channels(struct wiphy *wiphy,
  ...
  WL_INFORM_MEM(
      ("usable channel param band:%u iface:%u filter:%u max_size:%u\n",
       u_info.band_mask, u_info.iface_mode_mask, u_info.filter_mask,
       u_info.max_size));

  if (u_info.max_size == 0 || u_info.band_mask == 0 ||
      u_info.iface_mode_mask == 0) {
  ...
  if (u_info.max_size > USABLE_CHAN_MAX_SIZE) {
    WL_ERR(
        ("Too big max_size band:%u iface:%u filter:%u max_size:%u limit:%u\n",
         u_info.band_mask, u_info.iface_mode_mask, u_info.filter_mask,
         u_info.max_size, USABLE_CHAN_MAX_SIZE));
    return -EINVAL;
  }

  u_info.channels =
      MALLOCZ(cfg->osh, sizeof(*u_info.channels) * u_info.max_size);
  if (!u_info.channels) {
  ...
  for (i = 0; i < u_info.size; i++) {
    memcpy_s(uchan_data + off, uchan_data_len - off, &u_info.channels[i],
             uchan_item_size);
    off += uchan_item_size;
  }
  ...
  skb = cfg80211_vendor_cmd_alloc_reply_skb(
      wiphy,
      nla_total_size(uchan_data_len) + nla_total_size(sizeof(u_info.size)));
  if (!skb) {
    WL_ERR(("skb allocation is failed\n"));
    ret = -ENOMEM;
    goto exit;
...
exit:
  ...
  if (u_info.channels) {
    MFREE(cfg->osh, u_info.channels,
          sizeof(*u_info.channels) * u_info.max_size);
  }
...
#if defined(WLAN_ACCEL_BOOT)
static int wl_cfgvendor_trigger_ssr(struct wiphy *wiphy,
                                    struct wireless_dev *wdev, const void *data,
                                    int len) {
...
const struct nla_policy
    wifi_tx_power_limits_attr_policy[TX_POWER_ATTRIBUTE_MAX] = {
        [TX_POWER_CAP_ENABLE_ATTRIBUTE] = {.type = NLA_U8},
};

int wl_cfgvendor_set_tx_power_policy_handler(struct wiphy *wiphy,
                                             struct wireless_dev *wdev,
                                             const void *data, int len) {
  int ret = BCME_OK;
  int attr_type;
  ...
  ret = wldev_iovar_setint(wdev_to_ndev(wdev), "phy_peak_curr_txpwrcap",
                           pwr_lmt_enab);
  if (unlikely(ret)) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0))
const struct nla_policy
    wifi_radio_combo_attr_policy[ANDR_WIFI_ATTRIBUTE_RADIO_COMBO_MAX] = {
...
wifi_radio_configuration radio25_2x2[] = {
    {WLAN_MAC_2_4_BAND, WIFI_ANTENNA_2X2},
    {WLAN_MAC_5_0_BAND, WIFI_ANTENNA_2X2}};
wifi_radio_configuration radio26_2x2[] = {
    {WLAN_MAC_2_4_BAND, WIFI_ANTENNA_2X2},
    {WLAN_MAC_6_0_BAND, WIFI_ANTENNA_2X2}};

static int wl_cfgvendor_get_radio_combo_matrix(struct wiphy *wiphy,
                                               struct wireless_dev *wdev,
                                               const void *data, int len) {
  struct net_device *ndev = wdev_to_ndev(wdev);
  struct sk_buff *skb = NULL;
  ...
  if (sizeof(buf) < MAX_RADIO_MATRIX_SIZE) {
    WL_ERR(("Buff too short: %zd, exp max_radio matrix size: %zd\n",
  ...
  (void)(memcpy_s(radio_combinations->radio_configurations, sizeof(radio2_2x2),
                  (void *)radio2_2x2, sizeof(radio2_2x2)));
  ...
  radio_combinations->num_radio_combinations = 1;
  (void)(memcpy_s(radio_combinations->radio_configurations, sizeof(radio5_2x2),
                  (void *)radio5_2x2, sizeof(radio5_2x2)));
...
const struct nla_policy brcm_drv_attr_policy[BRCM_ATTR_DRIVER_MAX] = {
    ...
    [BRCM_ATTR_DRIVER_KEY_PMK] = {.type = NLA_BINARY,
                                  .len = WSEC_MAX_PASSPHRASE_LEN},
    [BRCM_ATTR_DRIVER_FEATURE_FLAGS] =
        {.type = NLA_BINARY, .len = ((BRCM_WLAN_VENDOR_FEATURES_MAX / 8) + 1)},
...
const struct nla_policy nan_attr_policy[NAN_ATTRIBUTE_MAX] = {
    ...
    [NAN_ATTRIBUTE_SYNC_DISC_2G_BEACON] = {.type = NLA_U8,
                                           .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SYNC_DISC_5G_BEACON] = {.type = NLA_U8,
                                           .len = sizeof(uint8)},
    ...
    [NAN_ATTRIBUTE_SERVICE_SPECIFIC_INFO_LEN] = {.type = NLA_U16,
                                                 .len = sizeof(uint16)},
    [NAN_ATTRIBUTE_SERVICE_SPECIFIC_INFO] =
        {.type = NLA_BINARY, .len = NAN_MAX_SERVICE_SPECIFIC_INFO_LEN},
    ...
    [NAN_ATTRIBUTE_SERVICE_NAME] = {.type = NLA_BINARY,
                                    .len = WL_NAN_SVC_HASH_LEN},
    ...
    [NAN_ATTRIBUTE_SSIREQUIREDFORMATCHINDICATION] = {.type = NLA_U8,
                                                     .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SUBSCRIBE_MATCH] = {.type = NLA_U8, .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_PUBLISH_MATCH] = {.type = NLA_U8, .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SERVICERESPONSEFILTER] = {.type = NLA_U8,
                                             .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SERVICERESPONSEINCLUDE] = {.type = NLA_U8,
                                              .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_USESERVICERESPONSEFILTER] = {.type = NLA_U8,
                                                .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_RX_MATCH_FILTER_LEN] = {.type = NLA_U16,
                                           .len = sizeof(uint16)},
    [NAN_ATTRIBUTE_RX_MATCH_FILTER] = {.type = NLA_BINARY,
                                       .len = MAX_MATCH_FILTER_LEN},
    [NAN_ATTRIBUTE_TX_MATCH_FILTER_LEN] = {.type = NLA_U16,
                                           .len = sizeof(uint16)},
    [NAN_ATTRIBUTE_TX_MATCH_FILTER] = {.type = NLA_BINARY,
                                       .len = MAX_MATCH_FILTER_LEN},
    [NAN_ATTRIBUTE_MAC_ADDR_LIST_NUM_ENTRIES] = {.type = NLA_U16,
                                                 .len = sizeof(uint16)},
    [NAN_ATTRIBUTE_MAC_ADDR_LIST] = {.type = NLA_BINARY,
                                     .len = (NAN_SRF_MAX_MAC * ETHER_ADDR_LEN)},
    [NAN_ATTRIBUTE_TX_TYPE] = {.type = NLA_U8, .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SDE_CONTROL_CONFIG_DP] = {.type = NLA_U8,
                                             .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SDE_CONTROL_RANGE_SUPPORT] = {.type = NLA_U8,
                                                 .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SDE_CONTROL_DP_TYPE] = {.type = NLA_U8,
                                           .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SDE_CONTROL_SECURITY] = {.type = NLA_U8,
                                            .len = sizeof(uint8)},
    ...
    [NAN_ATTRIBUTE_RSSI_THRESHOLD_FLAG] = {.type = NLA_U8,
                                           .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_SDEA_SERVICE_SPECIFIC_INFO_LEN] = {.type = NLA_U16,
                                                      .len = sizeof(uint16)},
    [NAN_ATTRIBUTE_SDEA_SERVICE_SPECIFIC_INFO] = {.type = NLA_BINARY,
                                                  .len = MAX_SDEA_SVC_INFO_LEN},
    [NAN_ATTRIBUTE_SECURITY] = {.type = NLA_U8, .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_RANGING_INTERVAL] = {.type = NLA_U32, .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_RANGING_INGRESS_LIMIT] = {.type = NLA_U32,
                                             .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_RANGING_EGRESS_LIMIT] = {.type = NLA_U32,
                                            .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_RANGING_INDICATION] = {.type = NLA_U32,
                                          .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_SVC_RESPONDER_POLICY] = {.type = NLA_U8,
                                            .len = sizeof(uint8)},
    ...
    [NAN_ATTRIBUTE_PEER_DISC_MAC_ADDR] = {.type = NLA_BINARY,
                                          .len = ETHER_ADDR_LEN},
    [NAN_ATTRIBUTE_PEER_NDI_MAC_ADDR] = {.type = NLA_BINARY,
                                         .len = ETHER_ADDR_LEN},
    [NAN_ATTRIBUTE_IF_ADDR] = {.type = NLA_BINARY, .len = ETHER_ADDR_LEN},
    [NAN_ATTRIBUTE_NO_CONFIG_AVAIL] = {.type = NLA_U8, .len = sizeof(uint8)},
    [NAN_ATTRIBUTE_CHANNEL_INFO] = {.type = NLA_BINARY,
                                    .len = sizeof(nan_channel_info_t) *
                                           NAN_MAX_CHANNEL_INFO_SUPPORTED},
    [NAN_ATTRIBUTE_NUM_CHANNELS] = {.type = NLA_U32, .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_INSTANT_MODE_ENABLE] = {.type = NLA_U32,
                                           .len = sizeof(uint32)},
    [NAN_ATTRIBUTE_INSTANT_COMM_CHAN] = {.type = NLA_U32,
                                         .len = sizeof(uint32)},
...
#ifdef WL_CUSTOM_MAPPING_OF_DSCP
const struct nla_policy
    custom_setting_attr_policy[CUSTOM_SETTING_ATTRIBUTE_MAX] = {
...
static struct wiphy_vendor_command wl_vendor_cmds[] = {
...
#endif /* WL_SAE */
    {{.vendor_id = OUI_BRCM, .subcmd = BRCM_VENDOR_SCMD_SET_CONNECT_PARAMS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_BRCM, .subcmd = BRCM_VENDOR_SCMD_SET_START_AP_PARAMS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_BRCM, .subcmd = BRCM_VENDOR_SCMD_SET_TD_POLICY},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#ifdef DHD_HAL_RING_DUMP
    {{.vendor_id = OUI_GOOGLE, .subcmd = DEBUG_GET_BUF_RING_MAP},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = NAN_WIFI_SUBCMD_ENABLE_MERGE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = APF_SUBCMD_READ_FILTER_DATA},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = DEBUG_SET_HAL_PID},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#endif /* WL_CFG80211 */
#ifdef WL_LATENCY_MODE
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_SET_LATENCY_MODE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#ifdef WL_P2P_RAND
    {{.vendor_id = OUI_BRCM, .subcmd = BRCM_VENDOR_SCMD_SET_MAC},
...
#endif /* WL_P2P_RAND */
#ifdef WL_THERMAL_MITIGATION
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_THERMAL_MITIGATION},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#endif /* WL_THERMAL_MITIGATION */
#ifdef WL_SAR_TX_POWER
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_TX_POWER_SCENARIO},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV,
...
#endif /* WL_SAR_TX_POWER */
#ifdef WL_SOFTAP_ACS
    {{.vendor_id = OUI_BRCM, .subcmd = BRCM_VENDOR_SCMD_ACS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
     .doit = wl_cfgscan_acs,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0))
...
#endif /* WL_SOFTAP_ACS */
#ifdef WL_CUSTOM_MAPPING_OF_DSCP
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_CUSTOM_MAPPING_OF_DSCP},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV,
...
#endif /* WL_CELLULAR_CHAN_AVOID */
#ifdef TPUT_DEBUG_DUMP
    {{.vendor_id = OUI_GOOGLE, .subcmd = DEBUG_SET_TPUT_DEBUG_DUMP_CMD},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_SET_MULTISTA_USE_CASE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_CONFIG_INDOOR_STATE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#if !defined(WL_TWT) && defined(WL_TWT_HAL_IF)
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_SETUP},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_TEARDOWN},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_INFO_FRAME},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_GET_CAP},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_GET_STATS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_CLR_STATS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = ANDR_TWT_SUBCMD_GET_RESPONSE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#endif /* !WL_TWT && WL_TWT_HAL_IF */
#ifdef SUPPORT_OTA_UPDATE
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_GET_OTA_CURRUNT_INFO},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_OTA_UPDATE},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#endif /* SUPPORT_OTA_UPDATE */
#ifdef OEM_ANDROID
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_SET_DTIM_CONFIG},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#endif /* OEM_ANDROID */
#ifdef WL_USABLE_CHAN
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_USABLE_CHAN},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_TRIGGER_SSR},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
     .doit = wl_cfgvendor_trigger_ssr},
#endif /* WLAN_ACCEL_BOOT */
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_GET_RADIO_COMBO_MATRIX},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
    ...
    {{.vendor_id = OUI_GOOGLE, .subcmd = WIFI_SUBCMD_SET_TX_POWER_LIMITS},
     .flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0))
static void wl_cfgvendor_apply_cmd_policy(struct wiphy *wiphy) {
  int i;
...
int wl_cfgvendor_attach(struct wiphy *wiphy, dhd_pub_t *dhd) {
  ...
  dhd_os_dbg_register_callback(DRIVER_LOG_RING_ID,
                               wl_cfgvendor_dbg_ring_send_evt);
  dhd_os_dbg_register_callback(ROAM_STATS_RING_ID,
                               wl_cfgvendor_dbg_ring_send_evt);
#ifdef DHD_DEBUGABILITY_DEBUG_DUMP
  dhd_os_dbg_register_callback(DEBUG_DUMP_RING1_ID,
                               wl_cfgvendor_dbg_ring_send_evt);
  dhd_os_dbg_register_callback(DEBUG_DUMP_RING2_ID,
                               wl_cfgvendor_dbg_ring_send_evt);
#endif /* DHD_DEBUGABILITY_DEBUG_DUMP */
#endif /* DHD_DEBUGABILITY_LOG_DUMP_RING */
#ifdef DHD_PKT_LOGGING_DBGRING
  dhd_os_dbg_register_callback(PACKET_LOG_RING_ID,
                               wl_cfgvendor_dbg_ring_send_evt);
#endif /* DHD_PKT_LOGGING_DBGRING */
#endif /* DEBUGABILITY */
#ifdef DHD_LOG_DUMP
...
void wl_cfgvendor_send_hang_event(struct net_device *dev, u16 reason,
  ...
  if (strlen(string) == 0 || (reason_hang_info != reason)) {
    ...
    bytes_written +=
        scnprintf(&hang_info[bytes_written], len,
                  "%d %d %s %08x %08x %08x %08x %08x %08x %08x", reason,
                  VENDOR_SEND_HANG_EXT_INFO_VER, dhd->debug_dump_time_hang_str,
                  0, 0, 0, 0, 0, 0, 0);
    if (dhd) {
  ...
  while (cnt < HANG_FIELD_CNT_MAX) {
    ...
    bytes_written += scnprintf(&hang_info[bytes_written], len, "%c%08x",
  ...
  wl_cfgvendor_send_async_event(wiphy, bcmcfg_to_prmry_ndev(cfg),
                                BRCM_VENDOR_EVENT_HANGED, hang_info,
...
void wl_cfgvendor_simple_hang_event(struct net_device *dev, u16 reason) {
  ...
  /* Allocate the skb for vendor event */
  msg = CFG80211_VENDOR_EVENT_ALLOC(wiphy, ndev_to_wdev(dev), hang_event_len,
  ...
  nla_put(msg, DEBUG_ATTRIBUTE_HANG_REASON, DHD_MEMDUMP_LONGSTR_LEN,
          dhd->memdump_str);
#endif
...
void wl_copy_hang_info_if_falure(struct net_device *dev, u16 reason, s32 ret) {
  ...
  if (!dhd || !dhd->hang_info) {
    WL_ERR(("%s dhd=%p hang_info=%p\n", __FUNCTION__, dhd,
            (dhd ? dhd->hang_info : NULL)));
    return;
  }

  err = wldev_iovar_getbuf_bsscfg(dev, "memuse", NULL, 0, ioctl_buf,
  ...
  bytes_written += scnprintf(&dhd->hang_info[bytes_written], remain_len,
                             "%d %d %s %d %d %d %d %d %08x %08x", reason,
                             VENDOR_SEND_HANG_EXT_INFO_VER,
                             dhd->debug_dump_time_hang_str, ret, mu.arena_size,
                             mu.arena_free, mu.inuse_size, mu.mf_count, 0, 0);
...
static void wl_cfgvendor_custom_advlog_scan_start(void *plog, uint32 armcycle) {
  ...
  SUPP_ADVLOG(
      ("[ROAM] SCAN_START reason=%d rssi=%d cu=%d full_scan=%d "
       "rssi_thresh=%d [%d]\n",
       cust_roam_rsn, log->rssi, log->current_cu, log->full_scan,
       (rssi_thresh ? log->low_rssi.rssi_threshold : (0)), armcycle));
}

static int wl_cfgvendor_custom_advlog_get_rcc(chanspec_t *chanspecs) {
  struct bcm_cfg80211 *cfg;
  struct net_device *ndev;
  int rcc_chan_cnt = 0;
...
static void wl_cfgvendor_custom_advlog_scan_cmpl(void *plog, uint32 armcycle) {
  ...
  for (i = 0; i < ch_cnt; i++) {
    ...
    buf_pos += snprintf(&ch_list[buf_pos], CH_LIST_LEN - buf_pos, "%d ", freq);
  }
  ...
  SUPP_ADVLOG(
      ("[ROAM] SCORE_CUR_AP bssid=" MACDBG_FULL " freq=%d rssi=%d cu=%d "
       "score=%d.%02d\n",
       MAC2STRDBG_FULL((uint8 *)&log->cur_info.addr), freq, log->cur_info.rssi,
       log->cur_info.cu_avail ? (log->cur_info.cu * 100 / WL_MAX_CHANNEL_USAGE)
                              : WL_CU_NOT_AVAIL,
       log->cur_info.score / 100, log->cur_info.score % 100));

  for (i = 0; i < log->scan_list_size; i++) {
    freq =
        wl_channel_to_frequency(wf_chspec_ctlchan(log->scan_list[i].chanspec),
...
static void wl_cfgvendor_custom_advlog_roam_cmpl(void *plog, uint32 armcycle) {
  ...
  } else if (log->status == WLC_E_STATUS_NO_NETWORKS ||
             log->status == WLC_E_STATUS_TIMEOUT) {
    /* Roaming No Networks */
    /* do not print to the kernel, only for framework (MACDBG_FULL) */
...
static void wl_cfgvendor_custom_advlog_btm_req(void *plog, uint32 armcycle) {
  ...
  SUPP_ADVLOG(
      ("[BTM] REQ token=%d mode=%d disassoc=%d validity=%d "
...
static void wl_cfgvendor_custom_advlog_btm_resp(void *plog, uint32 armcycle) {
  ...
  /* do not print to the kernel, only for framework (MACDBG_FULL) */
  SUPP_ADVLOG(("[BTM] RESP token=%d status=%d delay=%d target=" MACDBG_FULL
               " [%d]\n",
