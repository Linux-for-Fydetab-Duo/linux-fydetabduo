...
#ifdef SENDPROB
static int wl_ext_add_del_ie_hex(struct net_device *dev, uint pktflag,
                                 char *ie_data, int ie_len,
  ...
  memcpy((char *)vndr_ie->vndr_ie_buffer.vndr_ie_list[0].vndr_ie_data.oui,
         ie_data, ie_len);

  err = wldev_iovar_setbuf(dev, "vndr_ie", vndr_ie, tot_len, iovar_buf,
                           sizeof(iovar_buf), NULL);
  if (err != 0) AGENL_ERROR(dev->name, "vndr_ie, ret=%d\n", err);
...
static int wl_ext_send_probersp(struct net_device *dev, char *buf,
                                int buf_len) {
  ...
  err =
      wl_ext_add_del_ie_hex(dev, VNDR_IE_PRBRSP_FLAG, pVndrOUI, ie_len, "add");
  if (err) goto exit;
...
static int wl_ext_set_probreq(struct net_device *dev, bool set) {
  int bytes_written = 0;
...
void wl_ext_probreq_event(struct net_device *dev, void *argu,
  ...
  if (num_ie) {
    ...
    if (android_msg_level & ANDROID_INFO_LEVEL) prhex("buf", buf, event_len);
...
static int wl_ext_genl_send(struct genl_params *zconf, struct net_device *dev,
  ...
  if (skb) {
    msg_head = genlmsg_put(skb, 0, 0, &wl_ext_genl_family, 0,
                           GENL_CUSTOM_COMMAND_RECV);
    if (msg_head == NULL) {
      nlmsg_free(skb);
