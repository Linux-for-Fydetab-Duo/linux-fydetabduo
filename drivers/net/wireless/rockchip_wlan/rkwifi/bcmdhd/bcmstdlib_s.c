...
/* Don't use compiler builtins for stdlib APIs within the implementation of the
 * stdlib itself. */
 ...
 */
int BCMPOSTTRAPFN(memmove_s)(void *dest, size_t destsz, const void *src,
                             size_t n) {
  int err = BCME_OK;
 ...
 * than RSIZE_MAX, writes destsz zero bytes into the dest object.
 */
int BCMPOSTTRAPFN(memcpy_s)(void *dest, size_t destsz, const void *src,
                            size_t n) {
  int err = BCME_OK;
  char *d = dest;
  const char *s = src;
 ...
 */
int BCMPOSTTRAPFN(memset_s)(void *dest, size_t destsz, int c, size_t n) {
  int err = BCME_OK;
  if ((!dest) || (((char *)dest + destsz) < (char *)dest)) {
    err = BCME_BADARG;
    goto exit;
...
#endif /* !__STDC_WANT_SECURE_LIB__ && !(__STDC_LIB_EXT1__ && \
 ...
 * return: string leng of src (which is always < size) on success or size on
 * failure
 *
...
size_t BCMPOSTTRAPFN(strlcpy)(char *dest, const char *src, size_t size) {
  ...
  return size;
          ...
          !defined(BCM_USE_PLATFORM_STRLCPY) */
