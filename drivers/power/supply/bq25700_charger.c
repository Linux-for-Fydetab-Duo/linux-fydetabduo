...
static void bq25700_enable_charger(struct bq25700_device *charger,
				   u32 input_current)
{
	int vol_idx = bq25700_find_idx(DEFAULT_INPUTVOL, TBL_INPUTVOL);
		...
		unsigned long val, void *v)
{
	...
	if (ret != 0)
		return NOTIFY_OK;
	/* online=0: USB out */
  dev_info(bq->dev, "POWER_SUPPLY_PROP_ONLINE:%d\n", prop.intval);
	if (prop.intval == 0) {
	...
	if (prop.intval > 0) {
		...
		if (ret != 0)
			return NOTIFY_OK;
		vol_idx = bq25700_find_idx(prop.intval, TBL_INPUTVOL);
	...
	if (charger->nb.notifier_call) {
		if (charger->dev->of_node) {
			...
			if (IS_ERR_OR_NULL(notify_psy)) {
				dev_dbg(charger->dev, "No USB PD power supply, charger will work in basic mode\n");
				notify_psy = NULL;
			}
...
static long bq25700_init_usb(struct bq25700_device *charger)
{
	...
	if (IS_ERR(edev1)) {
		if (PTR_ERR(edev1) != -EPROBE_DEFER)
			dev_dbg(dev, "No second extcon device (single Type-C port configuration)\n");
