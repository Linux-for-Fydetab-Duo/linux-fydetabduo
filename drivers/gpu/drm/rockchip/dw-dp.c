...
static int dw_dp_connector_get_modes(struct drm_connector *connector)
{
	...
	if (!num_modes) {
		...
		if (ret) {
			dev_err(dp->dev, "failed to power on PHY for EDID: %d\n", ret);
		} else {
...
static bool dw_dp_detect_dpcd(struct dw_dp *dp)
{
	...
	/*
	 * After phy_power_on, the USBDP PHY may perform a full reconfiguration
	 * if switching from USB to DP mode. The built-in 10ms delay in the PHY
	 * isn't sufficient for mode changes. Give extra time for stabilization.
	 */
	if (dp->usbdp_hpd) {
		dev_info(dp->dev, "Waiting for USBDP PHY stabilization after mode change\n");
	}
...
static const struct drm_edid *dw_dp_bridge_get_edid(struct drm_bridge *bridge,
					  struct drm_connector *connector)
{
	struct dw_dp *dp = bridge_to_dp(bridge);
...
static const struct drm_bridge_funcs dw_dp_bridge_funcs = {
	...
	.edid_read = dw_dp_bridge_get_edid,
};
...
static int dw_dp_typec_mux_set(struct typec_mux_dev *mux, struct typec_mux_state *state)
{
	...
	mutex_lock(&dp->irq_lock);
	dev_info(dp->dev, "DW-DP mux_set: mode=%lu alt=%p\n", state->mode, state->alt);
	if (state->alt && state->alt->svid == USB_TYPEC_DP_SID) {
		struct typec_displayport_data *data = state->data;

		if (!data) {
			dev_info(dp->dev, "hotunplug from the usbdp (no data)\n");
			dp->hotplug.long_hpd = true;
			dp->hotplug.status = false;
			schedule_work(&dp->hpd_work);
			...
			dev_info(dp->dev, "hotplug from the usbdp status=0x%x conf=0x%x\n",
				 data->status, data->conf);
			dp->hotplug.long_hpd = true;
			dp->hotplug.status = true;
			schedule_work(&dp->hpd_work);
		} else {
			dev_info(dp->dev, "hotunplug from the usbdp status=0x%x\n", data->status);
			dp->hotplug.long_hpd = true;
			dp->hotplug.status = false;
			schedule_work(&dp->hpd_work);
		}
	} else {
		dev_info(dp->dev, "DP mux_set: not DP Alt Mode\n");
	}
	mutex_unlock(&dp->irq_lock);
...
static int dw_dp_probe(struct platform_device *pdev)
{
	...
	if (device_property_present(dev, "mode-switch")) {
		...
		if (ret)
			return ret;
		dp->usbdp_hpd = true;
		dev_info(dev, "DW-DP: usbdp_hpd enabled\n");
	} else {
		dev_warn(dev, "DW-DP: mode-switch property NOT found\n");
	}
