 ...
 * @返回值：0成功，否则失败
 */
static int mas_sync(u8 *txb, u8 *rxb, int len) {
  int ret = 0;
#if 1
...
// static int mas_ioctl (struct inode *node, struct file *filp, unsigned int
// cmd, uns igned long arg) this function only supported while the linux kernel
// version under v2.6.36,while the kernel version under v2.6.36, use this line
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
    ...
    case WAKEUP:  // wake up, schedule the process into the runqueue
      ...
      break;
    case ENABLE_CLK:
      mas_enable_spi_clock(smas->spi);  // if the spi clock is not opening
                                        // always, do this methods
      break;
    case DISABLE_CLK:
      mas_disable_spi_clock(smas->spi);  // disable the spi clock
      break;
    case ENABLE_INTERRUPT:
      enable_irq(irq);  // enable the irq,in fact, you can make irq enable
                        // always
      break;
    ...
    case GET_MODE:
      mutex_lock(&ioctl_lock);
      ret =
          copy_to_user((unsigned int *)arg, &ma_drv_reg, sizeof(unsigned int));
      mutex_unlock(&ioctl_lock);
      break;
    case MA_IOC_GVER: {
    ...
    case SET_SPI_SPEED:
      ret =
          copy_from_user(&ma_speed, (unsigned int *)arg, sizeof(unsigned int));
    ...
    case WAIT_SCREEN_STATUS_CHANGE:
      ...
      mutex_lock(&ioctl_lock);
      tmp = copy_to_user((unsigned int *)arg, &is_screen_on,
                         sizeof(unsigned int));
      mutex_unlock(&ioctl_lock);
      break;
    case GET_INTERRUPT_STATUS:
      ...
      mutex_lock(&ioctl_lock);
      tmp = copy_to_user((unsigned int *)arg, &int_pin_state,
                         sizeof(unsigned int));
      mutex_unlock(&ioctl_lock);
      break;
...
static ssize_t mas_write(struct file *filp, const char __user *buf,
  ...
  int val = 0;
  // MALOGD("start");
  if (count == 6) {  // cmd ioctl, old version used the write interface to do
                     // ioctl, this is only for the old version
    int cmd, arg;
    u8 tmp[6];
...
static int mas_mmap(struct file *filp, struct vm_area_struct *vma) {
  unsigned long page;
  if (!kernel_memaddr) {
...
#ifdef COMPATIBLE_VERSION3
static unsigned int mas_poll(struct file *filp,
                             struct poll_table_struct *wait) {
  unsigned int mask = 0;

  printd("%s: start. f_irq=%d f_repo=%d f_wake=%d\n", __func__, smas->f_irq,
         smas->f_repo, smas->f_wake);
...
static const struct file_operations sfops = {
    ...
    .mmap = mas_mmap,
//.ioctl = mas_ioctl,
// using the previous line replacing the unlock_ioctl while the linux kernel
// under version2.6.36
#ifdef CONFIG_COMPAT
...
static int init_file_node(void) {
  ...
  sdev->cls = class_create(MA_CHR_DEV_NAME);
  if (IS_ERR(sdev->cls)) {
    MALOGE("class_create");
    return -1;
  }
...
static int init_interrupt(void) {
  ...
  ret = request_irq(irq, mas_interrupt,
                    IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, tname, NULL);
#else
...
static int init_vars(void) {
  ...
  if (sdev == NULL || smas == NULL || stxb == NULL || srxb == NULL) {
    ...
    if (stxb != NULL)
      free_pages((unsigned long)stxb, get_order(IMAGE_DMA_SIZE));
    if (srxb != NULL)
      free_pages((unsigned long)srxb, get_order(IMAGE_DMA_SIZE));
    return -ENOMEM;
  }
...
static int __maybe_unused deinit_connect(void) {
  mas_free_dts_info();
  return 0;
}
...
void mas_plat_remove(struct platform_device *pdev) {
...
static int __init madev_init(void) {
  int ret = 0;
  MALOGF("start");
  compatible = 1;
