...
struct pinctrl_state *mas_finger_power2v8_on,
    *mas_finger_power2v8_off,                           // power2v8
...
/**
 *    the platform struct start,for getting the platform device to set gpio
 * state
 */
...
static struct platform_driver spdrv = {.probe = mas_plat_probe,
...
/**
 *  the spi struct date start,for getting the spi_device to set the spi clock
 * enable end
 */
...
int mas_do_some_for_probe(struct spi_device *spi) { return 0; }
...
int mas_finger_get_gpio_info(struct platform_device *pdev) {
  ...
  mas_finger_power2v8_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_power_en1");
  ...
  mas_finger_power2v8_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_power_en0");
  ...
  mas_finger_power1v8_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_power_18v_en1");
  ...
  mas_finger_power1v8_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_power_18v_en0");
  ...
  mas_spi_mi_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_mi_as_spi0_mi");
  ...
  mas_spi_mi_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_mi_as_gpio");
  ...
  mas_spi_mo_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_mo_as_spi0_mo");
  ...
  mas_spi_mo_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_mo_as_gpio");
  ...
  mas_spi_ck_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_clk_as_spi0_clk");
  ...
  mas_spi_ck_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_clk_as_gpio");
  ...
  mas_spi_cs_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_cs_as_spi0_cs");
  ...
  mas_spi_cs_off =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_spi0_cs_as_gpio");
  ...
  mas_finger_eint_on =
      pinctrl_lookup_state(mas_finger_pinctrl, "finger_int_as_int");
...
static int mas_finger_set_spi(int cmd) {
  switch (cmd) {
    case 0:
      if ((!IS_ERR(mas_spi_cs_off)) && (!IS_ERR(mas_spi_ck_off)) &&
          (!IS_ERR(mas_spi_mi_off)) && (!IS_ERR(mas_spi_mo_off))) {
      ...
      break;
    case 1:
      if ((!IS_ERR(mas_spi_cs_on)) && (!IS_ERR(mas_spi_ck_on)) &&
          (!IS_ERR(mas_spi_mi_on)) && (!IS_ERR(mas_spi_mo_on))) {
...
static int mas_finger_set_power(int cmd) {
...
static int __maybe_unused mas_finger_set_eint(int cmd) {
  switch (cmd) {
    case 0:
...
unsigned int mas_get_irq(struct device *dev) {
  int irq_num;
  finger_int_pin =
      of_get_named_gpio(dev->of_node, "fingerprint,touch-int-gpio", 0);
 ...
 *  		under 0  the of_property_read_u32_index return errno,check the
 *dts as below: last but not least use this function must checkt the label on
 *dts file, after is an example: ma_finger: ma_finger{ compatible =
 *"mediatek,afs120x"; finger_int_pin = <100 0>;
 * }
 */
