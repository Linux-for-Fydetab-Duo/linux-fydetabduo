...
#define MALOGD_LEVEL \
  KERN_EMERG  //[KERN_DEBUG|KERN_EMERG] usually, the debug level is used for the
              // release version
...
#define MALOGD(fmt, args...)                                      \
  do {                                                            \
    printk(KERN_ERR "<mafp>-dbg-[%04d]-[@%s]" fmt "\n", __LINE__, \
           __FUNCTION__, ##args);                                 \
  } while (0)
#define MALOGE(fmt, args...)                                      \
  do {                                                            \
    printk(KERN_ERR "<mafp>-err-[%04d]-[@%s]" fmt "\n", __LINE__, \
           __FUNCTION__, ##args);                                 \
  } while (0)
#define MALOGF(x)                                                     \
  do {                                                                \
    printk(KERN_ERR "<mafp>-fun-[%04d]-[@%s] is called!\n", __LINE__, \
           __FUNCTION__);                                             \
  } while (0)
#define MALOGW(fmt, args...)                                       \
  do {                                                             \
    printk(KERN_ERR "<mafp>-warn-[%04d]-[@%s]" fmt "\n", __LINE__, \
           __FUNCTION__, ##args);                                  \
  } while (0)
#else  // DEBUG
#define MALOGD(fmt, args...)
...
#define IOCTL_IRQ_ENABLE \
  0x101                        // 中断使能 			//enable interrupt
#define IOCTL_SPI_SPEED 0x102  // SPI速度 			//spi speed
#define IOCTL_READ_FLEN \
  0x103  // 读帧长度(保留)		//the length of one frame
#define IOCTL_LINK_DEV \
  0x104  // 连接设备(保留)		//connect the device
#define IOCTL_COVER_NUM \
  0x105  // 材料编号			//the index of the material
#define IOCTL_GET_VDATE \
  0x106  // 版本日期			//the date fo the version
