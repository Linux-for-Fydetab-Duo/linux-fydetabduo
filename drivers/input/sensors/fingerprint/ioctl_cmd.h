...
#define MA_IOC_UKRP \
  _IOW(MA_IOC_MAGIC, 17, unsigned char)  // user define the report key
...
#define MA_IOC_GVER     \
  _IOR(                 \
      MA_IOC_MAGIC, 35, \
      unsigned int)  // get the driver version,the version mapping in the u32 is
                     // the final  4+4+8,as ******** ******* ****(major verson
                     // number) ****(minor version number) ********(revised
                     // version number), the front 16 byte is reserved.
...
#define WAIT_FINGERPRINTD_RESPONSE \
  _IOR(MA_IOC_MAGIC, 41, unsigned int)  // for factory test
#define WAKEUP_FACTORY_TEST_SEND_FINGERPRINTD_RESPONSE \
  _IOW(MA_IOC_MAGIC, 42, unsigned int)  // for fingerprintd
