...
static irqreturn_t hall_mh248_interrupt(int irq, void *dev_id) {
  ...
  input_report_switch(mh248->hall_input, SW_LID,
                      gpio_value != mh248->active_value);
...
static int hall_mh248_probe(struct platform_device *pdev) {
  ...
  ret = devm_gpio_request_one(mh248->dev, mh248->gpio_pin, GPIOD_IN,
                              "hall_mh248");
  if (ret < 0) {
  ...
  ret = devm_request_threaded_irq(
      mh248->dev, mh248->irq, NULL, hall_mh248_interrupt,
      irq_flags | IRQF_NO_SUSPEND | IRQF_ONESHOT, "hall_mh248", mh248);
  if (ret < 0) {
...
static const struct of_device_id hall_mh248_match[] = {
    {.compatible = "hall-mh248-fyde"}, {/* Sentinel */}};
