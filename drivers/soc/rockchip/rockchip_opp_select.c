				      ...
				      struct pvtm_config *pvtm)
{
	...
	if (IS_ERR(pvtm->tz))
		return -EINVAL;
	/* Check if thermal zone is valid by attempting to get temperature */
	int temp;
	if (thermal_zone_get_temp(pvtm->tz, &temp))
		return -EINVAL;
					    ...
					    int *target_value)
{
	 ...
	 */
	thermal_zone_get_temp(pvtm->tz, &cur_temp);
				   ...
				   const char *clk_name, const char *reg_name)
{
	...
	if (info->opp_token < 0) {
		dev_err(dev, "failed to set opp config clk_name:%s, reg_name:%s\n", clk_name, reg_name);
			     ...
			     bool scaling_down, struct rockchip_opp_info *info)
{
	...
	/* rate_clk_single field not available in v6.12 - use current rate */
	unsigned long current_rate = clk_get_rate(opp_table->clk);
